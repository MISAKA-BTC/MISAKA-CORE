...
export default async function AddressPage({ params }: { params: { address: string } }) {
  ...
  return (
    <div className="page-enter space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-fg tracking-tight mb-2">Address</h1>
        <div className="hash-text text-sm text-subtle break-all bg-hover px-4 py-2.5 rounded-none border border-line">
          {summary.address}
        </div>
      </div>
      ...
      {summary.privacyNote && (
        <div className="rounded-none bg-fg/3 border border-line px-4 py-3">
          <p className="text-xs text-subtle leading-relaxed">
            <span className="text-subtle font-semibold">Privacy Note:</span>{' '}
            {summary.privacyNote}
          </p>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-line">
                ...
                <th className="text-right py-3 px-4 text-xs font-semibold uppercase tracking-wider text-muted">Index</th>
                <th className="text-right py-3 px-4 text-xs font-semibold uppercase tracking-wider text-muted">Amount</th>
                <th className="text-right py-3 px-4 text-xs font-semibold uppercase tracking-wider text-muted">Time</th>
                <th className="text-right py-3 px-4 text-xs font-semibold uppercase tracking-wider text-muted">Spent</th>
              </tr>
            </thead>
            <tbody>
              {summary.outputs.map((o, i) => (
                <tr key={i} className="border-b border-surface-300/20 table-row-hover">
                  ...
                  <td className="py-3 px-4 text-right text-subtle font-mono text-xs">{o.outputIndex}</td>
                  <td className="py-3 px-4 text-right text-subtle font-mono text-xs">
...
function InfoCard({ label, value }: { label: string; value: string | null }) {
  return (
    <div className="rounded-none border border-line bg-surface p-4">
      <span className="text-xs font-medium uppercase tracking-widest text-muted block mb-1.5">{label}</span>
      {value != null ? (
...
function PrivacyShield() {
  return (
    <span className="inline-flex items-center gap-1 text-xs text-dim" title="Hidden for privacy">
