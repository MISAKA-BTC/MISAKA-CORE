...
export default async function BlockDetailPage({ params }: { params: { id: string } }) {
  ...
  const rows: [string, React.ReactNode][] = [
    ['Block Height', <span key="h" className="font-mono text-fg font-semibold">#{formatNumber(block.height)}</span>],
    ...
    ['Timestamp', <span key="t" className="text-subtle">{formatTimestamp(block.timestamp)} (<TimestampDisplay iso={block.timestamp} />)</span>],
    ['Proposer', (
      <Link key="p" href={`/validators/${block.proposer}`} className="font-mono text-subtle hover:text-fg text-sm transition-colors link-underline">
        {block.proposer}
      </Link>
    )],
    ['Validator Signatures', <span key="vs" className="text-subtle font-mono">{block.validatorSignatures}</span>],
    ['Transaction Count', <span key="tc" className="text-subtle font-mono">{block.txCount}</span>],
    ['Total Fees', <span key="tf" className="text-subtle font-mono">{formatFee(block.totalFees)}</span>],
    ['Block Size', <span key="bs" className="text-subtle">{formatBytes(block.size)}</span>],
  ];
  ...
  return (
    <div className="page-enter space-y-8">
      ...
      <div className="flex items-center gap-2 text-sm">
        <Link href="/blocks" className="text-muted hover:text-subtle transition-colors">Blocks</Link>
        <span className="text-dim">/</span>
        <span className="text-subtle">#{formatNumber(block.height)}</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        {block.height > 0 && (
          <Link href={`/blocks/${block.height - 1}`} className="px-3 py-1.5 rounded-none text-xs font-medium bg-hover text-subtle hover:bg-dim hover:text-fg transition-all">
      ...
      <section>
        <h2 className="font-semibold text-fg text-lg mb-4">
