...
impl BridgeRequest {
    ...
    ///
    /// Binds: domain_tag || request_id || source || dest || asset || amount
...
mod tests {
    ...
    fn test_different_sender_different_hash() {
        let mut req1 = BridgeRequest {
            request_id: [0xCC; 32], source_chain: 1, dest_chain: 2,
            asset_id: "SOL".into(), amount: 1000,
            sender: "alice".into(), recipient: "r".into(), nonce: 1,
        };
        let mut req2 = req1.clone();
        req2.sender = "mallory".into();
        assert_ne!(
            req1.authorization_hash(b"TEST:"),
            req2.authorization_hash(b"TEST:"),
            "sender substitution must invalidate authorization hash"
        );
    }
}
