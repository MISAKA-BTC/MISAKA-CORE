//! Cross-chain Bridge — ML-DSA-65 committee authorization.
//!
//! Misaka side manages:
//! - Bridge request / receipt lifecycle
//! - Committee authorization (M-of-N ML-DSA-65 threshold verification)
//! - Asset registry (which Solana SPL tokens are bridged)
//! - Wrapped asset mint/burn accounting
//!
//! Production verifier: CommitteeVerifier (real PQ signature verification).
...
pub use verifier::{BridgeVerifier, CommitteeVerifier, CommitteeMember, AuthorizationProof, validate_verifier_for_production};
...
pub use isolation::{ExternalChain, BridgeOrigin, TokenType};
...
impl BridgeModule {
    ...
    pub fn process_lock_event(
        ...
        self.replay.mark_used(request.request_id)
            .map_err(|e| BridgeError::AuthorizationFailed(format!("replay persist failed: {}", e)))?;
