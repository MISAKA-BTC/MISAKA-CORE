        ...
        if chain_id == 1 {
            anyhow::bail!(
                "Mainnet genesis requires validator_keys.json with real ML-DSA-65 public keys. \
                 Generate with: misaka-cli keygen --validators {}", validator_count
            );
        }
        ...
        .map(|i| {
            ...
            let pk = ValidatorPublicKey {
                bytes: if i < loaded_keys.len() {
                    ...
                    let seed = Sha3_256::new().chain_update(b"MISAKA_TESTNET_VALIDATOR:")
                        .chain_update(&(i as u32).to_le_bytes()).finalize();
                    pk[..32].copy_from_slice(&seed);
                    pk
                },
            };
            ValidatorIdentity::new_active(id, 1_000_000, pk)
        })
        .collect();
