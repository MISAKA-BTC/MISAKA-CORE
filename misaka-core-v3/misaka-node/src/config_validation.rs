...
impl Default for TestnetConfig {
    fn default() -> Self {
        Self {
            ...
            min_ring_size: 4,
            default_ring_scheme: 0x10,  // UnifiedZkp-v1 (sole scheme)
            log_level: "info".into(),
...
impl TestnetConfig {
    ...
    pub fn validate(&self) -> Result<(), Vec<ConfigError>> {
        ...
        // Ports
        if self.p2p_port == 0 { errors.push(ConfigError::InvalidP2pPort); }
        if self.rpc_port == 0 { errors.push(ConfigError::InvalidRpcPort); }
        if self.p2p_port == self.rpc_port { errors.push(ConfigError::PortCollision); }
        ...
        // Ring scheme validation — UnifiedZkp only
        match self.default_ring_scheme {
            0x10 => {} // UnifiedZkp — sole supported scheme
            other => {
                errors.push(ConfigError::Custom(
                    format!("unsupported default_ring_scheme: 0x{:02x} — only UnifiedZkp (0x10) is supported", other)));
            }
        }
    ...
    ///
    /// Only UnifiedZkp (0x10) is allowed. removed.
    pub fn is_ring_scheme_allowed(&self, scheme: u8) -> bool {
        scheme == 0x10 // UnifiedZkp only
    }
}
...
mod tests {
    ...
    #[test]
    fn test_unified_zkp_only_allowed() {
        let cfg = TestnetConfig::default();
        assert!(cfg.is_ring_scheme_allowed(0x10));   // UnifiedZkp
        assert!(!cfg.is_ring_scheme_allowed(0x01));  // LRS removed
        assert!(!cfg.is_ring_scheme_allowed(0x02));  // Chipmunk removed
        assert!(!cfg.is_ring_scheme_allowed(0x03));  // removed
        assert!(!cfg.is_ring_scheme_allowed(0xFF));  // Unknown blocked
    }

    #[test]
    fn test_lrs_scheme_rejected() {
        let mut cfg = TestnetConfig::default();
        cfg.default_ring_scheme = 0x01; // LRS
        assert!(cfg.validate().is_err());
    }
