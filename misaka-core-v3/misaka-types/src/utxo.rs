...
#[deprecated(note = "Replaced by algebraic nullifiers")]
...
pub enum TxType {
    ...
    Transfer,
    /// Block reward transaction (no inputs, validator receives reward).
    Coinbase,
}
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[deprecated(note = "Use misaka_pqc::qdag_tx::QdagTransaction")]
pub struct UtxoTransaction {
    ...
    pub ring_scheme: u8,
    /// Transaction type — Transfer or Coinbase.
    #[serde(default)]
    pub tx_type: TxType,
    /// Ring-signed inputs.
...
#[deprecated(note = "LRS removed")]
pub const RING_SCHEME_LRS: u8 = 0x01;
#[deprecated(note = "Chipmunk removed")]
pub const RING_SCHEME_CHIPMUNK: u8 = 0x02;
/// LogRing O(log n) — system default.
#[deprecated(note = "LogRing removed — use UnifiedZkp 0x10")]
...
impl UtxoTransaction {
    ...
    pub fn validate_structure(&self) -> Result<(), MisakaError> {
        ...
        // Coinbase txs (block rewards) have empty inputs.
        // Regular txs MUST have at least one input.
        // Coinbase txs are identified by tx_type == Coinbase.
...
mod tests {
    ...
    fn make_utxo_tx_v1() -> UtxoTransaction {
        UtxoTransaction {
            ...
            outputs: vec![TxOutput { amount: 9900, one_time_address: [0xCC; 32], pq_stealth: None, spending_pubkey: None }],
            fee: 100,
            extra: vec![],
        }
    }
