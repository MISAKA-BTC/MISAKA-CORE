...
impl MisakaPublicKey {
    ...
    pub fn ml_dsa(bytes: Vec<u8>) -> Result<Self, MisakaError> {
        if bytes.len() != 1952 {
            return Err(MisakaError::InvalidPublicKeyLength { expected: 1952, got: bytes.len() });
        }
        Ok(Self { scheme: SignatureScheme::MlDsa65, bytes })
    }
    ...
    pub fn mcs1_decode(data: &[u8], offset: &mut usize) -> Result<Self, MisakaError> {
        if *offset >= data.len() {
            return Err(MisakaError::DeserializationError("EOF reading pk scheme".into()));
        }
        let scheme = SignatureScheme::from_u8(data[*offset])?;
        *offset += 1;
        let len = mcs1::read_u32(data, offset)? as usize;
        if *offset + len > data.len() {
            return Err(MisakaError::DeserializationError("EOF reading pk bytes".into()));
        }
    ...
    pub fn wire_len(&self) -> usize { 1 + 4 + self.bytes.len() }
}
...
impl MisakaSignature {
    pub fn ml_dsa(bytes: Vec<u8>) -> Self {
        Self { scheme: SignatureScheme::MlDsa65, bytes }
    }
    ...
    pub fn mcs1_decode(data: &[u8], offset: &mut usize) -> Result<Self, MisakaError> {
        if *offset >= data.len() {
            return Err(MisakaError::DeserializationError("EOF reading sig scheme".into()));
        }
        let scheme = SignatureScheme::from_u8(data[*offset])?;
        *offset += 1;
        let len = mcs1::read_u32(data, offset)? as usize;
        if *offset + len > data.len() {
            return Err(MisakaError::DeserializationError("EOF reading sig bytes".into()));
        }
        let bytes = data[*offset..*offset + len].to_vec();
        *offset += len;
        Ok(Self { scheme, bytes })
...
impl Drop for MisakaSecretKey {
    fn drop(&mut self) { for b in self.bytes.iter_mut() { *b = 0; } }
}
...
mod tests {
    ...
    fn test_scheme_from_u8() {
        assert_eq!(SignatureScheme::from_u8(0x01).unwrap(), SignatureScheme::MlDsa65);
        assert_eq!(SignatureScheme::from_u8(0x02).unwrap(), SignatureScheme::LatticeRing);
