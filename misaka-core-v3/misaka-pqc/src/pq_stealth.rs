...
// Phase 3 fix: Separate nonces for amount and payload AEAD.
// The old single LABEL_NONCE has been removed. Each AEAD context gets
// its own nonce derivation path to prevent catastrophic nonce reuse
// if keys are ever accidentally shared in a future refactor.
const LABEL_NONCE_AMT: &[u8] = b"misaka/pq-stealth/nonce-amt/v1";
const LABEL_NONCE_PAY: &[u8] = b"misaka/pq-stealth/nonce-pay/v1";
...
///
/// Phase 3 fix: nonce_amt and nonce_pay are derived independently.
/// The old single nonce_bytes field has been removed.
struct DerivedMaterials {
    k_addr: [u8; 32],
    k_amt: [u8; 32],
    k_payload: [u8; 32],
    scan_tag: [u8; 16],
    /// Nonce for amount AEAD encryption (24 bytes for XChaCha20).
    nonce_amt: [u8; 24],
    /// Nonce for payload AEAD encryption (24 bytes for XChaCha20).
    nonce_pay: [u8; 24],
}

impl Drop for DerivedMaterials {
    fn drop(&mut self) {
        crate::secret::zeroize_bytes(&mut self.k_addr);
        crate::secret::zeroize_bytes(&mut self.k_amt);
        crate::secret::zeroize_bytes(&mut self.k_payload);
        crate::secret::zeroize_bytes(&mut self.scan_tag);
        crate::secret::zeroize_bytes(&mut self.nonce_amt);
        crate::secret::zeroize_bytes(&mut self.nonce_pay);
    }
}
...
fn derive_materials(shared_secret: &MlKemSharedSecret) -> Result<DerivedMaterials, CryptoError> {
    ...
    hk.expand(LABEL_SCAN_TAG, &mut scan_tag)
        .map_err(|_| CryptoError::StealthDomainMismatch)?;
    // SEC-AUDIT-V4 CRITICAL-002: Derive separate nonces for amount and payload.
    // XChaCha20-Poly1305 security proof requires (key, nonce) uniqueness.
    // Using the same nonce with different keys is technically safe today,
    // but fragile — a future refactor sharing keys would be catastrophic.
    let mut nonce_amt = [0u8; 24];
    hk.expand(LABEL_NONCE_AMT, &mut nonce_amt)
        .map_err(|_| CryptoError::StealthDomainMismatch)?;
    let mut nonce_pay = [0u8; 24];
    hk.expand(LABEL_NONCE_PAY, &mut nonce_pay)
        .map_err(|_| CryptoError::StealthDomainMismatch)?;

    Ok(DerivedMaterials { k_addr, k_amt, k_payload, scan_tag, nonce_amt, nonce_pay })
}

/// Derive one-time destination address from k_addr.
///
/// `addr = SHA3-256("MISAKA_STEALTH_ADDR_V2:" || k_addr)` — full 32 bytes.
fn derive_one_time_address(k_addr: &[u8; 32]) -> [u8; 32] {
    let mut h = Sha3_256::new();
    h.update(b"MISAKA_STEALTH_ADDR_V2:");
    h.update(k_addr);
    h.finalize().into()
}
...
fn build_aad(
    ...
    let mut aad = Vec::with_capacity(1 + 32 + 4 + 32);
...
fn aead_encrypt(key: &[u8; 32], nonce: &[u8; 24], plaintext: &[u8], aad: &[u8]) -> Result<Vec<u8>, CryptoError> {
    let cipher = XChaCha20Poly1305::new(AeadKey::from_slice(key));
    let nonce = XNonce::from_slice(nonce);
    cipher.encrypt(nonce, Payload { msg: plaintext, aad })
        .map_err(|_| CryptoError::StealthPayloadTooShort { min: 0, got: plaintext.len() })
}

fn aead_decrypt(key: &[u8; 32], nonce: &[u8; 24], ciphertext: &[u8], aad: &[u8]) -> Result<Vec<u8>, CryptoError> {
    let cipher = XChaCha20Poly1305::new(AeadKey::from_slice(key));
    let nonce = XNonce::from_slice(nonce);
...
pub struct StealthOutput {
    /// One-time destination address (32 bytes — PQ collision-resistant).
...
pub fn create_stealth_output(
    ...
    // 7. Encrypt amount (XChaCha20-Poly1305) — uses amount-specific nonce
    let amount_bytes = amount.to_le_bytes();
    let amount_ct = aead_encrypt(&mat.k_amt, &mat.nonce_amt, &amount_bytes, &aad)?;

    // 8. Encrypt payload — uses payload-specific nonce
    let payload_ct = if payload.is_empty() {
        // Empty payload → encrypt zero-length → just the 16-byte auth tag
        aead_encrypt(&mat.k_payload, &mat.nonce_pay, &[], &aad)?
    } else {
        aead_encrypt(&mat.k_payload, &mat.nonce_pay, payload, &aad)?
    };
...
impl StealthScanner {
    ...
    pub fn try_recover(
        ...
        if !constant_time_eq(&mat.scan_tag, &stealth_data.scan_tag) {
        ...
        // 7. Decrypt amount — uses amount-specific nonce
        ...
        // 8. Decrypt payload — uses payload-specific nonce
        let payload = match aead_decrypt(&mat.k_payload, &mat.nonce_pay, &stealth_data.payload_ct, &aad) {
            Ok(pt) => pt,
    ...
    pub fn scan_batch(
        ...
        outputs.iter()
            .filter_map(|(data, tx_id, idx)| {
                self.try_recover(data, tx_id, *idx).ok().flatten()
            })
            .collect()
    }
}
...
mod tests {
    ...
    fn test_stealth_roundtrip() {
        ...
        let output = create_stealth_output(
            &kp.public_key, amount, payload, &tx_id, 0
        ).unwrap();

        let scanner = StealthScanner::new(kp.secret_key);
        let recovered = scanner.try_recover(&output.stealth_data, &tx_id, 0)
    ...
    fn test_wrong_view_key_no_match() {
        ...
        let result = scanner.try_recover(&output.stealth_data, &tx_id, 0).unwrap();
    ...
    fn test_tampered_payload_ct_fails() {
        let kp = ml_kem_keygen().unwrap();
        let tx_id = test_tx_id();
        let mut output = create_stealth_output(&kp.public_key, 100, b"real memo", &tx_id, 0).unwrap();
    ...
    fn test_batch_scan() {
        ...
        // 2 for recipient
        for i in 0..2u32 {
            let o = create_stealth_output(
                &recipient.public_key, (i + 1) as u64 * 100, b"", &tx_id, i
            ).unwrap();
