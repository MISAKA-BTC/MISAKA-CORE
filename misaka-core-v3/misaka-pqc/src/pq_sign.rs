...
use pqcrypto_traits::sign::{
    DetachedSignature, PublicKey as PqPk, SecretKey as PqSk,
};
...
pub fn ml_dsa_verify(pk: &MlDsaPublicKey, msg: &[u8], sig: &MlDsaSignature) -> Result<(), CryptoError> {
    let mut domain_msg = Vec::with_capacity(TX_SIGN_DOMAIN.len() + msg.len());
    domain_msg.extend_from_slice(TX_SIGN_DOMAIN);
    domain_msg.extend_from_slice(msg);
...
pub fn ml_dsa_verify_raw(pk: &MlDsaPublicKey, msg: &[u8], sig: &MlDsaSignature) -> Result<(), CryptoError> {
    let pq_pk = pk.to_pqcrypto()
        .ok_or(CryptoError::MlDsaVerifyFailed)?;
    let pq_sig = sig.to_pqcrypto()
        .ok_or(CryptoError::MlDsaVerifyFailed)?;
