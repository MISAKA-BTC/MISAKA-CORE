...
//!
//! [u8; 20] → [u8; 32]: collision-resistant, future-proof, no domain mixup.
...
///
/// Improvement M: Previously [u8; 20] (too short for PQ security margins).
...
impl OneTimeAddress {
    ...
    pub fn scheme(&self) -> u8 { self.0[1] }
    pub fn chain_id(&self) -> u16 { u16::from_le_bytes([self.0[2], self.0[3]]) }
    pub fn as_bytes(&self) -> &[u8; 32] { &self.0 }

    /// Backward compatibility: extract 20-byte legacy address.
    #[deprecated(note = "Use 32-byte address directly")]
    pub fn to_legacy_20(&self) -> [u8; 20] {
        let mut legacy = [0u8; 20];
        legacy.copy_from_slice(&self.0[..20]);
        legacy
    }
}
...
impl TxPaddingPolicy {
    ...
    pub fn dummy_outputs_needed(&self, real_output_count: usize) -> usize {
        if real_output_count >= self.min_outputs { 0 }
        else { self.min_outputs - real_output_count }
    }

    /// How many extra bytes to add for size padding.
    pub fn extra_padding_needed(&self, current_tx_size: usize) -> usize {
        if current_tx_size >= self.tx_size_target { 0 }
        else { self.tx_size_target - current_tx_size }
    }

    /// Validate that a TX meets padding requirements.
    pub fn validate(&self, ring_size: usize, output_count: usize) -> Result<(), String> {
        if ring_size != self.fixed_ring_size {
            return Err(format!(
                "ring size {} != fixed {}", ring_size, self.fixed_ring_size));
        }
        if output_count < self.min_outputs {
            return Err(format!(
                "output count {} < minimum {}", output_count, self.min_outputs));
        }
        Ok(())
    }
}
...
pub fn generate_scan_tag(shared_secret_material: &[u8]) -> [u8; SCAN_TAG_LEN] {
    ...
    hk.expand(SCAN_TAG_LABEL, &mut tag)
        .expect("HKDF expand for scan tag should never fail");
    tag
}
