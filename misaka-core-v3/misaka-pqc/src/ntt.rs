...
use super::pq_ring::{Poly, Q, N};

use std::sync::Once;

/// SEC-AUDIT-V4 HIGH-003: Verify NTT primitive root on first use.
///
/// ψ must be a primitive 512th root of unity mod Q=12289:
///   ψ^512 ≡ 1 (mod Q)   AND   ψ^256 ≢ 1 (mod Q)
///
/// If this fails, ALL polynomial multiplications silently produce
/// wrong results, potentially enabling ring signature forgery.
static NTT_ROOT_CHECK: Once = Once::new();
fn assert_ntt_roots() {
    NTT_ROOT_CHECK.call_once(|| {
        let q = Q as i64;
        let psi = mod_pow(11, 24, q);
        // ψ^512 must be 1 (order divides 512)
        assert_eq!(
            mod_pow(psi, 512, q), 1,
            "FATAL: ψ = 11^24 mod {} is NOT a 512th root of unity", Q
        );
        // ψ^256 must NOT be 1 (order is exactly 512, not a divisor)
        assert_ne!(
            mod_pow(psi, 256, q), 1,
            "FATAL: ψ = 11^24 mod {} has order 256, not 512 (not primitive)", Q
        );
        // ω = ψ² must be a primitive 256th root
        let omega = psi * psi % q;
        assert_eq!(
            mod_pow(omega, 256, q), 1,
            "FATAL: ω = ψ² is NOT a 256th root of unity"
        );
        assert_ne!(
            mod_pow(omega, 128, q), 1,
            "FATAL: ω = ψ² has order 128, not 256"
        );
    });
}
...
fn power_table(root: i32, n: usize) -> Vec<i32> {
    ...
    t[0] = 1;
    for i in 1..n { t[i] = mod_mul(t[i - 1], root); }
    t
}

/// Forward negacyclic NTT.
pub fn forward_ntt(a: &mut [i32; N]) {
    assert_ntt_roots(); // HIGH-003: verify primitive root on first call
    let q = Q as i64;
...
pub fn pointwise_mul(a: &[i32; N], b: &[i32; N]) -> [i32; N] {
    let mut c = [0i32; N];
    for i in 0..N { c[i] = mod_mul(a[i], b[i]); }
    c
}
...
pub fn ntt_mul(a: &Poly, b: &Poly) -> Poly {
    ...
    for i in 0..N { result.coeffs[i] = mod_reduce(c_ntt[i]); }
    result
}
...
mod tests {
    ...
    fn test_ntt_mul_vs_schoolbook() {
        let a = derive_public_param(&DEFAULT_A_SEED);
        let mut b = Poly::zero();
        b.coeffs[0] = 1; b.coeffs[1] = 2; b.coeffs[100] = Q - 1;
    ...
    fn test_ntt_mul_identity() {
        let a = derive_public_param(&DEFAULT_A_SEED);
        let mut one = Poly::zero(); one.coeffs[0] = 1;
    ...
    fn test_ntt_mul_commutative() {
        let a = derive_public_param(&DEFAULT_A_SEED);
        let mut b = Poly::zero();
        for i in 0..10 { b.coeffs[i] = ((i * 137 + 42) % Q as usize) as i32; }
