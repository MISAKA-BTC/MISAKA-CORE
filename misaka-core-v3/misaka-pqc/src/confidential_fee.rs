...
use crate::range_proof::{RangeProof, prove_range, verify_range};
...
pub fn create_confidential_fee(
    ...
    if fee < MIN_FEE {
        return Err(CryptoError::RingSignatureInvalid(
            format!("fee {} < minimum {}", fee, MIN_FEE)));
    }
    if fee > MAX_FEE {
        return Err(CryptoError::RingSignatureInvalid(
            format!("fee {} > maximum {}", fee, MAX_FEE)));
    }
...
pub fn verify_confidential_fee(
    crs: &BdlopCrs,
    fee: &ConfidentialFee,
) -> Result<(), CryptoError> {
    // 1. Range proof
    ...
    min_poly.coeffs[0] = (MIN_FEE % Q as u64) as i32;
...
fn derive_fee_hint_key_nonce(ss: &[u8; 32]) -> Result<([u8; 32], [u8; 24]), CryptoError> {
    ...
    hk.expand(FEE_HINT_KEY_LABEL, &mut key)
        .map_err(|_| CryptoError::RingSignatureInvalid("HKDF expand for fee hint key failed".into()))?;
    hk.expand(FEE_HINT_NONCE_LABEL, &mut nonce)
        .map_err(|_| CryptoError::RingSignatureInvalid("HKDF expand for fee hint nonce failed".into()))?;
    Ok((key, nonce))
}
...
pub fn encrypt_fee_hint(
    ...
    let aead_ct = cipher.encrypt(
        XNonce::from_slice(&nonce),
        Payload { msg: &fee.to_le_bytes(), aad: &aad },
    ).map_err(|_| CryptoError::RingSignatureInvalid("fee hint AEAD encrypt failed".into()))?;
...
pub fn decrypt_fee_hint(
    ...
    let plaintext = cipher.decrypt(
        XNonce::from_slice(&nonce),
        Payload { msg: &hint.aead_ct, aad: &aad },
    ).map_err(|_| CryptoError::RingSignatureInvalid(
        "fee hint AEAD decrypt failed (wrong key or tampered)".into()
    ))?;

    if plaintext.len() != 8 {
        return Err(CryptoError::RingSignatureInvalid(
            format!("fee hint plaintext length {} != 8", plaintext.len())));
    }
...
mod tests {
    ...
    fn test_confidential_fee_balance_integration() {
        ...
        for i in 0..N {
            let v = r_in.as_poly().coeffs[i] - r_out.as_poly().coeffs[i] - r_fee.as_poly().coeffs[i];
        ...
        for i in 0..N {
            assert_eq!(
                ...
                "balance diff must equal A₁·r_excess at coefficient {}", i
            );
        }
    }
    ...
    fn test_fee_hint_encrypt_decrypt_roundtrip() {
        ...
        let hint = encrypt_fee_hint(
            fee, &kp.public_key, chain_id, &txid, &proposer_id, epoch,
        ).unwrap();
        ...
        let decrypted = decrypt_fee_hint(
            &hint, &kp.secret_key, chain_id, &txid, &proposer_id, epoch,
        ).unwrap();
    ...
    fn test_fee_hint_wrong_key_fails() {
        ...
        let hint = encrypt_fee_hint(
            1000, &kp1.public_key, 2, &[0xAA; 32], &[0xBB; 32], 1,
        ).unwrap();

        // Decrypting with wrong key: Kyber implicit rejection produces
        // a wrong shared secret → AEAD decryption fails
        let result = decrypt_fee_hint(
            &hint, &kp2.secret_key, 2, &[0xAA; 32], &[0xBB; 32], 1,
        );
    ...
    fn test_fee_hint_wrong_aad_fails() {
        ...
        let hint = encrypt_fee_hint(
            500, &kp.public_key, 2, &[0xAA; 32], &[0xBB; 32], 10,
        ).unwrap();

        // Tamper with epoch in AAD
        let result = decrypt_fee_hint(
            &hint, &kp.secret_key, 2, &[0xAA; 32], &[0xBB; 32], 999,
        );
        assert!(result.is_err(), "wrong AAD (epoch) must fail AEAD");

        // Tamper with chain_id
        let result2 = decrypt_fee_hint(
            &hint, &kp.secret_key, 99, &[0xAA; 32], &[0xBB; 32], 10,
        );
    ...
    fn test_fee_hint_tampered_ct_fails() {
        ...
        hint.aead_ct[0] ^= 0xFF;
        let result = decrypt_fee_hint(
            &hint, &kp.secret_key, 2, &[0xAA; 32], &[0xBB; 32], 10,
        );
