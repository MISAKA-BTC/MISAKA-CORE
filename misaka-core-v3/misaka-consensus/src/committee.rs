//! Committee vote verification — only Active validators.
...
/// Verify a single vote from an Active validator.
/// Non-Active validators are rejected.
pub fn verify_vote(vs: &ValidatorSet, vote: &CommitteeVote) -> Result<u128, MisakaError> {
    let vi = vs.get(&vote.voter)
        .ok_or_else(|| MisakaError::SignatureVerificationFailed("unknown or non-active voter".into()))?;
    vs.verify_validator_sig(&vote.voter, &vote.signing_bytes(), &vote.signature)?;
    Ok(vi.stake_weight)
}

/// Verify a set of committee votes, accumulating stake weight.
...
mod tests {
    ...
    #[test]
    fn test_non_active_vote_rejected() {
        let kp = generate_validator_keypair();
        let mut vid = [0u8; 20]; vid[0] = 99;
        let mut vi = ValidatorIdentity::new_active(
            vid, 100, ValidatorPublicKey { bytes: kp.public_key.to_bytes() },
        );
        vi.status = ValidatorStatus::Unbonding;
        let vs = ValidatorSet::new(vec![vi]);
        let vote = make_vote(&kp, vid, 1, [0xAA; 32]);
        assert!(verify_vote(&vs, &vote).is_err(), "unbonding validator vote must be rejected");
    }
}
