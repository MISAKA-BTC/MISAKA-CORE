...
impl Drop for ValidatorPqSecretKey {
    fn drop(&mut self) {
        zeroize_bytes_local(&mut self.pq_sk);
    }
}

/// Local zeroize helper for misaka-crypto (cannot depend on misaka-pqc::secret).
/// Same semantics as `misaka_pqc::secret::zeroize_bytes()`.
/// TODO: Extract to shared `misaka-zeroize` crate when workspace is restructured.
#[inline(never)]
fn zeroize_bytes_local(buf: &mut [u8]) {
