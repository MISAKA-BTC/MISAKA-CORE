...
/// 順序付けされた個別 TX のデータ。
///
/// Phase 8: `nullifiers` は Q-DAG-CT のリング非依存 nullifier。
/// 同一 output を異なるリングで使っても同一 nullifier になるため、
/// DAG 上のどのブランチでも衝突検出が確実に動作する。
#[derive(Debug)]
pub struct OrderedTxData {
    /// TX ハッシュ。
    pub tx_hash: [u8; 32],
    /// Phase 4/8: Ring-independent nullifiers (NOT link_tags).
    /// Same output → same nullifier regardless of ring choice.
    /// The DAG state manager uses these for double-spend detection.
    pub nullifiers: Vec<[u8; 32]>,
    /// Coinbase TX か (inputs が空)。
...
/// 4. 無効な TX を FailedNullifierConflict としてマークする
///
/// ## 不変条件 (Invariants)
///
...
pub struct ApplyStats {
    ...
    /// 正常に適用された TX 数。
    pub txs_applied: u64,
    /// Key Image 競合で無効化された TX 数。
    pub txs_failed_null_conflict: u64,
    /// 署名不正で無効化された TX 数。
...
impl DagStateManager {
    ...
    /// # 引数
    ///
    /// - `known_nullifiers`: 既に UTXO Set に記録済みの Key Image 群。
    ///   チェックポイントからの復元時に使用。空から開始する場合は空の HashSet。
    pub fn new(known_nullifiers: HashSet<[u8; 32]>) -> Self {
        let applied_nullifiers = known_nullifiers.into_iter()
            .map(|ki| (ki, NullifierRecord {
                first_tx_hash: [0; 32],
                block_hash: [0; 32],
                order_index: 0,
            }))
    ...
    ///       → 無条件で適用 (Key Image なし)
    ///     else:
    ///       for each nullifier in tx.nullifiers:
    ///         if nullifier ∈ applied_nullifiers:
    ///           → TX を FailedNullifierConflict としてマーク
    ///           → この TX の出力は生成しない
    ///           → break (この TX の処理を中断)
    ///       if all nullifiers are fresh:
    ///         → TX を Applied としてマーク
    ///         → 全 nullifiers を applied_nullifiers に追加
    ///         → 出力を UTXO Set に追加
    /// ```
    ///
    /// # 引数
    ...
    ///   `(tx_hash, block_hash, output_index, amount)` を受け取る。
    ///
    /// # 戻り値
    ///
    /// 全 TX の適用結果リスト。
        ...
        F: FnMut(UtxoAction),
    {
        ...
        let mut global_order_index: u64 = self.stats.txs_applied + self.stats.txs_failed_null_conflict;
    ...
    /// 2. いずれかの Key Image が `applied_nullifiers` に既に存在する場合:
    ///    - TX 全体を `FailedNullifierConflict` としてマーク
    ///    - **TX の出力は一切生成しない** (部分適用は行わない)
    ///    - **TX の他の Key Image も記録しない** (未使用 KI として残す)
    /// 3. 全 Key Image が未使用の場合:
    ///    - 全 Key Image を `applied_nullifiers` に追加
        ...
        F: FnMut(UtxoAction),
    {
        ...
        if tx.is_coinbase {
            ...
            for (idx, &amount) in tx.output_amounts.iter().enumerate() {
        ...
        // ── 通常 TX: Key Image 競合チェック ──
        for &ki in &tx.nullifiers {
            if let Some(existing) = self.applied_nullifiers.get(&ki) {
                // ⚠️ Key Image 競合検出!
                ...
                self.stats.txs_failed_null_conflict += 1;

                return TxApplyResult {
                    tx_hash: tx.tx_hash,
                    block_hash: *block_hash,
                    status: TxApplyStatus::FailedNullifierConflict {
                        conflicting_nullifier: ki,
        ...
        // Key Image を記録
        for &ki in &tx.nullifiers {
            self.applied_nullifiers.insert(ki, NullifierRecord {
    ...
    pub fn is_nullifier_spent(&self, ki: &[u8; 32]) -> bool {
        self.applied_nullifiers.contains_key(ki)
    }
...
pub enum UtxoAction {
    ...
    /// Nullifier を記録する (Phase 4: ring-independent)。
    RecordNullifier {
        nullifier: [u8; 32],
        tx_hash: [u8; 32],
    },
...
/// 2. `FailedNullifierConflict` でマークされた TX の出力は選択しない
/// 3. 自分自身の入力に関連する UTXO は選択しない (リンク可能性排除)
///
/// ## 使用方法
///
/// ```ignore
/// let filter = DecoyFilter::new(&ghostdag_manager, &dag_store, &state_manager);
/// let safe_utxos = filter.get_eligible_utxos(amount, ring_size);
/// ```
...
impl<'a, S: DagStore> DecoyFilter<'a, S> {
    pub fn new(
        ...
        Self { ghostdag, store, state_manager }
    }
    ...
    /// - `ring_size`: 必要なデコイ数
    /// - `exclude_nullifiers`: 自分の TX に含まれる Key Image (除外対象)
    /// - `all_utxos`: UTXO Set から取得した全候補
    ///
    /// # 戻り値
    ///
    /// 安全なデコイ候補のリスト (ring_size 以上あれば成功)。
    pub fn filter_eligible_decoys(
        &self,
        target_amount: u64,
        exclude_nullifiers: &HashSet<[u8; 32]>,
        all_utxos: &[DecoyCandidate],
    ) -> Vec<DecoyCandidate> {
        all_utxos.iter()
            .filter(|utxo| {
                // 条件 1: 確認深度が十分
                let depth = self.ghostdag.confirmation_depth(&utxo.block_hash, self.store);
...
mod tests {
    ...
    fn make_tx_data(hash: [u8; 32], nullifiers: Vec<[u8; 32]>, outputs: Vec<u64>) -> OrderedTxData {
        OrderedTxData {
            tx_hash: hash,
            nullifiers,
            is_coinbase: false,
    ...
    #[test]
    fn test_basic_apply() {
        let mut manager = DagStateManager::new(HashSet::new());
        ...
        assert!(manager.is_nullifier_spent(&[0xBB; 32]));
    }

    #[test]
    fn test_nullifier_conflict_resolution() {
        let mut manager = DagStateManager::new(HashSet::new());
        ...
        let blocks = vec![
            ...
            OrderedBlockData {
                ...
                blue_score: 1,
                transactions: vec![
                    make_tx_data([0xA1; 32], vec![ki], vec![5_000]),
                ],
            },
            ...
            OrderedBlockData {
                ...
                blue_score: 1,
                transactions: vec![
                    make_tx_data([0xB1; 32], vec![ki], vec![4_000]),
                ],
            },
        ];
        ...
        assert!(matches!(
            results[1].status,
            TxApplyStatus::FailedNullifierConflict { conflicting_nullifier, prior_tx_hash }
            if conflicting_nullifier == ki && prior_tx_hash == [0xA1; 32]
        ));

        // 統計確認
        assert_eq!(manager.stats.txs_applied, 1);
        assert_eq!(manager.stats.txs_failed_null_conflict, 1);
    }
    ...
    fn test_multi_ki_partial_conflict() {
        ...
        let blocks = vec![
            ...
            OrderedBlockData {
                ...
                blue_score: 1,
                transactions: vec![
                    make_tx_data([0x10; 32], vec![ki_shared], vec![1_000]),
                ],
            },
            ...
            OrderedBlockData {
                ...
                blue_score: 2,
                transactions: vec![
                    make_tx_data([0x20; 32], vec![ki_shared, ki_unique], vec![2_000]),
                ],
            },
        ];
        ...
        assert_eq!(results[0].status, TxApplyStatus::Applied);
        assert!(matches!(results[1].status, TxApplyStatus::FailedNullifierConflict { .. }));

        // ki_unique は使われていない (TX 全体が無効化されたため)
        assert!(!manager.is_nullifier_spent(&ki_unique));
    }
    ...
    fn test_failed_tx_outputs_not_created() {
        ...
        let blocks = vec![
            OrderedBlockData {
                ...
                blue_score: 1,
                transactions: vec![
                    make_tx_data([0x10; 32], vec![ki], vec![5_000]),
                ],
            },
            OrderedBlockData {
                block_hash: [0x02; 32],
        ...
        manager.apply_ordered_transactions(&blocks, |action| {
            if let UtxoAction::CreateOutput { amount, .. } = action {
                created_outputs.push(amount);
            }
        });
