//! # MISAKA-DAG: Privacy BlockDAG (UnifiedZKP + GhostDAG)
//!
...
//!  │                                                                 │
//!  │  ┌──────────────────────────────────────────────────────┐      │
//!  │  │         Unified ZKP / PQC Layer (misaka-pqc)              │      │
//!  │  │  • Unified ZKP membership proofs                   │      │
...
//! ### 2. v1 型の再利用
//!
//! All transactions use `QdagTransaction` with confidential amounts (Item 4).
...
// ─── Q-DAG-CT: Confidential Transaction Verification ───
pub mod qdag_verify;

// ─── Medium-term: Persistent Storage + Wire Protocol ───
pub mod persistent_store;
pub mod wire_protocol;

// ─── Long-term: Advanced Privacy ───
pub mod decoy_selection;

// ─── Improvements: Validation + Optimization ───
pub mod validation_pipeline;  // C+L: TX auth boundary + verification order
pub mod header_validation;    // E+J: Hardened header + VRF tie-break
pub mod reachability;         // I+K: O(1) ancestor queries + finality coupling
...
pub use ghostdag::{
    GhostDagManager, DagStore, InMemoryDagStore,
    MIN_DECOY_DEPTH, DEFAULT_K, DEFAULT_PRUNING_DEPTH,
};
...
pub use dag_block_producer::{
    ...
    qdag_tx_to_ordered, build_ordered_block_data,
};
...
pub use qdag_verify::{verify_qdag_tx, QdagVerifyResult};

// ═══════════════════════════════════════════════════════════════
//  Re-exports — Medium-term: Persistent Storage + Wire Protocol
// ═══════════════════════════════════════════════════════════════

pub use persistent_store::{
    PersistentDagBackend, RocksDbDagStore, DagStoreError,
    CF_HEADERS, CF_GHOSTDAG, CF_CHILDREN, CF_TIPS, CF_NULLIFIERS,
};
pub use wire_protocol::{
    WireMessage, WireMessageType, WireError,
    encode_hash_list, decode_hash_list, encode_new_block, encode_nullifier_status,
    MAGIC, WIRE_VERSION, MAX_PAYLOAD_SIZE, MAX_HASHES_PER_REQ,
};

// ═══════════════════════════════════════════════════════════════
//  Re-exports — Long-term: Advanced Privacy
// ═══════════════════════════════════════════════════════════════

pub use decoy_selection::{
    GammaDecoySelector, RingCandidate, DecoySelectionError,
    MetadataLeakageProfile,
    GAMMA_SHAPE, GAMMA_RATE, DEFAULT_RING_SIZE, MAX_RING_SIZE_CT,
};
...
pub use validation_pipeline::{
    PreVerifyResult, ProofCache, ProposerScorecard,
    pre_verify_tx_structure, check_block_nullifier_uniqueness,
};
pub use header_validation::{
    HeaderCheckResult, TieBreakKey,
    validate_header_full, sort_by_tiebreak,
};
pub use reachability::{
    ReachabilityStore, Interval,
    RetentionLevel, NodeRetentionRole,
    is_decoy_eligible, retention_level,
    FINALITY_DEPTH, PRUNING_DEPTH, ACCUMULATOR_RETENTION_DEPTH,
};
