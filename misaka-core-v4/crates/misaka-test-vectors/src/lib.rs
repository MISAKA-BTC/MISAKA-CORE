...
use misaka_pqc::pq_kem::{
    ml_kem_decapsulate, ml_kem_encapsulate, ml_kem_keygen, ML_KEM_CT_LEN, ML_KEM_PK_LEN,
};
use misaka_pqc::pq_ring::{
    derive_public_param, ring_sign, ring_verify, SpendingKeypair, DEFAULT_A_SEED,
};
use misaka_pqc::pq_sign::{
    ml_dsa_sign, ml_dsa_verify, MlDsaKeypair, ML_DSA_PK_LEN, ML_DSA_SIG_LEN,
};
...
pub fn run_all_vectors() -> (usize, usize, usize) {
    ...
    if kp.public_key.as_bytes().len() == ML_DSA_PK_LEN {
        p += 1;
    } else {
        f += 1;
    }
    ...
    if sig.as_bytes().len() == ML_DSA_SIG_LEN {
        p += 1;
    } else {
        f += 1;
    }
    if ml_dsa_verify(&kp.public_key, b"test", &sig).is_ok() {
        p += 1;
    } else {
        f += 1;
    }
    if ml_dsa_verify(&kp.public_key, b"wrong", &sig).is_err() {
        p += 1;
    } else {
        f += 1;
    }
    ...
    if kem_kp.public_key.as_bytes().len() == ML_KEM_PK_LEN {
        p += 1;
    } else {
        f += 1;
    }
    let (ct, ss1) = ml_kem_encapsulate(&kem_kp.public_key).unwrap();
    if ct.as_bytes().len() == ML_KEM_CT_LEN {
        p += 1;
    } else {
        f += 1;
    }
    if ml_kem_decapsulate(&kem_kp.secret_key, &ct).unwrap() == ss1 {
        p += 1;
    } else {
        f += 1;
    }
    ...
    match scanner.try_recover(&st.stealth_data, &tx_id, 0) {
        Ok(Some(r)) if r.amount == 42000 => p += 1,
        _ => f += 1,
    }
    ...
    match ring_sign(&a, &ring_pks, 1, &ring_kps[1].secret_poly, &msg) {
        Ok(sig) => {
            p += 1;
            if ring_verify(&a, &ring_pks, &msg, &sig).is_ok() {
                p += 1;
            } else {
                f += 1;
            }
            if ring_verify(&a, &ring_pks, &[0xFF; 32], &sig).is_err() {
                p += 1;
            } else {
                f += 1;
            }
    ...
    if skp.key_image == ring_kps[0].key_image {
        p += 1;
    } else {
        f += 1;
    }
