...
impl From<&GhostdagData> for CompactGhostdagData {
    fn from(v: &GhostdagData) -> Self {
        Self {
            ...
            blue_work: v.blue_work,
            selected_parent: v.selected_parent,
        }
    }
}
...
impl GhostdagData {
    pub fn new(
        ...
        blues_anticone_sizes: Vec<(Hash, KType)>,
    ) -> Self {
        Self {
            blue_score,
            blue_work,
    ...
    pub fn new_with_selected_parent(selected_parent: Hash, k: KType) -> Self {
        ...
        Self {
            ...
            blues_anticone_sizes: vec![(selected_parent, 0)],
        }
    }
    pub fn genesis_data() -> Self {
        Self::default()
    }
...
impl DbGhostdagStore {
    pub fn new(db: Arc<DB>, level: u8, cache_policy: CachePolicy) -> Self {
        Self {
            db: db.clone(),
            full_access: CachedDbAccess::new(
                db.clone(),
                cache_policy,
    ...
    pub fn insert(&self, hash: Hash, data: &GhostdagData) -> StoreResult<()> {
        let compact: CompactGhostdagData = data.into();
        self.full_access
            .write(DirectDbWriter::new(self.db.clone()), hash, data.clone())?;
        self.compact_access
            .write(DirectDbWriter::new(self.db.clone()), hash, compact)?;
        Ok(())
    }
}
...
impl GhostdagStoreReader for DbGhostdagStore {
    ...
    fn get_data(&self, hash: &Hash) -> StoreResult<GhostdagData> {
        self.full_access.read(*hash)
    }
    fn get_compact_data(&self, hash: &Hash) -> StoreResult<CompactGhostdagData> {
        self.compact_access.read(*hash)
    }
    fn has(&self, hash: &Hash) -> StoreResult<bool> {
        self.full_access.has(*hash)
    }
}
