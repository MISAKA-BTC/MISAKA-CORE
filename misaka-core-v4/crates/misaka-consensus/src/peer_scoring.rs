...
/// Double-spend attempt (nullifier already in chain state).
...
/// Reward for a valid block relayed (not proposed) by this peer.
const REWARD_VALID_RELAY: i64 = 0; // No reward for relaying (anti-sybil)
...
pub enum PenaltyReason {
    ...
    /// Transaction contains a nullifier already in chain state.
...
impl PeerScoring {
    ...
    pub fn penalize(&mut self, peer: &PeerId, reason: PenaltyReason) -> PenaltyAction {
        ...
        if record.score <= DISCONNECT_THRESHOLD {
            ...
            let backoff_factor = 2u32.saturating_pow(
                record.total_offenses.saturating_sub(1).min(10)
            );
            ...
            PenaltyAction::Ban { new_score: record.score, duration: ban_duration }
        } else if record.score <= DEGRADED_THRESHOLD {
            record.state = PeerState::Degraded;
            PenaltyAction::ScoreReduced { new_score: record.score }
        } else {
    ...
    pub fn evict_stale(&mut self, max_idle: Duration) {
        let now = Instant::now();
        self.records.retain(|_, r| now.duration_since(r.last_activity) < max_idle);
    }
}
...
mod tests {
    ...
    fn test_double_spend_immediate_ban() {
        let mut scoring = PeerScoring::new();
        // Double-spend: 100 - 80 = 20 (still Active)
    ...
    fn test_cheap_needs_many_offenses_to_ban() {
        ...
        for i in 0..9 {
            let action = scoring.penalize(&peer_a(), PenaltyReason::CheapParseFail);
            assert!(matches!(action, PenaltyAction::ScoreReduced { .. }),
                "offense {} should not ban", i);
        }
