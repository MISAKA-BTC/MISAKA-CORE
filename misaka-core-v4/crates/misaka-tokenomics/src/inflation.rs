...
pub fn epoch_emission(total_supply: u128, year: u64, epochs_per_year: u64) -> u128 {
    ...
    // Audit R7: Use checked_mul to detect overflow instead of saturating
    // (saturating would silently produce u128::MAX / denominators = wrong result).
    // Division order: divide by BPS first (smaller divisor) to keep intermediates smaller.
    match total_supply.checked_mul(rate_bps) {
        Some(product) => product / BPS_DENOMINATOR / epochs_per_year as u128,
        None => {
            // Fallback: divide first to avoid overflow (loses some precision)
            (total_supply / BPS_DENOMINATOR) * rate_bps / epochs_per_year as u128
        }
    }
}
...
pub fn annual_emission(total_supply: u128, year: u64) -> u128 {
    ...
    match total_supply.checked_mul(rate_bps) {
        ...
        None => (total_supply / BPS_DENOMINATOR) * rate_bps,
    }
}
