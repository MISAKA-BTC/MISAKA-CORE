...
compile_error!(
    "DO NOT compile production build with 'experimental_dag' feature. \
     DAG consensus is incomplete (no P2P, single-node snapshot restore only, no durable multi-node recovery). \
...
async fn main() -> anyhow::Result<()> {
    ...
    let runtime_config = startup_config::resolve_runtime_config(&matches, &mut cli)?;
    ...
    if let Some(path) = runtime_config.config_path.as_ref() {
        info!("Loaded startup config from {}", path.display());
    }
    ...
    #[cfg(feature = "experimental_dag")]
    {
        start_dag_node(cli, node_mode, role, p2p_config, runtime_config).await
    }

    #[cfg(not(feature = "experimental_dag"))]
    {
        start_v1_node(cli, node_mode, role, p2p_config, runtime_config).await
    }
}
...
async fn start_dag_node(
    ...
    runtime_config: startup_config::RuntimeConfig,
) -> anyhow::Result<()> {
    use misaka_dag::{
    ...
    let (
        ...
        Ok(None) => {
            ...
            let genesis_header = DagBlockHeader {
                version: DAG_VERSION,
                parents: vec![],
                timestamp_ms: runtime_config.genesis_timestamp_ms,
                tx_root: ZERO_HASH,
...
async fn start_v1_node(
    ...
    // Genesis
    let genesis_timestamp_ms = runtime_config.genesis_timestamp_ms;
    let mut chain = ChainStore::new();
    let genesis = chain.store_genesis(genesis_timestamp_ms);
    info!(
    ...
    // Periodically retry configured bootstrap peers so operators can update
    // seeds.txt without rebuilding the package.
    if !p2p_config.seed_nodes.is_empty() || runtime_config.seed_file.is_some() {
        let bootstrap_state = state.clone();
        let bootstrap_p2p = p2p.clone();
        let bootstrap_static_peers = cli.peers.clone();
        let bootstrap_seed_nodes = p2p_config.seed_nodes.clone();
        let bootstrap_runtime = runtime_config.clone();
        tokio::spawn(async move {
            let retry_after =
                tokio::time::Duration::from_secs(bootstrap_runtime.seed_refresh_secs.max(5));
            loop {
                tokio::time::sleep(retry_after).await;

                let mut peer_specs = bootstrap_static_peers.clone();
                peer_specs.extend(bootstrap_seed_nodes.clone());
                match bootstrap_runtime.load_dynamic_seed_nodes() {
                    Ok(extra) => {
                        for seed in extra {
                            if !peer_specs.iter().any(|existing| existing == &seed) {
                                peer_specs.push(seed);
                            }
                        }
                    }
                    Err(e) => warn!("Bootstrap seed refresh failed: {}", e),
                }

                let mut retry_addrs = Vec::new();
                for peer in peer_specs {
                    match peer.parse::<SocketAddr>() {
                        Ok(addr) => retry_addrs.push(addr),
                ...
                let connected = bootstrap_p2p.connected_peer_addrs().await;
                retry_addrs.retain(|addr| !connected.contains(addr));
                if retry_addrs.is_empty() {
                    continue;
                }

                let height = bootstrap_state.read().await.height;
                info!("Bootstrap retry: attempting {} peer(s)", retry_addrs.len());
                bootstrap_p2p.connect_to_peers(&retry_addrs, height).await;
            }
        });
    }
