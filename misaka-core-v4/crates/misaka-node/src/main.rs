...
// ── DAG mode: RELEASE GATE ──
// The compile_error! has been replaced by a layered safety mechanism:
//
// Layer 1 (Runtime): MISAKA_DAG_EXPERIMENTAL=1 env var required at startup.
//          Enforced in start_dag_node() — refuses to start without it.
//
// Layer 2 (CI):      tests/multi_node_chaos.rs MUST pass before any
//          tagged release that includes experimental_dag.
//          See: scripts/dag_release_gate.sh
//
// Layer 3 (Checklist): All of the following must be true for mainnet:
...
//
// When all layers are satisfied, remove the MISAKA_DAG_EXPERIMENTAL guard
// in start_dag_node() to enable unattended DAG node startup.
...
struct Cli {
    ...
    /// Explicitly use the experimental zero-knowledge block path for txs that
    /// already carry `zk_proof`. Default mainline stays RingSignature.
...
async fn main() -> anyhow::Result<()> {
    ...
    // Catches edge cases where debug builds accidentally run against mainnet.
    {
        ...
        if cli.experimental_zk_path {
            warn!(
                "⚠ experimental_zk_path is active — only txs with zk_proof will use the explicit ZK block route"
            );
        }
    }
...
fn load_or_create_local_dag_validator(
    ...
    let key_path = local_validator_key_path(data_dir, validator_index);
    let keypair_and_identity = if key_path.exists() {
        ...
        if validator_id_vec.len() != validator_id.len() {
            anyhow::bail!(
                "invalid validator id length in '{}': expected 20 bytes, got {}",
                key_path.display(),
        ...
        let secret_key = ValidatorPqSecretKey {
            pq_sk: hex::decode(&persisted.secret_key_hex)?,
        };
        ...
        info!(
            "Layer 2: loaded local DAG validator key | id={} | file={}",
        ...
        let persisted = LocalDagValidatorKeyFile {
            validator_id_hex: hex::encode(identity.validator_id),
            public_key_hex: hex::encode(&identity.public_key.bytes),
            secret_key_hex: hex::encode(&keypair.secret_key.pq_sk),
            stake_weight: identity.stake_weight,
        };
        std::fs::write(&key_path, serde_json::to_vec_pretty(&persisted)?)?;
        info!(
            "Layer 2: created local DAG validator key | id={} | file={}",
            hex::encode(identity.validator_id),
            key_path.display()
        );
        (keypair, identity)
...
#[cfg(feature = "experimental_dag")]
pub(crate) fn experimental_dag_validator_set(
...
fn recompute_latest_checkpoint_finality(
    ...
    let validator_set = experimental_dag_validator_set(state);
...
async fn start_dag_node(
    ...
    use misaka_dag::{
        ...
        GhostDagEngine, UniformStakeProvider, ZERO_HASH,
    };
    ...
    //  - single-node JSON snapshot restore exists, but durable multi-node recovery is not done
    //  - finality checkpoints are only persisted inside the local runtime snapshot
    //  - wallet / explorer / bridge semantics are not yet DAG-native
    //
    ...
    if std::env::var("MISAKA_DAG_EXPERIMENTAL").as_deref() != Ok("1") {
        ...
        error!("║  • Durable multi-node state recovery                     ║");
        error!("║  • DAG-native wallet / explorer / bridge semantics       ║");
        error!("║                                                          ║");
    ...
    prune_checkpoint_attestation_state(&mut dag_node_state);
    if let Err(e) = refresh_local_checkpoint_attestation(&mut dag_node_state) {
        warn!(
            "Failed to refresh local checkpoint attestation on startup: {}",
            e
        );
    }
    let startup_gossip = local_vote_gossip_payload(&dag_node_state);
    ...
    // ── 5a. Crash-Safe Recovery: WAL scan + rollback (fail-closed) ──
    {
        use misaka_storage::dag_recovery::{self, RecoveryMode, RecoveryDecision};

        // Validator mode → maximally strict recovery.
        // Any anomaly = refuse to start (prevents corrupt state consensus).
        let recovery_mode = if role.produces_blocks() {
            RecoveryMode::Validator
        } else {
            RecoveryMode::Relay
        };

        let decision = dag_recovery::bootstrap(
            std::path::Path::new(&cli.data_dir),
            recovery_mode,
        );
        match decision {
            RecoveryDecision::SnapshotClean { rollback_report } => {
                info!(
                    "Layer 5: DAG recovery — WAL clean ({} committed entries)",
                    rollback_report.committed_count
                );
            }
            RecoveryDecision::SnapshotWithRollback { rollback_report } => {
                warn!(
                    "Layer 5: DAG recovery rolled back {} incomplete block(s), {} committed",
                    rollback_report.rolled_back.len(),
                    rollback_report.committed_count,
                );
            }
            RecoveryDecision::FreshStart => {
                info!("Layer 5: DAG recovery — fresh start (no existing state)");
            }
            RecoveryDecision::Fatal { error } => {
                error!("╔═══════════════════════════════════════════════════════════╗");
                error!("║  DAG RECOVERY FAILED — NODE CANNOT START SAFELY         ║");
                error!("╚═══════════════════════════════════════════════════════════╝");
                error!("Reason: {}", error);
                error!("Recovery mode: {:?}", recovery_mode);
                if recovery_mode == RecoveryMode::Validator {
                    error!("Validator mode: refusing to start with potentially corrupt state.");
                    error!("Options: (1) restore from backup, (2) delete data dir and re-sync");
                }
                std::process::exit(1);
            }
        }
    ...
    // ── 5b. P2P Event Loop ──
    let (p2p_event_loop, p2p_inbound_tx, mut p2p_outbound_rx) =
    ...
    // Spawn outbound message consumer (placeholder — connects to transport layer)
    let _outbound_handle = tokio::spawn(async move {
        while let Some(event) = p2p_outbound_rx.recv().await {
            // In production, this sends the message over the TCP/QUIC transport.
            // For now, log it for debugging.
            let target = event.peer_id
    ...
    loop {
        ...
        if finality.should_checkpoint(max_score) {
            ...
            let cp = finality.create_checkpoint(
                // Use the highest blue-score tip from the current DAG snapshot.
                checkpoint_tip,
                max_score,
...
mod tests {
    ...
    async fn test_remote_vote_gossip_forms_live_local_quorum_when_checkpoint_matches() {
        ...
        let server = tokio::spawn(async move {
            crate::dag_rpc::run_dag_rpc_server(server_state, addr)
                .await
        ...
        assert_eq!(
            chain_info["validatorAttestation"]["currentCheckpointStatus"]["explorerConfirmationLevel"],
