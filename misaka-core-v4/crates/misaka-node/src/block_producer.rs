...
fn is_explicit_zk_candidate(_tx: &UtxoTransaction) -> bool {
    false
}
            ...
            Ok(result) => {
                ...
                for vtx in &block.transactions {
                    ...
                    for inp in &tx.inputs {
                        s.mempool.mark_spent(inp.key_image);
                        // v4 Q-DAG-CT: key_image field carries the nullifier
                        if tx.is_qdag() {
