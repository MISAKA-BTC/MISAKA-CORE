...
pub struct NotificationProcessor {
    ...
    _address_tracker: Arc<parking_lot::RwLock<AddressTracker>>,
    _filters: Vec<Box<dyn NotificationFilter + Send + Sync>>,
        ...
        match &notification.payload {
            NotificationPayload::UtxosChanged(utxo_notif) => {
                ...
                utxo_notif
                    .added
                    .iter()
                    .chain(utxo_notif.removed.iter())
                    .any(|entry| {
                        tracker
                            .listeners_for_address(&entry.address)
...
impl RateLimitFilter {
    pub fn new() -> Self {
        ...
        limits.insert(
            ...
            (60, std::time::Duration::from_secs(60)),
        );
        limits.insert(
        ...
        limits.insert(
            ...
            (30, std::time::Duration::from_secs(60)),
        );
        Self {
            limits,
...
impl NotificationProcessor {
    pub fn new(
        ...
        Self {
            broadcaster,
            _address_tracker: address_tracker,
            _filters: vec![
...
pub fn build_utxos_changed_notification(
    ...
    Notification {
        ...
        payload: NotificationPayload::UtxosChanged(UtxosChangedNotification { added, removed }),
    }
}
