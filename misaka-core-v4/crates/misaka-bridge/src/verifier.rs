...
//! - MockVerifier is ONLY available behind `dev-bridge-mock` feature.
...
use misaka_pqc::pq_sign::{
    ml_dsa_verify_raw, MlDsaPublicKey, MlDsaSignature, ML_DSA_PK_LEN, ML_DSA_SIG_LEN,
};
    ...
    pub fn new(
        ...
        for (i, m) in members.iter().enumerate() {
            ...
            if m.pk_hash != expected_hash {
                return Err(format!(
                    "member[{}] pk_hash does not match public_key hash",
                    i
                ));
            }
        }
