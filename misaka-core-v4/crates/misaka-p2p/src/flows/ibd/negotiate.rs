...
pub async fn negotiate_chain_segment(
    ...
    let mut syncer_pruning_point = *locator
        .last()
        .expect("INVARIANT: locator.is_empty() returned Err above; last() must succeed");
    ...
    loop {
        ...
        if !zoom_locator.is_empty() {
            ...
            syncer_pruning_point = *locator
                .last()
                .expect("INVARIANT: locator non-empty guaranteed by initial_len > 0 check");
        }
    }
}
