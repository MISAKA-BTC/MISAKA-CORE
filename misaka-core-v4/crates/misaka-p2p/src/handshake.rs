...
fn build_transcript(
    ...
    // HIGH-T6-1 (SEC-HS-BIND): The responder public key MUST NOT be zero.
    // A zero responder_pk means no identity is bound into the transcript,
    // allowing any attacker to impersonate the responder (MITM).
    // NOTE: initiator_pk is intentionally zero here — the initiator's identity
    // is proven via ML-DSA-65 signature over the transcript (verified in step 7).
    if responder_pk.is_zero() {
        return Err(
            "SEC-HS-BIND: responder public key must not be zero (identity binding required)"
                .to_string(),
        );
    }
