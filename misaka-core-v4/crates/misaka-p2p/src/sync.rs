...
impl IbdEngine {
    ...
    pub fn on_bodies_received(
        ...
        let next_batch: Vec<Hash> = remaining.drain(..remaining.len().min(IBD_BODY_BATCH)).collect();

        if next_batch.is_empty() && in_flight.is_empty() {
            // All bodies fetched and applied
            self.phase = IbdPhase::SteadyRelay;
            actions.push(IbdAction::CommitIbdState);
            actions.push(IbdAction::Log {
                level: LogLevel::Info,
                message: format!("IBD complete: {} bodies applied, entering relay mode", bodies_applied),
            });
