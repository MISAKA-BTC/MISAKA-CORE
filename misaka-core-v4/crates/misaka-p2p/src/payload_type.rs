...
/// covering MISAKA's additional PQ features.
...
pub enum MisakaPayloadType {
    ...
    DecoySet = 0x0602,
    ShieldedSpendTagBroadcast = 0x0603,
    ...
    // ── Narwhal Block Relay ──
    NarwhalBlockProposal = 0x0900,
    NarwhalBlockRequest = 0x0901,
    NarwhalBlockResponse = 0x0902,
    NarwhalCommitVote = 0x0903,
}
...
impl MisakaPayloadType {
    ...
    pub fn name(self) -> &'static str {
        match self {
            ...
            MisakaPayloadType::ShieldedSpendTagBroadcast => "ShieldedSpendTagBroadcast",
            ...
            MisakaPayloadType::NarwhalBlockProposal => "NarwhalBlockProposal",
            MisakaPayloadType::NarwhalBlockRequest => "NarwhalBlockRequest",
            MisakaPayloadType::NarwhalBlockResponse => "NarwhalBlockResponse",
            MisakaPayloadType::NarwhalCommitVote => "NarwhalCommitVote",
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn narwhal_block_request_is_marked_as_request() {
        assert!(MisakaPayloadType::NarwhalBlockRequest.is_request());
        assert!(!MisakaPayloadType::NarwhalBlockProposal.is_request());
    }

    #[test]
    fn narwhal_payload_types_keep_disconnect_overflow_policy() {
        assert_eq!(
            MisakaPayloadType::NarwhalBlockProposal.overflow_policy(),
            OverflowPolicy::Disconnect
        );
        assert_eq!(
            MisakaPayloadType::NarwhalBlockResponse.overflow_policy(),
            OverflowPolicy::Disconnect
        );
    }
}
...
impl MisakaMessage {
    ...
    pub fn reject(reason: &str) -> Self {
        Self::new(MisakaPayloadType::Reject, reason.as_bytes().to_vec())
    }
}
