...
pub trait DiscoveryBackend {
    ...
    ///
    /// # v5 change: `&mut self` — MUST update `last_gossip` timestamp.
    /// v4 used `&self` which meant the cooldown check worked but the
    /// timestamp was never updated, causing unlimited gossip floods.
    fn next_gossip_batch(&mut self, peer: &PeerId, limit: usize) -> Vec<PeerRecord>;
}
...
impl PeerStore {
    ...
    pub fn evict_expired(&mut self) {
        let now_unix = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        self.records.retain(|_, stored| stored.record.expires_at_unix > now_unix);
    }
...
impl DiscoveryBackend for PeerStore {
    ...
    fn ingest_peer_record(&mut self, record: PeerRecord) -> Result<bool, PeerRecordError> {
        ...
        if self.records.len() >= MAX_PEER_STORE_SIZE && !self.records.contains_key(&record.peer_id) {
            ...
            if let Some(oldest_id) = self.records.iter()
        ...
        self.records.insert(peer_id, StoredRecord {
            record,
    ...
    fn select_dial_candidates(&self, limit: usize) -> Vec<PeerRecord> {
        let mut candidates: Vec<&StoredRecord> = self.records.values()
            .filter(|s| {
        ...
        candidates.sort_by(|a, b| {
            a.dial_failures.cmp(&b.dial_failures)
    ...
    fn next_gossip_batch(&mut self, peer: &PeerId, limit: usize) -> Vec<PeerRecord> {
        // Rate limit gossip — prevent flood
        ...
        let batch: Vec<PeerRecord> = self.records.values()
            .filter(|s| {
        ...
        // v5 FIX: Actually mark the gossip timestamp.
        // v4 used &self so this was impossible → unlimited gossip floods.
        if !batch.is_empty() {
            self.last_gossip.insert(*peer, Instant::now());
        }
...
mod tests {
    ...
    use crate::peer_record::{CapabilityFlags, PeerRoles, NetworkAddress};
    ...
    fn make_record(chain_id: u32) -> PeerRecord {
        ...
        PeerRecord::create_signed(
            ...
            &kp.secret_key,
            vec![NetworkAddress { protocol: "tcp".into(), addr: "1.2.3.4:6690".into() }],
    ...
    fn test_peer_store_dedup_by_peer_id() {
        ...
        let r1 = PeerRecord::create_signed(
            &kp.public_key.to_bytes(), &kp.secret_key,
            vec![], CapabilityFlags::RELAY, PeerRoles::full_node(),
            2, "test".into(), 0, now_unix(), 3600,
        ).expect("signed");
        let r2 = PeerRecord::create_signed(
            &kp.public_key.to_bytes(), &kp.secret_key,
            vec![], CapabilityFlags::RELAY, PeerRoles::full_node(),
            2, "test".into(), 0, now_unix() - 10, 3600, // older
        ).expect("signed");
    ...
    fn test_dial_candidates_exclude_connected() {
        ...
        assert!(candidates.is_empty(), "connected peers should not be dial candidates");
    }
