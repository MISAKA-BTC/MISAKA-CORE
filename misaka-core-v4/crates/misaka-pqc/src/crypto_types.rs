...
impl PublicNullifier {
    ...
    pub fn from_raw(bytes: [u8; 32]) -> Self { Self(bytes) }
}
...
impl CommittedLeaf {
    pub fn as_poly(&self) -> &Poly { &self.0 }
    pub fn into_poly(self) -> Poly { self.0 }
}
...
impl SecretWitness {
    ...
    pub fn expose(&self) -> &[u8] { &self.data }
}
...
impl fmt::Debug for SecretWitness {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "SecretWitness({}: [REDACTED {} bytes])", self.label, self.data.len())
    }
}
...
impl SharedSecret {
    pub fn new(data: [u8; 32]) -> Self { Self { data } }

    /// Access the shared secret. Use ONLY for key derivation (HKDF).
    pub fn expose(&self) -> &[u8; 32] { &self.data }
}
...
impl VerifiedNullifier {
    ...
    pub(crate) fn new_verified(nullifier: PublicNullifier) -> Self {
        Self { nullifier, _verified: () }
    }

    pub fn nullifier(&self) -> &PublicNullifier { &self.nullifier }
    pub fn into_nullifier(self) -> PublicNullifier { self.nullifier }
}
