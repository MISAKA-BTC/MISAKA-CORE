...
pub const MAX_MEMBERSHIP_PROOF_SIZE: usize = PROOF_V2_FIXED_SIZE + MAX_MERKLE_DEPTH * LEVEL_WIRE_SIZE;
...
impl ZkMerkleLevelV2 {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        if data.len() != LEVEL_WIRE_SIZE {
            return Err(CryptoError::RingSignatureInvalid(
                format!("ZkMerkleLevelV2: expected {} bytes, got {}", LEVEL_WIRE_SIZE, data.len())));
        }
        let mut off = 0;
        let sib_commitment = BdlopCommitment::from_bytes(&data[off..off + N * 2])?; off += N * 2;
        let parent_commitment = BdlopCommitment::from_bytes(&data[off..off + N * 2])?; off += N * 2;
        let mut c0 = [0u8; 32];
        c0.copy_from_slice(&data[off..off + 32]); off += 32;
        let mut c1 = [0u8; 32];
        c1.copy_from_slice(&data[off..off + 32]); off += 32;
        let z0 = Poly::from_bytes(&data[off..off + N * 2])?; off += N * 2;
        let z1 = Poly::from_bytes(&data[off..off + N * 2])?;
        debug_assert_eq!(off + N * 2, LEVEL_WIRE_SIZE);
        Ok(Self { sib_commitment, parent_commitment, c0, c1, z0, z1 })
    }
}
...
impl ZkMembershipProofV2 {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        ...
        if data.len() < PROOF_V2_FIXED_SIZE {
            return Err(CryptoError::RingSignatureInvalid(
                format!("ZkMembershipProofV2: too short ({} < {})", data.len(), PROOF_V2_FIXED_SIZE)));
        }
        if data.len() > MAX_MEMBERSHIP_PROOF_SIZE {
            return Err(CryptoError::RingSignatureInvalid(
                format!("ZkMembershipProofV2: exceeds max size ({} > {})", data.len(), MAX_MEMBERSHIP_PROOF_SIZE)));
        }
        ...
        if version != MEMBERSHIP_PROOF_VERSION {
            return Err(CryptoError::RingSignatureInvalid(
                format!("ZkMembershipProofV2: unknown version 0x{:02X} (expected 0x{:02X})",
                    version, MEMBERSHIP_PROOF_VERSION)));
        }

        // ── 2. Fixed fields ──
        let leaf_commitment = BdlopCommitment::from_bytes(&data[off..off + N * 2])?; off += N * 2;

        let mut sis_root_hash = [0u8; 32];
        sis_root_hash.copy_from_slice(&data[off..off + 32]); off += 32;

        // P0-2: root_poly as public input
        let root_poly = Poly::from_bytes(&data[off..off + N * 2])?; off += N * 2;

        let mut root_opening_challenge = [0u8; 32];
        root_opening_challenge.copy_from_slice(&data[off..off + 32]); off += 32;

        let root_opening_response = Poly::from_bytes(&data[off..off + N * 2])?; off += N * 2;

        // ── 3. Level count with bounds check ──
        let num_levels = u16::from_le_bytes([data[off], data[off + 1]]) as usize; off += 2;
        if num_levels > MAX_MERKLE_DEPTH {
            return Err(CryptoError::RingSignatureInvalid(
                format!("ZkMembershipProofV2: depth {} exceeds MAX_MERKLE_DEPTH {}", num_levels, MAX_MERKLE_DEPTH)));
        }
        ...
        if data.len() != expected_total {
            return Err(CryptoError::RingSignatureInvalid(
                format!("ZkMembershipProofV2: size mismatch (got {} expected {} for {} levels)",
                    data.len(), expected_total, num_levels)));
        }
...
mod tests {
    ...
    fn test_prove_verify_membership_v2() {
        ...
        let (proof, _r_leaf) = prove_membership_v2(
