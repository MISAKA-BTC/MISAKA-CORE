...
use crate::pq_kem::{
    ml_kem_decapsulate, ml_kem_encapsulate, MlKemPublicKey, MlKemSecretKey, ML_KEM_CT_LEN,
};
...
fn derive_keys_v2(
    shared_secret: &[u8; 32],
    tx_context: &[u8],
) -> Result<DerivedKeysV2, CryptoError> {
...
pub fn create_stealth_v2(
    ...
    tx_id: &[u8],
    output_index: u32,
) -> Result<(/* one_time_address */ [u8; 32], StealthPayloadV2), CryptoError> {
    // 1. KEM encapsulate
...
impl StealthScannerV2 {
    ...
    pub fn recover(
        ...
        // Check addr_commit
        if keys.addr_commit != payload.addr_commit {
            return Ok(None);
        }
        ...
        // Decrypt amount
        let amount_bytes =
            decrypt_aead(&keys.k_amount, &keys.nonce_bytes, &payload.amount_ct, &aad)?;
        if amount_bytes.len() != 8 {
            return Err(CryptoError::StealthPayloadTooShort {
                min: 8,
                got: amount_bytes.len(),
            });
        }
...
mod tests {
    ...
    fn test_stealth_v2_roundtrip() {
        ...
        let (ota, payload) =
            create_stealth_v2(&kp.public_key, amount, Some(b"hello bob"), &tx_id, 0).unwrap();
    ...
    fn test_stealth_v2_wire_size() {
        let kp = ml_kem_keygen().unwrap();
        let (_, payload) =
            create_stealth_v2(&kp.public_key, 1000, Some(b"test"), &[0; 32], 0).unwrap();
    ...
    fn test_stealth_v2_tampered_memo_ct() {
        ...
        let tx_id = [0x66; 32];
        let (_, mut payload) =
            create_stealth_v2(&kp.public_key, 1000, Some(b"secret memo"), &tx_id, 0).unwrap();
    ...
    fn test_quick_scan_is_optimization_only() {
        ...
        let tx_id = [0xAB; 32];
        let (_, payload) =
            create_stealth_v2(&kp.public_key, 5000, Some(b"test"), &tx_id, 0).unwrap();
        let scanner = StealthScannerV2::new(kp.secret_key);
        ...
        assert!(
            full.is_some(),
            "recover must succeed after positive quick_scan"
        );
    ...
    fn test_stealth_v2_scan_tag_collision_resistance() {
        ...
        assert_eq!(
            unique.len(),
            10,
            "scan_tag collision detected among 10 outputs"
        );
    }
}
