...
use misaka_pqc::{
    ...
    pq_ring::{
        derive_public_param, derive_secret_poly, compute_pubkey,
        Poly, SpendingKeypair, DEFAULT_A_SEED, N, Q,
    },
    // BDLOP commitments (Module-SIS hardness)
    bdlop::{BdlopCrs, BdlopCommitment, BlindingFactor, BalanceExcessProof},
    // Nullifier (algebraic: a_null · s, ring-independent)
    nullifier::{OutputId, compute_nullifier, canonical_nullifier_hash},
    ...
    membership::{SisMerkleCrs, sis_leaf, compute_sis_root, sis_root_hash},
...
use misaka_mempool::{UtxoMempool, MempoolError};
use misaka_storage::utxo_set::UtxoSet;
use misaka_consensus::zkp_budget::{
    ZkpVerificationBudget, BudgetError,
    COST_UNIFIED_MEMBERSHIP, COST_RANGE_PROOF, COST_BALANCE_EXCESS,
    MAX_BLOCK_VERIFICATION_UNITS,
};
use misaka_consensus::peer_scoring::{PeerScoring, PenaltyReason};
use misaka_dag::dag_state_manager::{
    DagStateManager, OrderedBlockData, OrderedTxData, UtxoAction, TxApplyStatus,
};
...
impl WalletSimulator {
    ...
    fn pubkey(&self) -> &Poly { &self.spending.public_poly }
    fn secret(&self) -> &Poly { &self.spending.secret_poly }
    ...
    fn build_zkp_transfer_tx(
        ...
        let (nullifier_hash, _null_poly) = compute_nullifier(
            self.secret(), output_id, CHAIN_ID,
        );

        // ── 2. BDLOP Commitments ──
        let r_in = BlindingFactor::random();
        let c_in = BdlopCommitment::commit(crs, &r_in, GENESIS_AMOUNT);

        let r_send = BlindingFactor::random();
        let c_send = BdlopCommitment::commit(crs, &r_send, send_amount);

        let r_change = BlindingFactor::random();
        let c_change = BdlopCommitment::commit(crs, &r_change, change_amount);
        ...
        let leaf_polys: Vec<Poly> = all_pks.iter()
        ...
        let leaf_hashes: Vec<[u8; 32]> = leaf_polys.iter()
            .map(|p| {
                let mut h = [0u8; 32];
        ...
        let r_excess_poly = r_in.as_poly()
            .sub(r_send.as_poly())
...
impl TestNode {
    ...
    fn submit_tx(&mut self, tx: &QdagTransaction) -> Result<[u8; 32], MempoolError> {
        ...
        self.mempool.admit(utxo_tx, &self.utxo_set, self.block_counter * 1000)
    }
    ...
    fn produce_and_apply_block(
        ...
        let ordered_txs: Vec<OrderedTxData> = txs.iter().map(|tx| {
            OrderedTxData {
                tx_hash: tx.tx_hash(),
                key_images: vec![],
                nullifiers: tx.nullifiers(),
                is_coinbase: tx.tx_type == QdagTxType::Coinbase,
                outputs: tx.outputs.iter().enumerate().map(|(i, out)| {
                    TxOutput {
        ...
        let results = self.state_manager.apply_ordered_transactions(
            &[block],
            |action| {
                match action {
                    UtxoAction::CreateOutput { tx_hash, block_hash, output_index, output } => {
    ...
    UtxoTransaction {
        ...
        inputs: tx.inputs.iter().map(|inp| {
            RingInput {
                ...
                ring_signature: inp.membership_proof.clone(),
...
fn setup_genesis() -> (TestNode, WalletSimulator, WalletSimulator, Vec<WalletSimulator>, OutputId) {
    ...
    for (i, d) in decoys.iter().enumerate() {
        ...
        node.utxo_set.register_spending_key(outref, d.pubkey().to_bytes());
    }
...
fn test_lattice_zkp_transfer_success() {
    ...
    let built = alice.build_zkp_transfer_tx(
        &alice_output_id,
        &decoy_pks,
        SEND_AMOUNT,
        CHANGE_AMOUNT,
    ...
    assert_eq!(built.nullifier, null_again,
    ...
    let tx_hash = node.submit_tx(&built.tx)
    ...
    assert!(node.state_manager.is_key_image_spent(&built.nullifier),
    ...
    verify_confidential_fee(&crs, &built.tx.fee)
        .expect("confidential fee proofs must verify");
...
fn test_lattice_nullifier_double_spend_rejected() {
    ...
    // ── First TX: succeeds ──
    let built = alice.build_zkp_transfer_tx(
        &alice_output_id, &decoy_pks, SEND_AMOUNT, CHANGE_AMOUNT, FEE_AMOUNT,
    ).unwrap();

    node.submit_tx(&built.tx).expect("first TX must be accepted");

    // ── Second TX with SAME nullifier: must be rejected at O(1) check ──
    let built2 = alice.build_zkp_transfer_tx(
        &alice_output_id, &decoy_pks, SEND_AMOUNT, CHANGE_AMOUNT, FEE_AMOUNT,
    ).unwrap();

    // Same key + same output_id = same nullifier (deterministic)
    assert_eq!(built.nullifier, built2.nullifier,
        "same (secret, output_id, chain_id) must produce identical nullifier");

    let err = node.submit_tx(&built2.tx)
    ...
    match &err {
        MempoolError::NullifierConflict(msg) => {
            assert!(msg.contains("in mempool"),
                "must be caught by O(1) mempool nullifier check, got: {}", msg);
        }
        other => panic!(
            "expected NullifierConflict, got {:?} — \
             double spend must be caught BEFORE ZKP verification (cheap check)", other
        ),
    }
    ...
    let built3 = alice.build_zkp_transfer_tx(
        &alice_output_id, &decoy_pks, SEND_AMOUNT, CHANGE_AMOUNT, FEE_AMOUNT,
    ).unwrap();
    let err3 = node.submit_tx(&built3.tx)
        .expect_err("post-confirmation double-spend must be rejected");

    match &err3 {
        MempoolError::NullifierConflict(msg) => {
            assert!(msg.contains("spent on-chain"),
                "must be caught by chain-state nullifier check, got: {}", msg);
        }
...
fn test_invalid_lattice_range_proof_rejected() {
    ...
    let c_inflated = BdlopCommitment::commit(&crs, &r_inflated, 2_000);
    ...
    let r_in = BlindingFactor::random();
    let c_in = BdlopCommitment::commit(&crs, &r_in, 1_000);
    ...
    let balance_diff = BdlopCommitment(
        c_in.0.sub(&c_inflated.0).sub(&conf_fee.commitment.0)
    );
    ...
    match proof_result {
        Ok(proof) => {
            ...
            let verify_result = misaka_pqc::bdlop::verify_balance_with_excess(
                &crs, &balance_diff, &proof,
            );
...
fn test_block_lattice_zkp_budget_exceeded() {
    ...
    for tx_idx in 0..300 {
        ...
        match budget.charge_confidential_tx(2, 3) {
            ...
            Err(BudgetError::UnitsExceeded { consumed, max }) => {
                ...
                exceeded_at_tx = tx_idx;
                assert!(consumed > max,
                    "consumed {} must exceed max {}", consumed, max);
                break;
            }
            Err(e) => {
                budget_exceeded = true;
    ...
    assert!(exceeded_at_tx < 300,
        "budget must be exceeded before processing all 300 TXs (got: {})", exceeded_at_tx);
    ...
    assert!(peer_scoring.should_disconnect(&proposer),
        "repeated budget violator must be disconnected (score: {})", new_score2);
    assert!(peer_scoring.is_banned(&proposer),
        "repeated budget violator must be banned");

    // Verify budget summary
    let summary = budget.summary();
    assert!(summary.units_consumed > 0, "some units must have been consumed");
    assert!(summary.proofs_verified > 0, "some proofs must have been counted");

    // Verify that a normal block (100 txs, 1 input, 2 outputs) would pass
    let mut normal_budget = ZkpVerificationBudget::new();
    for _ in 0..100 {
        normal_budget.charge_confidential_tx(1, 2)
            .expect("normal block (100 × 1in+2out) must fit within budget");
    }
    let normal_summary = normal_budget.summary();
    assert!(normal_summary.units_consumed <= MAX_BLOCK_VERIFICATION_UNITS,
        "normal block must be within budget: {} <= {}",
        normal_summary.units_consumed, MAX_BLOCK_VERIFICATION_UNITS);

    println!("[Scenario D] ✅ Budget exceeded at TX #{}, proposer penalized & banned", exceeded_at_tx);
}
...
fn test_nullifier_hash_consistency_across_proof_paths() {
    let alice = WalletSimulator::new();
    let output_id = OutputId { tx_hash: [0xAA; 32], output_index: 0 };
    ...
    assert_ne!(null_direct, null_other_chain,
