...
use misaka_pqc::bdlop::{
    BdlopCommitment, BdlopCrs, BlindingFactor, BalanceExcessProof, compute_balance_diff,
};
...
use misaka_pqc::qdag_tx::{
    ConfidentialInput, ConfidentialOutput, ConfidentialStealthData,
    QdagTransaction, QdagTxType, QDAG_VERSION,
};
use misaka_pqc::range_proof::prove_range;
use misaka_pqc::unified_zkp::unified_prove;
use misaka_pqc::membership::{SisMerkleCrs, sis_leaf, compute_sis_root, sis_root_hash};
use std::fs;
...
pub async fn run(
    ...
    println!("   Input:  [HIDDEN] ({} MISAKA from {}:{})",
        input_amount,
        &input_utxo.tx_hash[..16], input_utxo.output_index);
    if change > 0 {
    ...
    let ring_data = fetch_anonymity_set(
        &client, &input_utxo, CT_RING_SIZE, &sis_crs,
    ).await?;
    ...
    let (nullifier_hash, _null_poly) = compute_nullifier(
        &spending.secret_poly, &output_id, chain_id,
    );
    ...
    let c_change = r_change.as_ref().map(|r| BdlopCommitment::commit(&crs, r, change));
    ...
    let rp_change = if let (Some(ref r_ch), Some(ch_val)) = (&r_change, if change > 0 { Some(change) } else { None }) {
    ...
    let recipient_kem_pk_bytes = hex::decode(recipient_ml_kem_pk_hex)
        .context("invalid recipient ML-KEM public key hex")?;
    let recipient_kem_pk = MlKemPublicKey::from_bytes(&recipient_kem_pk_bytes)
    ...
    let submit_body = serde_json::json!({
        ...
        "balanceProof": hex::encode(balance_proof.to_bytes()),
...
async fn query_spendable_utxo(
    ...
    let resp = client.post_json("/api/get_address_outputs", &serde_json::json!({
        "address": address,
    })).await?;

    let outputs = resp["outputs"].as_array()
    ...
    for o in outputs {
        ...
        if amount >= min_amount {
            ...
            return Ok(SpendableUtxo { tx_hash, tx_hash_bytes, output_index, amount });
        }
    }
...
async fn fetch_anonymity_set(
    ...
    let resp = client.post_json("/api/get_anonymity_set", &serde_json::json!({
    ...
    Ok(AnonymitySetData { leaf_hashes, signer_index, root_hash })
}
