...
pub fn cheap_size_gate(tx: &UtxoTransaction) -> Result<(), MempoolError> {
    // Check total serialized size (approximate — uses ring_signature + ki_proof + extra + zk_proof)
    let mut total_bytes: usize = 0;
    for inp in &tx.inputs {
        total_bytes = total_bytes.saturating_add(inp.ring_signature.len());
    ...
    for out in &tx.outputs {
        if let Some(ref stealth) = out.pq_stealth {
            ...
            total_bytes = total_bytes.saturating_add(stealth.encrypted_payload.len());
        }
    }
    ...
    for (i, inp) in tx.inputs.iter().enumerate() {
        // In v4, ring_signature carries the UnifiedMembershipProof bytes
        let proof_len = inp.ring_signature.len();
...
mod tests {
    ...
    use misaka_types::utxo::{
        OutputRef, RingInput, TxOutput, UtxoTransaction, UTXO_TX_VERSION_V4,
    };
    ...
    fn sample_v4_tx() -> UtxoTransaction {
        UtxoTransaction {
            ...
            inputs: vec![RingInput {
                ...
                ring_signature: vec![0u8; 4096], // Simulated proof
                key_image: [9u8; 32],
                ki_proof: vec![],
            }],
    ...
    fn test_zkp_pipeline_has_cheap_gate_before_zkp() {
        ...
        let gate_idx = stages.iter().position(|s| *s == MempoolAdmissionStage::CheapSizeGate).unwrap();
        let zk_idx = stages.iter().position(|s| *s == MempoolAdmissionStage::ZeroKnowledgeProof).unwrap();
        assert!(gate_idx < zk_idx, "cheap gate must come before ZKP verification");
    }
    ...
    fn test_cheap_size_gate_rejects_oversized_proof() {
        ...
        tx.inputs[0].ring_signature = vec![0u8; MAX_PROOF_BYTES_PER_INPUT + 1];
    ...
    fn test_nullifier_conflict_before_zkp() {
        let stages = mempool_admission_pipeline(PrivacyBackendFamily::ZeroKnowledge);
        let null_idx = stages.iter().position(|s| *s == MempoolAdmissionStage::NullifierConflict).unwrap();
        let zk_idx = stages.iter().position(|s| *s == MempoolAdmissionStage::ZeroKnowledgeProof).unwrap();
