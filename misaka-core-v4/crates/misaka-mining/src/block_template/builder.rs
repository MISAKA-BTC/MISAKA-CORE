...
use super::{BlockTemplate, CoinbaseData, TemplateBuildMode, TemplateTransaction};
...
impl BlockTemplateBuilder {
    ...
    pub fn build(
        ...
        if total_mass > self.policy.max_block_mass {
            return Err(MiningError::TemplateBuildFailed(format!(
                "block mass {} exceeds limit {}",
                total_mass, self.policy.max_block_mass
            )));
        }
    ...
    pub fn modify_coinbase(template: &BlockTemplate, new_coinbase: CoinbaseData) -> BlockTemplate {
        ...
        new.header_hash =
            compute_template_hash(&new.parent_hashes, new.timestamp, new.daa_score, new.bits);
        new
    }
