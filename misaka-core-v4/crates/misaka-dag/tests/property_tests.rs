        ...
        while !remaining.is_empty() {
            ...
            for &idx in &remaining {
                ...
                if parent_ids.is_empty() {
                    ...
                    let data = engine.calculate(&hash, &parent_hashes, &store, &reach, &stake);
...
fn tiebreak_determinism_under_insertion_order() {
    ...
    for seed in 0..50u64 {
        ...
        for &id in &block_ids {
            ...
            let data = engine.calculate(&hash, &[genesis_hash], &store, &reach, &stake);
            store.set_ghostdag_data(hash, data);
        }
        ...
        let data = engine.calculate(&merge_hash, &used_parents, &store, &reach, &stake);
