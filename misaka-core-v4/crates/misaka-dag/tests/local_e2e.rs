...
use misaka_dag::{
    ...
    UniformStakeProvider, ThreadSafeDagStore, TxApplyStatus, VirtualBlock, ZERO_HASH,
};
...
fn make_zkmp_materialized_transfer_tx(
    ...
    let ring_members: Vec<OutputRef> = ring_pubkeys
        .iter()
        .enumerate()
        .map(|(idx, _)| OutputRef {
...
fn test_parallel_conflict_replay_and_checkpoint() {
    ...
    let a_data = ghostdag.try_calculate(&a_hash, &block_a.header.parents, &snapshot, &reachability, &stake).unwrap();
    ...
    let b_data = ghostdag.try_calculate(&b_hash, &block_b.header.parents, &snapshot, &reachability, &stake).unwrap();
    snapshot.set_ghostdag_data(b_hash, b_data.clone());
    dag_store.set_ghostdag(b_hash, b_data);

    let mut block_c = assemble_dag_block(&[a_hash, b_hash], vec![], [0x0C; 32], 1_700_000_002_000, misaka_dag::daa::INITIAL_BITS);
    ...
    let c_data = ghostdag.try_calculate(&c_hash, &block_c.header.parents, &snapshot, &reachability, &stake).unwrap();
...
fn test_snapshot_restore_preserves_checkpoint_and_state_root() {
    ...
    let mut block_c = assemble_dag_block(&[a_hash, b_hash], vec![], [0x1C; 32], 1_700_000_102_000, misaka_dag::daa::INITIAL_BITS);
    let c_hash = block_c.hash();
    dag_store
        .insert_block(c_hash, block_c.header.clone(), block_c.transactions.clone())
        .unwrap();
...
fn test_zkmp_materialized_parallel_conflict_replay_and_checkpoint() {
    ...
    let mut block_c = assemble_dag_block(&[a_hash, b_hash], vec![], [0x2C; 32], 1_700_000_202_000, misaka_dag::daa::INITIAL_BITS);
    let c_hash = block_c.hash();
    dag_store
        .insert_block(c_hash, block_c.header.clone(), block_c.transactions.clone())
        .unwrap();
