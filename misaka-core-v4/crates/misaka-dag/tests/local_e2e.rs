...
fn test_parallel_conflict_replay_and_checkpoint() {
    ...
    let a_data = ghostdag.calculate(&a_hash, &block_a.header.parents, &snapshot, &reachability, &stake);
    ...
    let b_data = ghostdag.calculate(&b_hash, &block_b.header.parents, &snapshot, &reachability, &stake);
    ...
    let c_data = ghostdag.calculate(&c_hash, &block_c.header.parents, &snapshot, &reachability, &stake);
