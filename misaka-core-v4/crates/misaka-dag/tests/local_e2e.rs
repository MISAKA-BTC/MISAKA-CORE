...
fn test_parallel_conflict_replay_and_checkpoint() {
    ...
    let mut block_c = assemble_dag_block(&[a_hash, b_hash], vec![], [0x0C; 32], 1_700_000_002_000);
...
fn test_snapshot_restore_preserves_checkpoint_and_state_root() {
    ...
    let mut block_c = assemble_dag_block(&[a_hash, b_hash], vec![], [0x1C; 32], 1_700_000_102_000);
    let c_hash = block_c.hash();
    dag_store
        .insert_block(c_hash, block_c.header.clone(), block_c.transactions.clone())
        .unwrap();
...
fn test_zkmp_materialized_parallel_conflict_replay_and_checkpoint() {
    ...
    let mut block_c = assemble_dag_block(&[a_hash, b_hash], vec![], [0x2C; 32], 1_700_000_202_000);
    let c_hash = block_c.hash();
    dag_store
        .insert_block(c_hash, block_c.header.clone(), block_c.transactions.clone())
        .unwrap();
