...
pub enum HeaderCheckResult {
    ...
    DuplicateParent,
    FutureTimestamp { block_ms: u64, now_ms: u64 },
    TxRootMismatch { declared: [u8; 32], computed: [u8; 32] },
    ProposerNotEligible([u8; 32]),
    ProposerSignatureInvalid,
    BluScoreMismatch { declared: u64, computed: u64 },
    ChainIdMismatch { header: u32, expected: u32 },
...
pub fn sort_by_tiebreak(
    hashes: &mut Vec<[u8; 32]>,
    get_key: impl Fn(&[u8; 32]) -> TieBreakKey,
) {
...
mod tests {
    ...
    fn sig_ok(_hash: &[u8; 32], _sig: &[u8]) -> bool { true }
    /// Helper: always-invalid signature verifier.
    fn sig_fail(_hash: &[u8; 32], _sig: &[u8]) -> bool { false }

    #[test]
    fn test_valid_header() {
        let result = validate_header_full(
            &[0xAA; 32], EXPECTED_VERSION, &[[0xBB; 32]], 1000,
            &[0xCC; 32], &[0xCC; 32], // tx_root matches
            &[0xDD; 32], b"sig", // proposer with sig
            5, 5, // blue_score matches
            2, 2, // chain_id matches
            2000, false, true, |_| true,
            &sig_ok,
        );
    ...
    fn test_proposer_sig_cryptographically_verified() {
        ...
        let result = validate_header_full(
            &[0xAA; 32], EXPECTED_VERSION, &[[0xBB; 32]], 1000,
            &[0xCC; 32], &[0xCC; 32],
            &[0xDD; 32], b"sig_bytes_that_look_fine",
            5, 5, 2, 2, 2000, false, true, |_| true,
            &sig_fail,
        );
        assert!(matches!(result, HeaderCheckResult::ProposerSignatureInvalid),
    ...
    fn test_tx_root_mismatch_rejected() {
        let result = validate_header_full(
            &[0xAA; 32], EXPECTED_VERSION, &[[0xBB; 32]], 1000,
            &[0xCC; 32], &[0xDD; 32], // MISMATCH
            &[0xDD; 32], b"sig", 5, 5, 2, 2, 2000, false, true, |_| true,
            &sig_ok,
        );
    ...
    fn test_blue_score_mismatch_rejected() {
        let result = validate_header_full(
            ...
            &[0xCC; 32], &[0xCC; 32],
            &[0xDD; 32], b"sig",
            10, 5, // declared 10, computed 5 — MISMATCH
            2, 2, 2000, false, true, |_| true,
            &sig_ok,
        );
