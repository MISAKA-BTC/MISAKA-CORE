...
impl DaaWindow {
    ...
    pub fn build<S: DagStore>(tip: &Hash, store: &S, window_size: u64) -> Self {
        ...
        if let Some(header) = store.get_header(&current) {
            let blue_score = store.get_ghostdag_data(&current)
                .map(|d| d.blue_score)
                .unwrap_or(0);
        ...
        loop {
            ...
            if let Some(data) = store.get_ghostdag_data(&current) {
                ...
                if let Some(header) = store.get_header(&sp) {
                    let sp_score = store.get_ghostdag_data(&sp)
                        .map(|d| d.blue_score)
                        .unwrap_or(0);
                    blocks.push(DaaWindowBlock {
                        hash: sp,
    ...
    pub fn average_interval(&self) -> u64 {
        ...
        if count == 0 { return TARGET_BLOCK_INTERVAL_MS; }
        span / count
    }
}
...
pub fn compute_bounded_past_median_time<S: DagStore>(
    parents: &[Hash],
    store: &S,
) -> u64 {
    if parents.is_empty() {
    ...
    if window.len() >= 3 {
        let mut timestamps: Vec<u64> = window.blocks.iter()
...
pub fn compute_direct_parents_median<S: DagStore>(
    parents: &[Hash],
    store: &S,
) -> u64 {
    if parents.is_empty() {
...
pub fn compute_proposer_cadence_ms<S: DagStore>(
    tip: &Hash,
    store: &S,
) -> u64 {
    let (actual_interval, block_count) = compute_block_rate(tip, store, DAA_WINDOW_SIZE);
...
mod tests {
    ...
    fn test_timestamp_valid() {
        ...
        store.set_ghostdag_data(h(1), GhostDagData {
