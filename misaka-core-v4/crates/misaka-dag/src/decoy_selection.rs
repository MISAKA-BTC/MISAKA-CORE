...
//! - Selection was O(ring_size) per transaction
//!
...
use crate::ghostdag::{DagStore, MIN_DECOY_DEPTH};
...
pub fn select_anonymity_set(
    ...
    let candidates: Vec<&EligibleOutput> = eligible_outputs.iter()
    ...
    let real_output = eligible_outputs.iter()
    ...
    if real_output.chain_id != chain_id {
        return Err(DecoySelectionError::ChainIdMismatch {
            expected: chain_id, got: real_output.chain_id,
        });
    }
...
mod tests {
    ...
    fn make_eligible(id: u8, score: u64) -> EligibleOutput {
        EligibleOutput {
            tx_hash: [id; 32], output_index: 0, blue_score: score,
            spending_pubkey: vec![id; 512], commitment_bytes: vec![],
            chain_id: 2,
        }
    }
    ...
    fn test_select_anonymity_set_valid() {
        ...
        let (set, idx) = select_anonymity_set(
            &[0; 32], 0, 2, 200, &outputs,
        ).unwrap();
    ...
    fn test_no_age_bias() {
        ...
        let (set, _) = select_anonymity_set(
            &[0; 32], 0, 2, 200, &outputs,
        ).unwrap();
