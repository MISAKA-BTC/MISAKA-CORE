...
pub fn build_block_diff(
        ...
        if let Some(nf) = conflicting_nf {
            ...
            for inp in &tx.inputs {
                ...
                if let Some(source_ref) = inp.ring_members.first() {
                    if let Some(output) = virtual_state.get_utxo(source_ref) {
                        utxos_spent.push(SpentUtxo {
                            outref: source_ref.clone(),
                            output: output.clone(),
                            creation_tx_hash: source_ref.tx_hash,
                            creation_blue_score: 0, // TODO: retrieve from block metadata
    ...
    StateDiff {
        block_hash,
        blue_score,
        epoch: crate::daa::DaaScore(blue_score).epoch(), // v9: DAA-derived epoch
        nullifiers_added,
        utxos_created,
        utxos_spent,
        tx_results,
        ...
        for tx_result in &diff.tx_results {
            let status = match tx_result.status {
                DiffTxStatus::Applied | DiffTxStatus::Coinbase => {
                    crate::dag_state_manager::TxApplyStatus::Applied
                }
                DiffTxStatus::FailedNullifierConflict => {
                    crate::dag_state_manager::TxApplyStatus::FailedNullifierConflict {
                        conflicting_nullifier: tx_result.nullifiers.first().copied().unwrap_or(ZERO_HASH),
                        prior_tx_hash: ZERO_HASH, // 先行 TX の特定は resolve 結果から
                    }
                }
                DiffTxStatus::FailedInvalidProof => {
                    crate::dag_state_manager::TxApplyStatus::FailedInvalidSignature
                }
            };
            s.dag_store.set_tx_status(tx_result.tx_hash, status);
        }
