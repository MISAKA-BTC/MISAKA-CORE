...
use crate::state_diff::{StateDiff, CreatedUtxo, DiffTxResult, DiffTxStatus};
...
impl StoreWriteBatch {
    ...
    pub fn commit<S: PersistentDagBackend>(
        self,
        storage: &S,
    ) -> Result<(), AtomicPipelineError> {
        // ── Write header + ghostdag atomically ──
        if let (Some((hash, header)), Some((_, ghostdag))) = (&self.header, &self.ghostdag) {
            storage.insert_block_atomic(*hash, header.clone(), ghostdag.clone())
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "insert_block_atomic: {}", e
                )))?;
        }

        // ── Write nullifiers ──
        for (nullifier, tx_hash) in &self.nullifiers {
            storage.record_nullifier(*nullifier, *tx_hash)
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "record_nullifier: {}", e
                )))?;
        }
        ...
        if let Some((hash, ref diff)) = self.state_diff {
            let diff_json = serde_json::to_vec(diff)
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "serialize state_diff: {}", e
                )))?;
            storage.save_state_diff(hash, &diff_json)
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "save_state_diff: {}", e
                )))?;
        }
        ...
        if let Some((hash, ref data)) = self.acceptance {
            storage.save_acceptance(hash, data)
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "save_acceptance: {}", e
                )))?;
        }
        ...
        if let Some(ref snapshot) = self.virtual_snapshot {
            storage.save_virtual_snapshot(snapshot)
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "save_virtual_snapshot: {}", e
                )))?;
        }

        // ── Write last accepted ──
        if let Some(hash) = self.last_accepted {
            storage.save_last_accepted(hash)
                .map_err(|e| AtomicPipelineError::CommitFailed(format!(
                    "save_last_accepted: {}", e
                )))?;
        }
    ...
    W: StakeWeightProvider,
{
    ...
    if store.get_header(&block_hash).is_some() {
        ...
        return Err(AtomicPipelineError::Duplicate(
    ...
    validate_header_topology(&header.parents, header.blue_score, store)
        .map_err(|e| AtomicPipelineError::ValidationFailed(
            format!("header topology: {}", e),
        ))?;
    ...
    for (i, sealed_tx) in block.transactions.iter().enumerate() {
        if !sealed_tx.is_coinbase && sealed_tx.nullifiers.is_empty() {
            return Err(AtomicPipelineError::ValidationFailed(
                format!("tx[{}] ({}) is Transfer but has no nullifiers",
                    i, hex::encode(&sealed_tx.tx_hash[..4])),
            ));
        }
    }
    ...
    if !header.is_genesis() {
        ...
        match validate_timestamp(header.timestamp_ms, &header.parents, store, now_ms) {
            ...
            TimestampCheck::TooOld { block_ms, past_median_ms } => {
                return Err(AtomicPipelineError::BadTimestamp(
                    format!("{}ms < past_median {}ms", block_ms, past_median_ms),
                ));
            }
            TimestampCheck::TooFuture { block_ms, max_ms } => {
                return Err(AtomicPipelineError::BadTimestamp(
                    format!("{}ms > max_allowed {}ms", block_ms, max_ms),
                ));
            }
        }
    }

    debug!("Stage 1 passed: block {} validated", hex::encode(&block_hash[..4]));
    ...
    debug!("Stage 2 passed: reachability updated (parent={})",
    ...
    let ghostdag_data = engine.try_calculate(
        ...
        reachability,
        stake,
    ).map_err(|e| AtomicPipelineError::GhostDagFailed(
        format!("{}", e),
    ))?;
    ...
    debug!("Stage 3 passed: GhostDAG computed (blue_score={})", computed_blue_score);
    ...
    for tx_result in &diff.tx_results {
        if matches!(tx_result.status, DiffTxStatus::Applied | DiffTxStatus::Coinbase) {
    ...
    // この操作はメモリ上のみで完結する。
    virtual_state.apply_block(diff.clone())
        .map_err(|e| AtomicPipelineError::VirtualResolveFailed(
            format!("{}", e),
        ))?;
    ...
    let snapshot_json = serde_json::to_vec(&snapshot)
        .map_err(|e| AtomicPipelineError::VirtualResolveFailed(
            format!("serialize virtual snapshot: {}", e),
        ))?;
    batch.put_virtual_snapshot(snapshot_json);
    batch.put_last_accepted(block_hash);

    debug!("Stage 4 passed: StateDiff built ({} nullifiers, {} created, {} spent)",
        diff.nullifiers_added.len(), diff.utxos_created.len(), diff.utxos_spent.len());
...
fn build_qdag_block_diff(
    ...
    for sealed_tx in txs {
        if sealed_tx.is_coinbase {
            ...
            tx_results.push(DiffTxResult {
                tx_hash: sealed_tx.tx_hash,
                status: DiffTxStatus::Coinbase,
                nullifiers: vec![],
            });
            continue;
        }
        ...
        if conflicting {
            tx_results.push(DiffTxResult {
                tx_hash: sealed_tx.tx_hash,
                status: DiffTxStatus::FailedNullifierConflict,
                nullifiers: vec![],
            });
        } else {
            // TX accepted
            ...
            //
            // Nullifier に対応する UTXO を VirtualState の全 UTXO セットから
            // 検索する。Q-DAG-CT モデルでは nullifier は algebraic に
            // (secret, output_id) から決定論的に生成されるため、
            // ring member の概念は存在しない。
            //
            // 現時点では SealedTransaction から直接 source outpoint を
            // 取得する API がないため、utxos_spent は空になる場合がある。
            // フルの UTXO 消費追跡は dag_block_producer.rs の
            // build_block_diff (UtxoTransaction ベース) で行われる。
            //
            // TODO: SealedTransaction に source outpoints を追加し、
            // ここで完全な SpentUtxo を記録する。

            tx_results.push(DiffTxResult {
                tx_hash: sealed_tx.tx_hash,
                status: DiffTxStatus::Applied,
                nullifiers: sealed_tx.nullifiers.clone(),
            });
        }
    }
    ...
    StateDiff {
        block_hash,
        blue_score,
        epoch: crate::daa::DaaScore(blue_score).epoch(),
        nullifiers_added,
        utxos_created,
        utxos_spent,
        tx_results,
...
mod tests {
    ...
    fn test_write_batch_drop_discards() {
        ...
        let mut batch = StoreWriteBatch::new();
        batch.put_header([1; 32], DagBlockHeader {
            version: 1,
            parents: vec![],
            timestamp_ms: 0,
            tx_root: [0; 32],
    ...
    fn test_qdag_diff_nullifier_conflict() {
        ...
        assert_eq!(diff.tx_results[0].status, DiffTxStatus::FailedNullifierConflict);
        assert_eq!(diff.nullifiers_added.len(), 0, "conflicting nullifier must not be added");
        assert_eq!(diff.utxos_created.len(), 0, "conflicting TX must not create outputs");
    }
    ...
    fn test_qdag_diff_intra_block_conflict() {
        ...
        let tx1 = SealedTransaction::seal(
            [0x01; 32],
            vec![[0xAA; 32]],
            false, 1, vec![[0xDD; 32]], 1,
        );
        let tx2 = SealedTransaction::seal(
            [0x02; 32],
            vec![[0xAA; 32]], // same nullifier as tx1
            false, 1, vec![[0xEE; 32]], 1,
        );
        ...
        assert_eq!(diff.tx_results[0].status, DiffTxStatus::Applied, "first TX wins");
        assert_eq!(diff.tx_results[1].status, DiffTxStatus::FailedNullifierConflict,
    ...
    fn test_atomic_pipeline_rejects_duplicate() {
        ...
        let engine = GhostDagEngine::new(18, 256);
        let mut vs = VirtualState::new(genesis);

        // Set up genesis
        store.insert_header(genesis, DagBlockHeader {
            version: DAG_VERSION, parents: vec![], timestamp_ms: 0,
            tx_root: [0; 32], proposer_id: [0; 32], nonce: 0,
            blue_score: 0, bits: 0,
        });
        store.set_ghostdag_data(genesis, GhostDagData {
            selected_parent: ZERO_HASH, mergeset_blues: vec![],
            mergeset_reds: vec![], blue_score: 0, blue_work: 0,
        ...
        let block = QdagBlock::new(
            DagBlockHeader {
                version: DAG_VERSION, parents: vec![genesis],
                timestamp_ms: 1000, tx_root: [0; 32],
                proposer_id: [1; 32], nonce: 0, blue_score: 1,
        ...
        let result = process_new_block_atomic(
            &block, &store, &mut reach, &engine,
            &UniformStakeProvider, &mut vs,
        );

        assert!(matches!(result, Err(AtomicPipelineError::Duplicate(_))),
