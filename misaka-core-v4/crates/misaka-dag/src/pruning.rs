...
impl PruningManager {
    ...
    pub fn find_new_pruning_point<S: DagStore>(
        ...
        // Tips から canonical sort で最上位のブロックを取得
        let tips = store.get_tips();
        if tips.is_empty() {
            return None;
        }

        // 最高 blue_score の tip を探す (canonical sort の簡易版)
        let virtual_tip = tips.iter()
            .max_by_key(|t| {
                store.get_ghostdag_data(t).map(|d| d.blue_score).unwrap_or(0)
            })
            .copied()?;
