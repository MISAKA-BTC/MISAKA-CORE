...
impl DagSyncManager {
    ...
    pub fn on_remote_tips(&mut self, remote_tips: Vec<Hash>, remote_blue_score: u64) -> Vec<Hash> {
        let unknown: Vec<Hash> = remote_tips
            .iter()
    ...
    pub fn on_block_received(&mut self, block_hash: Hash, parents: &[Hash]) -> Vec<Hash> {
        self.local_known.insert(block_hash);

        // parents のうち未知のものを追加要求
        let new_requests: Vec<Hash> = parents
            .iter()
