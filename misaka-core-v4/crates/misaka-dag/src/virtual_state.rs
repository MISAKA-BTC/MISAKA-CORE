        ...
        let acceptance_data: Vec<BlockAcceptanceData> = new_diffs.iter()
            .map(|diff| BlockAcceptanceData {
                ...
                tx_results: diff.tx_results.iter().map(|tr| TxAcceptance {
                    tx_hash: tr.tx_hash,
                    accepted: matches!(tr.status, DiffTxStatus::Applied),
                    rejection_reason: match &tr.status {
                        DiffTxStatus::Applied => String::new(),
                        DiffTxStatus::DoubleSpend { conflicting_nullifier } =>
                            format!("double-spend: nullifier {}", hex::encode(&conflicting_nullifier[..8])),
                        DiffTxStatus::InvalidProof { reason } =>
                            format!("invalid proof: {}", reason),
                    },
                }).collect(),
            })
            .collect();
