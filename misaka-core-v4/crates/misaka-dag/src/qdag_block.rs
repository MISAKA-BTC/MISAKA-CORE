...
impl SealedTransaction {
    ...
    pub fn seal(
        ...
        Self {
            ...
            output_addresses,
            chain_id,
            tx_bytes: vec![], // Populated by caller if archival needed
        }
    }
}
...
impl QdagBlock {
    ...
    pub fn to_ordered_block(&self) -> super::dag_state_manager::OrderedBlockData {
        ...
        OrderedBlockData {
            ...
            transactions: self.transactions.iter().map(|sealed| {
                OrderedTxData {
                    ...
                    outputs: (0..sealed.output_count).map(|i| {
                        TxOutput {
                            amount: 0, // Confidential
                            one_time_address: sealed.output_addresses
                                .get(i as usize)
                                .copied()
                                .unwrap_or([0u8; 32]),
                            pq_stealth: None,
        ...
        let block = QdagBlock::new(
            ...
            vec![
                SealedTransaction::seal(
                    [1; 32], vec![[0xAA; 32], [0xBB; 32]], false, 2,
                    vec![[0x11; 32], [0x22; 32]], 2,
                ),
                SealedTransaction::seal(
                    [2; 32], vec![[0xCC; 32]], false, 1,
                    vec![[0x33; 32]], 2,
                ),
            ],
        );
    ...
    fn test_to_ordered_block_nullifiers_populated() {
        let block = QdagBlock::new(
            ...
            vec![SealedTransaction::seal(
                [1; 32], vec![[0xDD; 32]], false, 1, vec![[0x44; 32]], 2,
            )],
        );
        let ordered = block.to_ordered_block();
        assert_eq!(ordered.transactions[0].nullifiers, vec![[0xDD; 32]],
