...
use crate::parent_selection::{self, ParentSortKey, canonical_compare, sort_mergeset_canonical, mergeset_compare};
...
impl GhostDagV2 {
        ...
        W: StakeWeightProvider,
    {
        ...
        let ClassifyResult { blues, reds, blues_anticone_sizes } =
            self.classify_mergeset_spec(
    ...
    /// Non-fallible wrapper — **DEPRECATED**.
    ///
    /// On mergeset overflow, treats the block as having an empty mergeset
    /// and logs a warning. **All production code should use `try_calculate()`.**
    /// This wrapper exists only for backward-compatible test code.
    #[deprecated(note = "Use try_calculate() for Fail-Closed error handling. calculate() silently handles mergeset overflow.")]
    ...
    fn compute_mergeset_failclosed<S: DagStore>(
        ...
        while let Some(current) = queue.pop_front() {
            ...
            if reachability::is_dag_ancestor_conclusive(
                &current, selected_parent, reachability, store,
            )? {
                continue;
            }
    ...
    ///
    /// Returned by `classify_mergeset_spec()`.
    ...
    fn classify_mergeset_spec<S: DagStore>(
        ...
        for (i, blue_block) in blues.iter().enumerate() {
            ...
            // k-cluster 違反: 分類結果が不正 → このブロックは reject すべき
            // (通常は起こらないが、DAG が正しく構築されていれば k を超えない)
            if final_count > self.k {
                tracing::warn!(
                    "k-cluster violation: blue block {} has anticone size {} > k={}",
                    hex::encode(&blue_block[..4]),
                    final_count,
                    self.k,
                );
                // Note: Kaspa では k-cluster 違反は起こり得ないが、
                // 実装の健全性チェックとして検出する。
                // 違反ブロックを Red に降格することも可能だが、
                // 分類アルゴリズムの correctness 問題なので warn に留める。
            }
        }

        Ok(ClassifyResult { blues, reds, blues_anticone_sizes })
    }
...
mod tests {
    ...
    fn test_mergeset_discovers_side_branches() {
        ...
        let f_data = engine.try_calculate(&f, &[c, d], &store, &reach, &stake).unwrap();
    ...
    fn test_mergeset_overflow_returns_error() {
        ...
        for i in 1..=n {
            ...
            let data = engine.try_calculate(&h, &[g], &store, &reach, &stake).unwrap();
    ...
    fn test_diamond_dag_total_order() {
        ...
        store.insert_header(a, make_header(vec![g]));
        reach.add_child(g, a).unwrap();
        let a_d = engine.try_calculate(&a, &[g], &store, &reach, &stake).unwrap();
        store.set_ghostdag_data(a, a_d);

        store.insert_header(b, make_header(vec![g]));
        reach.add_child(g, b).unwrap();
        let b_d = engine.try_calculate(&b, &[g], &store, &reach, &stake).unwrap();
        ...
        let c_d = engine.try_calculate(&c, &[a, b], &store, &reach, &stake).unwrap();
