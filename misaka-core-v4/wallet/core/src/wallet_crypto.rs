...
///
/// SEC-FIX H-9: Debug is manually implemented to redact secret material.
/// The derived Debug would print raw key bytes in logs/panics.
...
pub fn encrypt_secrets(
    ...
    // SEC-FIX NM-7: Zeroize plaintext and key material
    plaintext.zeroize();
    key.zeroize();
...
pub fn decrypt_secrets(
    ...
    // SEC-FIX NM-6: Validate KDF params to prevent weakened key derivation.
    // An attacker who tampers with the envelope could set m_cost=1 to make
    // offline brute-force trivially fast.
    const MIN_M_COST: u32 = 16 * 1024; // 16 MiB minimum
    const MIN_T_COST: u32 = 2;
    if envelope.kdf_params.m_cost < MIN_M_COST {
        return Err(WalletCryptoError::InvalidFormat(format!(
            "KDF m_cost {} below minimum {} (possible tamper)",
            envelope.kdf_params.m_cost, MIN_M_COST
        )));
    }
    if envelope.kdf_params.t_cost < MIN_T_COST {
        return Err(WalletCryptoError::InvalidFormat(format!(
            "KDF t_cost {} below minimum {}",
            envelope.kdf_params.t_cost, MIN_T_COST
        )));
    }
    if salt.len() < 16 {
        return Err(WalletCryptoError::InvalidFormat(format!(
    ...
    // Decrypt
    let cipher =
        XChaCha20Poly1305::new_from_slice(&key).map_err(|_| WalletCryptoError::DecryptionFailed)?;
    let nonce = chacha20poly1305::XNonce::from_slice(&nonce_bytes);
    ...
    let secrets: WalletSecrets =
        serde_json::from_slice(&plaintext).map_err(|_| WalletCryptoError::DecryptionFailed)?;

    // SEC-FIX NM-7: Zeroize decrypted plaintext after deserialization
    plaintext.zeroize();
...
mod tests {
    ...
    fn test_unsupported_version_rejected() {
        ...
        assert!(matches!(
            result,
            Err(WalletCryptoError::UnsupportedVersion(99))
        ));
    }
}
