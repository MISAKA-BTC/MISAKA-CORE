...
impl BroadcastManagerState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, BroadcastManagerError> {
        ...
        let entry = BroadcastManagerEntry {
            ...
            created_at: now,
            updated_at: now,
            state: BroadcastManagerEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(BroadcastManagerTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), BroadcastManagerError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(BroadcastManagerError::NotFound(id))?;
        ...
        self.emit(BroadcastManagerEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BroadcastManagerError> {
        ...
        self.emit(BroadcastManagerEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<BroadcastManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    BroadcastManagerEntryState::Completed
                        | BroadcastManagerEntryState::Failed
                        | BroadcastManagerEntryState::Cancelled
                        | BroadcastManagerEntryState::Expired
        ...
        if removed > 0 {
            self.emit(BroadcastManagerEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> BroadcastManagerStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<BroadcastManagerEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = BroadcastManagerConfig {
            retry_count: 2,
            ..Default::default()
        };
