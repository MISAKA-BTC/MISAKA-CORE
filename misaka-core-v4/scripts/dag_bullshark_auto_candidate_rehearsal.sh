...
state_dir="${MISAKA_BULLSHARK_AUTO_CANDIDATE_REHEARSAL_DIR:-$repo_root/.tmp/dag-bullshark-auto-candidate-rehearsal}"
result_file="${MISAKA_BULLSHARK_AUTO_CANDIDATE_REHEARSAL_RESULT:-$state_dir/result.json}"
target_dir="${MISAKA_CARGO_TARGET_DIR:-$repo_root/.tmp/dag-bullshark-auto-candidate-target}"
log_file="$state_dir/cargo-test.log"
test_filter="live_bullshark_candidate_preview_auto_advances_after_runtime_commit"

usage() {
  cat <<'EOF'
Usage: ./scripts/dag_bullshark_auto_candidate_rehearsal.sh

Runs the confined Bullshark automatic candidate-preview rehearsal and writes:

  .tmp/dag-bullshark-auto-candidate-rehearsal/result.json

Optional env:
  MISAKA_BULLSHARK_AUTO_CANDIDATE_REHEARSAL_DIR=/custom/output/dir
  MISAKA_BULLSHARK_AUTO_CANDIDATE_REHEARSAL_RESULT=/custom/result.json
...
export MISAKA_BULLSHARK_AUTO_CANDIDATE_REHEARSAL_RESULT="$result_file"
rm -f "$result_file"
...
if ! cargo test -p misaka-node --bin misaka-node \
  ...
  write_failure "bullshark automatic candidate rehearsal regression failed"
  cat "$log_file" >&2
  exit 1
fi

if [[ ! -f "$result_file" ]]; then
  write_failure "bullshark automatic candidate rehearsal passed but did not emit result artifact"
  exit 1
fi
...
preview = payload.get("afterProducerAutoCandidatePreview", {}).get("chainInfo", {})
preview_dissemination = payload.get("afterProducerAutoCandidatePreview", {}).get("txDissemination", {})
preview_hashes = payload.get("afterProducerAutoCandidatePreview", {}).get("previewHashes", {})
...
if payload.get("flow") != "live_bullshark_candidate_preview_auto_advance_after_runtime_commit":
...
if preview_state.get("candidatePreviewQueued") != 2:
    errors.append("candidatePreviewQueued should be 2 after producer auto preview")
...
if preview_state.get("commitPreviewQueued") != 2:
    errors.append("commitPreviewQueued should be 2 after producer auto preview")
...
for key in (
    ...
    "producerAutoPreviewVisibleAfterResolve",
    "commitPreviewAutoAdvancedAlongsideCandidate",
    "candidatePreviewHashesVisibleAfterAutoAdvance",
...
echo "bullshark automatic candidate rehearsal passed"
echo "  $result_file"
