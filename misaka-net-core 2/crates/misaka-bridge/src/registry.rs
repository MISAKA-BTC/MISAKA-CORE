...
impl AssetRegistry {
    ...
    pub fn register(&mut self, asset_id: String, source_chain: u32, source_address: String) {
        self.assets.insert(asset_id.clone(), BridgedAsset {
            asset_id, source_chain, source_address, decimals: 9, is_active: true,
        });
    }

    pub fn is_registered(&self, asset_id: &str) -> bool {
        self.assets.get(asset_id).map(|a| a.is_active).unwrap_or(false)
    }
    ...
    pub fn deactivate(&mut self, asset_id: &str) {
        if let Some(a) = self.assets.get_mut(asset_id) { a.is_active = false; }
    }
}
