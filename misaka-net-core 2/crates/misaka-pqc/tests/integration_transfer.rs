...
impl Wallet {
    fn new() -> Self {
        Self {
            spending: SpendingKeypair::from_ml_dsa(MlDsaKeypair::generate().secret_key),
...
fn test_full_transfer_with_mempool_verification() {
    ...
    for (i, w) in wallets.iter().enumerate() {
        ...
        pool.register_spending_key(outref, w.spending.public_poly.clone());
    }
    ...
    let kip = prove_key_image(&a, &alice.spending.secret_poly, &alice.spending.public_poly, &alice.spending.key_image).unwrap();
    ...
    let mut tx2 = UtxoTransaction { ring_scheme: RING_SCHEME_LRS,
        ...
        inputs: vec![RingInput {
            ...
            ring_signature: vec![],
            key_image: alice.spending.key_image, // same key image!
            ki_proof: vec![],
        }],
        outputs: vec![TxOutput { amount: 5000, one_time_address: [0xDD; 20], pq_stealth: None }],
        fee: 200,
        extra: vec![],
    };
    ...
    let kip2 = prove_key_image(&a, &alice.spending.secret_poly, &alice.spending.public_poly, &alice.spending.key_image).unwrap();
    tx2.inputs[0].ki_proof = kip2.to_bytes();
...
fn test_mempool_rejects_invalid_ring_sig() {
    ...
    for (i, w) in wallets.iter().enumerate() {
        ...
        utxo_set.add_output(outref.clone(),
            TxOutput { amount: 10_000, one_time_address: [0xAA; 20], pq_stealth: None }, 0).unwrap();
    ...
    let mut tx = UtxoTransaction { ring_scheme: RING_SCHEME_LRS,
        ...
        outputs: vec![TxOutput { amount: 9900, one_time_address: [0xBB; 20], pq_stealth: None }],
        fee: 100, extra: vec![],
    };
