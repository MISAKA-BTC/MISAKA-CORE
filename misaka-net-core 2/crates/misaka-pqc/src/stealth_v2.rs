...
impl Drop for DerivedKeysV2 {
    fn drop(&mut self) {
        self.k_addr.fill(0);
        self.k_amount.fill(0);
        self.k_memo.fill(0);
    }
}

fn derive_keys_v2(shared_secret: &[u8; 32], tx_context: &[u8]) -> DerivedKeysV2 {
    let hk = Hkdf::<Sha256>::new(Some(tx_context), shared_secret);

    let mut k_addr = [0u8; 32];
    hk.expand(DST_V2_ADDRESS, &mut k_addr).unwrap();

    let mut scan_buf = [0u8; 16];
    hk.expand(DST_V2_SCAN, &mut scan_buf).unwrap();

    let mut k_amount = [0u8; 32];
    hk.expand(DST_V2_AMOUNT, &mut k_amount).unwrap();

    let mut k_memo = [0u8; 32];
    hk.expand(DST_V2_MEMO, &mut k_memo).unwrap();

    let mut nonce_bytes = [0u8; 24];
    hk.expand(DST_V2_NONCE, &mut nonce_bytes).unwrap();
    ...
    DerivedKeysV2 { k_addr, scan_tag: scan_buf, k_amount, k_memo, nonce_bytes, addr_commit }
}
