...
impl Drop for DerivedMaterials {
    fn drop(&mut self) {
        self.k_addr = [0u8; 32];
        self.k_amt = [0u8; 32];
        self.k_payload = [0u8; 32];
        // scan_tag and nonce are not secret
    }
}

/// Derive all sub-keys from the KEM shared secret (§5 steps 3-4).
fn derive_materials(shared_secret: &MlKemSharedSecret) -> DerivedMaterials {
    // root_key = HKDF-Extract("", shared_secret)
    let hk = Hkdf::<Sha256>::new(None, shared_secret.as_bytes());

    let mut k_addr = [0u8; 32];
    hk.expand(LABEL_ADDRESS, &mut k_addr).expect("valid HKDF");
    let mut k_amt = [0u8; 32];
    hk.expand(LABEL_AMOUNT, &mut k_amt).expect("valid HKDF");
    let mut k_payload = [0u8; 32];
    hk.expand(LABEL_PAYLOAD, &mut k_payload).expect("valid HKDF");
    let mut scan_tag = [0u8; 16];
    hk.expand(LABEL_SCAN_TAG, &mut scan_tag).expect("valid HKDF");
    let mut nonce_bytes = [0u8; 24];
    hk.expand(LABEL_NONCE, &mut nonce_bytes).expect("valid HKDF");

    DerivedMaterials { k_addr, k_amt, k_payload, scan_tag, nonce_bytes }
}

/// Derive one-time destination address from k_addr.
///
/// `addr = SHA3-256(k_addr)[0..20]`
fn derive_one_time_address(k_addr: &[u8; 32]) -> [u8; 20] {
    let full: [u8; 32] = {
...
impl StealthScanner {
    ...
    pub fn try_recover(
        ...
        // 7. Decrypt amount
        let amount_plain = match aead_decrypt(&mat.k_amt, &mat.nonce_bytes, &stealth_data.amount_ct, &aad) {
            Ok(pt) => pt,
        ...
        let amount = u64::from_le_bytes(amount_plain.try_into().unwrap());

        // 8. Decrypt payload
        let payload = match aead_decrypt(&mat.k_payload, &mat.nonce_bytes, &stealth_data.payload_ct, &aad) {
            Ok(pt) => pt,
