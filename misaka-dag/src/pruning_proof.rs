...
impl PruningProof {
    ...
    pub fn build<S: DagStore>(
        ...
        let _pp_header = store.get_header(&pruning_point_hash)?;
    ...
    pub fn build_legacy<S: DagStore>(pruning_point_hash: Hash, store: &S) -> Option<Self> {
...
impl DagSnapshot {
    ...
    pub fn export<S: DagStore>(
        ...
        for hash in &all_hashes {
            if let (Some(header), Some(gd)) =
                (store.get_header(hash), store.get_ghostdag_data(hash))
            {
                // Active window: blue_score >= pruning_point.blue_score
                if gd.blue_score >= pp_data.blue_score {
                    headers.push((*hash, header.clone()));
                    ghostdag_data.push((*hash, gd.clone()));
                }
            }
        }
...
mod tests {
    ...
    fn test_commitment_computation_deterministic() {
        let utxos = vec![([0x02; 32], 0u32, 100u64), ([0x01; 32], 0u32, 200u64)];
