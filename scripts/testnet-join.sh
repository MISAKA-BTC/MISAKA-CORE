#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
#  MISAKA Testnet — Join as Validator
#
#  Usage:
#    ./scripts/testnet-join.sh --seeds 163.43.225.27:6690 --index 1 --validators 3
#
#  Prerequisites:
#    - misaka-node binary built with --features testnet
#    - Or run: cargo build -p misaka-node --release --features testnet
# ═══════════════════════════════════════════════════════════════
set -euo pipefail
...
# Read seeds from file if not provided
if [ -z "$SEEDS" ]; then
...
# Build RPC peers from seeds (convert host:p2p_port to http://host:rpc_port)
if [ -z "$RPC_PEERS" ]; then
