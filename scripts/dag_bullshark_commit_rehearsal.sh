...
state_dir="${MISAKA_BULLSHARK_COMMIT_REHEARSAL_DIR:-$repo_root/.tmp/dag-bullshark-commit-rehearsal}"
result_file="${MISAKA_BULLSHARK_COMMIT_REHEARSAL_RESULT:-$state_dir/result.json}"
target_dir="${MISAKA_CARGO_TARGET_DIR:-$repo_root/.tmp/dag-bullshark-commit-target}"
log_file="$state_dir/cargo-test.log"
test_filter="live_bullshark_commit_handoff_through_rpc_service"

usage() {
  cat <<'EOF'
Usage: ./scripts/dag_bullshark_commit_rehearsal.sh

Runs the confined Bullshark commit handoff rehearsal and writes:

  .tmp/dag-bullshark-commit-rehearsal/result.json

Optional env:
  MISAKA_BULLSHARK_COMMIT_REHEARSAL_DIR=/custom/output/dir
  MISAKA_BULLSHARK_COMMIT_REHEARSAL_RESULT=/custom/result.json
...
export MISAKA_BULLSHARK_COMMIT_REHEARSAL_RESULT="$result_file"
rm -f "$result_file"
...
if ! cargo test -p misaka-node --bin misaka-node \
  ...
  write_failure "bullshark commit rehearsal regression failed"
  cat "$log_file" >&2
  exit 1
fi

if [[ ! -f "$result_file" ]]; then
  write_failure "bullshark commit rehearsal passed but did not emit result artifact"
  exit 1
fi
...
commit_preview = payload.get("afterCommitPreview", {}).get("chainInfo", {})
committed = payload.get("afterCommit", {}).get("chainInfo", {})
...
if payload.get("flow") != "live_bullshark_commit_handoff_through_rpc_service":
...
commit_preview_state = commit_preview.get("completionTargetShadowState", {})
committed_state = committed.get("completionTargetShadowState", {})
orch = committed.get("orchestration", {})
caps = committed.get("completionTargetShadowCapabilities", {})

if delivered_state.get("committedQueued") != 0:
    errors.append("delivered committedQueued should be 0")
if candidate_state.get("candidatePreviewQueued") != 2:
    errors.append("candidatePreviewQueued should be 2 after candidate handoff")
if commit_preview_state.get("commitPreviewQueued") != 2:
    errors.append("commitPreviewQueued should be 2 after commit preview handoff")
if committed_state.get("committedQueued") != 2:
    errors.append("committedQueued should be 2 after commit handoff")
...
if orch.get("commitCallerReady") is not True:
    errors.append("ordering orchestration commitCallerReady is not true")
if caps.get("bullsharkCommitReady") is not True:
    errors.append("ordering capability bullsharkCommitReady is not true")
...
if runtime_recovery.get("lastBullsharkCommitCount") != 2:
    errors.append("runtime recovery commit count should be 2")
if runtime_recovery.get("lastBullsharkCommitTxHashes") != tx_hashes:
    errors.append("runtime recovery commit hashes do not match txHashes")
if runtime_recovery.get("bullsharkCommitObserved") is not True:
    errors.append("runtime recovery bullsharkCommitObserved should be true")
...
for key in (
    ...
    "commitVisibleAfterHandoff",
    "commitHashesVisibleAfterHandoff",
...
echo "bullshark commit rehearsal passed"
echo "  $result_file"
