...
impl Drop for DerivedMaterials {
    fn drop(&mut self) {
        // Zeroize ALL key material with write_volatile to prevent optimization
        for b in self.k_addr.iter_mut() {
            unsafe { std::ptr::write_volatile(b, 0u8); }
        }
        for b in self.k_amt.iter_mut() {
            unsafe { std::ptr::write_volatile(b, 0u8); }
        }
        for b in self.k_payload.iter_mut() {
            unsafe { std::ptr::write_volatile(b, 0u8); }
        }
        for b in self.scan_tag.iter_mut() {
            unsafe { std::ptr::write_volatile(b, 0u8); }
        }
        for b in self.nonce_bytes.iter_mut() {
            unsafe { std::ptr::write_volatile(b, 0u8); }
        }
...
fn derive_materials(shared_secret: &MlKemSharedSecret) -> Result<DerivedMaterials, CryptoError> {
    ...
    Ok(DerivedMaterials { k_addr, k_amt, k_payload, scan_tag, nonce_bytes })
}
