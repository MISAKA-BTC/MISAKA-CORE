...
pub fn decode_transaction(data: &[u8]) -> Result<UtxoTransaction, String> {
    ...
    let ring_scheme = if off < data.len() && (version == 0x01 || version == 0x02) {
    ...
    for _ in 0..n_inputs {
        ...
        for _ in 0..n_members {
            ...
            ring_members.push(OutputRef { tx_hash, output_index });
        }
        ...
        inputs.push(RingInput { ring_members, ring_signature, key_image, ki_proof });
    }
    ...
    for _ in 0..n_outputs {
        ...
        let mut one_time_address = [0u8; 20];
        read_fixed(data, &mut off, &mut one_time_address)?;
        let has_stealth = read_u8(data, &mut off)?;
        let pq_stealth = if has_stealth == 1 {
            Some(PqStealthData::mcs1_decode(data, &mut off)
                .map_err(|e| format!("stealth decode: {}", e))?)
        } else {
            None
        };
        outputs.push(TxOutput { amount, one_time_address, pq_stealth, spending_pubkey: None });
    }
    ...
    Ok(UtxoTransaction { version, ring_scheme, tx_type: TxType::Transfer, inputs, outputs, fee, extra })
}

/// Compute wire size without actually encoding.
pub fn wire_size(tx: &UtxoTransaction) -> usize {
    let mut sz = 1 + 1 + 4; // version + ring_scheme + num_inputs
    for inp in &tx.inputs {
...
fn read_u8(data: &[u8], off: &mut usize) -> Result<u8, String> {
    if *off >= data.len() { return Err("EOF".into()); }
    let v = data[*off]; *off += 1; Ok(v)
}

fn read_u32(data: &[u8], off: &mut usize) -> Result<u32, String> {
    if *off + 4 > data.len() { return Err("EOF".into()); }
    let v = u32::from_le_bytes(data[*off..*off + 4].try_into().map_err(|_| "u32 slice".to_string())?);
    *off += 4; Ok(v)
}

fn read_u64(data: &[u8], off: &mut usize) -> Result<u64, String> {
    if *off + 8 > data.len() { return Err("EOF".into()); }
    let v = u64::from_le_bytes(data[*off..*off + 8].try_into().map_err(|_| "u64 slice".to_string())?);
    *off += 8; Ok(v)
}

fn read_fixed(data: &[u8], off: &mut usize, out: &mut [u8]) -> Result<(), String> {
    if *off + out.len() > data.len() { return Err("EOF".into()); }
    out.copy_from_slice(&data[*off..*off + out.len()]);
    *off += out.len(); Ok(())
}

fn read_bytes(data: &[u8], off: &mut usize, len: usize) -> Result<Vec<u8>, String> {
    if *off + len > data.len() { return Err("EOF".into()); }
...
mod tests {
 ...
 tx_type: TxType::Transfer,
            ...
            inputs: vec![RingInput {
                ring_members: vec![
                    OutputRef { tx_hash: [0x01; 32], output_index: 0 },
                    OutputRef { tx_hash: [0x02; 32], output_index: 1 },
                    OutputRef { tx_hash: [0x03; 32], output_index: 2 },
                    OutputRef { tx_hash: [0x04; 32], output_index: 3 },
                ],
    ...
    fn test_encode_decode_with_stealth() {
        ...
        let tx_id = [0x42; 32];
        let stealth = create_stealth_output(
            &kem_kp.public_key, 7777, b"memo", &tx_id, 0
        ).unwrap();
 ...
 tx_type: TxType::Transfer,
    ...
    fn test_full_tx_with_real_ring_sig() {
        ...
        let tx_id = [0x99; 32];
        let stealth = create_stealth_output(
            &kem_kp.public_key, 10000, b"real tx", &tx_id, 0
        ).unwrap();

        // Use strong-binding canonical key image for ki_proof
        let (_, strong_ki) = crate::ki_proof::canonical_strong_ki(&kps[1].public_poly, &kps[1].secret_poly);
 ...
 tx_type: TxType::Transfer,
            ...
            inputs: vec![RingInput {
                ring_members: vec![
                    OutputRef { tx_hash: [0x01; 32], output_index: 0 },
                    OutputRef { tx_hash: [0x02; 32], output_index: 0 },
                    OutputRef { tx_hash: [0x03; 32], output_index: 0 },
                    OutputRef { tx_hash: [0x04; 32], output_index: 0 },
                ],
                ring_signature: vec![],
                key_image: strong_ki,
                ki_proof: vec![],
        ...
        let kip = crate::ki_proof::prove_key_image(&a, &kps[1].secret_poly, &kps[1].public_poly, &strong_ki).unwrap();
        ...
        let decoded_sig = crate::packing::unpack_ring_sig_v2(
            &decoded.inputs[0].ring_signature, 4
        ).unwrap();
        ring_verify(&a, &ring_pks, &digest, &decoded_sig).unwrap();

        println!("\n  ═══ Real TX Wire Size Breakdown ═══");
        println!("  Total wire:    {} bytes", wire_bytes.len());
        println!("  Ring sig (v2): {} bytes", tx.inputs[0].ring_signature.len());
        println!("  Stealth data:  {} bytes",
            tx.outputs[0].pq_stealth.as_ref().unwrap().wire_len());
        println!("  Overhead:      {} bytes (headers/refs/amounts)",
            wire_bytes.len() - tx.inputs[0].ring_signature.len()
                - tx.outputs[0].pq_stealth.as_ref().unwrap().wire_len());

        let raw_sig_size = sig.to_bytes().len();
        println!("\n  Sig compression: {} → {} bytes ({:.1}% saved)",
            raw_sig_size, tx.inputs[0].ring_signature.len(),
 ...
 tx_type: TxType::Transfer,
            ...
            outputs: vec![TxOutput {
                amount: 1000, one_time_address: [0; 20], pq_stealth: None,
            spending_pubkey: None,
        }],
            fee: 10,
            extra: vec![1, 2, 3],
