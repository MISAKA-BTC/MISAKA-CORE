...
impl KiProof {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        ...
        Ok(Self { challenge, response, ki_poly })
    }
}
...
pub fn prove_key_image(
    ...
    if *key_image != expected_nullifier {
        return Err(CryptoError::RingSignatureInvalid(
    ...
    for _ in 0..MAX_SIGN_ATTEMPTS {
        ...
        if z.norm_inf() >= BETA { continue; }

        return Ok(KiProof { challenge, response: z, ki_poly });
    }

    Err(CryptoError::RingSignatureInvalid("ki_proof: max attempts".into()))
}
...
pub fn verify_key_image_proof(
    ...
    if z.norm_inf() >= BETA {
        return Err(CryptoError::RingSignatureInvalid(
            format!("ki_proof norm {} >= beta", z.norm_inf()),
        ));
    }

    // 2. Verify key_image matches ki_poly hash
    let expected_nullifier = ki_poly_to_nullifier(&proof.ki_poly);
    if *key_image != expected_nullifier {
        return Err(CryptoError::RingSignatureInvalid(
            "ki_proof: key_image does not match ki_poly".into(),
        ));
    }
    ...
    // 7. Challenge must match
    if expected_c != proof.challenge {
        return Err(CryptoError::RingSignatureInvalid(
            "ki_proof: challenge mismatch (strong binding violated)".into(),
        ));
    }
...
pub fn verify_key_image(
    a: &Poly, public_key: &Poly, key_image: &[u8; 32], proof: &KiProof,
) -> Result<(), CryptoError> {
...
mod tests {
    use super::*;
    use crate::pq_ring::{derive_public_param, derive_secret_poly, compute_pubkey,
                         DEFAULT_A_SEED, SpendingKeypair};
