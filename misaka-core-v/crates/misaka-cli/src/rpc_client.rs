...
impl RpcClient {
            ...
            match self.do_post(&url, body).await {
                ...
                Err(e) => {
                    ...
                    if is_transient && attempt < MAX_RETRIES {
                        eprintln!("   ⚠ RPC attempt {} failed ({}), retrying...", attempt + 1, e);
                        last_err = Some(e);
                        continue;
                    }
                    return Err(e);
    ...
    async fn do_post(&self, url: &str, body: &Value) -> Result<Value> {
        ...
        if !status.is_success() {
            ...
            anyhow::bail!(
                "RPC error: HTTP {} from {} — {}",
                status.as_u16(), url, &body_text[..body_text.len().min(200)]
            );
        }
...
pub async fn get_status(rpc_url: &str) -> Result<()> {
    ...
    let resp = client.post_json("/api/get_chain_info", &serde_json::json!({})).await?;
