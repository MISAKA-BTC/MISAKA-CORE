...
use misaka_pqc::pq_ring::{
    SpendingKeypair, Poly, ring_sign, derive_public_param, DEFAULT_A_SEED,
};
...
pub async fn run(
    ...
    let key_json = fs::read_to_string(key_path)
        .context("failed to read wallet key file")?;
    ...
    let master_sk_bytes = hex::decode(&wallet.ml_dsa_sk)
        .context("invalid hex in ml_dsa_sk")?;
    ...
    let decoys = fetch_same_amount_decoys(
        &client, input_amount, &input_tx_hash, input_output_index, MIN_RING_SIZE - 1,
    ).await?;
    ...
    if decoys.len() < MIN_RING_SIZE - 1 {
        bail!(
            ...
            MIN_RING_SIZE - 1, decoys.len(), input_amount
        );
    }
    ...
    let decoy_pks: Vec<Poly> = decoys.iter()
        .map(|d| {
            let pk_hex = d.spending_pubkey.as_ref()
                .ok_or_else(|| anyhow::anyhow!("decoy UTXO missing spending_pubkey"))?;
            let pk_bytes = hex::decode(pk_hex)
                .context("invalid hex in decoy spending_pubkey")?;
    ...
    let change_info: Option<(u32, String, String)> = if change > 0 {
        ...
        Some((idx, child.derive_address(), hex::encode(child.key_image)))
    } else {
        None
    };
...
async fn fetch_same_amount_decoys(
    ...
    let resp = client.post_json("/api/get_decoy_utxos", &serde_json::json!({
    ...
    let utxos_raw = resp["utxos"].as_array()
    ...
    for u in utxos_raw {
        ...
        if tx_hash == exclude_tx && output_index == exclude_idx { continue; }
        if amount != target_amount { continue; }
        if spending_pubkey.is_none() { continue; }

        decoys.push(ChainUtxo { tx_hash, output_index, amount, spending_pubkey });
        if decoys.len() >= count { break; }
    }
...
async fn sync_wallet_from_chain(client: &RpcClient, state: &mut WalletState) -> Result<()> {
    let resp = match client.post_json("/api/get_address_outputs", &serde_json::json!({
    ...
    if let Some(spent_kis) = resp["spentKeyImages"].as_array() {
        let spent_set: std::collections::HashSet<&str> = spent_kis.iter()
