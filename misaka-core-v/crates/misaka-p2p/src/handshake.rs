...
impl InitiatorHandshake {
    ...
    fn complete_inner(
        ...
        Ok(HandshakeResult { session_key, peer_pk: reply.responder_pk.clone(), our_signature: our_sig })
    }
}
...
fn build_transcript(
    kem_pk: &MlKemPublicKey, ct: &MlKemCiphertext, _session_key: &[u8; 32],
) -> Vec<u8> {
...
mod tests {
    ...
    fn test_mutual_auth_handshake() {
        ...
        let initiator_result = hs.complete_verified(&reply, &initiator_kp.secret_key, &responder_kp.public_key).unwrap();

        // Step 7: Responder verifies initiator's signature (MUTUAL AUTH)
        let responder_result = reply.verify_initiator(
            &initiator_result.our_signature,
