    ...
    match req.method.as_str() {
        "misaka_getStatus" => {
            let status = NodeStatus {
                chain_id, height, utxo_count, mempool_size,
            ...
            JsonRpcResponse::success(req.id.clone(),
                serde_json::to_value(status).unwrap_or(serde_json::json!({"error": "serialization failed"})))
        }
        "misaka_getUtxoCount" => {
