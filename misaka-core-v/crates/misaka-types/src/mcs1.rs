...
pub fn read_u32(data: &[u8], offset: &mut usize) -> Result<u32, MisakaError> {
    ...
    let v = u32::from_le_bytes(
        data[*offset..*offset + 4].try_into()
            .map_err(|_| MisakaError::DeserializationError("u32 slice conversion failed".into()))?
    );
    *offset += 4;
    Ok(v)
}
...
pub fn read_u64(data: &[u8], offset: &mut usize) -> Result<u64, MisakaError> {
    ...
    let v = u64::from_le_bytes(
        data[*offset..*offset + 8].try_into()
            .map_err(|_| MisakaError::DeserializationError("u64 slice conversion failed".into()))?
    );
    *offset += 8;
    Ok(v)
}
...
pub fn read_u128(data: &[u8], offset: &mut usize) -> Result<u128, MisakaError> {
    ...
    let v = u128::from_le_bytes(
        data[*offset..*offset + 16].try_into()
            .map_err(|_| MisakaError::DeserializationError("u128 slice conversion failed".into()))?
    );
    *offset += 16;
    Ok(v)
}
...
pub fn read_bytes(data: &[u8], offset: &mut usize) -> Result<Vec<u8>, MisakaError> {
    ...
    if *offset + len > data.len() {
        return Err(MisakaError::DeserializationError("EOF reading bytes".into()));
    }
...
pub fn read_fixed(data: &[u8], offset: &mut usize, len: usize) -> Result<Vec<u8>, MisakaError> {
    if *offset + len > data.len() {
        return Err(MisakaError::DeserializationError("EOF reading fixed bytes".into()));
    }
    let v = data[*offset..*offset + len].to_vec();
    *offset += len;
    Ok(v)
