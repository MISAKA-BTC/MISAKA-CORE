...
impl GasMeter {
    pub fn new(budget: u64, price: u64) -> Self {
        Self { budget, price, consumed: 0, exhausted: false }
    }
    ...
    pub fn charge(&mut self, amount: u64) -> Result<(), MisakaError> {
        ...
        if new > self.budget {
            ...
            return Err(MisakaError::GasBudgetExceeded { charged: new, limit: self.budget });
        }
        self.consumed = new;
        Ok(())
    }

    pub fn gas_charged(&self) -> u64 { self.consumed }

    pub fn fee_charged(&self) -> Result<u64, MisakaError> {
        self.consumed.checked_mul(self.price).ok_or(MisakaError::ArithmeticOverflow)
    }

    pub fn fee_refund(&self) -> Result<u64, MisakaError> {
        let remaining = self.budget.saturating_sub(self.consumed);
        remaining.checked_mul(self.price).ok_or(MisakaError::ArithmeticOverflow)
    }

    pub fn fee_max(&self) -> Result<u64, MisakaError> {
        self.budget.checked_mul(self.price).ok_or(MisakaError::ArithmeticOverflow)
    }
}
