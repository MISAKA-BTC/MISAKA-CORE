...
impl Drop for ValidatorPqSecretKey {
    fn drop(&mut self) {
        // Volatile write prevents compiler from eliding the zeroing.
        // This is critical for secret key material.
        for b in self.pq_sk.iter_mut() {
            unsafe { std::ptr::write_volatile(b, 0u8); }
        }
        // Fence to ensure the volatile writes are not reordered past this point.
