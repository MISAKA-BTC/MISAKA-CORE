...
impl DagStateManager {
    ...
    /// # 引数
    ///
    /// - `known_key_images`: 既に UTXO Set に記録済みの Key Image 群。
    ///   チェックポイントからの復元時に使用。空から開始する場合は空の HashSet。
    pub fn new(known_key_images: HashSet<[u8; 32]>) -> Self {
        let applied_key_images = known_key_images.into_iter()
            .map(|ki| (ki, KeyImageRecord {
                first_tx_hash: [0; 32],
                block_hash: [0; 32],
                order_index: 0,
            }))
        ...
        F: FnMut(UtxoAction),
    {
        ...
        let mut global_order_index: u64 = self.stats.txs_applied + self.stats.txs_failed_ki_conflict;
        ...
        F: FnMut(UtxoAction),
    {
        ...
        // Key Image を記録
        for &ki in &tx.key_images {
            self.applied_key_images.insert(ki, KeyImageRecord {
...
mod tests {
    ...
    fn test_multi_ki_partial_conflict() {
        ...
        assert_eq!(results[0].status, TxApplyStatus::Applied);
        assert!(matches!(results[1].status, TxApplyStatus::FailedKeyImageConflict { .. }));
