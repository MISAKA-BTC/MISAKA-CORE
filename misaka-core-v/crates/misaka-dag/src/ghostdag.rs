...
impl GhostDagManager {
    ...
    pub fn calculate_ghostdag_data<S: DagStore>(
        ...
        // ── Step 3: Blue/Red 分類 ──
        let (blues, reds) = self.classify_mergeset(
    ...
    fn classify_mergeset<S: DagStore>(
        ...
        for &block in mergeset {
            ...
            let blue_anticone_count = current_blues.iter()
                .filter(|&&blue_block| {
    ...
    pub fn confirmation_depth<S: DagStore>(
        &self,
        block_hash: &Hash,
        store: &S,
    ) -> u64 {
...
mod tests {
    ...
    fn test_simple_diamond_dag() {
        ...
        let manager = GhostDagManager::new(18, [0x00; 32]);
