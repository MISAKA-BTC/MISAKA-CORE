...
impl ConnectionManager {
    ...
    ///
    /// SEC-FIX: Uses a single write lock for the entire check-and-insert
    /// operation. Previously used a read lock for the count check, dropped it,
    /// then acquired a write lock for the insert — allowing concurrent threads
    /// to both pass the limit check and exceed max_inbound/max_outbound.
    pub fn register(&self, addr: SocketAddr, direction: ConnectionDirection) -> Option<u64> {
        // Acquire write lock upfront to prevent TOCTOU race between
        // count check and insert.
        ...
        drop(connections); // Release before acquiring by_addr lock
