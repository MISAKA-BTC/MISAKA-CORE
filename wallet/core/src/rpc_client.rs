...
impl NodeClient {
    ...
    pub async fn chain_info(&self) -> Result<ChainInfo> {
        self.post_with_retry("/api/get_chain_info", &serde_json::json!({}))
            .await
    }
    ...
    // ── Decoy UTXOs (for ML-DSA signature transfers) ──
    ...
    // ── Anonymity Set (for ZKP privacy transfers) ──
    ...
    pub async fn get_tx(&self, hash: &str) -> Result<serde_json::Value> {
        self.post_with_retry("/api/get_tx_by_hash", &serde_json::json!({ "hash": hash }))
            .await
    }
