...
impl HardwareBridgeState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, HardwareBridgeError> {
        ...
        let entry = HardwareBridgeEntry {
            ...
            created_at: now,
            updated_at: now,
            state: HardwareBridgeEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(HardwareBridgeTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), HardwareBridgeError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(HardwareBridgeError::NotFound(id))?;
        ...
        self.emit(HardwareBridgeEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, HardwareBridgeError> {
        ...
        self.emit(HardwareBridgeEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<HardwareBridgeEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    HardwareBridgeEntryState::Completed
                        | HardwareBridgeEntryState::Failed
                        | HardwareBridgeEntryState::Cancelled
                        | HardwareBridgeEntryState::Expired
        ...
        if removed > 0 {
            self.emit(HardwareBridgeEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> HardwareBridgeStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<HardwareBridgeEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = HardwareBridgeConfig {
            retry_count: 2,
            ..Default::default()
        };
