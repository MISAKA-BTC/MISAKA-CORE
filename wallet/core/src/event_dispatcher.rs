...
impl EventDispatcherState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, EventDispatcherError> {
        ...
        let entry = EventDispatcherEntry {
            ...
            created_at: now,
            updated_at: now,
            state: EventDispatcherEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(EventDispatcherTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), EventDispatcherError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(EventDispatcherError::NotFound(id))?;
        ...
        self.emit(EventDispatcherEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, EventDispatcherError> {
        ...
        self.emit(EventDispatcherEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<EventDispatcherEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    EventDispatcherEntryState::Completed
                        | EventDispatcherEntryState::Failed
                        | EventDispatcherEntryState::Cancelled
                        | EventDispatcherEntryState::Expired
        ...
        if removed > 0 {
            self.emit(EventDispatcherEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> EventDispatcherStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<EventDispatcherEvent> {
