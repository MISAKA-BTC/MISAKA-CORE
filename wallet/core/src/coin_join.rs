...
impl CoinJoinState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, CoinJoinError> {
        ...
        let entry = CoinJoinEntry {
            ...
            created_at: now,
            updated_at: now,
            state: CoinJoinEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(CoinJoinTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), CoinJoinError> {
        ...
        self.emit(CoinJoinEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, CoinJoinError> {
        ...
        self.emit(CoinJoinEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<CoinJoinEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    CoinJoinEntryState::Completed
                        | CoinJoinEntryState::Failed
                        | CoinJoinEntryState::Cancelled
                        | CoinJoinEntryState::Expired
        ...
        if removed > 0 {
            self.emit(CoinJoinEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> CoinJoinStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<CoinJoinEvent> {
