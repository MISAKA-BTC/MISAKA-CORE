...
impl AddressDerivationState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, AddressDerivationError> {
        ...
        let entry = AddressDerivationEntry {
            ...
            created_at: now,
            updated_at: now,
            state: AddressDerivationEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(AddressDerivationTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), AddressDerivationError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(AddressDerivationError::NotFound(id))?;
        ...
        self.emit(AddressDerivationEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AddressDerivationError> {
        ...
        self.emit(AddressDerivationEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<AddressDerivationEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    AddressDerivationEntryState::Completed
                        | AddressDerivationEntryState::Failed
                        | AddressDerivationEntryState::Cancelled
                        | AddressDerivationEntryState::Expired
        ...
        if removed > 0 {
            self.emit(AddressDerivationEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> AddressDerivationStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<AddressDerivationEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = AddressDerivationConfig {
            retry_count: 2,
            ..Default::default()
        };
