...
// CRIT-2: argon2 crate for real memory-hard KDF (replaces HKDF emulation).
...
impl EncryptedKeystore {
    ...
    pub fn decrypt(&self, password: &str) -> Result<Vec<u8>, KeystoreError> {
        let derived_key = derive_key(password, &self.crypto.salt, &self.crypto.kdf_params)?;
...
fn derive_key(
    ...
    // CRIT-2 FIX: Real Argon2id — memory-hard KDF resistant to GPU/ASIC attacks.
    //
    // Previous implementation used HKDF-SHA3 which provides ZERO memory-hardness.
    // An attacker with an encrypted wallet file could brute-force the password
    // using GPU clusters at ~1000x the speed of CPU.
    //
    // Argon2id (memory_cost=64MB, time_cost=3, parallelism=4) requires ~64MB RAM
    // per attempt, making GPU attacks economically infeasible.
    let argon2_params = argon2::Params::new(
        params.memory_cost,  // memory in KiB (default: 65536 = 64 MB)
        params.time_cost,    // iterations (default: 3)
        params.parallelism,  // threads (default: 4)
        Some(params.output_len as usize), // output length (default: 32)
    ...
    argon2.hash_password_into(
        ...
        salt,
        &mut okm,
    ).map_err(|e| KeystoreError::Kdf(format!("Argon2id failed: {}", e)))?;
...
fn generate_salt() -> Vec<u8> {
    ...
    // CRIT-4 FIX: Use OsRng consistently for all key material.
