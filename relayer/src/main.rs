...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Relayer keypair: {}", config.relayer_keypair_path);
    ...
    let mut last_lock_cursor: Option<String> = store.get_cursor("lock_cursor").unwrap_or(None);
    if let Some(ref cursor) = last_lock_cursor {
        info!(
            "Resuming lock poll from cursor: {}...",
                ...
                for event in events {
                    ...
                    match misaka_watcher::submit_mint_request(&config, &event).await {
                        Ok(receipt_id) => {
                            ...
                            info!(
                                "[LOCK] Mint OK: {}",
                                &receipt_id[..16.min(receipt_id.len())]
                            );
                        }
                        Err(e) => {
