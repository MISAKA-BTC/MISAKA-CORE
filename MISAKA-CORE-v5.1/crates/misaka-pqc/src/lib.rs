...
pub mod pq_ring;
pub mod pq_sign;
// Legacy stealth v1 remains compiled during the v4 transition because
// wallet recovery, test vectors, and migration tests still depend on it.
pub mod pq_stealth;
pub mod tx_codec;
...
pub use privacy_dispatch::{
    verify_ring_family_input, RingFamilyVerifyError, RingFamilyVerifyInput,
    select_privacy_backend, verify_unified_zkp_input,
    read_composite_proof_from_tx, verify_composite_tx,
    PrivacyBackendPreference, SelectedPrivacyBackend, UnifiedZkpVerifyInput,
};
...
pub use composite_proof::{
    CompositeProof, OutputWitness, SCHEME_COMPOSITE, COMPOSITE_VERSION,
    prove_composite, verify_composite, compute_binding_digest,
};
...
pub use secret::{SecretPoly, SecureBuffer, SecretKey32, ZeroizeGuard, ct_eq, ct_eq_32};
...
pub use unified_zkp::{
    UnifiedMembershipProof, unified_prove, unified_verify, unified_verify_ctx, compute_merkle_root,
