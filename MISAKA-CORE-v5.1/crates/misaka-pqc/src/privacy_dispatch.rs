...
fn select_ring_signature_backend(
    ...
    Ok(SelectedPrivacyBackend {
        ...
        selection_reason: format!(
            "v1-v3 ring signature TX (scheme=0x{:02x})",
            tx.ring_scheme
        ),
    })
}
...
pub fn verify_ring_family_input(
    ...
    match tx.ring_scheme {
        ...
        0x01 => {
            ...
            let ki_proof = crate::ki_proof::KiProof::from_bytes(verify.raw_ki_proof)
                .map_err(|e| {
...
pub fn read_composite_proof_from_tx(
    ...
    if carrier.backend_tag != crate::composite_proof::SCHEME_COMPOSITE {
        return Err(CryptoError::RingSignatureInvalid(format!(
            ...
            carrier.backend_tag, crate::composite_proof::SCHEME_COMPOSITE,
        )));
    }
...
mod tests {
    ...
    use misaka_types::utxo::{
        OutputRef, RingInput, TxOutput, TxType, UtxoTransaction,
        UTXO_TX_VERSION, UTXO_TX_VERSION_V3, UTXO_TX_VERSION_V4,
        RING_SCHEME_LRS, RING_SCHEME_LOGRING,
    };
    ...
    fn test_v1_explicit_ring_routes_to_ring_signature() {
        let tx = sample_lrs_tx();
        let selected = select_privacy_backend(&tx, PrivacyBackendPreference::RingSignature).unwrap();
        assert_eq!(selected.backend_family, PrivacyBackendFamily::RingSignature);
