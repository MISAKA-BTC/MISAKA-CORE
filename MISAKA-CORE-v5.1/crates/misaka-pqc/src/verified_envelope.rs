...
use crate::crypto_types::{PublicNullifier, AnonymityRoot, TxDigest};
use crate::qdag_tx::QdagTransaction;
use crate::bdlop::{BdlopCrs, verify_balance_with_excess, compute_balance_diff};
use crate::range_proof::verify_range;
use crate::confidential_fee::verify_confidential_fee;
use crate::unified_zkp::{UnifiedMembershipProof, unified_verify};
...
const ALL_CHECKS: u32 = CHECK_STRUCTURAL | CHECK_NULLIFIERS | CHECK_MEMBERSHIP
                       | CHECK_RANGE | CHECK_BALANCE | CHECK_FEE;
...
impl VerifiedTransactionEnvelope {
    ...
    pub fn tx(&self) -> &QdagTransaction { &self.tx }

    /// Get verified nullifiers (typed).
    pub fn nullifiers(&self) -> &[PublicNullifier] { &self.nullifiers }

    /// Get the transaction digest.
    pub fn tx_digest(&self) -> &TxDigest { &self.tx_digest }

    /// Get the transaction hash.
    pub fn tx_hash(&self) -> [u8; 32] { self.tx.tx_hash() }

    /// Get verified anonymity roots (one per input).
    pub fn anonymity_roots(&self) -> &[AnonymityRoot] { &self.anonymity_roots }
    ...
    pub fn proof_weight(&self) -> u64 { self.proof_weight }

    /// Does this TX comply with fee policy?
    pub fn fee_policy_ok(&self) -> bool { self.fee_policy_ok }
}

impl std::fmt::Debug for VerifiedTransactionEnvelope {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "VerifiedTxEnvelope(digest={:?}, nullifiers={}, weight={}, fee_ok={})",
            self.tx_digest, self.nullifiers.len(), self.proof_weight, self.fee_policy_ok)
    }
}
...
pub fn verify_and_seal(
    ...
    for (i, inp) in tx.inputs.iter().enumerate() {
        // Deserialize proof
        let proof = UnifiedMembershipProof::from_bytes(&inp.membership_proof)
            .map_err(|e| TxVerificationError::MembershipProof {
                index: i, reason: format!("deserialize: {}", e),
            })?;
    ...
    for (i, out) in tx.outputs.iter().enumerate() {
        verify_range(&crs, &out.commitment, &out.range_proof)
            .map_err(|e| TxVerificationError::RangeProof {
                index: i, reason: e.to_string(),
            })?;
    }
    checks |= CHECK_RANGE;
    ...
    verify_confidential_fee(&crs, &tx.fee)
        .map_err(|e| TxVerificationError::FeeProof {
            reason: e.to_string(),
        })?;
    checks |= CHECK_FEE;
    ...
    verify_balance_with_excess(&crs, &balance_diff, &tx.balance_proof)
        .map_err(|e| TxVerificationError::BalanceProof {
            reason: e.to_string(),
        })?;
    checks |= CHECK_BALANCE;

    // ── All checks passed — seal the envelope ──
    assert_eq!(checks, ALL_CHECKS, "BUG: not all verification stages executed");

    let nullifiers = tx.inputs.iter()
        .map(|inp| PublicNullifier::from_raw(inp.nullifier))
        .collect();

    let anonymity_roots = tx.inputs.iter()
    ...
    Ok(VerifiedTransactionEnvelope {
        ...
        _seal: VerificationSeal { checks_passed: checks },
    })
}
...
mod tests {
    use super::*;
    use crate::qdag_tx::{QdagTxType, ConfidentialInput, QDAG_VERSION};
