    ...
    pub fn from_tx_and_input_amounts_for_backend(
        ...
        let sum_inputs = input_amounts.iter().try_fold(0u64, |acc, amt| {
            acc.checked_add(*amt).ok_or_else(|| {
                CryptoError::RingSignatureInvalid("privacy constraints input sum overflow".into())
            })
        })?;
        ...
        let sum_outputs = output_amounts.iter().try_fold(0u64, |acc, amt| {
            acc.checked_add(*amt).ok_or_else(|| {
                CryptoError::RingSignatureInvalid("privacy constraints output sum overflow".into())
            })
        })?;

        let required = sum_outputs.checked_add(tx.fee).ok_or_else(|| {
            CryptoError::RingSignatureInvalid("privacy constraints required amount overflow".into())
        })?;
...
mod tests {
    ...
    use misaka_types::utxo::{
        OutputRef, RingInput, TxOutput, UtxoTransaction, RING_SCHEME_LOGRING, UTXO_TX_VERSION_V3,
    };
