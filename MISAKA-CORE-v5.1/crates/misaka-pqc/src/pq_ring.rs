...
pub fn ring_sign(
    ...
    if n_ring < MIN_RING_SIZE || n_ring > MAX_RING_SIZE {
        return Err(CryptoError::RingSignatureInvalid(format!(
            "ring size {} out of [{}, {}]",
            n_ring, MIN_RING_SIZE, MAX_RING_SIZE
        )));
    }
    ...
    for _attempt in 0..MAX_SIGN_ATTEMPTS {
        ...
        for i in 0..N {
            let y_centered = if y.coeffs[i] > Q / 2 {
                y.coeffs[i] - Q
            } else {
                y.coeffs[i]
            };
            let cs_centered = if cs.coeffs[i] > Q / 2 {
                cs.coeffs[i] - Q
            } else {
                cs.coeffs[i]
            };
...
pub fn ring_verify(
    ...
    if n_ring < MIN_RING_SIZE || n_ring > MAX_RING_SIZE {
        return Err(CryptoError::RingSignatureInvalid(
            "ring size out of range".into(),
        ));
    }
    if sig.responses.len() != n_ring {
        return Err(CryptoError::RingSignatureInvalid(
            "response count mismatch".into(),
        ));
    }
...
impl SpendingKeypair {
    ...
    pub fn derive_child(master_sk_bytes: &[u8], index: u32) -> Result<Self, CryptoError> {
        ...
        let child_sk = MlDsaSecretKey::from_bytes(&child_bytes).map_err(|e| {
