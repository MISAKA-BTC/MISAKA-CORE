...
use crate::bdlop::{BdlopCrs, BdlopCommitment, BlindingFactor, BalanceExcessProof};
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        serde_json::from_slice(data)
            .map_err(|e| CryptoError::RingSignatureInvalid(
                format!("CompositeProof deserialize: {}", e)))
    }
...
pub fn verify_composite(
    ...
    for (i, (commitment, range_proof)) in proof.output_commitments.iter()
        .zip(proof.range_proofs.iter())
        .enumerate()
    {
        verify_range(crs, commitment, range_proof).map_err(|e| {
            CryptoError::RingSignatureInvalid(format!(
                "range proof failed for output {}: {}", i, e))
        })?;
    }
...
mod tests {
    ...
    fn test_prove_verify_roundtrip() {
        ...
        let out_witness = OutputWitness { amount: 9_900, blinding: BlindingFactor::random() };
    ...
    fn test_wrong_tx_digest_rejected() {
        ...
        let out_witness = OutputWitness { amount: 900, blinding: BlindingFactor::random() };
        let fee_blind = BlindingFactor::random();

        let proof = prove_composite(
            &crs, &[0x01; 32],
            &[in_commitment.clone()], &[in_blind],
            &[out_witness], 100, &fee_blind, &[[0xBB; 32]],
        ).expect("prove");
        ...
        let result = verify_composite(
            &crs, &proof, &[0x02; 32],
            &[in_commitment], &fee_commitment, &[[0xBB; 32]],
        );
    ...
    fn test_wrong_nullifier_rejected() {
        ...
        let result = verify_composite(
            &crs, &proof, &[0x01; 32],
            &[in_commitment], &fee_commitment, &[[0xCC; 32]],
        );
    ...
    fn test_serialization_roundtrip() {
        ...
        let out_witness = OutputWitness { amount: 4950, blinding: BlindingFactor::random() };
        let fee_blind = BlindingFactor::random();

        let proof = prove_composite(
            &crs, &[0x33; 32],
            &[in_commitment], &[in_blind],
            &[out_witness], 50, &fee_blind, &[[0xDD; 32]],
        ).expect("prove");
    ...
    fn test_multiple_outputs() {
        ...
        let out1 = OutputWitness { amount: 6_000, blinding: BlindingFactor::random() };
        let out2 = OutputWitness { amount: 3_900, blinding: BlindingFactor::random() };
        let fee_blind = BlindingFactor::random();

        let proof = prove_composite(
            &crs, &[0x44; 32],
            &[in_commitment.clone()], &[in_blind],
            &[out1, out2], 100, &fee_blind, &[[0xEE; 32]],
        ).expect("prove");
        ...
        verify_composite(
            &crs, &proof, &[0x44; 32],
            &[in_commitment], &fee_commitment, &[[0xEE; 32]],
