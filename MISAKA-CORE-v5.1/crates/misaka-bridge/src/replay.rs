    ...
    pub fn open(path: &Path) -> Result<Self, String> {
        ...
        if path.exists() {
            ...
            for line in content.lines() {
                ...
                if line.len() != 64 {
                    return Err(format!(
                        "invalid replay file line (expected 64 hex chars): '{}'",
                        line
                    ));
                }
                let bytes =
                    hex::decode(line).map_err(|e| format!("invalid hex in replay file: {}", e))?;
                let id: [u8; 32] = bytes
                    .try_into()
...
impl ReplayProtection {
    ...
    pub fn new_volatile_for_test() -> Self {
        Self {
            backend: Box::new(VolatileReplayProtection::new()),
        }
    }
    ...
    pub fn durable(path: &Path) -> Result<Self, String> {
        Ok(Self {
            backend: Box::new(DurableReplayProtection::open(path)?),
        })
    }
