...
impl PeerScore {
    ...
    pub fn decay(&mut self) {
        ...
        fn decay_toward_zero(v: &mut i32, rate: i32) {
            if *v > 0 { *v = (*v - rate).max(0); }
            else if *v < 0 { *v = (*v + rate).min(0); }
        }
...
impl ScoreEvent {
    ...
    pub fn apply(&self, score: &mut PeerScore) {
        match self {
            ...
            Self::DuplicateAddressSpam => score.penalty_score = (score.penalty_score - 10).max(-1000),
            Self::HandshakeTimeout => score.transport_score = (score.transport_score - 10).max(-100),
            ...
            Self::ExcessiveOrphanSpam => score.penalty_score = (score.penalty_score - 15).max(-1000),
            Self::EquivocationEvidence => score.penalty_score = (score.penalty_score - 100).max(-1000),
            Self::NullGossipFlood => score.penalty_score = (score.penalty_score - 10).max(-1000),
            Self::EclipseClusteringDetected => score.penalty_score = (score.penalty_score - 30).max(-1000),
        }
    }
}
...
impl ScoreManager {
    ...
    pub fn record_event(&mut self, peer: PeerId, event: ScoreEvent) -> bool {
        ...
        if score.should_temp_ban() {
            self.temp_bans.insert(
                peer,
                Instant::now() + Duration::from_secs(TEMP_BAN_SECS),
            );
            return true;
        }
        false
    ...
    pub fn select_dial_candidates(&self, candidates: &[PeerId], limit: usize) -> Vec<PeerId> {
        let mut scored: Vec<(PeerId, i32)> = candidates.iter()
    ...
    pub fn banned_count(&self) -> usize {
        let temp = self.temp_bans.values().filter(|e| Instant::now() < **e).count();
...
mod tests {
    ...
    fn test_temp_ban_expires() {
        ...
        mgr.temp_bans.insert(peer, Instant::now() - Duration::from_secs(1));
