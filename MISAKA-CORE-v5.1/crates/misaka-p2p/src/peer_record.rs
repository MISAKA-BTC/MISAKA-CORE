...
pub struct NetworkAddress {
    ...
    pub protocol: String,
    /// Address string (e.g., "133.167.126.51:6690").
    pub addr: String,
}
...
impl PeerRoles {
    pub fn full_node() -> Self {
        Self { validator: false, sentry: false, bootstrap: false }
    }

    pub fn sentry() -> Self {
        Self { validator: false, sentry: true, bootstrap: false }
    }

    pub fn validator() -> Self {
        Self { validator: true, sentry: false, bootstrap: false }
    }

    pub fn bootstrap() -> Self {
        Self { validator: false, sentry: false, bootstrap: true }
    }
}
...
impl PeerRecord {
    ...
    pub fn signing_bytes(&self) -> Vec<u8> {
        ...
        let mut addrs: Vec<String> = self.addresses.iter()
    ...
    pub fn verify(&self, now_unix: u64) -> Result<(), PeerRecordError> {
        ...
        let pk = misaka_crypto::validator_sig::ValidatorPqPublicKey::from_bytes(&self.transport_pubkey)
...
mod tests {
    ...
    fn test_peer_record_tampered_signature_rejected() {
        ...
        let result = record.verify(now_unix());
        assert!(matches!(result, Err(PeerRecordError::SignatureVerificationFailed(_))));
    }
    ...
    fn test_peer_record_wrong_chain_id_peer_id_mismatch() {
        ...
        record.chain_id = 99;
        let result = record.verify(now_unix());
        assert!(matches!(result, Err(PeerRecordError::PeerIdMismatch { .. })));
    }
    ...
    fn test_peer_record_too_many_addresses() {
        ...
        let addrs: Vec<NetworkAddress> = (0..MAX_ADDRESSES + 1)
            .map(|i| NetworkAddress { protocol: "tcp".into(), addr: format!("1.2.3.4:{}", 6690 + i) })
            .collect();
        let record = PeerRecord::create_signed(
            &kp.public_key.to_bytes(),
            &kp.secret_key,
        ...
        let result = record.verify(now_unix());
        assert!(matches!(result, Err(PeerRecordError::TooManyAddresses { .. })));
    }
}
