//! # Epoch Reward Distribution — Workload Tracking + sqrt(stake) Rewards
//!
...
use misaka_tokenomics::{
    WorkloadConfig, WorkloadAccumulator, ValidatorWorkloadSnapshot,
    RewardWeightConfig, ValidatorRewardInput, RewardBreakdownSnapshot,
    NetworkWorkloadSummary, distribute_epoch_rewards, compute_network_summary,
};
...
impl RewardEpochTracker {
    ...
    pub fn register_validator(
        ...
        self.accumulators.entry(id).or_insert_with(|| {
            (WorkloadAccumulator::default(), stake, initial_smoothed_score)
        });
    }
    ...
    pub fn on_vote(&mut self, validator: &ValidatorId, signed: bool) {
        if let Some((acc, _, _)) = self.accumulators.get_mut(validator) {
            if signed { acc.signed_votes += 1; } else { acc.missed_votes += 1; }
        }
    }
    ...
    pub fn transition_epoch(&mut self) -> EpochRewardOutput {
        ...
        for (vid, (acc, stake, _smoothed)) in &self.accumulators {
            let snap: ValidatorWorkloadSnapshot = acc.clone().into_snapshot(
                hex::encode(vid),
                epoch,
                &self.config,
            );
    ...
    pub fn epoch_fee_pool(&self) -> u128 { self.epoch_fee_pool }
    pub fn validator_count(&self) -> usize { self.accumulators.len() }
}
...
mod tests {
    ...
    fn vid(n: u8) -> ValidatorId { ValidatorId([n; 32]) }

    #[test]
    fn test_tracker_basic_flow() {
        let mut tracker = RewardEpochTracker::new(
            0,
    ...
    #[test]
    fn test_sqrt_dampens_stake_in_rewards() {
        let mut tracker = RewardEpochTracker::new(
            0,
            WorkloadConfig::default(),
            RewardWeightConfig::default(),
        ...
        // V1: low stake, V2: 100x stake, same work
        ...
        if r1 > 0 {
            let ratio = r2 / r1;
            assert!(ratio < 20, "100x stake should yield ~10x reward, got {ratio}x");
        }
    }

    #[test]
    fn test_carry_forward_preserved() {
        let mut tracker = RewardEpochTracker::new(0, WorkloadConfig::default(), RewardWeightConfig::default());
        tracker.register_validator(vid(1), 100, 0);
