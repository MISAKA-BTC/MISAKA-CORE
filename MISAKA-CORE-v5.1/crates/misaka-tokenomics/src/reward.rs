//! # Reward Distribution — Fee-Only Model with sqrt(stake) Weighting
//!
//! # Core Formula
//!
//! ```text
...
//! 2. **sqrt(stake)**: Dampens large-stake advantage (100x stake → 10x weight)
...
pub struct RewardWeightConfig {
    /// Enable sqrt(stake) compression. If false, stake is unused (pure score).
    pub stake_root_enabled: bool,
    // Note: stake_root_power is always 0.5 (hardcoded as integer sqrt).
    // Future: generalize to nth-root if needed.
}

impl Default for RewardWeightConfig {
    fn default() -> Self {
        Self {
            stake_root_enabled: true,
        }
    }
}
...
/// from stake → sqrt → weight → share → payout.
...
pub struct RewardBreakdownSnapshot {
    ...
    /// `isqrt(active_stake)` — the compressed stake component.
    pub sqrt_scaled_stake: u64,
    /// Smoothed performance score (EMA or epoch score).
    pub smoothed_score: u64,
    /// `sqrt_scaled_stake * smoothed_score` — the full reward weight.
    #[serde(with = "serde_u128_string")]
...
/// - `validator_pool_total`: Total fee pool available (smallest token unit)
/// - `validators`: Slice of per-validator inputs (stake + score)
/// - `config`: Reward weight configuration
///
/// # Returns
///
/// `EpochRewardResult` with per-validator breakdowns and carry-over dust.
///
/// # Determinism
///
...
pub fn distribute_epoch_rewards(
    ...
    // Step 1: Compute reward weights
    let mut entries: Vec<(String, u128, u64, u64, u128)> = validators
        .iter()
        .map(|v| {
            let sqrt_stake = if config.stake_root_enabled {
                sqrt_scaled(v.active_stake)
            } else {
                // If sqrt disabled, stake component is 1 (pure score mode)
                1u64
            };
            let weight = sqrt_stake as u128 * v.smoothed_score as u128;
            (
    ...
    let total_reward_weight: u128 = entries.iter().map(|e| e.4).sum();

    if total_reward_weight == 0 {
        // All validators have zero weight → carry everything forward
        let breakdowns = entries
            .iter()
            .map(|(id, stake, sqrt_s, score, _w)| RewardBreakdownSnapshot {
                validator_id: id.clone(),
                epoch,
                active_stake: *stake,
                sqrt_scaled_stake: *sqrt_s,
    ...
    let breakdowns: Vec<RewardBreakdownSnapshot> = entries
        .iter()
        .map(|(id, stake, sqrt_s, score, weight)| {
            let epoch_reward = if *weight == 0 {
                0u128
            } else {
                // validator_reward = pool * weight / total_weight
                validator_pool_total * *weight / total_reward_weight
            };
            distributed += epoch_reward;

            let share_ppm = (*weight * 1_000_000 / total_reward_weight) as u64;
    ...
    // Step 5: Remainder → carry
    let next_epoch_carry = validator_pool_total - distributed;
...
mod tests {
    ...
    fn test_zero_pool() {
        ...
        assert_eq!(result.breakdowns[0].epoch_reward, 0);
        assert_eq!(result.next_epoch_carry, 0);
    }

    #[test]
    fn test_single_validator_gets_all() {
        let vals = vec![ValidatorRewardInput {
            validator_id: "v1".into(),
            active_stake: 10000,
            smoothed_score: 500,
        ...
        assert_eq!(result.breakdowns[0].epoch_reward, pool);
        assert_eq!(result.next_epoch_carry, 0);
        assert_eq!(result.breakdowns[0].reward_share_ppm, 1_000_000);
    }
    ...
    fn test_equal_validators_equal_reward() {
        ...
        assert_eq!(result.breakdowns[0].epoch_reward, 500_000);
        assert_eq!(result.breakdowns[1].epoch_reward, 500_000);
        assert_eq!(result.next_epoch_carry, 0);
    }
    ...
    fn test_reward_weight_formula_consistency() {
        ...
        // Verify: reward_weight = sqrt_scaled_stake * smoothed_score
        assert_eq!(
            bd.reward_weight,
            bd.sqrt_scaled_stake as u128 * bd.smoothed_score as u128,
            "reward_weight must equal sqrt_scaled_stake * smoothed_score"
        );
    }

    #[test]
    fn test_sqrt_dampens_stake_advantage() {
        // V1: 100 stake, high score
        // V2: 10000 stake (100x), same score
        let vals = vec![
            ValidatorRewardInput {
                validator_id: "v1".into(),
                active_stake: 100,
        ...
        // V2 has 100x stake but should NOT get 100x reward
        // sqrt(10000)/sqrt(100) = 100/10 = 10x weight ratio
        let ratio = r2 / r1.max(1);
        assert!(
            ratio < 20, // Some rounding tolerance
            "100x stake should yield ~10x reward, got {ratio}x"
        );
        assert!(
            ratio >= 8,
            "100x stake should yield ~10x reward, got {ratio}x"
    ...
    #[test]
    fn test_score_dominance_over_stake() {
        ...
        // V1: sqrt(100)*10000 = 10*10000 = 100_000
        // V2: sqrt(1_000_000)*10 = 1000*10 = 10_000
        // V1 should earn MORE than V2 despite 10000x less stake
        assert!(
            r1 > r2,
            "high-score low-stake must beat low-score high-stake: r1={r1}, r2={r2}"
        );
    }
    ...
    fn test_zero_weight_gets_zero_reward() {
        ...
        let idle = result.breakdowns.iter().find(|b| b.validator_id == "idle").expect("idle");
        assert_eq!(idle.epoch_reward, 0, "zero-score validator must get zero reward");
    ...
    fn test_breakdown_serde_roundtrip() {
        let bd = RewardBreakdownSnapshot {
            ...
            sqrt_scaled_stake: 500_000,
            smoothed_score: 831_500,
            reward_weight: 415_750_000_000,
    ...
    #[test]
    fn test_disabled_sqrt_pure_score_mode() {
        let config = RewardWeightConfig {
            stake_root_enabled: false,
        };
        let vals = vec![
            ValidatorRewardInput {
        ...
        // With sqrt disabled, stake is ignored → equal scores = equal reward
        assert_eq!(
            result.breakdowns[0].epoch_reward,
            result.breakdowns[1].epoch_reward,
            "with sqrt disabled, equal scores must yield equal rewards"
        );
    }
}
