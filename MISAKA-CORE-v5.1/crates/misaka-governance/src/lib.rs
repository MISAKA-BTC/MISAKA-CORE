...
impl Proposal {
    ...
    pub fn compute_id(proposer: &Address, title: &str, submitted_at: u64) -> ProposalId {
        let mut h = Sha3_256::new();
        h.update(b"MISAKA:governance:proposal_id:v1:");
        h.update(proposer);
    ...
    pub fn evaluate(&mut self, params: &GovernanceParams) {
        ...
        let quorum_required =
            self.total_power_snapshot * params.quorum_bps / 10_000;
        ...
        let approval_required = yes_plus_no * params.approval_threshold_bps / 10_000;
...
impl GovernanceRegistry {
    ...
    pub fn submit_proposal(
        ...
        let id = Proposal::compute_id(&proposer, &title, current_epoch);
...
mod tests {
    ...
    fn test_submit_and_vote_pass() {
        ...
        let id = gov
            .submit_proposal(
                ...
                "Test Proposal".into(),
                "Description".into(),
                ProposalAction::Signal { text: "test".into() },
                1000,
                100,
                1000, // total power
                &params,
        ...
        gov.cast_vote(&id, [1; 20], VoteChoice::Yes, 600, 105).unwrap();
        gov.cast_vote(&id, [2; 20], VoteChoice::No, 100, 105).unwrap();
    ...
    fn test_quorum_not_met() {
        ...
        gov.cast_vote(&id, [1; 20], VoteChoice::Yes, 200, 5).unwrap();
    ...
    fn test_execution_timelock() {
        ...
        gov.cast_vote(&id, [1; 20], VoteChoice::Yes, 500, 5).unwrap();
        gov.finalize_expired(11, &params);
        assert_eq!(gov.get(&id).unwrap().status, ProposalStatus::Passed);
    ...
    fn test_duplicate_vote_rejected() {
        ...
        let id = gov
            .submit_proposal(
                [1; 20], "Dup Vote".into(), "".into(),
                ProposalAction::Signal { text: "".into() },
                1000, 0, 1000, &params,
            )
            .unwrap();

        gov.cast_vote(&id, [1; 20], VoteChoice::Yes, 500, 3).unwrap();
        assert!(gov.cast_vote(&id, [1; 20], VoteChoice::No, 500, 4).is_err());
    }
