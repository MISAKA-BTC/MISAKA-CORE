...
fn default_ranking_epoch() -> String { "current".into() }
fn default_ranking_sort() -> String { "workload".into() }
fn default_ranking_limit() -> usize { 100 }
...
pub fn v4_privacy_path_surface_status(
    ...
    PrivacyPathSurfaceStatus {
        ...
        note: "runtimePath reflects the currently active path; targetPath reflects the v4 direction.".into(),
    }
}
    ...
    match req.method.as_str() {
        "misaka_getStatus" => {
            let status = NodeStatus {
                ...
                validator_attestation: dag_consumer_surfaces_status(
                    "checkpointDependent",
                    "checkpointAware",
                )
                .validator_attestation,
                tx_status_vocabulary: dag_consumer_surfaces_status(
                    "checkpointDependent",
                    "checkpointAware",
                )
        ...
        "misaka_getValidatorWorkload" => {
            ...
            JsonRpcResponse::error(
                req.id.clone(),
                ERR_INTERNAL,
                "misaka_getValidatorWorkload: requires backend wiring (see misaka-node/dag_rpc.rs)".into(),
            )
        }
...
mod tests {
    ...
    fn test_linear_consumer_surface_defaults() {
        ...
        assert_eq!(surfaces.validator_attestation.bridge_readiness, "notAvailable");
    ...
    fn test_workload_response_serde() {
        let resp = ValidatorWorkloadResponse {
            ...
            workload: WorkloadMetrics {
                proposed_blocks: 21, accepted_blocks: 19, rejected_blocks: 2,
                validated_blocks: 610, signed_votes: 602, missed_votes: 8,
                attestation_count: 599, finalized_contribution_count: 588,
                mempool_tx_seen: 220000, mempool_tx_included: 185000,
                relayed_messages: 92000, uptime_checks_passed: 1438,
                uptime_checks_failed: 2, active_time_slots: 1435,
