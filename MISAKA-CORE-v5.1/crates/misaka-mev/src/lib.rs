...
//! - Sender/receiver are hidden (ring sigs / stealth) → no address-based frontrunning
//! - BUT: fee-based priority ordering can still be manipulated
//! - AND: nullifier observation can leak timing information
//!
...
//!
//! 3. **Nullifier timing protection**: The mempool does not reveal which
//!    nullifiers are pending to RPC queries. Only confirmed nullifiers
...
pub fn fee_anomaly_score(tx_fee: u64, median_fee: u64) -> f64 {
    if median_fee == 0 || tx_fee <= median_fee * 2 {
        return 0.0;
    }
...
pub fn analyze_block_mev(
    tx_fees_and_hashes: &[(u64, [u8; 32])],
) -> (Vec<MevAssessment>, u64) {
