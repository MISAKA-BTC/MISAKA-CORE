...
use crate::dag_block::{Hash, DagBlockHeader, ZERO_HASH};
...
impl WireMessage {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, WireError> {
        ...
        // Type
        let msg_type = WireMessageType::from_u8(data[5])
            .ok_or(WireError::UnknownMessageType(data[5]))?;
...
mod tests {
    ...
    fn test_wire_bad_checksum_rejected() {
        ...
        encoded[last] ^= 0xFF;
        assert!(matches!(WireMessage::decode(&encoded), Err(WireError::ChecksumMismatch)));
    }
    ...
    fn test_wire_oversized_rejected() {
        ...
        assert!(matches!(WireMessage::decode(&data), Err(WireError::PayloadTooLarge(_))));
    }
    ...
    fn test_hash_list_too_many_rejected() {
        ...
        assert!(matches!(decode_hash_list(&data), Err(WireError::TooManyHashes(_))));
    }

    #[test]
    fn test_all_message_types_valid() {
        for id in 0x01..=0x0Au8 {
            assert!(WireMessageType::from_u8(id).is_some(),
                "message type 0x{:02x} should be valid", id);
        }
