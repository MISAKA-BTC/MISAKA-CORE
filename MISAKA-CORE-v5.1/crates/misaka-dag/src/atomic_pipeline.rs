...
fn build_qdag_block_diff(
    ...
    for sealed_tx in txs {
        ...
        if let Some(nf) = conflicting_nf {
            tx_results.push(DiffTxResult::failed_nullifier(sealed_tx.tx_hash, nf, [0u8; 32]));
        } else {
            // TX accepted
            for nf in &sealed_tx.nullifiers {
            ...
            tx_results.push(DiffTxResult::applied(sealed_tx.tx_hash, sealed_tx.nullifiers.clone()));
        }
    }
