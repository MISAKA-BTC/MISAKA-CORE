...
impl PruningPoint {
    ...
    pub fn with_state(
        ...
        Self { block_hash, blue_score, state_root, utxo_commitment, nullifier_commitment }
    }
...
impl PruningManager {
    ...
    pub fn find_new_pruning_point_legacy<S: DagStore>(
        ...
        self.find_new_pruning_point(
            current_max_score, store, ZERO_HASH, ZERO_HASH, ZERO_HASH,
        )
    }
    ...
    pub fn update_and_prune<S: DagStore>(
        ...
        let new_pp = self.find_new_pruning_point(
            current_max_score, store, state_root, utxo_commitment, nullifier_commitment,
        )?;
...
mod tests {
    ...
    fn test_pruning_point_with_state_commitment() {
        let pp = PruningPoint::with_state(
            [0xAA; 32], 500,
    ...
    fn test_find_pruning_point_on_chain() {
        ...
        let pm = PruningManager::new();
        let pp = pm.find_new_pruning_point(
            1200, &store, [0xAA; 32], [0xBB; 32], [0xCC; 32],
        );
        assert!(pp.is_some());
        let pp = pp.unwrap();
    ...
    fn test_collect_prunable_blocks_respects_reachability() {
        ...
        store.set_ghostdag_data(b, GhostDagData {
            ...
            blue_score: 2, blue_work: 2, blues_anticone_sizes: vec![],
        });
    ...
    fn test_prune_level() {
        ...
        store.insert_header(g, make_header(vec![]));
        store.set_ghostdag_data(g, GhostDagData {
            selected_parent: ZERO_HASH, ..Default::default()
        });

        store.insert_header(a, make_header(vec![g]));
        reach.add_child(g, a).unwrap();
        store.set_ghostdag_data(a, GhostDagData {
            selected_parent: g, blue_score: 1, blue_work: 1,
            ..Default::default()
        });
        ...
        reach.add_child(a, b).unwrap();
        store.set_ghostdag_data(b, GhostDagData {
            selected_parent: a, blue_score: 2, blue_work: 2,
            ..Default::default()
        });
        ...
        assert_eq!(pm.prune_level_of(&a, &store, &reach), PruneLevel::BodyPruned);
    }

    #[test]
    fn test_pruning_point_serialization() {
        let pp = PruningPoint::with_state(
            [0xAA; 32], 500, [0x11; 32], [0x22; 32], [0x33; 32],
        );
