...
impl VirtualState {
    ...
    pub fn from_snapshot(
        ...
        Self {
            tip, tip_score, nullifiers, utxos: tracked,
            diff_stack: Vec::new(),
            max_depth: MAX_REORG_DEPTH,
            stats,
        }
    ...
    pub fn get_utxo_with_score(&self, outref: &OutputRef) -> Option<(&TxOutput, u64)> {
        self.utxos.get(outref).map(|(output, score)| (output, *score))
    }
    ...
    pub fn apply_block(&mut self, diff: StateDiff) -> Result<(), VirtualStateError> {
        ...
        for created in &diff.utxos_created {
            self.utxos.insert(created.outref.clone(), (created.output.clone(), creation_score));
        }
    ...
    fn revert_last(&mut self) -> Result<StateDiff, VirtualStateError> {
        ...
        for spent in &diff.utxos_spent {
            self.utxos.insert(spent.outref.clone(), (spent.output.clone(), spent.creation_blue_score));
        }
    ...
    pub fn update_virtual<S: DagStore>(
        ...
        for (i, diff) in self.diff_stack.iter().rev().enumerate() {
            if reachability::is_dag_ancestor_conclusive(
                &diff.block_hash, &new_tip, reachability, store,
            )? {
                revert_count = i;
                break;
            }
    ...
    pub fn resolve<S: DagStore>(
        ...
        let update = self.update_virtual(
            new_tip, new_tip_score, new_diffs.clone(),
            reachability, store,
        )?;

        // Build acceptance data from the applied diffs
        let acceptance_data: Vec<BlockAcceptanceData> = new_diffs.iter()
            .map(|diff| BlockAcceptanceData {
                block_hash: diff.block_hash,
                tx_results: diff.tx_results.iter().map(|tr| TxAcceptance {
                    tx_hash: tr.tx_hash,
                    accepted: matches!(tr.status, DiffTxStatus::Applied | DiffTxStatus::Coinbase),
                    rejection_reason: match &tr.status {
                        ...
                        DiffTxStatus::FailedInvalidProof =>
        ...
        Ok(ResolveResult {
            ...
            blocks_reverted: update.blocks_reverted,
            chain_changes,
...
mod tests {
    ...
    fn test_simple_advance() {
        ...
        store.insert_header(genesis, DagBlockHeader {
            version: DAG_VERSION, parents: vec![], timestamp_ms: 0, tx_root: [0; 32],
        ...
        for (hash, parent, score) in [([1; 32], genesis, 1u64), ([2; 32], [1; 32], 2)] {
            store.insert_header(hash, DagBlockHeader {
                version: DAG_VERSION, parents: vec![parent], timestamp_ms: 0, tx_root: [0; 32],
                proposer_id: [0; 32], nonce: 0, blue_score: 0, bits: 0,
            });
        ...
        let result = vs.update_virtual([2; 32], 2, diffs, &reach, &store).unwrap();
