...
pub enum DagP2pMessage {
    ...
    GetDagBlocks { hashes: Vec<Hash> },
    GetDagTips,
    DagTips { tips: Vec<Hash>, max_blue_score: u64 },
    ...
    NewTx { tx_hash: [u8; 32], fee: u64, size: usize },
    GetTx { tx_hash: [u8; 32] },
    TxData { tx_json: Vec<u8> },
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerInfo {
    /// Peer's advertised listen address (e.g., "49.212.136.189:6690").
...
impl PeerQuality {
    pub fn new() -> Self {
        Self {
            penalty: 0, good_responses: 0, bad_responses: 0,
            last_response: None, banned: false, ban_reason: None,
        }
    }
...
pub fn build_block_locator<F>(
    tip: Hash,
    get_selected_parent: F,
    genesis: Hash,
) -> Vec<Hash>
where
    F: Fn(&Hash) -> Option<Hash>,
{
    ...
    loop {
        if locator.len() >= MAX_LOCATOR_SIZE { break; }
        locator.push(current);
        if current == genesis || current == ZERO_HASH { break; }

        for _ in 0..step {
            match get_selected_parent(&current) {
                Some(parent) if parent != ZERO_HASH => current = parent,
                _ => {
                    if current != genesis { locator.push(genesis); }
                    return locator;
                }
            }
        }
        ...
        steps_taken += 1;
        if steps_taken > 10 { step *= 2; }
    }

    if locator.last() != Some(&genesis) { locator.push(genesis); }
    locator
}
...
pub enum SyncAction {
    Send(DagP2pMessage),
    Ban(String),
    ProcessBlock { hash: Hash, header_json: Vec<u8>, txs_json: Vec<u8> },
    ValidateHeader { hash: Hash, header_json: Vec<u8> },
}
...
impl DagSyncManager {
    ...
    pub fn add_known(&mut self, hash: Hash) { self.local_known.insert(hash); }
    pub fn is_known(&self, hash: &Hash) -> bool { self.local_known.contains(hash) }
    pub fn is_synced(&self) -> bool { matches!(self.state, DagSyncState::Synced) }
    pub fn is_banned(&self) -> bool { self.peer_quality.banned }
    ...
    pub fn on_dag_hello(
        ...
        self.state = DagSyncState::NegotiatingPast {
            remote_tips, remote_blue_score, remote_pruning_point,
        };
        ...
        let (headers_received, validated_headers, invalid_hdrs, peer_has_more) =
            match &mut self.state {
                DagSyncState::DownloadingHeaders {
                    headers_received, validated_headers, invalid_headers, peer_has_more, ..
                } => (headers_received, validated_headers, invalid_headers, peer_has_more),
                _ => {
        ...
        for (hash, header_json) in headers {
            *headers_received += 1;
            ...
            actions.push(SyncAction::ValidateHeader {
                hash: *hash, header_json: header_json.clone(),
            });
        }
        ...
        if has_more && validated_headers.len() < MAX_PENDING_HEADERS {
            ...
            if self.check_rate_limit(true) {
                ...
                actions.push(SyncAction::Send(DagP2pMessage::GetHeaders {
                    after_hash: last, limit: HEADER_BATCH_SIZE as u32,
                }));
            }
        } else {
            ...
            let need: Vec<Hash> = validated_headers.iter()
    ...
    pub fn on_bodies(
        ...
        let (pending, received) = match &mut self.state {
            DagSyncState::DownloadingBodies { pending_bodies, bodies_received } =>
        ...
        for (hash, hdr, txs) in blocks {
            ...
            *received += 1;
            actions.push(SyncAction::ProcessBlock {
                hash: *hash, header_json: hdr.clone(), txs_json: txs.clone(),
            });
        }

        let next: Vec<Hash> = pending.iter()
            .filter(|h| !self.local_known.contains(*h))
            .copied().take(BODY_BATCH_SIZE).collect();
    ...
    pub fn on_new_block(&mut self, hash: Hash, parents: &[Hash]) -> Vec<SyncAction> {
        self.local_known.insert(hash);
        self.peer_quality.record_good_response();
        let missing: Vec<Hash> = parents.iter()
            .filter(|p| **p != ZERO_HASH && !self.local_known.contains(*p))
            .copied().collect();
        if missing.is_empty() { vec![] }
        else { vec![SyncAction::Send(DagP2pMessage::GetDagBlocks { hashes: missing })] }
    }
    ...
    pub fn on_validation_failed(&mut self, hash: &Hash, reason: &str) {
        ...
        if let DagSyncState::DownloadingHeaders { invalid_headers, .. } = &mut self.state {
    ...
    pub fn on_pruning_proof(
        ...
        let result = proof.verify_with_state(
            expected_utxo_commitment,
            expected_nullifier_commitment,
        );
        ...
        match result {
            ...
            other => {
                ...
                if self.peer_quality.banned {
                    self.state = DagSyncState::Banned { reason: reason.clone() };
                }
                Err(reason)
            }
        }
...
mod tests {
    ...
    fn test_block_locator_linear() {
        let locator = build_block_locator(
            h(20),
            |hash| if hash[0] > 0 { Some(h(hash[0] - 1)) } else { None },
            h(0),
        );
    ...
    fn test_full_sync_flow() {
        ...
        assert!(matches!(sync.state, DagSyncState::DownloadingHeaders { .. }));
        ...
        assert_eq!(actions.iter().filter(|a| matches!(a, SyncAction::ValidateHeader { .. })).count(), 5);

        // 4. Bodies
        let bodies: Vec<(Hash, Vec<u8>, Vec<u8>)> = (1..=5u8).map(|i| (h(i), vec![i], vec![])).collect();
        let actions = sync.on_bodies(&bodies);
        assert!(sync.is_synced());
        assert_eq!(actions.iter().filter(|a| matches!(a, SyncAction::ProcessBlock { .. })).count(), 5);
    }
    ...
    fn test_multi_batch_headers() {
        ...
        assert!(matches!(sync.state, DagSyncState::DownloadingHeaders { .. }));
        assert!(actions.iter().any(|a| matches!(a, SyncAction::Send(DagP2pMessage::GetHeaders { .. }))));
    ...
    fn test_validation_failure_tracks_invalid_header() {
        ...
        if let DagSyncState::DownloadingHeaders { invalid_headers, .. } = &sync.state {
