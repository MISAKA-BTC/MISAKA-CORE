...
impl PersistentDagBackend for RocksDbDagStore {
    fn insert_block_atomic(
        ...
        // Duplicate check
        let cf_h = self.db.cf_handle(CF_HEADERS)
            .ok_or_else(|| DagStoreError::Io(format!("CF {} not found", CF_HEADERS)))?;
        if self.db.get_cf(&cf_h, &hash).map_err(|e| DagStoreError::Io(e.to_string()))?.is_some() {
            return Err(DagStoreError::BlockExists(hex::encode(&hash[..8])));
        }

        let h_bytes = serde_json::to_vec(&header).map_err(|e| DagStoreError::Serde(e.to_string()))?;
        let g_bytes = serde_json::to_vec(&ghostdag).map_err(|e| DagStoreError::Serde(e.to_string()))?;

        let cf_g = self.db.cf_handle(CF_GHOSTDAG)
            .ok_or_else(|| DagStoreError::Io(format!("CF {} not found", CF_GHOSTDAG)))?;
        let cf_c = self.db.cf_handle(CF_CHILDREN)
            .ok_or_else(|| DagStoreError::Io(format!("CF {} not found", CF_CHILDREN)))?;
        let cf_t = self.db.cf_handle(CF_TIPS)
        ...
        for parent in &header.parents {
            let existing = self.db.get_cf(&cf_c, parent)
                .map_err(|e| DagStoreError::Io(e.to_string()))?;
        ...
        let tips_bytes = self.db.get_cf(&cf_t, TIPS_KEY)
            .map_err(|e| DagStoreError::Io(e.to_string()))?;
        ...
        let tips_json = serde_json::to_vec(&tips_vec)
            .map_err(|e| DagStoreError::Serde(e.to_string()))?;
        batch.put_cf(&cf_t, TIPS_KEY, &tips_json);

        self.db.write(batch).map_err(|e| DagStoreError::Io(e.to_string()))?;
        Ok(())
    }

    fn get_header(&self, hash: &Hash) -> Option<DagBlockHeader> {
        let cf = self.db.cf_handle(CF_HEADERS)?;
        self.db.get_cf(&cf, hash).ok()?
            .and_then(|b| serde_json::from_slice(&b).ok())
    }
    ...
    fn get_tips(&self) -> Vec<Hash> {
        let cf = match self.db.cf_handle(CF_TIPS) { Some(c) => c, None => return vec![] };
        self.db.get_cf(&cf, TIPS_KEY).ok()
            .flatten()
    ...
    fn get_children(&self, parent: &Hash) -> Vec<Hash> {
        let cf = match self.db.cf_handle(CF_CHILDREN) { Some(c) => c, None => return vec![] };
        self.db.get_cf(&cf, parent).ok()
            .flatten()
            .and_then(|b| serde_json::from_slice(&b).ok())
            .unwrap_or_default()
    }
    ...
    fn is_nullifier_spent(&self, nullifier: &[u8; 32]) -> bool {
        let cf = match self.db.cf_handle(CF_NULLIFIERS) { Some(c) => c, None => return false };
    ...
    fn record_nullifier(&self, nullifier: [u8; 32], tx_hash: [u8; 32]) -> Result<(), DagStoreError> {
        ...
        if self.db.get_cf(&cf, &nullifier).map_err(|e| DagStoreError::Io(e.to_string()))?.is_some() {
            return Err(DagStoreError::NullifierAlreadySpent(hex::encode(&nullifier[..8])));
        }
        self.db.put_cf(&cf, &nullifier, &tx_hash)
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }

    fn block_count(&self) -> u64 {
        let cf = match self.db.cf_handle(CF_HEADERS) { Some(c) => c, None => return 0 };
        let mut count = 0u64;
    ...
    fn max_blue_score(&self) -> u64 {
        let cf = match self.db.cf_handle(CF_GHOSTDAG) { Some(c) => c, None => return 0 };
        let mut max_score = 0u64;
        let iter = self.db.iterator_cf(&cf, rocksdb::IteratorMode::Start);
        for item in iter {
            if let Ok((_key, value)) = item {
                if let Ok(gd) = serde_json::from_slice::<GhostDagData>(&value) {
                    if gd.blue_score > max_score { max_score = gd.blue_score; }
                }
            }
        }
        max_score
    ...
    fn flush(&self) -> Result<(), DagStoreError> {
        self.db.flush().map_err(|e| DagStoreError::Io(e.to_string()))
    }

    fn save_virtual_snapshot(&self, snapshot_json: &[u8]) -> Result<(), DagStoreError> {
        let cf = self.db.cf_handle(CF_VIRTUAL)
            .ok_or_else(|| DagStoreError::Io(format!("CF {} not found", CF_VIRTUAL)))?;
        self.db.put_cf(&cf, VIRTUAL_SNAPSHOT_KEY, snapshot_json)
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }
    ...
    fn save_state_diff(&self, block_hash: Hash, diff_json: &[u8]) -> Result<(), DagStoreError> {
        ...
        self.db.put_cf(&cf, &block_hash, diff_json)
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }
    ...
    fn save_acceptance(&self, block_hash: Hash, data_json: &[u8]) -> Result<(), DagStoreError> {
        ...
        self.db.put_cf(&cf, &block_hash, data_json)
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }
    ...
    fn save_last_accepted(&self, hash: Hash) -> Result<(), DagStoreError> {
        ...
        self.db.put_cf(&cf, META_LAST_ACCEPTED_KEY, &hash)
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }
    ...
    fn save_pruning_point(&self, pp_json: &[u8]) -> Result<(), DagStoreError> {
        let cf = self.db.cf_handle(CF_META)
            .ok_or_else(|| DagStoreError::Io(format!("CF {} not found", CF_META)))?;
        self.db.put_cf(&cf, META_PRUNING_POINT_KEY, pp_json)
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }
    ...
    fn save_schema_version(&self, version: u32) -> Result<(), DagStoreError> {
        let cf = self.db.cf_handle(CF_META)
            .ok_or_else(|| DagStoreError::Io(format!("CF {} not found", CF_META)))?;
        self.db.put_cf(&cf, META_SCHEMA_VERSION_KEY, &version.to_le_bytes())
            .map_err(|e| DagStoreError::Io(e.to_string()))
    }
    ...
    fn validate_recovery(&self) -> RecoveryValidation {
        ...
        // Virtual snapshot
        v.virtual_snapshot_valid = self.load_virtual_snapshot().is_some()
            || self.load_last_accepted().is_none(); // Fresh DB OK
        ...
        if let Some(hash) = self.load_last_accepted() {
            v.diff_journal_valid = self.load_state_diff(&hash).is_some()
                || self.block_count() <= 1; // Genesis-only DB is OK
        }
...
pub fn import_from_memory_dump(
    ...
    for record in &dump.headers {
        let ghostdag = dump.ghostdag.iter()
