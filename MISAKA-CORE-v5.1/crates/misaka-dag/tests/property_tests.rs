...
use misaka_dag::ghostdag::{InMemoryDagStore, DagStore, GhostDagEngine, UniformStakeProvider, DEFAULT_K};
...
impl DagTopology {
    ...
    fn build_with_order(
        ...
        while !remaining.is_empty() {
            ...
            for &idx in &remaining {
                ...
                let parent_hashes: Vec<Hash> = parent_ids.iter().map(|&pid| make_hash(pid)).collect();
                ...
                if parent_ids.is_empty() {
                    ...
                    let data = engine.try_calculate(&hash, &parent_hashes, &store, &reach, &stake).unwrap();
    ...
    fn build_with_order_and_parent_perm(
        ...
        while !remaining.is_empty() {
            max_iterations -= 1;
            if max_iterations == 0 { panic!("cycle"); }
            ...
            for &idx in &remaining {
                ...
                if parent_ids.is_empty() {
                    ...
                    let mut parent_hashes: Vec<Hash> = parent_ids.iter()
                        .map(|&pid| make_hash(pid)).collect();
                    let permuted: Vec<Hash> = perm.iter()
...
fn tiebreak_determinism_under_insertion_order() {
    ...
    for seed in 0..50u64 {
        ...
        store.set_ghostdag_data(genesis_hash, GhostDagData {
            selected_parent: ZERO_HASH, mergeset_blues: vec![], mergeset_reds: vec![],
            blue_score: 0, blue_work: 1,
            blues_anticone_sizes: vec![],
        });
        ...
        for &id in &block_ids {
            ...
            let data = engine.try_calculate(&hash, &[genesis_hash], &store, &reach, &stake).unwrap();
            store.set_ghostdag_data(hash, data);
        }
        ...
        let used_parents: Vec<Hash> = shuffled_parents.into_iter()
        ...
        let data = engine.try_calculate(&merge_hash, &used_parents, &store, &reach, &stake).unwrap();
