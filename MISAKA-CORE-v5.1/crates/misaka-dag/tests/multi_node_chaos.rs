...
impl TestNode {
    ...
    fn insert_block(&mut self, hash: Hash, header: DagBlockHeader) -> bool {
        ...
        if ghostdag_data.selected_parent != ZERO_HASH {
            self.reachability.add_block(hash, ghostdag_data.selected_parent);
        }
    ...
    fn selected_tip(&self) -> Hash {
        ...
        tips.into_iter()
            .max_by(|a, b| {
                let a_score = snapshot.get_ghostdag_data(a).map(|d| d.blue_score).unwrap_or(0);
                let b_score = snapshot.get_ghostdag_data(b).map(|d| d.blue_score).unwrap_or(0);
    ...
    fn test_wide_dag_convergence() {
        ...
        for round in 1..=5u64 {
            for proposer in 0..3usize {
                ...
                let (hash, header) = make_block(
                    vec![prev_per_proposer[proposer]],
                    nonce,
                    round,
                );
