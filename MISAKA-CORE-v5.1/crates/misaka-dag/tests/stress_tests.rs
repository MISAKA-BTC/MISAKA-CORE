...
use misaka_dag::ghostdag::{InMemoryDagStore, DagStore};
use misaka_dag::ghostdag_v2::{GhostDagV2, UniformStakeProvider, DEFAULT_K};
use misaka_dag::dag_block::{Hash, DagBlockHeader, GhostDagData, DAG_VERSION, ZERO_HASH};
use misaka_dag::virtual_state::VirtualState;
use misaka_dag::state_diff::{StateDiff, CreatedUtxo};
...
fn insert_block(
    ...
    let data = engine.try_calculate(&hash, &parents, store, reach, stake)
        .unwrap_or_else(|e| {
            // For stress tests, allow overflow by creating minimal data
            let parent_score = store.get_ghostdag_data(&sp)
...
fn test_wide_dag_200_branches() {
    ...
    let result = engine.try_calculate(
        &merge, &[all_tips[0], all_tips[100]], &store, &reach, &stake,
    );

    // Should either succeed or return MergesetTooLarge (not panic)
    assert!(result.is_ok() || format!("{:?}", result).contains("MergesetTooLarge"),
...
fn test_determinism_insertion_order() {
    ...
    let data1 = {
        ...
        insert_block(hn(1), vec![genesis], &mut store, &mut reach, &engine, &stake);
        insert_block(hn(2), vec![genesis], &mut store, &mut reach, &engine, &stake);
        insert_block(hn(3), vec![hn(1), hn(2)], &mut store, &mut reach, &engine, &stake)
    };
...
fn test_linear_chain_1000_blocks() {
    ...
    for i in 1..=1000u32 {
        ...
        assert_eq!(data.blue_score, i as u64,
            "blue_score mismatch at block {}", i);

        parent = block;
    }

    let elapsed = start.elapsed();
    println!("1000 linear blocks: {:?} ({:?}/block)", elapsed, elapsed / 1000);
...
fn test_virtual_state_snapshot_restore_identity() {
    ...
    for i in 1..=10u8 {
        let diff = StateDiff {
            ...
            utxos_created: vec![CreatedUtxo {
                outref: OutputRef { tx_hash: [i; 32], output_index: 0 },
                output: TxOutput {
    ...
    for i in 1..=10u8 {
        utxos.insert(
            OutputRef { tx_hash: [i; 32], output_index: 0 },
            TxOutput {
                amount: i as u64 * 1000,
                one_time_address: [0xAA; 32],
                pq_stealth: None,
    ...
    assert_eq!(root_after_apply, root_restored,
...
fn test_adversarial_parent_shuffle() {
    ...
    let build_with_parent_order = |parent_order: &[Hash]| -> GhostDagData {
        ...
        for i in 1..=5u32 {
            insert_block(hn(i), vec![genesis], &mut store, &mut reach, &engine, &stake);
        }
        ...
        let _ = reach.add_child(sp, merge);
        engine.try_calculate(&merge, parent_order, &store, &reach, &stake)
...
fn test_reachability_scale_5000() {
    ...
    let start = std::time::Instant::now();
    for i in 1..5000u32 {
        reach.add_child(parent, hn(i))
            .unwrap_or_else(|e| panic!("failed at {}: {}", i, e));
        parent = hn(i);
    }
    let elapsed = start.elapsed();

    println!("5000 reachability insertions: {:?} ({:?}/insert)", elapsed, elapsed / 5000);
...
fn test_multi_diamond_convergence() {
    ...
    for _diamond in 0..3 {
        let left = hn(block_id); block_id += 1;
        let right = hn(block_id); block_id += 1;
        let merge = hn(block_id); block_id += 1;

        insert_block(left, vec![last_merge], &mut store, &mut reach, &engine, &stake);
        insert_block(right, vec![last_merge], &mut store, &mut reach, &engine, &stake);
        insert_block(merge, vec![left, right], &mut store, &mut reach, &engine, &stake);
    ...
    for h in &all_hashes {
        assert!(order_set.contains(h),
            "block {} must be in total ordering", hex::encode(&h[..4]));
    }

    // No duplicates in ordering
    assert_eq!(order.len(), order_set.len(), "total ordering must have no duplicates");
}
