...
use crate::dag_p2p_surface::{
    observe_dag_p2p_message, DagP2pDirection, DagP2pObservationState,
};
...
impl DagP2pEventLoop {
    ...
    async fn handle_inbound(&mut self, event: InboundDagEvent) {
        ...
        self.observation
            .write()
            .await
            .record(DagP2pDirection::Inbound, &event.message, Some(&peer_id));
        ...
        match event.message {
            ...
            DagP2pMessage::GetPeers => {
                ...
                let peers: Vec<misaka_dag::dag_p2p::PeerInfo> = self.inventories.iter()
                    .filter_map(|(id, inv)| {
                        inv.listen_addr.as_ref().map(|addr| misaka_dag::dag_p2p::PeerInfo {
                            listen_addr: addr.clone(),
                            peer_id: hex::encode(id),
                ...
                if !peers.is_empty() {
                    self.send_to_peer(peer_id, DagP2pMessage::Peers { peers }).await;
                }
            }
            ...
            DagP2pMessage::Peers { peers } => {
                ...
                // Store discovered addresses in observation state for the transport
                // layer to pick up via periodic polling.
                let mut obs = self.observation.write().await;
                for peer_info in peers {
                    obs.discovered_peers.push(peer_info.listen_addr);
                }
                // Cap discovered list to prevent memory growth
                if obs.discovered_peers.len() > 100 {
                    obs.discovered_peers.drain(0..obs.discovered_peers.len() - 100);
                }
            }
    ...
    async fn send_to_peer(&self, peer_id: [u8; 20], message: DagP2pMessage) {
        ...
        self.observation
            .write()
            .await
            .record(DagP2pDirection::OutboundUnicast, &message, Some(&peer_id));
    ...
    async fn broadcast(&self, message: DagP2pMessage) {
        observe_dag_p2p_message(
            DagP2pDirection::OutboundBroadcast,
            &message,
            None,
        );
        self.observation
...
mod tests {
    ...
    async fn test_event_loop_serves_get_dag_tips() {
        ...
        match outbound.message {
            DagP2pMessage::DagTips { tips, max_blue_score } => {
