...
async fn main() -> anyhow::Result<()> {
    ...
    // Catches edge cases where debug builds accidentally run against mainnet.
    {
        ...
        if cli.experimental_zk_path {
            info!(
                "ZK block path ENABLED — txs with zk_proof will use CompositeProof verification"
            );
        }
    }
...
fn load_or_create_local_dag_validator(
    ...
    use misaka_crypto::keystore::{
        encrypt_keystore, decrypt_keystore, save_keystore, load_keystore,
        is_plaintext_keyfile,
    };
    ...
    let keypair_and_identity = if encrypted_path.exists() {
        ...
        if validator_id_vec.len() != 20 {
            anyhow::bail!(
                ...
                encrypted_path.display(), validator_id_vec.len()
            );
        }
        validator_id.copy_from_slice(&validator_id_vec);

        let public_key = ValidatorPqPublicKey::from_bytes(
            &hex::decode(&keystore.public_key_hex)?
        ).map_err(anyhow::Error::msg)?;

        let secret_key = ValidatorPqSecretKey { pq_sk: secret_bytes };
        ...
        let public_key = ValidatorPqPublicKey::from_bytes(
            &hex::decode(&persisted.public_key_hex)?
        ).map_err(anyhow::Error::msg)?;
        ...
        if let Err(e) = std::fs::rename(&plaintext_path, &backup_path) {
            warn!("Could not rename plaintext key file: {} (delete manually)", e);
        } else {
            warn!(
...
async fn start_dag_node(
    ...
    )> = local_validator.as_ref().map(|lv| {
        (lv.keypair.public_key.clone(), lv.keypair.secret_key.clone())
    });
    ...
    #[cfg(feature = "rocksdb")]
    {
        use misaka_dag::persistent_store::{RocksDbDagStore, PersistentDagBackend};
        ...
        match RocksDbDagStore::open(&rocks_path, genesis_hash, {
            ...
            snapshot.get_header(&genesis_hash).cloned().unwrap_or_else(|| {
            ...
            Ok(rocks) => {
                ...
                if rocks_count <= 1 && mem_count > 1 {
                    ...
                    if let Err(e) = misaka_dag::persistent_store::import_from_memory_dump(&rocks, &dump) {
                        error!("RocksDB migration failed: {} — continuing with in-memory only", e);
                    } else {
                ...
                info!(
                    ...
                    rocks_path.display(),
                    dag_node_state.persistent_backend.as_ref()
                        .map(|r| r.block_count())
                        .unwrap_or(0),
                );
            }
    ...
    let seed_addrs: Vec<SocketAddr> = cli.seeds.iter()
        .filter_map(|s| {
    ...
    let staking_config = if cli.chain_id == 1 {
        misaka_consensus::staking::StakingConfig::default() // Mainnet: 10M MISAKA
    } else {
        misaka_consensus::staking::StakingConfig::testnet()
    };
    let validator_registry = Arc::new(RwLock::new(
        misaka_consensus::staking::StakingRegistry::new(staking_config),
    ));
    let current_epoch: Arc<RwLock<u64>> = Arc::new(RwLock::new(0));
    info!("Layer 6: Validator staking registry initialized (min_stake={})",
          if cli.chain_id == 1 { "10M MISAKA" } else { "10 MISAKA (testnet)" });
...
async fn start_v1_node(
    ...
    let (utxo_set, restored_height) = match misaka_storage::utxo_set::UtxoSet::load_from_file(
        &utxo_snapshot_path,
        1000,
    ) {
        Ok(Some(restored)) => {
    ...
    info!(
        ...
        cli.validator_index,
        cli.validators,
        if cli.experimental_zk_path { " | privacyPath=ZK" } else { "" }
    );
    ...
    async fn test_remote_vote_gossip_forms_live_local_quorum_when_checkpoint_matches() {
        ...
        let server = tokio::spawn(async move {
            crate::dag_rpc::run_dag_rpc_server_with_observation(
                server_state, None, None, Arc::new(RwLock::new(0)), addr,
            )
                .await
