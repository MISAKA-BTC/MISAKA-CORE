...
use crate::rpc_auth::{ApiKeyState, require_api_key};

use misaka_mempool::UtxoMempool;
use misaka_rpc::{dag_consumer_surfaces_status, v4_privacy_path_surface_status};
...
fn dag_consumer_surfaces_json(state: &DagNodeState) -> serde_json::Value {
    ...
    serde_json::to_value(dag_consumer_surfaces_status(
        bridge_readiness,
        explorer_confirmation_level,
    ))
...
pub async fn run_dag_rpc_server_with_observation(
    ...
    let write_routes = Router::new()
        ...
        .route("/api/submit_checkpoint_vote", post(dag_submit_checkpoint_vote))
    ...
    if let Some(registry) = validator_registry {
        ...
        let validator_router = crate::validator_api::validator_api_router(validator_state);
        app = app.merge(Router::new().nest("/api/v1/validators", validator_router));
    ...
    let app = app
        .layer(cors)
        // Keep DAG RPC aligned with linear RPC guardrails.
...
mod tests {
    ...
    fn test_dag_consumer_surfaces_json_tracks_checkpoint_status() {
        ...
        assert_eq!(
            ...
            serde_json::Value::String("checkpointDependent".into())
        );
        assert_eq!(
    ...
    fn test_dag_privacy_path_surface_json_targets_v4_path() {
        ...
        assert_eq!(json["runtimePath"], serde_json::Value::String("zeroKnowledge".into()));
        assert_eq!(json["targetPath"], serde_json::Value::String("zeroKnowledge".into()));
        assert_eq!(
