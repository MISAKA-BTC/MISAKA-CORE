                ...
                match TcpStream::connect(addr).await {
                    Ok(stream) => {
                        ...
                        if let Err(e) = write_p2p_message(
                            &mut writer,
                            &hello,
                            P2pTransportLane::HandshakeHello,
                        )
                        .await
                        {
                                    ...
                                    Ok(msg) => {
                                        ...
                                        let entries = addrs
                                            ...
                                            .map(|(address, node_name)| {
                                                PeerDiscoveryEntry::from_legacy_tuple(address, node_name)
                                            })
                                            .collect::<Vec<_>>();
                                        let summary = sanitize_peer_discovery_entries(
                                            &entries,
                                            |address| validate_advertise_addr(address).is_some(),
                                        );
                                        ...
                                        for entry in &summary.accepted {
                                            debug!("  discovered: {} ({})", entry.address, entry.node_name);
                                        }
                                        ...
                                        let advertisable = peer_list
                                            ...
                                            .collect::<Vec<_>>();
                                        drop(peer_list);
                                        let peer_list = build_peer_discovery_response(&advertisable);
...
mod tests {
    ...
    fn test_relay_transport_lane_classifies_sync_messages() {
        ...
        assert_eq!(relay_transport_lane(&relay), P2pTransportLane::BroadcastRelay);
    }
