...
export default function Error({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-32 text-center page-enter">
      ...
      {error.message && (
        <p className="text-xs text-slate-600 font-mono bg-surface-200 px-3 py-1.5 rounded-lg mb-6 max-w-lg break-all">
          {error.message}
        </p>
      )}
      <button
        onClick={reset}
        className="px-5 py-2.5 rounded-lg text-sm font-medium bg-misaka-500/20 text-misaka-400 hover:bg-misaka-500/30 transition-colors"
      >
        Try again
      </button>
    </div>
