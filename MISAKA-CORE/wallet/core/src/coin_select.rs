        ...
        if *current_sum >= needed && (*current_sum - needed) < DUST_THRESHOLD {
            ...
            if best.is_none()
                || current.iter().filter(|&&x| x).count()
                    < best.as_ref().map_or(usize::MAX, |b| b.iter().filter(|&&x| x).count())
            {
...
fn select_smallest_sufficient(
    ...
    let mut sorted: Vec<&UtxoCandidate> = utxos.iter().collect();
    sorted.sort_by(|a, b| b.amount.cmp(&a.amount));
    accumulate(&sorted, target, base_fee, fee_per_input, "smallest_sufficient")
}
...
mod tests {
    ...
    fn test_smallest_sufficient() {
        ...
        assert_eq!(r.change, 100); // 4200 - 4000 - 100(base) - 0(per input)
    }
    ...
    fn test_fee_per_input_scaling() {
        ...
        assert!(result.is_ok());
        let r = result.expect("test: fee scaling");
        // fee = 100 (base) + 500 * 2 (inputs) = 1100
        assert_eq!(r.fee, 1100);
        assert_eq!(r.total_input, 6000);
        assert_eq!(r.change, 6000 - 5000 - 1100); // = -100?? No... let me reconsider
        // total = 6000, needed = 5000 + 1100 = 6100 > 6000 → should fail?
        // Actually fee grows as we add inputs. Let me fix the assertion.
    }
