//! Network connectivity monitoring for wallet sync
//!
...
/// Configuration for the NetworkMonitor subsystem.
...
/// Runtime state for NetworkMonitor.
pub struct NetworkMonitorRuntime {
    config: NetworkMonitorConfig,
    entries: parking_lot::RwLock<HashMap<u64, NetworkMonitorEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: NetworkMonitorStats,
    event_buffer: parking_lot::Mutex<Vec<NetworkMonitorEvent>>,
}

/// Individual entry tracked by NetworkMonitor.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkMonitorEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by NetworkMonitor.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for NetworkMonitor.
...
impl NetworkMonitorStats {
    pub fn snapshot(&self) -> NetworkMonitorStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        NetworkMonitorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkMonitorStatsSnapshot {
...
impl NetworkMonitorRuntime {
    /// Create a new NetworkMonitor runtime.
    pub fn new(config: NetworkMonitorConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: NetworkMonitorStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, NetworkMonitorError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(NetworkMonitorError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, NetworkMonitorEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(NetworkMonitorEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), NetworkMonitorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NetworkMonitorError::NotFound(id))?;
        ...
        self.emit_event(NetworkMonitorEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, NetworkMonitorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NetworkMonitorError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(NetworkMonitorEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<NetworkMonitorEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<NetworkMonitorEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(NetworkMonitorEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> NetworkMonitorStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<NetworkMonitorEvent> {
    ...
    fn emit_event(&self, event: NetworkMonitorEvent) {
...
/// Errors from NetworkMonitor operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = NetworkMonitorRuntime::new(NetworkMonitorConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = NetworkMonitorConfig { retry_count: 2, ..Default::default() };
        let rt = NetworkMonitorRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = NetworkMonitorConfig { max_entries: 2, ..Default::default() };
        let rt = NetworkMonitorRuntime::new(config);
