//! Persistent transaction history storage and indexing
//!
...
/// Configuration for TransactionStore.
...
/// TransactionStore operational state.
pub struct TransactionStoreState {
    config: TransactionStoreConfig,
    entries: parking_lot::RwLock<HashMap<u64, TransactionStoreEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<TransactionStoreTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: TransactionStoreStats,
    events: parking_lot::Mutex<Vec<TransactionStoreEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in TransactionStore.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TransactionStoreEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: TransactionStoreEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum TransactionStoreEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct TransactionStoreTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from TransactionStore.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Statistics for TransactionStore.
...
impl TransactionStoreStats {
    pub fn snapshot(&self) -> TransactionStoreStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TransactionStoreStatsSnapshot {
...
impl TransactionStoreState {
    pub fn new(config: TransactionStoreConfig) -> Self {
        Self {
            config,
            ...
            stats: TransactionStoreStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, TransactionStoreError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(TransactionStoreError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = TransactionStoreEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: TransactionStoreEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(TransactionStoreTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(TransactionStoreEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), TransactionStoreError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TransactionStoreError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = TransactionStoreEntryState::Completed;
        ...
        self.emit(TransactionStoreEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, TransactionStoreError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TransactionStoreError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = TransactionStoreEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = TransactionStoreEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(TransactionStoreEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), TransactionStoreError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TransactionStoreError::NotFound(id))?;
        entry.state = TransactionStoreEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<TransactionStoreEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<TransactionStoreEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: TransactionStoreEntryState) -> Vec<TransactionStoreEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, TransactionStoreEntryState::Completed | TransactionStoreEntryState::Failed | TransactionStoreEntryState::Cancelled | TransactionStoreEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(TransactionStoreEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> TransactionStoreStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<TransactionStoreEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = TransactionStoreState::new(TransactionStoreConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = TransactionStoreConfig { retry_count: 2, ..Default::default() };
        let state = TransactionStoreState::new(config);
