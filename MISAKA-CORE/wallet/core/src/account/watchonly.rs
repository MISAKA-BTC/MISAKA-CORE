...
impl WatchOnlyAccount {
    pub fn new(id: AccountId, name: String, pubkeys: Vec<Vec<u8>>) -> Self {
        let addresses: Vec<String> = pubkeys.iter().map(|pk| {
...
impl Account for WatchOnlyAccount {
    ...
    fn kind(&self) -> AccountKind { AccountKind::WatchOnly }
    fn receive_address(&self) -> String {
        self.addresses.first().cloned().unwrap_or_default()
    }
    fn change_address(&self) -> String { self.receive_address() }
    fn next_receive_address(&mut self) -> String { self.receive_address() }
    fn next_change_address(&mut self) -> String { self.receive_address() }
    fn can_sign(&self) -> bool { false }
}
