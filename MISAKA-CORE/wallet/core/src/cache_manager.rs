//! Wallet data caching for performance optimization
//!
...
/// CacheManager operational state.
pub struct CacheManagerState {
    config: CacheManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, CacheManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<CacheManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CacheManagerStats,
    events: parking_lot::Mutex<Vec<CacheManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in CacheManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CacheManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: CacheManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum CacheManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct CacheManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from CacheManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum CacheManagerEvent {
...
impl CacheManagerStats {
    pub fn snapshot(&self) -> CacheManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CacheManagerStatsSnapshot {
...
impl CacheManagerState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, CacheManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(CacheManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = CacheManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: CacheManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(CacheManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(CacheManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), CacheManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CacheManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = CacheManagerEntryState::Completed;
        ...
        self.emit(CacheManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, CacheManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CacheManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = CacheManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = CacheManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(CacheManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), CacheManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CacheManagerError::NotFound(id))?;
        entry.state = CacheManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<CacheManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<CacheManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: CacheManagerEntryState) -> Vec<CacheManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, CacheManagerEntryState::Completed | CacheManagerEntryState::Failed | CacheManagerEntryState::Cancelled | CacheManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(CacheManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> CacheManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<CacheManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: CacheManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = CacheManagerState::new(CacheManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = CacheManagerConfig { retry_count: 2, ..Default::default() };
        let state = CacheManagerState::new(config);
