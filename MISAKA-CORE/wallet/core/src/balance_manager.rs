...
impl BalanceManager {
    ...
    pub fn update_daa_score(&self, score: u64) {
        let old_score = self.current_daa_score.swap(score, std::sync::atomic::Ordering::Relaxed);
        if score > old_score {
    ...
    pub fn add_utxo(&self, utxo: TrackedUtxo) -> bool {
        ...
        self.by_account.write().entry(account_id).or_default().push(outpoint);
        self.by_address.write().entry(address).or_default().push(outpoint);
    ...
    pub fn mark_spent(&self, outpoint: &Outpoint, spending_tx: [u8; 32]) -> Option<u64> {
        ...
        if let Some(utxo) = utxos.get_mut(outpoint) {
            ...
            drop(utxos);
            self.invalidate_cache(account_id);
            self.emit_event(account_id, BalanceChangeType::UtxoSpent, amount,
    ...
    pub fn lock_utxos(&self, outpoints: &[Outpoint]) -> Result<(), String> {
        ...
        for op in outpoints {
            ...
            if utxo.is_spent { return Err("UTXO already spent".into()); }
            if utxo.is_locked { return Err("UTXO already locked".into()); }
        }
        // Lock all
        for op in outpoints {
    ...
    pub fn get_balance(&self, account_id: u64) -> DetailedBalance {
        ...
        self.balance_cache.write().insert(account_id, balance.clone());
        balance
    }
    ...
    fn compute_balance(&self, account_id: u64) -> DetailedBalance {
        ...
        let current_score = self.current_daa_score.load(std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn get_spendable_utxos(&self, account_id: u64) -> Vec<TrackedUtxo> {
        ...
        let mut result: Vec<TrackedUtxo> = by_account.get(&account_id)
            .map(|ops| ops.iter()
                .filter_map(|op| utxos.get(op))
                .filter(|u| !u.is_spent && !u.is_locked && u.is_confirmed
                    && (!u.is_coinbase || current_score.saturating_sub(u.block_daa_score) >= COINBASE_MATURITY)
    ...
    fn refresh_maturity(&self, current_score: u64) {
        ...
        for utxo in utxos.values_mut() {
            if !utxo.is_mature && utxo.is_confirmed && !utxo.is_spent {
                let depth = current_score.saturating_sub(utxo.block_daa_score);
                let required = if utxo.is_coinbase { COINBASE_MATURITY } else { MIN_CONFIRMATIONS };
                if depth >= required {
                    utxo.is_mature = true;
    ...
    pub fn cleanup_spent(&self, max_age_scores: u64) -> usize {
        ...
        let to_remove: Vec<Outpoint> = utxos.iter()
    ...
    fn emit_event(&self, account_id: u64, change_type: BalanceChangeType, amount: u64, tx_id: Option<String>) {
        let balance = self.compute_balance(account_id);
    ...
    pub fn total_utxo_count(&self) -> usize { self.utxos.read().len() }

    /// Get account count.
    pub fn account_count(&self) -> usize { self.by_account.read().len() }
}
...
mod tests {
    ...
    fn make_utxo(id: u8, amount: u64, account: u64, score: u64, coinbase: bool) -> TrackedUtxo {
        TrackedUtxo {
            outpoint: Outpoint { tx_id: [id; 32], index: 0 },
            amount,
    ...
    fn test_lock_unlock() {
        ...
        let op = Outpoint { tx_id: [1; 32], index: 0 };
