//! Wallet notification processing from node events
//!
...
/// Configuration for NotificationHandler.
...
/// NotificationHandler operational state.
pub struct NotificationHandlerState {
    config: NotificationHandlerConfig,
    entries: parking_lot::RwLock<HashMap<u64, NotificationHandlerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<NotificationHandlerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: NotificationHandlerStats,
    events: parking_lot::Mutex<Vec<NotificationHandlerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in NotificationHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NotificationHandlerEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: NotificationHandlerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum NotificationHandlerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct NotificationHandlerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from NotificationHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum NotificationHandlerEvent {
...
/// Statistics for NotificationHandler.
...
impl NotificationHandlerStats {
    pub fn snapshot(&self) -> NotificationHandlerStatsSnapshot {
        ...
        NotificationHandlerStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NotificationHandlerStatsSnapshot {
...
impl NotificationHandlerState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, NotificationHandlerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(NotificationHandlerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = NotificationHandlerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: NotificationHandlerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(NotificationHandlerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(NotificationHandlerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), NotificationHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NotificationHandlerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = NotificationHandlerEntryState::Completed;
        ...
        self.emit(NotificationHandlerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, NotificationHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NotificationHandlerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = NotificationHandlerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = NotificationHandlerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(NotificationHandlerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), NotificationHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NotificationHandlerError::NotFound(id))?;
        entry.state = NotificationHandlerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<NotificationHandlerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<NotificationHandlerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: NotificationHandlerEntryState) -> Vec<NotificationHandlerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, NotificationHandlerEntryState::Completed | NotificationHandlerEntryState::Failed | NotificationHandlerEntryState::Cancelled | NotificationHandlerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(NotificationHandlerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> NotificationHandlerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<NotificationHandlerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: NotificationHandlerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = NotificationHandlerState::new(NotificationHandlerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = NotificationHandlerConfig { retry_count: 2, ..Default::default() };
        let state = NotificationHandlerState::new(config);
