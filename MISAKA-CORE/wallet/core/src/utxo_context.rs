//! UTXO context resolution for transaction signing
//!
...
/// Configuration for UtxoContext.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoContextConfig {
    pub enabled: bool,
...
impl Default for UtxoContextConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// UtxoContext operational state.
pub struct UtxoContextState {
    config: UtxoContextConfig,
    entries: parking_lot::RwLock<HashMap<u64, UtxoContextEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<UtxoContextTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: UtxoContextStats,
    events: parking_lot::Mutex<Vec<UtxoContextEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in UtxoContext.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoContextEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: UtxoContextEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum UtxoContextEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct UtxoContextTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from UtxoContext.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum UtxoContextEvent {
...
/// Statistics for UtxoContext.
#[derive(Debug, Default)]
pub struct UtxoContextStats {
...
impl UtxoContextStats {
    pub fn snapshot(&self) -> UtxoContextStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoContextStatsSnapshot {
...
impl UtxoContextState {
    pub fn new(config: UtxoContextConfig) -> Self {
        Self {
            config,
            ...
            stats: UtxoContextStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, UtxoContextError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(UtxoContextError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = UtxoContextEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: UtxoContextEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(UtxoContextTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(UtxoContextEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), UtxoContextError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoContextError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = UtxoContextEntryState::Completed;
        ...
        self.emit(UtxoContextEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, UtxoContextError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoContextError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = UtxoContextEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = UtxoContextEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(UtxoContextEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), UtxoContextError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoContextError::NotFound(id))?;
        entry.state = UtxoContextEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<UtxoContextEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<UtxoContextEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: UtxoContextEntryState) -> Vec<UtxoContextEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, UtxoContextEntryState::Completed | UtxoContextEntryState::Failed | UtxoContextEntryState::Cancelled | UtxoContextEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(UtxoContextEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> UtxoContextStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<UtxoContextEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: UtxoContextEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = UtxoContextState::new(UtxoContextConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = UtxoContextConfig { retry_count: 2, ..Default::default() };
        let state = UtxoContextState::new(config);
