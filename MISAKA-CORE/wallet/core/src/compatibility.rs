//! Backward compatibility layer for wallet format migration
//!
...
/// Configuration for the Compatibility subsystem.
...
/// Runtime state for Compatibility.
pub struct CompatibilityRuntime {
    config: CompatibilityConfig,
    entries: parking_lot::RwLock<HashMap<u64, CompatibilityEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CompatibilityStats,
    event_buffer: parking_lot::Mutex<Vec<CompatibilityEvent>>,
}

/// Individual entry tracked by Compatibility.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Events emitted by Compatibility.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for Compatibility.
...
impl CompatibilityStats {
    pub fn snapshot(&self) -> CompatibilityStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        CompatibilityStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CompatibilityStatsSnapshot {
...
impl CompatibilityRuntime {
    /// Create a new Compatibility runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, CompatibilityError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(CompatibilityError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, CompatibilityEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(CompatibilityEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), CompatibilityError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CompatibilityError::NotFound(id))?;
        ...
        self.emit_event(CompatibilityEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, CompatibilityError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CompatibilityError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(CompatibilityEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<CompatibilityEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<CompatibilityEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(CompatibilityEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> CompatibilityStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<CompatibilityEvent> {
    ...
    fn emit_event(&self, event: CompatibilityEvent) {
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = CompatibilityRuntime::new(CompatibilityConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = CompatibilityConfig { retry_count: 2, ..Default::default() };
        let rt = CompatibilityRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = CompatibilityConfig { max_entries: 2, ..Default::default() };
        let rt = CompatibilityRuntime::new(config);
