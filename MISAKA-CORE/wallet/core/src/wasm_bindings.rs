//! WASM bindings for browser wallet integration
//!
...
/// Configuration for WasmBindings.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WasmBindingsConfig {
    pub enabled: bool,
...
impl Default for WasmBindingsConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// WasmBindings operational state.
pub struct WasmBindingsState {
    config: WasmBindingsConfig,
    entries: parking_lot::RwLock<HashMap<u64, WasmBindingsEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<WasmBindingsTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: WasmBindingsStats,
    events: parking_lot::Mutex<Vec<WasmBindingsEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in WasmBindings.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WasmBindingsEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: WasmBindingsEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum WasmBindingsEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct WasmBindingsTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from WasmBindings.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum WasmBindingsEvent {
...
/// Statistics for WasmBindings.
#[derive(Debug, Default)]
pub struct WasmBindingsStats {
...
impl WasmBindingsStats {
    pub fn snapshot(&self) -> WasmBindingsStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WasmBindingsStatsSnapshot {
...
impl WasmBindingsState {
    pub fn new(config: WasmBindingsConfig) -> Self {
        Self {
            config,
            ...
            stats: WasmBindingsStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, WasmBindingsError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(WasmBindingsError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = WasmBindingsEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: WasmBindingsEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(WasmBindingsTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(WasmBindingsEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), WasmBindingsError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WasmBindingsError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = WasmBindingsEntryState::Completed;
        ...
        self.emit(WasmBindingsEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, WasmBindingsError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WasmBindingsError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = WasmBindingsEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = WasmBindingsEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(WasmBindingsEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), WasmBindingsError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WasmBindingsError::NotFound(id))?;
        entry.state = WasmBindingsEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<WasmBindingsEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<WasmBindingsEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: WasmBindingsEntryState) -> Vec<WasmBindingsEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, WasmBindingsEntryState::Completed | WasmBindingsEntryState::Failed | WasmBindingsEntryState::Cancelled | WasmBindingsEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(WasmBindingsEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> WasmBindingsStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<WasmBindingsEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: WasmBindingsEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = WasmBindingsState::new(WasmBindingsConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = WasmBindingsConfig { retry_count: 2, ..Default::default() };
        let state = WasmBindingsState::new(config);
