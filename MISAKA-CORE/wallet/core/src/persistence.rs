...
impl PersistenceState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, PersistenceError> {
        ...
        let entry = PersistenceEntry {
            ...
            state: PersistenceEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(PersistenceTask { entry_id: id, scheduled_at: now, priority });
    ...
    pub fn complete(&self, id: u64) -> Result<(), PersistenceError> {
        ...
        self.emit(PersistenceEvent::Completed { id, duration_ms: duration });
        Ok(())
    }
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, PersistenceError> {
        ...
        self.emit(PersistenceEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }
    ...
    pub fn get(&self, id: u64) -> Option<PersistenceEntry> { self.entries.read().get(&id).cloned() }
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, PersistenceEntryState::Completed | PersistenceEntryState::Failed | PersistenceEntryState::Cancelled | PersistenceEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(PersistenceEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> PersistenceStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<PersistenceEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = PersistenceConfig { retry_count: 2, ..Default::default() };
