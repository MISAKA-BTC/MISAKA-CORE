//! Error recovery and retry logic for wallet operations
//!
...
/// Configuration for the ErrorRecovery subsystem.
...
/// Runtime state for ErrorRecovery.
pub struct ErrorRecoveryRuntime {
    config: ErrorRecoveryConfig,
    entries: parking_lot::RwLock<HashMap<u64, ErrorRecoveryEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ErrorRecoveryStats,
    event_buffer: parking_lot::Mutex<Vec<ErrorRecoveryEvent>>,
}

/// Individual entry tracked by ErrorRecovery.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ErrorRecoveryEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by ErrorRecovery.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ErrorRecoveryEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for ErrorRecovery.
...
impl ErrorRecoveryStats {
    pub fn snapshot(&self) -> ErrorRecoveryStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        ErrorRecoveryStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ErrorRecoveryStatsSnapshot {
...
impl ErrorRecoveryRuntime {
    /// Create a new ErrorRecovery runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, ErrorRecoveryError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(ErrorRecoveryError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, ErrorRecoveryEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(ErrorRecoveryEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), ErrorRecoveryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ErrorRecoveryError::NotFound(id))?;
        ...
        self.emit_event(ErrorRecoveryEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, ErrorRecoveryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ErrorRecoveryError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(ErrorRecoveryEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<ErrorRecoveryEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<ErrorRecoveryEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(ErrorRecoveryEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> ErrorRecoveryStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<ErrorRecoveryEvent> {
    ...
    fn emit_event(&self, event: ErrorRecoveryEvent) {
...
/// Errors from ErrorRecovery operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = ErrorRecoveryRuntime::new(ErrorRecoveryConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = ErrorRecoveryConfig { retry_count: 2, ..Default::default() };
        let rt = ErrorRecoveryRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = ErrorRecoveryConfig { max_entries: 2, ..Default::default() };
        let rt = ErrorRecoveryRuntime::new(config);
