...
use crate::{TxPlan, TxPlanInput, TxPlanOutput, OwnedUtxo};
...
impl TxBuilder {
    pub fn new(config: TxBuilderConfig) -> Self { Self { config } }
    ...
    pub fn build_unsigned(&self, plan: &TxPlan) -> UnsignedTransaction {
        let inputs: Vec<UnsignedInput> = plan.inputs.iter().map(|i| UnsignedInput {
        ...
        let outputs: Vec<TransactionOutput> = plan.outputs.iter().map(|o| TransactionOutput {
            value: o.amount,
