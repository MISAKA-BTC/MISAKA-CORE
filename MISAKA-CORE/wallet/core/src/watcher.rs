//! Address watcher service for real-time balance updates
//!
...
pub struct WatcherState {
    ...
    entries: parking_lot::RwLock<HashMap<u64, WatcherEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<WatcherTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: WatcherStats,
    events: parking_lot::Mutex<Vec<WatcherEvent>>,
...
impl WatcherState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, WatcherError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(WatcherError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = WatcherEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: WatcherEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(WatcherTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(WatcherEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), WatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WatcherError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = WatcherEntryState::Completed;
        ...
        self.emit(WatcherEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, WatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WatcherError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = WatcherEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = WatcherEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(WatcherEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), WatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WatcherError::NotFound(id))?;
        entry.state = WatcherEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<WatcherEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<WatcherEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: WatcherEntryState) -> Vec<WatcherEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, WatcherEntryState::Completed | WatcherEntryState::Failed | WatcherEntryState::Cancelled | WatcherEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(WatcherEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> WatcherStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<WatcherEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = WatcherState::new(WatcherConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = WatcherConfig { retry_count: 2, ..Default::default() };
        let state = WatcherState::new(config);
