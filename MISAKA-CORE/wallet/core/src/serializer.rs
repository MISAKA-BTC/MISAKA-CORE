...
impl WalletStateFile {
    ...
    pub fn save(&self, state: &WalletStateSnapshot) -> Result<(), WalletFileError> {
        let data = match self.format {
            ...
            Format::Binary => serde_json::to_vec(state)
                .map_err(|e| WalletFileError::Serialize(e.to_string()))?,
        };
    ...
    fn load_from_path(&self, path: &Path) -> Result<WalletStateSnapshot, WalletFileError> {
        let data = std::fs::read(path)
            .map_err(|e| WalletFileError::Read(e.to_string()))?;
...
pub enum WalletUpdate {
    UtxoAdded { outpoint: String, amount: u64, account_id: u64 },
    UtxoSpent { outpoint: String, tx_id: String },
    UtxoConfirmed { outpoint: String, block_score: u64 },
    AddressUsed { address: String },
    TxSubmitted { tx_id: String, amount: u64, fee: u64 },
    TxConfirmed { tx_id: String, block_hash: String },
    AccountCreated { id: u64, name: String, kind: String },
    SyncProgress { daa_score: u64 },
