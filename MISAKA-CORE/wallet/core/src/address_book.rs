//! Contact address book with labels and metadata
//!
...
/// AddressBook operational state.
pub struct AddressBookState {
    config: AddressBookConfig,
    entries: parking_lot::RwLock<HashMap<u64, AddressBookEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<AddressBookTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: AddressBookStats,
    events: parking_lot::Mutex<Vec<AddressBookEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in AddressBook.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressBookEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: AddressBookEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum AddressBookEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct AddressBookTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from AddressBook.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl AddressBookStats {
    pub fn snapshot(&self) -> AddressBookStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressBookStatsSnapshot {
...
impl AddressBookState {
    pub fn new(config: AddressBookConfig) -> Self {
        Self {
            ...
            stats: AddressBookStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, AddressBookError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(AddressBookError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = AddressBookEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: AddressBookEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(AddressBookTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(AddressBookEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), AddressBookError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressBookError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = AddressBookEntryState::Completed;
        ...
        self.emit(AddressBookEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AddressBookError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressBookError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = AddressBookEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = AddressBookEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(AddressBookEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), AddressBookError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressBookError::NotFound(id))?;
        entry.state = AddressBookEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<AddressBookEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<AddressBookEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: AddressBookEntryState) -> Vec<AddressBookEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, AddressBookEntryState::Completed | AddressBookEntryState::Failed | AddressBookEntryState::Cancelled | AddressBookEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(AddressBookEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> AddressBookStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<AddressBookEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: AddressBookEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = AddressBookState::new(AddressBookConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = AddressBookConfig { retry_count: 2, ..Default::default() };
        let state = AddressBookState::new(config);
