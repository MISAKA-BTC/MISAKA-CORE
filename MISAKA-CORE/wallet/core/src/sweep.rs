//! Sweep all UTXOs from an address to another
//!
...
/// Sweep operational state.
pub struct SweepState {
    config: SweepConfig,
    entries: parking_lot::RwLock<HashMap<u64, SweepEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<SweepTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SweepStats,
    events: parking_lot::Mutex<Vec<SweepEvent>>,
...
impl SweepStats {
    pub fn snapshot(&self) -> SweepStatsSnapshot {
...
impl SweepState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, SweepError> {
        ...
        let entry = SweepEntry {
            ...
            state: SweepEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(SweepTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(SweepEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), SweepError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SweepError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = SweepEntryState::Completed;
        ...
        self.emit(SweepEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, SweepError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SweepError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = SweepEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = SweepEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(SweepEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), SweepError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SweepError::NotFound(id))?;
        entry.state = SweepEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<SweepEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<SweepEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: SweepEntryState) -> Vec<SweepEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, SweepEntryState::Completed | SweepEntryState::Failed | SweepEntryState::Cancelled | SweepEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(SweepEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> SweepStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<SweepEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = SweepConfig { retry_count: 2, ..Default::default() };
        let state = SweepState::new(config);
