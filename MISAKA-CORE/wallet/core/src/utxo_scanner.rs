//! Full blockchain UTXO scanner for wallet sync
//!
...
/// Configuration for UtxoScanner.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoScannerConfig {
    pub enabled: bool,
...
impl Default for UtxoScannerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// UtxoScanner operational state.
pub struct UtxoScannerState {
    config: UtxoScannerConfig,
    entries: parking_lot::RwLock<HashMap<u64, UtxoScannerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<UtxoScannerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: UtxoScannerStats,
    events: parking_lot::Mutex<Vec<UtxoScannerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in UtxoScanner.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoScannerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: UtxoScannerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum UtxoScannerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct UtxoScannerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from UtxoScanner.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Statistics for UtxoScanner.
#[derive(Debug, Default)]
pub struct UtxoScannerStats {
...
impl UtxoScannerStats {
    pub fn snapshot(&self) -> UtxoScannerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoScannerStatsSnapshot {
...
impl UtxoScannerState {
    pub fn new(config: UtxoScannerConfig) -> Self {
        Self {
            config,
            ...
            stats: UtxoScannerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, UtxoScannerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(UtxoScannerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = UtxoScannerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: UtxoScannerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(UtxoScannerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(UtxoScannerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), UtxoScannerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoScannerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = UtxoScannerEntryState::Completed;
        ...
        self.emit(UtxoScannerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, UtxoScannerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoScannerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = UtxoScannerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = UtxoScannerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(UtxoScannerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), UtxoScannerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoScannerError::NotFound(id))?;
        entry.state = UtxoScannerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<UtxoScannerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<UtxoScannerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: UtxoScannerEntryState) -> Vec<UtxoScannerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, UtxoScannerEntryState::Completed | UtxoScannerEntryState::Failed | UtxoScannerEntryState::Cancelled | UtxoScannerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(UtxoScannerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> UtxoScannerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<UtxoScannerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: UtxoScannerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = UtxoScannerState::new(UtxoScannerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = UtxoScannerConfig { retry_count: 2, ..Default::default() };
        let state = UtxoScannerState::new(config);
