//! CoinJoin-compatible mixing protocol for privacy
//!
...
/// CoinJoin operational state.
pub struct CoinJoinState {
    config: CoinJoinConfig,
    entries: parking_lot::RwLock<HashMap<u64, CoinJoinEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<CoinJoinTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CoinJoinStats,
    events: parking_lot::Mutex<Vec<CoinJoinEvent>>,
...
/// Events from CoinJoin.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum CoinJoinEvent {
...
impl CoinJoinStats {
    pub fn snapshot(&self) -> CoinJoinStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CoinJoinStatsSnapshot {
...
impl CoinJoinState {
    pub fn new(config: CoinJoinConfig) -> Self {
        Self {
            ...
            stats: CoinJoinStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, CoinJoinError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(CoinJoinError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = CoinJoinEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: CoinJoinEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(CoinJoinTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(CoinJoinEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), CoinJoinError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CoinJoinError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = CoinJoinEntryState::Completed;
        ...
        self.emit(CoinJoinEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, CoinJoinError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CoinJoinError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = CoinJoinEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = CoinJoinEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(CoinJoinEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), CoinJoinError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CoinJoinError::NotFound(id))?;
        entry.state = CoinJoinEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<CoinJoinEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<CoinJoinEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: CoinJoinEntryState) -> Vec<CoinJoinEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, CoinJoinEntryState::Completed | CoinJoinEntryState::Failed | CoinJoinEntryState::Cancelled | CoinJoinEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(CoinJoinEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> CoinJoinStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<CoinJoinEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: CoinJoinEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = CoinJoinState::new(CoinJoinConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = CoinJoinConfig { retry_count: 2, ..Default::default() };
        let state = CoinJoinState::new(config);
