...
impl DiscoveryManager {
    ...
    pub fn init_account(&mut self, account_id: u64) {
        self.account_states.insert(account_id, AccountDiscoveryState {
            account_id,
    ...
    pub fn next_batch(&mut self, account_id: u64) -> Option<Vec<(String, u32, bool)>> {
        ...
        if state.is_complete { return None; }
        ...
        if self.config.include_change_addresses {
            ...
            let change_end = (change_start + batch_size / 2).min(self.config.max_addresses_per_account);
    ...
    pub fn process_results(&mut self, account_id: u64, result: DiscoveryBatchResult) {
        if let Some(state) = self.account_states.get_mut(&account_id) {
            ...
            let receive_gap = state.receive_scanned_to.saturating_sub(state.receive_last_used);
            let change_gap = state.change_scanned_to.saturating_sub(state.change_last_used);
            ...
            if receive_gap >= self.config.gap_limit && change_gap >= self.config.gap_limit {
                ...
                tracing::info!(
                    ...
                    account_id, state.total_addresses_found,
                    state.total_utxos_found, state.total_balance_found,
                );
            }
    ...
    pub fn total_utxos_discovered(&self) -> u64 { self.total_utxos }
    pub fn account_count(&self) -> usize { self.account_states.len() }
}
...
impl DiscoveryManager {
    pub fn stats(&self) -> DiscoveryStats {
        DiscoveryStats {
            accounts_scanning: self.account_states.values().filter(|s| !s.is_complete).count(),
            accounts_complete: self.account_states.values().filter(|s| s.is_complete).count(),
            total_addresses: self.discovered_addresses.len(),
            total_utxos: self.total_utxos,
            total_balance: self.account_states.values().map(|s| s.total_balance_found).sum(),
