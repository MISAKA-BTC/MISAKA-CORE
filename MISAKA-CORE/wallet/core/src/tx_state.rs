...
impl TxStatus {
    ...
    pub fn is_terminal(&self) -> bool {
        matches!(self, TxStatus::Confirmed { .. } | TxStatus::Dropped { .. } | TxStatus::Conflicting { .. })
    }
...
pub fn apply_transition(current: &TxStatus, event: &TxEvent) -> TransitionResult {
    match (current, event) {
        ...
        (TxStatus::PendingLocal, TxEvent::UserCancel) => {
            TransitionResult::Ok(TxStatus::Dropped {
        ...
        (
            TxStatus::PendingInBlock { block_height, .. },
            TxEvent::Finalized { .. },
        ) => TransitionResult::Ok(TxStatus::Confirmed {
            block_height: *block_height,
        }),
    ...
    pub fn register_tx(
        ...
        for input in &inputs {
            match self.utxo_locks.get(input) {
                Some(UtxoLock::LockedForTx { tx_hash: locked_by, .. }) => {
                    return Err(format!(
    ...
    for tx_hash in pending_hashes {
        ...
        let event = match (current_status, node_info) {
            ...
            (
                ...
                Some(NodeTxInfo::InBlock { block_height, .. }),
...
mod tests {
    ...
    fn test_happy_path_lifecycle() {
        ...
        let status = tracker
            ...
            .expect("included");
        assert!(matches!(status, TxStatus::PendingInBlock { block_height: 10, confirmations: 1 }));

        // Finalized
        let status = tracker
            .apply_event(
                &tx_hash,
                &TxEvent::Finalized { block_height: 10 },
                4000,
            )
            .expect("finalized");
