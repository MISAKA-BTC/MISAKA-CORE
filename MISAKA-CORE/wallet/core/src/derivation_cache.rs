//! Address derivation cache for HD wallets
//!
...
/// Configuration for DerivationCache.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DerivationCacheConfig {
    pub enabled: bool,
...
impl Default for DerivationCacheConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// DerivationCache operational state.
pub struct DerivationCacheState {
    config: DerivationCacheConfig,
    entries: parking_lot::RwLock<HashMap<u64, DerivationCacheEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<DerivationCacheTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: DerivationCacheStats,
    events: parking_lot::Mutex<Vec<DerivationCacheEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in DerivationCache.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DerivationCacheEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: DerivationCacheEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum DerivationCacheEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct DerivationCacheTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from DerivationCache.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DerivationCacheEvent {
...
/// Statistics for DerivationCache.
...
impl DerivationCacheStats {
    pub fn snapshot(&self) -> DerivationCacheStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DerivationCacheStatsSnapshot {
...
impl DerivationCacheState {
    pub fn new(config: DerivationCacheConfig) -> Self {
        Self {
            config,
            ...
            stats: DerivationCacheStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, DerivationCacheError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(DerivationCacheError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = DerivationCacheEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: DerivationCacheEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(DerivationCacheTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(DerivationCacheEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), DerivationCacheError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DerivationCacheError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = DerivationCacheEntryState::Completed;
        ...
        self.emit(DerivationCacheEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, DerivationCacheError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DerivationCacheError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = DerivationCacheEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = DerivationCacheEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(DerivationCacheEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), DerivationCacheError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DerivationCacheError::NotFound(id))?;
        entry.state = DerivationCacheEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<DerivationCacheEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<DerivationCacheEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: DerivationCacheEntryState) -> Vec<DerivationCacheEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, DerivationCacheEntryState::Completed | DerivationCacheEntryState::Failed | DerivationCacheEntryState::Cancelled | DerivationCacheEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(DerivationCacheEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> DerivationCacheStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<DerivationCacheEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: DerivationCacheEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = DerivationCacheState::new(DerivationCacheConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = DerivationCacheConfig { retry_count: 2, ..Default::default() };
        let state = DerivationCacheState::new(config);
