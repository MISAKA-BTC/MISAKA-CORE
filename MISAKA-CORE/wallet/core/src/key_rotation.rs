//! Key rotation policy — automated PQ key lifecycle management
//!
...
/// Configuration for the KeyRotation subsystem.
...
/// Runtime state for KeyRotation.
pub struct KeyRotationRuntime {
    config: KeyRotationConfig,
    entries: parking_lot::RwLock<HashMap<u64, KeyRotationEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: KeyRotationStats,
    event_buffer: parking_lot::Mutex<Vec<KeyRotationEvent>>,
}

/// Individual entry tracked by KeyRotation.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for KeyRotation.
...
impl KeyRotationStats {
    pub fn snapshot(&self) -> KeyRotationStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        KeyRotationStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyRotationStatsSnapshot {
...
impl KeyRotationRuntime {
    /// Create a new KeyRotation runtime.
    pub fn new(config: KeyRotationConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: KeyRotationStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, KeyRotationError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(KeyRotationError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, KeyRotationEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(KeyRotationEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), KeyRotationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KeyRotationError::NotFound(id))?;
        ...
        self.emit_event(KeyRotationEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, KeyRotationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KeyRotationError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(KeyRotationEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<KeyRotationEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<KeyRotationEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(KeyRotationEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> KeyRotationStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<KeyRotationEvent> {
...
/// Errors from KeyRotation operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = KeyRotationRuntime::new(KeyRotationConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = KeyRotationConfig { retry_count: 2, ..Default::default() };
        let rt = KeyRotationRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = KeyRotationConfig { max_entries: 2, ..Default::default() };
        let rt = KeyRotationRuntime::new(config);
