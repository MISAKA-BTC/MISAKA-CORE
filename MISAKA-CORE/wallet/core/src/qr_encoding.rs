//! QR code encoding for address and PSKT transport
//!
...
/// Configuration for QrEncoding.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QrEncodingConfig {
    pub enabled: bool,
...
impl Default for QrEncodingConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// QrEncoding operational state.
pub struct QrEncodingState {
    config: QrEncodingConfig,
    entries: parking_lot::RwLock<HashMap<u64, QrEncodingEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<QrEncodingTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: QrEncodingStats,
    events: parking_lot::Mutex<Vec<QrEncodingEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in QrEncoding.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QrEncodingEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: QrEncodingEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct QrEncodingTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from QrEncoding.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum QrEncodingEvent {
...
/// Statistics for QrEncoding.
#[derive(Debug, Default)]
pub struct QrEncodingStats {
...
impl QrEncodingStats {
    pub fn snapshot(&self) -> QrEncodingStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QrEncodingStatsSnapshot {
...
impl QrEncodingState {
    pub fn new(config: QrEncodingConfig) -> Self {
        Self {
            config,
            ...
            stats: QrEncodingStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, QrEncodingError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(QrEncodingError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = QrEncodingEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: QrEncodingEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(QrEncodingTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(QrEncodingEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), QrEncodingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(QrEncodingError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = QrEncodingEntryState::Completed;
        ...
        self.emit(QrEncodingEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, QrEncodingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(QrEncodingError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = QrEncodingEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = QrEncodingEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(QrEncodingEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), QrEncodingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(QrEncodingError::NotFound(id))?;
        entry.state = QrEncodingEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<QrEncodingEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<QrEncodingEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: QrEncodingEntryState) -> Vec<QrEncodingEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, QrEncodingEntryState::Completed | QrEncodingEntryState::Failed | QrEncodingEntryState::Cancelled | QrEncodingEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(QrEncodingEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> QrEncodingStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<QrEncodingEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: QrEncodingEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = QrEncodingState::new(QrEncodingConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = QrEncodingConfig { retry_count: 2, ..Default::default() };
        let state = QrEncodingState::new(config);
