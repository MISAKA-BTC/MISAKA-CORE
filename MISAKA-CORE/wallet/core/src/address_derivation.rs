//! Full HD address derivation with PQ extensions
//!
...
/// Configuration for AddressDerivation.
...
impl Default for AddressDerivationConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// AddressDerivation operational state.
pub struct AddressDerivationState {
    config: AddressDerivationConfig,
    entries: parking_lot::RwLock<HashMap<u64, AddressDerivationEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<AddressDerivationTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: AddressDerivationStats,
    events: parking_lot::Mutex<Vec<AddressDerivationEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in AddressDerivation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressDerivationEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: AddressDerivationEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum AddressDerivationEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct AddressDerivationTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from AddressDerivation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum AddressDerivationEvent {
...
/// Statistics for AddressDerivation.
#[derive(Debug, Default)]
pub struct AddressDerivationStats {
...
impl AddressDerivationStats {
    pub fn snapshot(&self) -> AddressDerivationStatsSnapshot {
        ...
        AddressDerivationStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressDerivationStatsSnapshot {
...
impl AddressDerivationState {
    pub fn new(config: AddressDerivationConfig) -> Self {
        Self {
            config,
            ...
            stats: AddressDerivationStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, AddressDerivationError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(AddressDerivationError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = AddressDerivationEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: AddressDerivationEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(AddressDerivationTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(AddressDerivationEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), AddressDerivationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressDerivationError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = AddressDerivationEntryState::Completed;
        ...
        self.emit(AddressDerivationEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AddressDerivationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressDerivationError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = AddressDerivationEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = AddressDerivationEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(AddressDerivationEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), AddressDerivationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressDerivationError::NotFound(id))?;
        entry.state = AddressDerivationEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<AddressDerivationEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<AddressDerivationEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: AddressDerivationEntryState) -> Vec<AddressDerivationEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, AddressDerivationEntryState::Completed | AddressDerivationEntryState::Failed | AddressDerivationEntryState::Cancelled | AddressDerivationEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(AddressDerivationEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> AddressDerivationStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<AddressDerivationEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: AddressDerivationEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = AddressDerivationState::new(AddressDerivationConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = AddressDerivationConfig { retry_count: 2, ..Default::default() };
        let state = AddressDerivationState::new(config);
