...
use crate::account::{Account, AccountId, AccountKind, AccountMeta};
...
use crate::{OwnedUtxo, WalletBalance, TxPlan};
...
impl Wallet {
    ...
    pub fn create(
        ...
        let keystore = EncryptedKeystore::create(
            &seed,
            password,
            name,
    ...
    pub fn create_watchonly_account(
        &mut self,
        name: String,
        pubkeys: Vec<Vec<u8>>,
    ) -> AccountId {
    ...
    pub fn get_balance(&self, account_id: AccountId) -> WalletBalance {
        let utxos = self.utxos.get(&account_id).map_or(&[] as &[OwnedUtxo], |v| v.as_slice());
    ...
    pub fn total_balance(&self) -> u64 {
        self.accounts.keys().map(|id| self.get_balance(*id).total).sum()
    }
    ...
    /// Get wallet state.
    pub fn state(&self) -> &WalletState { &self.state }
