//! Transaction broadcast with retry and monitoring
//!
...
pub struct BroadcastManagerConfig {
    pub enabled: bool,
...
impl Default for BroadcastManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// BroadcastManager operational state.
pub struct BroadcastManagerState {
    config: BroadcastManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, BroadcastManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BroadcastManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BroadcastManagerStats,
    events: parking_lot::Mutex<Vec<BroadcastManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BroadcastManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BroadcastManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BroadcastManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BroadcastManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BroadcastManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BroadcastManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BroadcastManagerEvent {
...
/// Statistics for BroadcastManager.
...
impl BroadcastManagerStats {
    pub fn snapshot(&self) -> BroadcastManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BroadcastManagerStatsSnapshot {
...
impl BroadcastManagerState {
    pub fn new(config: BroadcastManagerConfig) -> Self {
        Self {
            config,
            ...
            stats: BroadcastManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BroadcastManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BroadcastManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BroadcastManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BroadcastManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BroadcastManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BroadcastManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BroadcastManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BroadcastManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = BroadcastManagerEntryState::Completed;
        ...
        self.emit(BroadcastManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BroadcastManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BroadcastManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BroadcastManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BroadcastManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BroadcastManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BroadcastManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BroadcastManagerError::NotFound(id))?;
        entry.state = BroadcastManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BroadcastManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BroadcastManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BroadcastManagerEntryState) -> Vec<BroadcastManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BroadcastManagerEntryState::Completed | BroadcastManagerEntryState::Failed | BroadcastManagerEntryState::Cancelled | BroadcastManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BroadcastManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BroadcastManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BroadcastManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BroadcastManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BroadcastManagerState::new(BroadcastManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BroadcastManagerConfig { retry_count: 2, ..Default::default() };
        let state = BroadcastManagerState::new(config);
