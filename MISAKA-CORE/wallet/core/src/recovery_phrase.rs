//! Recovery phrase (mnemonic) generation and validation
//!
...
/// Configuration for RecoveryPhrase.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RecoveryPhraseConfig {
    pub enabled: bool,
...
impl Default for RecoveryPhraseConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// RecoveryPhrase operational state.
pub struct RecoveryPhraseState {
    config: RecoveryPhraseConfig,
    entries: parking_lot::RwLock<HashMap<u64, RecoveryPhraseEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<RecoveryPhraseTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: RecoveryPhraseStats,
    events: parking_lot::Mutex<Vec<RecoveryPhraseEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in RecoveryPhrase.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RecoveryPhraseEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: RecoveryPhraseEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum RecoveryPhraseEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct RecoveryPhraseTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from RecoveryPhrase.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RecoveryPhraseEvent {
...
/// Statistics for RecoveryPhrase.
#[derive(Debug, Default)]
pub struct RecoveryPhraseStats {
...
impl RecoveryPhraseStats {
    pub fn snapshot(&self) -> RecoveryPhraseStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RecoveryPhraseStatsSnapshot {
...
impl RecoveryPhraseState {
    pub fn new(config: RecoveryPhraseConfig) -> Self {
        Self {
            config,
            ...
            stats: RecoveryPhraseStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, RecoveryPhraseError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(RecoveryPhraseError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = RecoveryPhraseEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: RecoveryPhraseEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(RecoveryPhraseTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(RecoveryPhraseEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), RecoveryPhraseError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RecoveryPhraseError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = RecoveryPhraseEntryState::Completed;
        ...
        self.emit(RecoveryPhraseEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, RecoveryPhraseError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RecoveryPhraseError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = RecoveryPhraseEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = RecoveryPhraseEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(RecoveryPhraseEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), RecoveryPhraseError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RecoveryPhraseError::NotFound(id))?;
        entry.state = RecoveryPhraseEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<RecoveryPhraseEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<RecoveryPhraseEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: RecoveryPhraseEntryState) -> Vec<RecoveryPhraseEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, RecoveryPhraseEntryState::Completed | RecoveryPhraseEntryState::Failed | RecoveryPhraseEntryState::Cancelled | RecoveryPhraseEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(RecoveryPhraseEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> RecoveryPhraseStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<RecoveryPhraseEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: RecoveryPhraseEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = RecoveryPhraseState::new(RecoveryPhraseConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = RecoveryPhraseConfig { retry_count: 2, ..Default::default() };
        let state = RecoveryPhraseState::new(config);
