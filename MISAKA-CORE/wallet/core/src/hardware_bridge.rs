//! Hardware wallet communication bridge
//!
...
/// Configuration for HardwareBridge.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HardwareBridgeConfig {
    pub enabled: bool,
...
impl Default for HardwareBridgeConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// HardwareBridge operational state.
pub struct HardwareBridgeState {
    config: HardwareBridgeConfig,
    entries: parking_lot::RwLock<HashMap<u64, HardwareBridgeEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<HardwareBridgeTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: HardwareBridgeStats,
    events: parking_lot::Mutex<Vec<HardwareBridgeEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in HardwareBridge.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HardwareBridgeEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: HardwareBridgeEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum HardwareBridgeEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct HardwareBridgeTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from HardwareBridge.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum HardwareBridgeEvent {
...
/// Statistics for HardwareBridge.
#[derive(Debug, Default)]
pub struct HardwareBridgeStats {
...
impl HardwareBridgeStats {
    pub fn snapshot(&self) -> HardwareBridgeStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HardwareBridgeStatsSnapshot {
...
impl HardwareBridgeState {
    pub fn new(config: HardwareBridgeConfig) -> Self {
        Self {
            config,
            ...
            stats: HardwareBridgeStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, HardwareBridgeError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(HardwareBridgeError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = HardwareBridgeEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: HardwareBridgeEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(HardwareBridgeTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(HardwareBridgeEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), HardwareBridgeError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HardwareBridgeError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = HardwareBridgeEntryState::Completed;
        ...
        self.emit(HardwareBridgeEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, HardwareBridgeError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HardwareBridgeError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = HardwareBridgeEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = HardwareBridgeEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(HardwareBridgeEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), HardwareBridgeError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HardwareBridgeError::NotFound(id))?;
        entry.state = HardwareBridgeEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<HardwareBridgeEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<HardwareBridgeEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: HardwareBridgeEntryState) -> Vec<HardwareBridgeEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, HardwareBridgeEntryState::Completed | HardwareBridgeEntryState::Failed | HardwareBridgeEntryState::Cancelled | HardwareBridgeEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(HardwareBridgeEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> HardwareBridgeStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<HardwareBridgeEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: HardwareBridgeEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = HardwareBridgeState::new(HardwareBridgeConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = HardwareBridgeConfig { retry_count: 2, ..Default::default() };
        let state = HardwareBridgeState::new(config);
