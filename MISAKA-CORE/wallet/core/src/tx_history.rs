...
/// Configuration for the TxHistory subsystem.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TxHistoryConfig {
    pub enabled: bool,
...
impl Default for TxHistoryConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 10_000,
...
/// Runtime state for TxHistory.
pub struct TxHistoryRuntime {
    config: TxHistoryConfig,
    entries: parking_lot::RwLock<HashMap<u64, TxHistoryEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: TxHistoryStats,
    event_buffer: parking_lot::Mutex<Vec<TxHistoryEvent>>,
}

/// Individual entry tracked by TxHistory.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Events emitted by TxHistory.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum TxHistoryEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for TxHistory.
#[derive(Debug, Default)]
pub struct TxHistoryStats {
...
impl TxHistoryStats {
    pub fn snapshot(&self) -> TxHistoryStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        TxHistoryStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TxHistoryStatsSnapshot {
...
impl TxHistoryRuntime {
    /// Create a new TxHistory runtime.
    pub fn new(config: TxHistoryConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: TxHistoryStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, TxHistoryError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(TxHistoryError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, TxHistoryEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(TxHistoryEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), TxHistoryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TxHistoryError::NotFound(id))?;
        ...
        self.emit_event(TxHistoryEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, TxHistoryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TxHistoryError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(TxHistoryEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<TxHistoryEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<TxHistoryEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(TxHistoryEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> TxHistoryStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<TxHistoryEvent> {
    ...
    fn emit_event(&self, event: TxHistoryEvent) {
...
/// Errors from TxHistory operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = TxHistoryRuntime::new(TxHistoryConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = TxHistoryConfig { retry_count: 2, ..Default::default() };
        let rt = TxHistoryRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = TxHistoryConfig { max_entries: 2, ..Default::default() };
        let rt = TxHistoryRuntime::new(config);
