//! Multi-party signing coordination for multisig wallets
//!
...
/// Configuration for SigningCoordinator.
...
/// SigningCoordinator operational state.
pub struct SigningCoordinatorState {
    config: SigningCoordinatorConfig,
    entries: parking_lot::RwLock<HashMap<u64, SigningCoordinatorEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<SigningCoordinatorTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SigningCoordinatorStats,
    events: parking_lot::Mutex<Vec<SigningCoordinatorEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in SigningCoordinator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SigningCoordinatorEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: SigningCoordinatorEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum SigningCoordinatorEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct SigningCoordinatorTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from SigningCoordinator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SigningCoordinatorEvent {
...
/// Statistics for SigningCoordinator.
#[derive(Debug, Default)]
pub struct SigningCoordinatorStats {
...
impl SigningCoordinatorStats {
    pub fn snapshot(&self) -> SigningCoordinatorStatsSnapshot {
        ...
        SigningCoordinatorStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SigningCoordinatorStatsSnapshot {
...
impl SigningCoordinatorState {
    pub fn new(config: SigningCoordinatorConfig) -> Self {
        Self {
            config,
            ...
            stats: SigningCoordinatorStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, SigningCoordinatorError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(SigningCoordinatorError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = SigningCoordinatorEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: SigningCoordinatorEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(SigningCoordinatorTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(SigningCoordinatorEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), SigningCoordinatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SigningCoordinatorError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = SigningCoordinatorEntryState::Completed;
        ...
        self.emit(SigningCoordinatorEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, SigningCoordinatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SigningCoordinatorError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = SigningCoordinatorEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = SigningCoordinatorEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(SigningCoordinatorEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), SigningCoordinatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SigningCoordinatorError::NotFound(id))?;
        entry.state = SigningCoordinatorEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<SigningCoordinatorEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<SigningCoordinatorEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: SigningCoordinatorEntryState) -> Vec<SigningCoordinatorEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, SigningCoordinatorEntryState::Completed | SigningCoordinatorEntryState::Failed | SigningCoordinatorEntryState::Cancelled | SigningCoordinatorEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(SigningCoordinatorEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> SigningCoordinatorStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<SigningCoordinatorEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: SigningCoordinatorEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = SigningCoordinatorState::new(SigningCoordinatorConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = SigningCoordinatorConfig { retry_count: 2, ..Default::default() };
        let state = SigningCoordinatorState::new(config);
