//! Full multi-account lifecycle management
//!
...
/// Configuration for AccountManager.
...
/// AccountManager operational state.
pub struct AccountManagerState {
    config: AccountManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, AccountManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<AccountManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: AccountManagerStats,
    events: parking_lot::Mutex<Vec<AccountManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in AccountManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AccountManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: AccountManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum AccountManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct AccountManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from AccountManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl AccountManagerStats {
    pub fn snapshot(&self) -> AccountManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AccountManagerStatsSnapshot {
...
impl AccountManagerState {
    pub fn new(config: AccountManagerConfig) -> Self {
        Self {
            ...
            stats: AccountManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, AccountManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(AccountManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = AccountManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: AccountManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(AccountManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(AccountManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), AccountManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AccountManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = AccountManagerEntryState::Completed;
        ...
        self.emit(AccountManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AccountManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AccountManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = AccountManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = AccountManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(AccountManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), AccountManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AccountManagerError::NotFound(id))?;
        entry.state = AccountManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<AccountManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<AccountManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: AccountManagerEntryState) -> Vec<AccountManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, AccountManagerEntryState::Completed | AccountManagerEntryState::Failed | AccountManagerEntryState::Cancelled | AccountManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(AccountManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> AccountManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<AccountManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: AccountManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = AccountManagerState::new(AccountManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = AccountManagerConfig { retry_count: 2, ..Default::default() };
        let state = AccountManagerState::new(config);
