//! Wallet encryption management — key wrapping and secure storage
//!
...
/// Configuration for the EncryptionManager subsystem.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EncryptionManagerConfig {
    pub enabled: bool,
...
impl Default for EncryptionManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 10_000,
...
/// Runtime state for EncryptionManager.
pub struct EncryptionManagerRuntime {
    config: EncryptionManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, EncryptionManagerEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: EncryptionManagerStats,
    event_buffer: parking_lot::Mutex<Vec<EncryptionManagerEvent>>,
}

/// Individual entry tracked by EncryptionManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EncryptionManagerEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by EncryptionManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum EncryptionManagerEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for EncryptionManager.
#[derive(Debug, Default)]
pub struct EncryptionManagerStats {
...
impl EncryptionManagerStats {
    pub fn snapshot(&self) -> EncryptionManagerStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        EncryptionManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EncryptionManagerStatsSnapshot {
...
impl EncryptionManagerRuntime {
    /// Create a new EncryptionManager runtime.
    pub fn new(config: EncryptionManagerConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: EncryptionManagerStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, EncryptionManagerError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(EncryptionManagerError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, EncryptionManagerEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(EncryptionManagerEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), EncryptionManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EncryptionManagerError::NotFound(id))?;
        ...
        self.emit_event(EncryptionManagerEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, EncryptionManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EncryptionManagerError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(EncryptionManagerEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<EncryptionManagerEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<EncryptionManagerEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(EncryptionManagerEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> EncryptionManagerStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<EncryptionManagerEvent> {
    ...
    fn emit_event(&self, event: EncryptionManagerEvent) {
...
/// Errors from EncryptionManager operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = EncryptionManagerRuntime::new(EncryptionManagerConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = EncryptionManagerConfig { retry_count: 2, ..Default::default() };
        let rt = EncryptionManagerRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = EncryptionManagerConfig { max_entries: 2, ..Default::default() };
        let rt = EncryptionManagerRuntime::new(config);
