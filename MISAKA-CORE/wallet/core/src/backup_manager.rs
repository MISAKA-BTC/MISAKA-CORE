//! Wallet backup and recovery management
//!
...
/// Configuration for BackupManager.
...
/// BackupManager operational state.
pub struct BackupManagerState {
    config: BackupManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, BackupManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BackupManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BackupManagerStats,
    events: parking_lot::Mutex<Vec<BackupManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BackupManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BackupManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BackupManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BackupManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BackupManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BackupManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BackupManagerEvent {
...
impl BackupManagerStats {
    pub fn snapshot(&self) -> BackupManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BackupManagerStatsSnapshot {
...
impl BackupManagerState {
    pub fn new(config: BackupManagerConfig) -> Self {
        Self {
            config,
            ...
            stats: BackupManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BackupManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BackupManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BackupManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BackupManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BackupManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BackupManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BackupManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BackupManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = BackupManagerEntryState::Completed;
        ...
        self.emit(BackupManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BackupManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BackupManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BackupManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BackupManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BackupManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BackupManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BackupManagerError::NotFound(id))?;
        entry.state = BackupManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BackupManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BackupManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BackupManagerEntryState) -> Vec<BackupManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BackupManagerEntryState::Completed | BackupManagerEntryState::Failed | BackupManagerEntryState::Cancelled | BackupManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BackupManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BackupManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BackupManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BackupManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BackupManagerState::new(BackupManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BackupManagerConfig { retry_count: 2, ..Default::default() };
        let state = BackupManagerState::new(config);
