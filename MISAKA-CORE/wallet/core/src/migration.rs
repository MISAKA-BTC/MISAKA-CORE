//! Wallet format migration between versions
//!
...
/// Migration operational state.
pub struct MigrationState {
    config: MigrationConfig,
    entries: parking_lot::RwLock<HashMap<u64, MigrationEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<MigrationTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MigrationStats,
    events: parking_lot::Mutex<Vec<MigrationEvent>>,
...
impl MigrationState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, MigrationError> {
        ...
        let entry = MigrationEntry {
            ...
            state: MigrationEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(MigrationTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(MigrationEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), MigrationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MigrationError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = MigrationEntryState::Completed;
        ...
        self.emit(MigrationEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, MigrationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MigrationError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = MigrationEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = MigrationEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(MigrationEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), MigrationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MigrationError::NotFound(id))?;
        entry.state = MigrationEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<MigrationEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<MigrationEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: MigrationEntryState) -> Vec<MigrationEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, MigrationEntryState::Completed | MigrationEntryState::Failed | MigrationEntryState::Cancelled | MigrationEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(MigrationEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> MigrationStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<MigrationEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = MigrationState::new(MigrationConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = MigrationConfig { retry_count: 2, ..Default::default() };
        let state = MigrationState::new(config);
