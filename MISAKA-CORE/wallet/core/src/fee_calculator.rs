...
/// Configuration for FeeCalculator.
...
/// FeeCalculator operational state.
pub struct FeeCalculatorState {
    config: FeeCalculatorConfig,
    entries: parking_lot::RwLock<HashMap<u64, FeeCalculatorEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<FeeCalculatorTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: FeeCalculatorStats,
    events: parking_lot::Mutex<Vec<FeeCalculatorEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in FeeCalculator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FeeCalculatorEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: FeeCalculatorEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum FeeCalculatorEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct FeeCalculatorTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from FeeCalculator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum FeeCalculatorEvent {
...
impl FeeCalculatorStats {
    pub fn snapshot(&self) -> FeeCalculatorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FeeCalculatorStatsSnapshot {
...
impl FeeCalculatorState {
    pub fn new(config: FeeCalculatorConfig) -> Self {
        Self {
            config,
            ...
            stats: FeeCalculatorStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, FeeCalculatorError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(FeeCalculatorError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = FeeCalculatorEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: FeeCalculatorEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(FeeCalculatorTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(FeeCalculatorEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), FeeCalculatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FeeCalculatorError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = FeeCalculatorEntryState::Completed;
        ...
        self.emit(FeeCalculatorEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, FeeCalculatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FeeCalculatorError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = FeeCalculatorEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = FeeCalculatorEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(FeeCalculatorEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), FeeCalculatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FeeCalculatorError::NotFound(id))?;
        entry.state = FeeCalculatorEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<FeeCalculatorEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<FeeCalculatorEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: FeeCalculatorEntryState) -> Vec<FeeCalculatorEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, FeeCalculatorEntryState::Completed | FeeCalculatorEntryState::Failed | FeeCalculatorEntryState::Cancelled | FeeCalculatorEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(FeeCalculatorEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> FeeCalculatorStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<FeeCalculatorEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: FeeCalculatorEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = FeeCalculatorState::new(FeeCalculatorConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = FeeCalculatorConfig { retry_count: 2, ..Default::default() };
        let state = FeeCalculatorState::new(config);
