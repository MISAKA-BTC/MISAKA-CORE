//! UTXO compounding — merge small UTXOs for efficiency
//!
...
/// Compound operational state.
pub struct CompoundState {
    config: CompoundConfig,
    entries: parking_lot::RwLock<HashMap<u64, CompoundEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<CompoundTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CompoundStats,
    events: parking_lot::Mutex<Vec<CompoundEvent>>,
...
impl CompoundStats {
    pub fn snapshot(&self) -> CompoundStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CompoundStatsSnapshot {
...
impl CompoundState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, CompoundError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(CompoundError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = CompoundEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: CompoundEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(CompoundTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(CompoundEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), CompoundError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CompoundError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = CompoundEntryState::Completed;
        ...
        self.emit(CompoundEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, CompoundError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CompoundError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = CompoundEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = CompoundEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(CompoundEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), CompoundError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CompoundError::NotFound(id))?;
        entry.state = CompoundEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<CompoundEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<CompoundEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: CompoundEntryState) -> Vec<CompoundEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, CompoundEntryState::Completed | CompoundEntryState::Failed | CompoundEntryState::Cancelled | CompoundEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(CompoundEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> CompoundStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<CompoundEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: CompoundEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = CompoundState::new(CompoundConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = CompoundConfig { retry_count: 2, ..Default::default() };
        let state = CompoundState::new(config);
