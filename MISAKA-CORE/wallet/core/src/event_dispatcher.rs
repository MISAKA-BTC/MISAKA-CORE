//! Wallet event dispatching to UI and external subscribers
//!
...
/// EventDispatcher operational state.
pub struct EventDispatcherState {
    config: EventDispatcherConfig,
    entries: parking_lot::RwLock<HashMap<u64, EventDispatcherEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<EventDispatcherTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: EventDispatcherStats,
    events: parking_lot::Mutex<Vec<EventDispatcherEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in EventDispatcher.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EventDispatcherEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: EventDispatcherEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum EventDispatcherEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct EventDispatcherTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from EventDispatcher.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum EventDispatcherEvent {
...
/// Statistics for EventDispatcher.
...
impl EventDispatcherStats {
    pub fn snapshot(&self) -> EventDispatcherStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EventDispatcherStatsSnapshot {
...
impl EventDispatcherState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, EventDispatcherError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(EventDispatcherError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = EventDispatcherEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: EventDispatcherEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(EventDispatcherTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(EventDispatcherEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), EventDispatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EventDispatcherError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = EventDispatcherEntryState::Completed;
        ...
        self.emit(EventDispatcherEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, EventDispatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EventDispatcherError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = EventDispatcherEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = EventDispatcherEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(EventDispatcherEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), EventDispatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EventDispatcherError::NotFound(id))?;
        entry.state = EventDispatcherEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<EventDispatcherEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<EventDispatcherEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: EventDispatcherEntryState) -> Vec<EventDispatcherEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, EventDispatcherEntryState::Completed | EventDispatcherEntryState::Failed | EventDispatcherEntryState::Cancelled | EventDispatcherEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(EventDispatcherEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> EventDispatcherStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<EventDispatcherEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: EventDispatcherEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = EventDispatcherState::new(EventDispatcherConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = EventDispatcherConfig { retry_count: 2, ..Default::default() };
        let state = EventDispatcherState::new(config);
