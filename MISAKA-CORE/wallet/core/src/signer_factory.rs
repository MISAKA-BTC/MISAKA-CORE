//! Signature factory for different key types
//!
...
/// Configuration for SignerFactory.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SignerFactoryConfig {
    pub enabled: bool,
...
impl Default for SignerFactoryConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// SignerFactory operational state.
pub struct SignerFactoryState {
    config: SignerFactoryConfig,
    entries: parking_lot::RwLock<HashMap<u64, SignerFactoryEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<SignerFactoryTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SignerFactoryStats,
    events: parking_lot::Mutex<Vec<SignerFactoryEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in SignerFactory.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SignerFactoryEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: SignerFactoryEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum SignerFactoryEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct SignerFactoryTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from SignerFactory.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SignerFactoryEvent {
...
/// Statistics for SignerFactory.
#[derive(Debug, Default)]
pub struct SignerFactoryStats {
...
impl SignerFactoryStats {
    pub fn snapshot(&self) -> SignerFactoryStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SignerFactoryStatsSnapshot {
...
impl SignerFactoryState {
    pub fn new(config: SignerFactoryConfig) -> Self {
        Self {
            config,
            ...
            stats: SignerFactoryStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, SignerFactoryError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(SignerFactoryError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = SignerFactoryEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: SignerFactoryEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(SignerFactoryTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(SignerFactoryEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), SignerFactoryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SignerFactoryError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = SignerFactoryEntryState::Completed;
        ...
        self.emit(SignerFactoryEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, SignerFactoryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SignerFactoryError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = SignerFactoryEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = SignerFactoryEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(SignerFactoryEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), SignerFactoryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SignerFactoryError::NotFound(id))?;
        entry.state = SignerFactoryEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<SignerFactoryEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<SignerFactoryEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: SignerFactoryEntryState) -> Vec<SignerFactoryEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, SignerFactoryEntryState::Completed | SignerFactoryEntryState::Failed | SignerFactoryEntryState::Cancelled | SignerFactoryEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(SignerFactoryEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> SignerFactoryStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<SignerFactoryEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: SignerFactoryEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = SignerFactoryState::new(SignerFactoryConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = SignerFactoryConfig { retry_count: 2, ..Default::default() };
        let state = SignerFactoryState::new(config);
