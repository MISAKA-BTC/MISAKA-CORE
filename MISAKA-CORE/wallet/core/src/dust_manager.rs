//! Dust UTXO consolidation and management
//!
...
/// Configuration for DustManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DustManagerConfig {
    pub enabled: bool,
...
impl Default for DustManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// DustManager operational state.
pub struct DustManagerState {
    config: DustManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, DustManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<DustManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: DustManagerStats,
    events: parking_lot::Mutex<Vec<DustManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in DustManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DustManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: DustManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum DustManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct DustManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from DustManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DustManagerEvent {
...
/// Statistics for DustManager.
#[derive(Debug, Default)]
pub struct DustManagerStats {
...
impl DustManagerStats {
    pub fn snapshot(&self) -> DustManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DustManagerStatsSnapshot {
...
impl DustManagerState {
    pub fn new(config: DustManagerConfig) -> Self {
        Self {
            config,
            ...
            stats: DustManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, DustManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(DustManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = DustManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: DustManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(DustManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(DustManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), DustManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DustManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let old_state = format!("{:?}", entry.state);
        entry.state = DustManagerEntryState::Completed;
        ...
        self.emit(DustManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, DustManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DustManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = DustManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = DustManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(DustManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), DustManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DustManagerError::NotFound(id))?;
        entry.state = DustManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<DustManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<DustManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: DustManagerEntryState) -> Vec<DustManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, DustManagerEntryState::Completed | DustManagerEntryState::Failed | DustManagerEntryState::Cancelled | DustManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(DustManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> DustManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<DustManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: DustManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = DustManagerState::new(DustManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = DustManagerConfig { retry_count: 2, ..Default::default() };
        let state = DustManagerState::new(config);
