...
write_result_snapshot() {
  ...
  python3 - "$before_a" "$before_b" "$before_c" "$after_a" "$after_b" "$after_c" "$RESULT_FILE" "$LOG_DIR" <<'PY'
import json
import sys
...
before_nodes = {
    "nodeA": load(before_paths[0]),
    "nodeB": load(before_paths[1]),
    "nodeC": load(before_paths[2]),
}
after_nodes = {
    "nodeA": load(after_paths[0]),
    "nodeB": load(after_paths[1]),
    "nodeC": load(after_paths[2]),
}
