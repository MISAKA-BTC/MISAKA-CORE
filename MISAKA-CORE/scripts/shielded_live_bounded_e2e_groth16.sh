...
state_dir="${MISAKA_SHIELDED_LIVE_GROTH16_E2E_DIR:-$repo_root/.tmp/shielded-live-bounded-e2e-groth16}"
result_file="${MISAKA_SHIELDED_LIVE_GROTH16_E2E_RESULT:-$state_dir/result.json}"
log_file="$state_dir/cargo-test.log"
target_dir="${MISAKA_CARGO_TARGET_DIR:-$repo_root/.tmp/shielded-live-bounded-e2e-groth16-target}"
...
usage() {
  cat <<'EOF'
Usage: ./scripts/shielded_live_bounded_e2e_groth16.sh
Usage: ./scripts/shielded_live_bounded_e2e_groth16.sh --three-validator

Runs the feature-on live Groth16 bounded E2E and writes:

  .tmp/shielded-live-bounded-e2e-groth16/result.json

Optional env:
  MISAKA_SHIELDED_LIVE_GROTH16_E2E_DIR=/custom/output/dir
  MISAKA_SHIELDED_LIVE_GROTH16_E2E_RESULT=/custom/result.json
...
mode="groth16-two-validator"
if [[ "${1:-}" == "--three-validator" ]]; then
  mode="groth16-three-validator"
  shift
fi
...
case "$mode" in
  groth16-two-validator)
    test_filter="live_groth16_first_shielded_submit_transfer_tx_hash_continuity_through_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_submit_transfer_pending_ordered_confirmed_checkpoint"
    expected_vote_count="2"
    expected_quorum_threshold="2"
    ;;
  groth16-three-validator)
    test_filter="live_groth16_first_shielded_submit_transfer_tx_hash_continuity_through_three_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_three_validator_submit_transfer_pending_ordered_confirmed_checkpoint"
    expected_vote_count="3"
    expected_quorum_threshold="3"
    ;;
  *)
    write_failure "unsupported groth16 bounded E2E mode: $mode"
    exit 1
    ;;
esac

if ! cargo test -p misaka-node --bin misaka-node \
  --features shielded-groth16-verifier \
  "$test_filter" \
  -- --nocapture >"$log_file" 2>&1; then
  write_failure "live Groth16 shielded DAG RPC bounded E2E regression failed"
  cat "$log_file" >&2
  exit 1
fi

if [[ ! -f "$result_file" ]]; then
  write_failure "live Groth16 bounded E2E regression passed but did not emit result artifact"
  exit 1
fi
...
required = [
    ...
    any(entry.get("backendId") == "groth16-v1" for entry in layer4.get("registeredBackends", [])),
    pathlib.Path(artifacts.get("shieldedStateSnapshot", "")).exists(),
    pathlib.Path(artifacts.get("dagRuntimeSnapshot", "")).exists(),
]
if not all(required):
    raise SystemExit("live Groth16 bounded E2E artifact is incomplete")
print(result_path)
PY
