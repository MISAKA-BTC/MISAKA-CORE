...
state_dir="${MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_DIR:-}"
result_file="${MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_RESULT:-}"
target_dir="${MISAKA_CARGO_TARGET_DIR:-$repo_root/.tmp/shielded-full-path-e2e-groth16-target}"
mode="${MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_MODE:-groth16-first-three-validator}"
...
usage() {
  cat <<'EOF'
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --groth16-first
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --three-validator
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --three-validator-sequence
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --four-validator
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --four-validator-sequence
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --five-validator
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --five-validator-sequence
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --six-validator
Usage: ./scripts/shielded_live_full_path_e2e_groth16.sh --six-validator-sequence

Runs the Groth16-first live full-path restart continuity E2E and writes:

  .tmp/shielded-full-path-e2e-groth16/result.json

Optional env:
  MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_MODE=groth16-first|groth16-first-three-validator|groth16-first-three-validator-sequence|groth16-first-four-validator|groth16-first-four-validator-sequence|groth16-first-five-validator|groth16-first-five-validator-sequence|groth16-first-six-validator|groth16-first-six-validator-sequence
  MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_DIR=/custom/output/dir
  MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_RESULT=/custom/result.json
...
if [[ "${1:-}" == "--three-validator" ]]; then
  mode="groth16-first-three-validator"
  shift
elif [[ "${1:-}" == "--three-validator-sequence" ]]; then
  mode="groth16-first-three-validator-sequence"
  shift
elif [[ "${1:-}" == "--four-validator" ]]; then
  mode="groth16-first-four-validator"
  shift
elif [[ "${1:-}" == "--four-validator-sequence" ]]; then
  mode="groth16-first-four-validator-sequence"
  shift
elif [[ "${1:-}" == "--five-validator" ]]; then
  mode="groth16-first-five-validator"
  shift
elif [[ "${1:-}" == "--five-validator-sequence" ]]; then
  mode="groth16-first-five-validator-sequence"
  shift
elif [[ "${1:-}" == "--six-validator" ]]; then
  mode="groth16-first-six-validator"
  shift
elif [[ "${1:-}" == "--six-validator-sequence" ]]; then
  mode="groth16-first-six-validator-sequence"
  shift
elif [[ "${1:-}" == "--groth16-first" ]]; then
  mode="groth16-first"
  shift
fi
...
case "$mode" in
  groth16-first)
    test_filter="live_groth16_first_shielded_submit_transfer_full_path_restart_continuity_through_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_restart_continuity_full_path"
    expected_vote_count="2"
    expected_quorum_threshold="2"
    ;;
  groth16-first-three-validator)
    test_filter="live_groth16_first_shielded_submit_transfer_full_path_restart_continuity_through_three_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_three_validator_restart_continuity_full_path"
    expected_vote_count="3"
    expected_quorum_threshold="3"
    ;;
  groth16-first-three-validator-sequence)
    test_filter="live_groth16_first_shielded_submit_transfer_sequence_full_path_restart_continuity_through_three_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_three_validator_restart_continuity_full_path_sequence"
    expected_vote_count="3"
    expected_quorum_threshold="3"
    ;;
  groth16-first-four-validator)
    test_filter="live_groth16_first_shielded_submit_transfer_full_path_restart_continuity_through_four_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_four_validator_restart_continuity_full_path"
    expected_vote_count="4"
    expected_quorum_threshold="3"
    ;;
  groth16-first-four-validator-sequence)
    test_filter="live_groth16_first_shielded_submit_transfer_sequence_full_path_restart_continuity_through_four_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_four_validator_restart_continuity_full_path_sequence"
    expected_vote_count="4"
    expected_quorum_threshold="3"
    ;;
  groth16-first-five-validator)
    test_filter="live_groth16_first_shielded_submit_transfer_full_path_restart_continuity_through_five_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_five_validator_restart_continuity_full_path"
    expected_vote_count="5"
    expected_quorum_threshold="4"
    ;;
  groth16-first-five-validator-sequence)
    test_filter="live_groth16_first_shielded_submit_transfer_sequence_full_path_restart_continuity_through_five_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_five_validator_restart_continuity_full_path_sequence"
    expected_vote_count="5"
    expected_quorum_threshold="4"
    ;;
  groth16-first-six-validator)
    test_filter="live_groth16_first_shielded_submit_transfer_full_path_restart_continuity_through_six_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_six_validator_restart_continuity_full_path"
    expected_vote_count="6"
    expected_quorum_threshold="5"
    ;;
  groth16-first-six-validator-sequence)
    test_filter="live_groth16_first_shielded_submit_transfer_sequence_full_path_restart_continuity_through_six_validator_real_dag_rpc_commit_and_checkpoint_consumer"
    expected_flow="live_groth16_first_six_validator_restart_continuity_full_path_sequence"
    expected_vote_count="6"
    expected_quorum_threshold="5"
    ;;
  *)
    write_failure "unsupported Groth16 full-path E2E mode: $mode"
    exit 1
    ;;
esac
...
if [[ -z "$state_dir" && -z "$result_file" ]]; then
  case "$mode" in
    groth16-first)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16"
      ;;
    groth16-first-three-validator)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-3v"
      ;;
    groth16-first-three-validator-sequence)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-3v-seq"
      ;;
    groth16-first-four-validator)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-4v"
      ;;
    groth16-first-four-validator-sequence)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-4v-seq"
      ;;
    groth16-first-five-validator)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-5v"
      ;;
    groth16-first-five-validator-sequence)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-5v-seq"
      ;;
    groth16-first-six-validator)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-6v"
      ;;
    groth16-first-six-validator-sequence)
      state_dir="$repo_root/.tmp/shielded-full-path-e2e-groth16-6v-seq"
      ;;
  esac
...
export MISAKA_SHIELDED_FULL_PATH_GROTH16_E2E_RESULT="$result_file"
rm -f "$result_file"

cd "$repo_root"

if ! cargo test -p misaka-node --bin misaka-node \
  --features qdag_ct,shielded-groth16-verifier \
  "$test_filter" \
  -- --nocapture >"$log_file" 2>&1; then
  write_failure "Groth16 shielded DAG RPC full-path E2E regression failed ($mode)"
  cat "$log_file" >&2
  exit 1
fi

if [[ ! -f "$result_file" ]]; then
  write_failure "Groth16 shielded DAG RPC full-path E2E regression passed but did not emit result artifact ($mode)"
  exit 1
fi
...
if mode == "groth16-first-three-validator":
    required.extend([
    ...
    if not all(required):
        raise SystemExit(f"future Groth16 shielded DAG RPC full-path E2E artifact is incomplete ({mode})")
    print(result_path)
    raise SystemExit(0)
elif mode == "groth16-first-four-validator":
    required.extend([
        data.get("postRestart", {}).get("quorum", {}).get("validatorCount") == expected_vote_count,
    ])
elif mode == "groth16-first-five-validator":
    required.extend([
        data.get("postRestart", {}).get("quorum", {}).get("validatorCount") == expected_vote_count,
    ])
elif mode == "groth16-first-six-validator":
    required.extend([
        data.get("postRestart", {}).get("quorum", {}).get("validatorCount") == expected_vote_count,
    ])
if not all(required):
