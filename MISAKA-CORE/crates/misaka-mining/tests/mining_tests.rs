...
fn test_mempool_eviction() {
    let config = MempoolConfig { max_pool_size: 3, ..Default::default() };
    ...
    pool.add_transaction(test_tx(4, 100, 50)).expect("add better");
    assert_eq!(pool.transaction_count(), 3);
...
fn test_mining_manager() {
    ...
    let template = mgr.get_block_template(
        vec![[0u8; 32]], 1000, 0x1d00ffff, 5,
        vec![1, 2, 3], 50_000_000_000,
    );
