//! Block reward and fee distribution calculation
//!
...
/// Configuration for RewardCalculator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RewardCalculatorConfig {
    pub enabled: bool,
...
impl Default for RewardCalculatorConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// RewardCalculator operational state.
pub struct RewardCalculatorState {
    config: RewardCalculatorConfig,
    entries: parking_lot::RwLock<HashMap<u64, RewardCalculatorEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<RewardCalculatorTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: RewardCalculatorStats,
    events: parking_lot::Mutex<Vec<RewardCalculatorEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in RewardCalculator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RewardCalculatorEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: RewardCalculatorEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum RewardCalculatorEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct RewardCalculatorTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from RewardCalculator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RewardCalculatorEvent {
...
/// Statistics for RewardCalculator.
#[derive(Debug, Default)]
pub struct RewardCalculatorStats {
...
impl RewardCalculatorStats {
    pub fn snapshot(&self) -> RewardCalculatorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RewardCalculatorStatsSnapshot {
...
impl RewardCalculatorState {
    pub fn new(config: RewardCalculatorConfig) -> Self {
        Self {
            config,
            ...
            stats: RewardCalculatorStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, RewardCalculatorError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(RewardCalculatorError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = RewardCalculatorEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: RewardCalculatorEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(RewardCalculatorTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(RewardCalculatorEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), RewardCalculatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RewardCalculatorError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = RewardCalculatorEntryState::Completed;
        ...
        self.emit(RewardCalculatorEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, RewardCalculatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RewardCalculatorError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = RewardCalculatorEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = RewardCalculatorEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(RewardCalculatorEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), RewardCalculatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RewardCalculatorError::NotFound(id))?;
        entry.state = RewardCalculatorEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<RewardCalculatorEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<RewardCalculatorEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: RewardCalculatorEntryState) -> Vec<RewardCalculatorEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, RewardCalculatorEntryState::Completed | RewardCalculatorEntryState::Failed | RewardCalculatorEntryState::Cancelled | RewardCalculatorEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(RewardCalculatorEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> RewardCalculatorStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<RewardCalculatorEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: RewardCalculatorEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = RewardCalculatorState::new(RewardCalculatorConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = RewardCalculatorConfig { retry_count: 2, ..Default::default() };
        let state = RewardCalculatorState::new(config);
