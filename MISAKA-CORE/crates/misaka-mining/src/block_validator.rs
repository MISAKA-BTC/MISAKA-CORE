//! Block template validation before submission
//!
...
/// Configuration for the BlockValidator subsystem.
...
/// Runtime state for BlockValidator.
pub struct BlockValidatorRuntime {
    config: BlockValidatorConfig,
    entries: parking_lot::RwLock<HashMap<u64, BlockValidatorEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BlockValidatorStats,
    event_buffer: parking_lot::Mutex<Vec<BlockValidatorEvent>>,
}

/// Individual entry tracked by BlockValidator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockValidatorEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by BlockValidator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BlockValidatorEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for BlockValidator.
#[derive(Debug, Default)]
pub struct BlockValidatorStats {
...
impl BlockValidatorStats {
    pub fn snapshot(&self) -> BlockValidatorStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        BlockValidatorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockValidatorStatsSnapshot {
...
impl BlockValidatorRuntime {
    /// Create a new BlockValidator runtime.
    pub fn new(config: BlockValidatorConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: BlockValidatorStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, BlockValidatorError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(BlockValidatorError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, BlockValidatorEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(BlockValidatorEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), BlockValidatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockValidatorError::NotFound(id))?;
        ...
        self.emit_event(BlockValidatorEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, BlockValidatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockValidatorError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(BlockValidatorEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<BlockValidatorEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<BlockValidatorEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(BlockValidatorEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> BlockValidatorStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<BlockValidatorEvent> {
    ...
    fn emit_event(&self, event: BlockValidatorEvent) {
...
/// Errors from BlockValidator operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = BlockValidatorRuntime::new(BlockValidatorConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = BlockValidatorConfig { retry_count: 2, ..Default::default() };
        let rt = BlockValidatorRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = BlockValidatorConfig { max_entries: 2, ..Default::default() };
        let rt = BlockValidatorRuntime::new(config);
