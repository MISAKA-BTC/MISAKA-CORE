...
impl StratumServer {
    ...
    pub fn connect(&self) -> Option<u64> {
        ...
        if sessions.len() >= self.config.max_connections { return None; }
        drop(sessions);

        let id = self.next_session_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let extra_nonce = self.next_extra_nonce.fetch_add(1, std::sync::atomic::Ordering::Relaxed);

        self.sessions.write().insert(id, MinerSession {
            id,
    ...
    pub fn handle_authorize(&self, session_id: u64, worker: &str, password: &str) -> bool {
        let mut sessions = self.sessions.write();
    ...
    pub fn handle_submit(&self, session_id: u64, share: ShareSubmission) -> ShareResult {
        ...
        if current_job.as_ref().map_or(true, |j| j.job_id != share.job_id) {
        ...
        session.hashrate_estimate = session.shares_accepted as f64 * session.difficulty / elapsed as f64;
    ...
    pub fn adjust_difficulty(&self, session_id: u64) -> Option<f64> {
        if !self.config.vardiff_enabled { return None; }
        let mut sessions = self.sessions.write();
        let session = sessions.get_mut(&session_id)?;
