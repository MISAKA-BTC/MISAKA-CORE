//! Mining pool management — share tracking and payout
//!
...
/// Configuration for the PoolManager subsystem.
...
/// Runtime state for PoolManager.
pub struct PoolManagerRuntime {
    config: PoolManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, PoolManagerEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: PoolManagerStats,
    event_buffer: parking_lot::Mutex<Vec<PoolManagerEvent>>,
}

/// Individual entry tracked by PoolManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PoolManagerEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Operational statistics for PoolManager.
...
impl PoolManagerStats {
    pub fn snapshot(&self) -> PoolManagerStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        PoolManagerStatsSnapshot {
...
impl PoolManagerRuntime {
    /// Create a new PoolManager runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, PoolManagerError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(PoolManagerError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, PoolManagerEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(PoolManagerEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), PoolManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PoolManagerError::NotFound(id))?;
        ...
        self.emit_event(PoolManagerEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, PoolManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PoolManagerError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(PoolManagerEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<PoolManagerEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<PoolManagerEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(PoolManagerEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> PoolManagerStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<PoolManagerEvent> {
    ...
    fn emit_event(&self, event: PoolManagerEvent) {
...
/// Errors from PoolManager operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = PoolManagerRuntime::new(PoolManagerConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = PoolManagerConfig { retry_count: 2, ..Default::default() };
        let rt = PoolManagerRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = PoolManagerConfig { max_entries: 2, ..Default::default() };
        let rt = PoolManagerRuntime::new(config);
