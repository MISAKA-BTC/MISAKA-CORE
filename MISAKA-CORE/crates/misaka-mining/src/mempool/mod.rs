...
use crate::errors::{MiningError, MiningResult, MempoolRuleError};
...
impl Mempool {
    ...
    pub fn orphan_count(&self) -> usize { self.orphan_pool.len() }
    ...
    pub fn insert(&mut self, tx: MempoolTransaction) -> MiningResult<()> {
        ...
        self.counters.tx_accepted_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        Ok(())
    }
    ...
    pub fn insert_orphan(&mut self, tx: MempoolTransaction) -> MiningResult<()> {
        let tx_id = tx.tx_id;
        self.orphan_pool.insert(tx).map_err(|_| MiningError::OrphanPoolFull(self.orphan_pool.len()))?;
        self.counters.orphans_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        Ok(())
    }
    ...
    pub fn select_transactions(&self, max_mass: u64) -> Vec<[u8; 32]> {
        self.frontier.select(max_mass).into_iter().map(|k| k.tx_id).collect()
    }
    ...
    fn evict_lowest_fee_rate(&mut self, needed_mass: u64) -> MiningResult<()> {
        ...
        let all_keys: Vec<_> = self.frontier.select(self.total_mass).into_iter().rev().collect();
        for key in all_keys {
            if freed >= needed_mass { break; }
