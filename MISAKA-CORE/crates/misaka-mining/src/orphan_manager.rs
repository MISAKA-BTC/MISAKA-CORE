//! Orphan transaction tracking and resolution
//!
...
/// Configuration for OrphanManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OrphanManagerConfig {
    pub enabled: bool,
...
impl Default for OrphanManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// OrphanManager operational state.
pub struct OrphanManagerState {
    config: OrphanManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, OrphanManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<OrphanManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: OrphanManagerStats,
    events: parking_lot::Mutex<Vec<OrphanManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in OrphanManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OrphanManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: OrphanManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum OrphanManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct OrphanManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from OrphanManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum OrphanManagerEvent {
...
/// Statistics for OrphanManager.
#[derive(Debug, Default)]
pub struct OrphanManagerStats {
...
impl OrphanManagerStats {
    pub fn snapshot(&self) -> OrphanManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OrphanManagerStatsSnapshot {
...
impl OrphanManagerState {
    pub fn new(config: OrphanManagerConfig) -> Self {
        Self {
            config,
            ...
            stats: OrphanManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, OrphanManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(OrphanManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = OrphanManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: OrphanManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(OrphanManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(OrphanManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), OrphanManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(OrphanManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = OrphanManagerEntryState::Completed;
        ...
        self.emit(OrphanManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, OrphanManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(OrphanManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = OrphanManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = OrphanManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(OrphanManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), OrphanManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(OrphanManagerError::NotFound(id))?;
        entry.state = OrphanManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<OrphanManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<OrphanManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: OrphanManagerEntryState) -> Vec<OrphanManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, OrphanManagerEntryState::Completed | OrphanManagerEntryState::Failed | OrphanManagerEntryState::Cancelled | OrphanManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(OrphanManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> OrphanManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<OrphanManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: OrphanManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = OrphanManagerState::new(OrphanManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = OrphanManagerConfig { retry_count: 2, ..Default::default() };
        let state = OrphanManagerState::new(config);
