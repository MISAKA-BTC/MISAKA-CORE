...
use crate::block_template::{BlockTemplate, CoinbaseData, TemplateTransaction, TemplateBuildMode};
...
use crate::fee_rate::{FeeRateEstimator, FeerateEstimations, FeeEstimateVerbose};
...
impl MiningManager {
    pub fn new(
        ...
        let config = MempoolConfig::build_default(target_time_per_block, relay_non_std, max_block_mass);
    ...
    pub fn get_block_template(
        ...
        self.counters.template_builds.fetch_add(1, std::sync::atomic::Ordering::Relaxed);

        // Select transactions from mempool
        let mempool = self.mempool.read();
        let selected_ids = mempool.select_transactions(self.block_policy.max_block_mass);

        let transactions: Vec<TemplateTransaction> = selected_ids.iter().filter_map(|id| {
        ...
        let template = builder.build(parent_hashes, daa_score, timestamp, bits, coinbase, transactions)?;
    ...
    pub fn handle_new_block(&self, block_tx_ids: &[[u8; 32]], fee_rates: &[f64], total_mass: u64) {
        ...
        self.counters.block_tx_counts.fetch_add(removed as u64, std::sync::atomic::Ordering::Relaxed);
        self.fee_estimator.write().record_block(fee_rates, total_mass, self.config.max_block_mass);
    }
