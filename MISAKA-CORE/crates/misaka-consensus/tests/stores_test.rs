...
fn test_ghostdag_store_roundtrip() {
    ...
    let data = GhostdagData::new(10, 100, test_hash(0),
        vec![test_hash(0)], vec![test_hash(2)], vec![(test_hash(0), 0)]);
...
fn test_headers_store_roundtrip() {
    ...
    let header = Header {
        ...
        hash_merkle_root: ZERO_HASH, accepted_id_merkle_root: ZERO_HASH,
        utxo_commitment: ZERO_HASH, timestamp: 1000, bits: 0x1d00ffff,
        nonce: 42, daa_score: 100, blue_work: 500, blue_score: 50,
        pruning_point: ZERO_HASH, pqc_signature: vec![1,2,3],
...
fn test_relations_store() {
    ...
    store.insert_batch(&mut batch, child, vec![parent]).expect("c");
    db.write(batch).expect("commit");
...
fn test_utxo_diffs_store() {
    ...
    let mut batch = rocksdb::WriteBatch::default();
    store.insert_batch(&mut batch, hash, diff.clone()).expect("insert");
    db.write(batch).expect("commit");
...
fn test_depth_store() {
    ...
    let data = BlockDepthData { merge_depth_root: test_hash(1), finality_point: test_hash(2) };
    let mut batch = rocksdb::WriteBatch::default();
