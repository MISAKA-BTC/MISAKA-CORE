...
fn test_ghostdag_store_roundtrip() {
    ...
    let data = GhostdagData::new(10, 100, test_hash(0),
        vec![test_hash(0), test_hash(2)], vec![test_hash(3)],
    ...
    assert_eq!(store.get_selected_parent(&hash).expect("parent"), test_hash(0));
...
fn test_ghostdag_batch() {
    ...
    for i in 0u8..5 {
        ...
        store.insert_batch(&mut batch, test_hash(i + 10), &data).expect("batch");
    }
...
fn test_headers_store_roundtrip() {
    ...
    let hash = test_hash(1);
    let header = Header {
        hash, version: 1, parents: vec![test_hash(0)],
        hash_merkle_root: [0; 32], accepted_id_merkle_root: [0; 32],
        utxo_commitment: [0; 32], timestamp: 1000, bits: 0x1d00ffff,
        nonce: 42, daa_score: 5, blue_work: 50, blue_score: 5,
        pruning_point: [0; 32], pqc_signature: vec![1, 2, 3],
        proposer_pk: vec![4, 5, 6],
    };
    let mut batch = rocksdb::WriteBatch::default();
    store.insert_batch(&mut batch, hash, header.clone()).expect("insert");
    db.write(batch).expect("commit");
...
fn test_statuses_store() {
    ...
    store.set(hash, BlockStatus::StatusUTXOValid).expect("update");
...
fn test_relations_store() {
    ...
    store.insert_batch(&mut b, parent, vec![]).expect("insert parent");
    db.write(b).expect("commit");

    let mut b2 = rocksdb::WriteBatch::default();
    store.insert_batch(&mut b2, child, vec![parent]).expect("insert child");
...
fn test_utxo_diffs_store() {
    ...
    let diff = UtxoDiff {
        added: vec![UtxoEntry {
            outpoint: Outpoint { transaction_id: test_hash(10), index: 0 },
            amount: 1000, script_public_key: vec![1, 2], block_daa_score: 5, is_coinbase: false,
        }],
        removed: vec![],
    };
    let mut batch = rocksdb::WriteBatch::default();
...
fn test_selected_chain_store() {
    ...
    let mut batch = rocksdb::WriteBatch::default();
    store.apply_new_chain_block(&mut batch, 0, test_hash(1)).expect("apply");
    store.apply_new_chain_block(&mut batch, 1, test_hash(2)).expect("apply");
    db.write(batch).expect("commit");
...
fn test_block_transactions_store() {
    ...
    let txs = vec![StoredTransaction {
        tx_id: test_hash(10), inputs: vec![], outputs: vec![StoredTxOutput { amount: 500, script_public_key: vec![] }],
...
fn test_daa_store() {
    ...
    let mut batch = rocksdb::WriteBatch::default();
    store.insert_batch(&mut batch, test_hash(1), 12345).expect("insert");
    db.write(batch).expect("commit");
...
fn test_depth_store() {
    ...
    let mut batch = rocksdb::WriteBatch::default();
    store.insert_batch(&mut batch, test_hash(1), BlockDepthData {
        merge_depth_root: test_hash(0), finality_point: test_hash(0),
    }).expect("insert");
    db.write(batch).expect("commit");
...
fn test_pruning_store() {
    ...
    store.set(&PruningPointInfo {
        pruning_point: test_hash(5), candidate: test_hash(10), index: 0,
    }).expect("set");
