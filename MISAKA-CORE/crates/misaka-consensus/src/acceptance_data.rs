//! Block acceptance data processing for chain tracking
//!
...
/// Configuration for AcceptanceData.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AcceptanceDataConfig {
    pub enabled: bool,
...
impl Default for AcceptanceDataConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// AcceptanceData operational state.
pub struct AcceptanceDataState {
    config: AcceptanceDataConfig,
    entries: parking_lot::RwLock<HashMap<u64, AcceptanceDataEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<AcceptanceDataTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: AcceptanceDataStats,
    events: parking_lot::Mutex<Vec<AcceptanceDataEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in AcceptanceData.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AcceptanceDataEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: AcceptanceDataEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum AcceptanceDataEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct AcceptanceDataTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from AcceptanceData.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum AcceptanceDataEvent {
...
/// Statistics for AcceptanceData.
#[derive(Debug, Default)]
pub struct AcceptanceDataStats {
...
impl AcceptanceDataStats {
    pub fn snapshot(&self) -> AcceptanceDataStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AcceptanceDataStatsSnapshot {
...
impl AcceptanceDataState {
    pub fn new(config: AcceptanceDataConfig) -> Self {
        Self {
            config,
            ...
            stats: AcceptanceDataStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, AcceptanceDataError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(AcceptanceDataError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = AcceptanceDataEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: AcceptanceDataEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(AcceptanceDataTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(AcceptanceDataEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), AcceptanceDataError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AcceptanceDataError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = AcceptanceDataEntryState::Completed;
        ...
        self.emit(AcceptanceDataEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AcceptanceDataError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AcceptanceDataError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = AcceptanceDataEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = AcceptanceDataEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(AcceptanceDataEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), AcceptanceDataError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AcceptanceDataError::NotFound(id))?;
        entry.state = AcceptanceDataEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<AcceptanceDataEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<AcceptanceDataEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: AcceptanceDataEntryState) -> Vec<AcceptanceDataEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, AcceptanceDataEntryState::Completed | AcceptanceDataEntryState::Failed | AcceptanceDataEntryState::Cancelled | AcceptanceDataEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(AcceptanceDataEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> AcceptanceDataStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<AcceptanceDataEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: AcceptanceDataEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = AcceptanceDataState::new(AcceptanceDataConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = AcceptanceDataConfig { retry_count: 2, ..Default::default() };
        let state = AcceptanceDataState::new(config);
