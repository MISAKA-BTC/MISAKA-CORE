//! Coinbase transaction validation rules
//!
...
/// Configuration for the CoinbaseValidation subsystem.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CoinbaseValidationConfig {
    pub enabled: bool,
...
impl Default for CoinbaseValidationConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 10_000,
...
/// Runtime state for CoinbaseValidation.
pub struct CoinbaseValidationRuntime {
    config: CoinbaseValidationConfig,
    entries: parking_lot::RwLock<HashMap<u64, CoinbaseValidationEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CoinbaseValidationStats,
    event_buffer: parking_lot::Mutex<Vec<CoinbaseValidationEvent>>,
}

/// Individual entry tracked by CoinbaseValidation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CoinbaseValidationEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by CoinbaseValidation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum CoinbaseValidationEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for CoinbaseValidation.
#[derive(Debug, Default)]
pub struct CoinbaseValidationStats {
...
impl CoinbaseValidationStats {
    pub fn snapshot(&self) -> CoinbaseValidationStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        CoinbaseValidationStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CoinbaseValidationStatsSnapshot {
...
impl CoinbaseValidationRuntime {
    /// Create a new CoinbaseValidation runtime.
    pub fn new(config: CoinbaseValidationConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: CoinbaseValidationStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, CoinbaseValidationError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(CoinbaseValidationError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, CoinbaseValidationEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.stats.peak_count.fetch_max(active, std::sync::atomic::Ordering::Relaxed);

        self.emit_event(CoinbaseValidationEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), CoinbaseValidationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CoinbaseValidationError::NotFound(id))?;
        ...
        self.emit_event(CoinbaseValidationEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, CoinbaseValidationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CoinbaseValidationError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(CoinbaseValidationEvent::EntryFailed { id, reason });
            Ok(false) // No more retries
        } else {
            entry.state = EntryState::Pending; // Retry
            self.stats.total_retries.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            Ok(true) // Will retry
        }
    }

    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<CoinbaseValidationEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<CoinbaseValidationEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        let expired_ids: Vec<u64> = entries.iter()
            .filter(|(_, e)| {
        ...
        entries.retain(|_, e| {
            !(matches!(e.state, EntryState::Completed | EntryState::Failed | EntryState::Expired)
                && now.saturating_sub(e.updated_at) > self.config.cleanup_interval_secs)
        });

        let removed = before - entries.len();
        if removed > 0 {
            self.emit_event(CoinbaseValidationEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> CoinbaseValidationStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<CoinbaseValidationEvent> {
    ...
    pub fn active_count(&self) -> u64 {
        self.stats.active_count.load(std::sync::atomic::Ordering::Relaxed)
    }

    fn emit_event(&self, event: CoinbaseValidationEvent) {
...
/// Errors from CoinbaseValidation operations.
#[derive(Debug, thiserror::Error)]
pub enum CoinbaseValidationError {
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = CoinbaseValidationRuntime::new(CoinbaseValidationConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = CoinbaseValidationConfig { retry_count: 2, ..Default::default() };
        let rt = CoinbaseValidationRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = CoinbaseValidationConfig { max_entries: 2, ..Default::default() };
        let rt = CoinbaseValidationRuntime::new(config);
