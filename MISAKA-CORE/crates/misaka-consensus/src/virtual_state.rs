//! Virtual state computation for tip management
//!
...
/// Configuration for VirtualState.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VirtualStateConfig {
    pub enabled: bool,
...
impl Default for VirtualStateConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// VirtualState operational state.
pub struct VirtualStateState {
    config: VirtualStateConfig,
    entries: parking_lot::RwLock<HashMap<u64, VirtualStateEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<VirtualStateTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: VirtualStateStats,
    events: parking_lot::Mutex<Vec<VirtualStateEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in VirtualState.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VirtualStateEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: VirtualStateEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum VirtualStateEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct VirtualStateTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from VirtualState.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum VirtualStateEvent {
...
impl VirtualStateStats {
    pub fn snapshot(&self) -> VirtualStateStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VirtualStateStatsSnapshot {
...
impl VirtualStateState {
    pub fn new(config: VirtualStateConfig) -> Self {
        Self {
            config,
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, VirtualStateError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(VirtualStateError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = VirtualStateEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: VirtualStateEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(VirtualStateTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(VirtualStateEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), VirtualStateError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(VirtualStateError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = VirtualStateEntryState::Completed;
        ...
        self.emit(VirtualStateEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, VirtualStateError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(VirtualStateError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = VirtualStateEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = VirtualStateEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(VirtualStateEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), VirtualStateError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(VirtualStateError::NotFound(id))?;
        entry.state = VirtualStateEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<VirtualStateEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<VirtualStateEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: VirtualStateEntryState) -> Vec<VirtualStateEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, VirtualStateEntryState::Completed | VirtualStateEntryState::Failed | VirtualStateEntryState::Cancelled | VirtualStateEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(VirtualStateEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> VirtualStateStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<VirtualStateEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: VirtualStateEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = VirtualStateState::new(VirtualStateConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = VirtualStateConfig { retry_count: 2, ..Default::default() };
        let state = VirtualStateState::new(config);
