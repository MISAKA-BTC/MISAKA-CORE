...
impl StateDiffState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, StateDiffError> {
        ...
        let entry = StateDiffEntry {
            ...
            state: StateDiffEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(StateDiffTask { entry_id: id, scheduled_at: now, priority });
    ...
    pub fn complete(&self, id: u64) -> Result<(), StateDiffError> {
        ...
        self.emit(StateDiffEvent::Completed { id, duration_ms: duration });
        Ok(())
    }
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, StateDiffError> {
        ...
        self.emit(StateDiffEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }
    ...
    pub fn get(&self, id: u64) -> Option<StateDiffEntry> { self.entries.read().get(&id).cloned() }
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, StateDiffEntryState::Completed | StateDiffEntryState::Failed | StateDiffEntryState::Cancelled | StateDiffEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(StateDiffEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> StateDiffStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<StateDiffEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = StateDiffConfig { retry_count: 2, ..Default::default() };
