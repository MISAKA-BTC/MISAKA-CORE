...
impl BftStateMachine {
    ...
    pub fn on_proposal(
        ...
        let prevote_hash = self.decide_prevote(&proposal, is_valid_block);
    ...
    fn decide_prevote(&self, proposal: &BftProposal, is_valid_block: bool) -> Option<Hash> {
        if !is_valid_block {
        ...
        if let Some(ref locked) = self.locked_value {
            ...
            {
                // TODO: verify polka at valid_round (requires storing historical QCs)
                return Some(proposal.block_hash);
            }
...
mod tests {
    ...
    fn test_locked_value_enforced() {
        ...
        let result = sm.decide_prevote(&different_proposal, true);
