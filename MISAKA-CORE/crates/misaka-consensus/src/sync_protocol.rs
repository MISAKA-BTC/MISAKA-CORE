//! Consensus sync protocol state machine
//!
...
/// Configuration for the SyncProtocol subsystem.
...
/// Runtime state for SyncProtocol.
pub struct SyncProtocolRuntime {
    config: SyncProtocolConfig,
    entries: parking_lot::RwLock<HashMap<u64, SyncProtocolEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SyncProtocolStats,
    event_buffer: parking_lot::Mutex<Vec<SyncProtocolEvent>>,
}

/// Individual entry tracked by SyncProtocol.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SyncProtocolEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by SyncProtocol.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SyncProtocolEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for SyncProtocol.
#[derive(Debug, Default)]
pub struct SyncProtocolStats {
...
impl SyncProtocolStats {
    pub fn snapshot(&self) -> SyncProtocolStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        SyncProtocolStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SyncProtocolStatsSnapshot {
...
impl SyncProtocolRuntime {
    /// Create a new SyncProtocol runtime.
    pub fn new(config: SyncProtocolConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: SyncProtocolStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, SyncProtocolError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(SyncProtocolError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, SyncProtocolEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(SyncProtocolEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), SyncProtocolError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SyncProtocolError::NotFound(id))?;
        ...
        self.emit_event(SyncProtocolEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, SyncProtocolError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SyncProtocolError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(SyncProtocolEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<SyncProtocolEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<SyncProtocolEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(SyncProtocolEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> SyncProtocolStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<SyncProtocolEvent> {
    ...
    fn emit_event(&self, event: SyncProtocolEvent) {
...
/// Errors from SyncProtocol operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = SyncProtocolRuntime::new(SyncProtocolConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = SyncProtocolConfig { retry_count: 2, ..Default::default() };
        let rt = SyncProtocolRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = SyncProtocolConfig { max_entries: 2, ..Default::default() };
        let rt = SyncProtocolRuntime::new(config);
