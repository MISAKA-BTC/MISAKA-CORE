//! GhostDAG ordering computation service
//!
...
/// Configuration for GhostdagService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GhostdagServiceConfig {
    pub enabled: bool,
...
impl Default for GhostdagServiceConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// GhostdagService operational state.
pub struct GhostdagServiceState {
    config: GhostdagServiceConfig,
    entries: parking_lot::RwLock<HashMap<u64, GhostdagServiceEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<GhostdagServiceTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: GhostdagServiceStats,
    events: parking_lot::Mutex<Vec<GhostdagServiceEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in GhostdagService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GhostdagServiceEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: GhostdagServiceEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum GhostdagServiceEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct GhostdagServiceTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from GhostdagService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum GhostdagServiceEvent {
...
/// Statistics for GhostdagService.
#[derive(Debug, Default)]
pub struct GhostdagServiceStats {
...
impl GhostdagServiceStats {
    pub fn snapshot(&self) -> GhostdagServiceStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GhostdagServiceStatsSnapshot {
...
impl GhostdagServiceState {
    pub fn new(config: GhostdagServiceConfig) -> Self {
        Self {
            config,
            ...
            stats: GhostdagServiceStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, GhostdagServiceError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(GhostdagServiceError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = GhostdagServiceEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: GhostdagServiceEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(GhostdagServiceTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(GhostdagServiceEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), GhostdagServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(GhostdagServiceError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = GhostdagServiceEntryState::Completed;
        ...
        self.emit(GhostdagServiceEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, GhostdagServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(GhostdagServiceError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = GhostdagServiceEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = GhostdagServiceEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(GhostdagServiceEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), GhostdagServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(GhostdagServiceError::NotFound(id))?;
        entry.state = GhostdagServiceEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<GhostdagServiceEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<GhostdagServiceEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: GhostdagServiceEntryState) -> Vec<GhostdagServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, GhostdagServiceEntryState::Completed | GhostdagServiceEntryState::Failed | GhostdagServiceEntryState::Cancelled | GhostdagServiceEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(GhostdagServiceEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> GhostdagServiceStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<GhostdagServiceEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: GhostdagServiceEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = GhostdagServiceState::new(GhostdagServiceConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = GhostdagServiceConfig { retry_count: 2, ..Default::default() };
        let state = GhostdagServiceState::new(config);
