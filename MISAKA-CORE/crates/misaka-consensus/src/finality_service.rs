//! Finality tracking and conflict resolution
//!
...
/// Configuration for FinalityService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FinalityServiceConfig {
    pub enabled: bool,
...
impl Default for FinalityServiceConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// FinalityService operational state.
pub struct FinalityServiceState {
    config: FinalityServiceConfig,
    entries: parking_lot::RwLock<HashMap<u64, FinalityServiceEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<FinalityServiceTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: FinalityServiceStats,
    events: parking_lot::Mutex<Vec<FinalityServiceEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in FinalityService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FinalityServiceEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: FinalityServiceEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum FinalityServiceEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct FinalityServiceTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from FinalityService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum FinalityServiceEvent {
...
/// Statistics for FinalityService.
#[derive(Debug, Default)]
pub struct FinalityServiceStats {
...
impl FinalityServiceStats {
    pub fn snapshot(&self) -> FinalityServiceStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FinalityServiceStatsSnapshot {
...
impl FinalityServiceState {
    pub fn new(config: FinalityServiceConfig) -> Self {
        Self {
            config,
            ...
            stats: FinalityServiceStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, FinalityServiceError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(FinalityServiceError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = FinalityServiceEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: FinalityServiceEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(FinalityServiceTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(FinalityServiceEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), FinalityServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FinalityServiceError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = FinalityServiceEntryState::Completed;
        ...
        self.emit(FinalityServiceEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, FinalityServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FinalityServiceError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = FinalityServiceEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = FinalityServiceEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(FinalityServiceEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), FinalityServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FinalityServiceError::NotFound(id))?;
        entry.state = FinalityServiceEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<FinalityServiceEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<FinalityServiceEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: FinalityServiceEntryState) -> Vec<FinalityServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, FinalityServiceEntryState::Completed | FinalityServiceEntryState::Failed | FinalityServiceEntryState::Cancelled | FinalityServiceEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(FinalityServiceEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> FinalityServiceStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<FinalityServiceEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: FinalityServiceEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = FinalityServiceState::new(FinalityServiceConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = FinalityServiceConfig { retry_count: 2, ..Default::default() };
        let state = FinalityServiceState::new(config);
