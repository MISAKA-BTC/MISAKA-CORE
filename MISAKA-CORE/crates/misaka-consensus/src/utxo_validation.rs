//! UTXO validation — verify inputs exist and are unspent
//!
...
/// Configuration for the UtxoValidation subsystem.
...
impl Default for UtxoValidationConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 10_000,
...
/// Runtime state for UtxoValidation.
pub struct UtxoValidationRuntime {
    config: UtxoValidationConfig,
    entries: parking_lot::RwLock<HashMap<u64, UtxoValidationEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: UtxoValidationStats,
    event_buffer: parking_lot::Mutex<Vec<UtxoValidationEvent>>,
}

/// Individual entry tracked by UtxoValidation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoValidationEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by UtxoValidation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum UtxoValidationEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for UtxoValidation.
#[derive(Debug, Default)]
pub struct UtxoValidationStats {
...
impl UtxoValidationStats {
    pub fn snapshot(&self) -> UtxoValidationStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        UtxoValidationStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoValidationStatsSnapshot {
...
impl UtxoValidationRuntime {
    /// Create a new UtxoValidation runtime.
    pub fn new(config: UtxoValidationConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: UtxoValidationStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, UtxoValidationError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(UtxoValidationError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, UtxoValidationEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(UtxoValidationEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), UtxoValidationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoValidationError::NotFound(id))?;
        ...
        self.emit_event(UtxoValidationEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, UtxoValidationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoValidationError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(UtxoValidationEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<UtxoValidationEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<UtxoValidationEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(UtxoValidationEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> UtxoValidationStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<UtxoValidationEvent> {
    ...
    fn emit_event(&self, event: UtxoValidationEvent) {
...
/// Errors from UtxoValidation operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = UtxoValidationRuntime::new(UtxoValidationConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = UtxoValidationConfig { retry_count: 2, ..Default::default() };
        let rt = UtxoValidationRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = UtxoValidationConfig { max_entries: 2, ..Default::default() };
        let rt = UtxoValidationRuntime::new(config);
