//! DAG tip management and virtual parent selection
//!
...
/// Configuration for TipsManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TipsManagerConfig {
    pub enabled: bool,
...
impl Default for TipsManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// TipsManager operational state.
pub struct TipsManagerState {
    config: TipsManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, TipsManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<TipsManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: TipsManagerStats,
    events: parking_lot::Mutex<Vec<TipsManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in TipsManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TipsManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: TipsManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum TipsManagerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct TipsManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from TipsManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum TipsManagerEvent {
...
/// Statistics for TipsManager.
#[derive(Debug, Default)]
pub struct TipsManagerStats {
...
impl TipsManagerStats {
    pub fn snapshot(&self) -> TipsManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TipsManagerStatsSnapshot {
...
impl TipsManagerState {
    pub fn new(config: TipsManagerConfig) -> Self {
        Self {
            config,
            ...
            stats: TipsManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, TipsManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(TipsManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = TipsManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: TipsManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(TipsManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(TipsManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), TipsManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TipsManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = TipsManagerEntryState::Completed;
        ...
        self.emit(TipsManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, TipsManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TipsManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = TipsManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = TipsManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(TipsManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), TipsManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TipsManagerError::NotFound(id))?;
        entry.state = TipsManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<TipsManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<TipsManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: TipsManagerEntryState) -> Vec<TipsManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, TipsManagerEntryState::Completed | TipsManagerEntryState::Failed | TipsManagerEntryState::Cancelled | TipsManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(TipsManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> TipsManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<TipsManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: TipsManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = TipsManagerState::new(TipsManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = TipsManagerConfig { retry_count: 2, ..Default::default() };
        let state = TipsManagerState::new(config);
