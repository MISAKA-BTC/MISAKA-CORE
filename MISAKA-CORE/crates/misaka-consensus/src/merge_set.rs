//! Merge set computation for GhostDAG ordering
//!
...
/// Configuration for MergeSet.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MergeSetConfig {
    pub enabled: bool,
...
impl Default for MergeSetConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// MergeSet operational state.
pub struct MergeSetState {
    config: MergeSetConfig,
    entries: parking_lot::RwLock<HashMap<u64, MergeSetEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<MergeSetTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MergeSetStats,
    events: parking_lot::Mutex<Vec<MergeSetEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in MergeSet.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MergeSetEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: MergeSetEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Events from MergeSet.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum MergeSetEvent {
...
/// Statistics for MergeSet.
#[derive(Debug, Default)]
pub struct MergeSetStats {
...
impl MergeSetStats {
    pub fn snapshot(&self) -> MergeSetStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MergeSetStatsSnapshot {
...
impl MergeSetState {
    pub fn new(config: MergeSetConfig) -> Self {
        Self {
            config,
            ...
            stats: MergeSetStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, MergeSetError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(MergeSetError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = MergeSetEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: MergeSetEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(MergeSetTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(MergeSetEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), MergeSetError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MergeSetError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = MergeSetEntryState::Completed;
        ...
        self.emit(MergeSetEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, MergeSetError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MergeSetError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = MergeSetEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = MergeSetEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(MergeSetEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), MergeSetError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MergeSetError::NotFound(id))?;
        entry.state = MergeSetEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<MergeSetEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<MergeSetEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: MergeSetEntryState) -> Vec<MergeSetEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, MergeSetEntryState::Completed | MergeSetEntryState::Failed | MergeSetEntryState::Cancelled | MergeSetEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(MergeSetEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> MergeSetStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<MergeSetEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: MergeSetEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = MergeSetState::new(MergeSetConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = MergeSetConfig { retry_count: 2, ..Default::default() };
        let state = MergeSetState::new(config);
