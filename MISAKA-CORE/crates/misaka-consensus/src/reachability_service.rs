//! DAG reachability query service
//!
...
/// Configuration for ReachabilityService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReachabilityServiceConfig {
    pub enabled: bool,
...
impl Default for ReachabilityServiceConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// ReachabilityService operational state.
pub struct ReachabilityServiceState {
    config: ReachabilityServiceConfig,
    entries: parking_lot::RwLock<HashMap<u64, ReachabilityServiceEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<ReachabilityServiceTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ReachabilityServiceStats,
    events: parking_lot::Mutex<Vec<ReachabilityServiceEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in ReachabilityService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReachabilityServiceEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: ReachabilityServiceEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum ReachabilityServiceEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct ReachabilityServiceTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from ReachabilityService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ReachabilityServiceEvent {
...
/// Statistics for ReachabilityService.
#[derive(Debug, Default)]
pub struct ReachabilityServiceStats {
...
impl ReachabilityServiceStats {
    pub fn snapshot(&self) -> ReachabilityServiceStatsSnapshot {
        ...
        ReachabilityServiceStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReachabilityServiceStatsSnapshot {
...
impl ReachabilityServiceState {
    pub fn new(config: ReachabilityServiceConfig) -> Self {
        Self {
            config,
            ...
            stats: ReachabilityServiceStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, ReachabilityServiceError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(ReachabilityServiceError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = ReachabilityServiceEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: ReachabilityServiceEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(ReachabilityServiceTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(ReachabilityServiceEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), ReachabilityServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ReachabilityServiceError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = ReachabilityServiceEntryState::Completed;
        ...
        self.emit(ReachabilityServiceEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, ReachabilityServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ReachabilityServiceError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = ReachabilityServiceEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = ReachabilityServiceEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(ReachabilityServiceEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), ReachabilityServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ReachabilityServiceError::NotFound(id))?;
        entry.state = ReachabilityServiceEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<ReachabilityServiceEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<ReachabilityServiceEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: ReachabilityServiceEntryState) -> Vec<ReachabilityServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, ReachabilityServiceEntryState::Completed | ReachabilityServiceEntryState::Failed | ReachabilityServiceEntryState::Cancelled | ReachabilityServiceEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(ReachabilityServiceEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> ReachabilityServiceStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<ReachabilityServiceEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: ReachabilityServiceEvent) {
...
pub enum ReachabilityServiceError {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = ReachabilityServiceState::new(ReachabilityServiceConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = ReachabilityServiceConfig { retry_count: 2, ..Default::default() };
        let state = ReachabilityServiceState::new(config);
