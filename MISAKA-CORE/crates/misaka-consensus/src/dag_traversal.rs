//! DAG graph traversal utilities for consensus
//!
...
/// Configuration for DagTraversal.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DagTraversalConfig {
    pub enabled: bool,
...
impl Default for DagTraversalConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// DagTraversal operational state.
pub struct DagTraversalState {
    config: DagTraversalConfig,
    entries: parking_lot::RwLock<HashMap<u64, DagTraversalEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<DagTraversalTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: DagTraversalStats,
    events: parking_lot::Mutex<Vec<DagTraversalEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in DagTraversal.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DagTraversalEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: DagTraversalEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum DagTraversalEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct DagTraversalTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from DagTraversal.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DagTraversalEvent {
...
/// Statistics for DagTraversal.
#[derive(Debug, Default)]
pub struct DagTraversalStats {
...
impl DagTraversalStats {
    pub fn snapshot(&self) -> DagTraversalStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DagTraversalStatsSnapshot {
...
impl DagTraversalState {
    pub fn new(config: DagTraversalConfig) -> Self {
        Self {
            config,
            ...
            stats: DagTraversalStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, DagTraversalError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(DagTraversalError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = DagTraversalEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: DagTraversalEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(DagTraversalTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(DagTraversalEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), DagTraversalError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DagTraversalError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = DagTraversalEntryState::Completed;
        ...
        self.emit(DagTraversalEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, DagTraversalError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DagTraversalError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = DagTraversalEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = DagTraversalEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(DagTraversalEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), DagTraversalError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DagTraversalError::NotFound(id))?;
        entry.state = DagTraversalEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<DagTraversalEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<DagTraversalEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: DagTraversalEntryState) -> Vec<DagTraversalEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, DagTraversalEntryState::Completed | DagTraversalEntryState::Failed | DagTraversalEntryState::Cancelled | DagTraversalEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(DagTraversalEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> DagTraversalStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<DagTraversalEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: DagTraversalEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = DagTraversalState::new(DagTraversalConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = DagTraversalConfig { retry_count: 2, ..Default::default() };
        let state = DagTraversalState::new(config);
