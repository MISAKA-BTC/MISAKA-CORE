//! Consensus block builder for validators
//!
...
/// Configuration for BlockBuilder.
...
/// BlockBuilder operational state.
pub struct BlockBuilderState {
    config: BlockBuilderConfig,
    entries: parking_lot::RwLock<HashMap<u64, BlockBuilderEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BlockBuilderTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BlockBuilderStats,
    events: parking_lot::Mutex<Vec<BlockBuilderEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BlockBuilder.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockBuilderEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BlockBuilderEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BlockBuilderEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BlockBuilderTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BlockBuilder.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BlockBuilderEvent {
...
impl BlockBuilderStats {
    pub fn snapshot(&self) -> BlockBuilderStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockBuilderStatsSnapshot {
...
impl BlockBuilderState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BlockBuilderError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BlockBuilderError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BlockBuilderEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BlockBuilderEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BlockBuilderTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BlockBuilderEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BlockBuilderError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockBuilderError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = BlockBuilderEntryState::Completed;
        ...
        self.emit(BlockBuilderEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BlockBuilderError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockBuilderError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BlockBuilderEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BlockBuilderEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BlockBuilderEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BlockBuilderError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockBuilderError::NotFound(id))?;
        entry.state = BlockBuilderEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BlockBuilderEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BlockBuilderEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BlockBuilderEntryState) -> Vec<BlockBuilderEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BlockBuilderEntryState::Completed | BlockBuilderEntryState::Failed | BlockBuilderEntryState::Cancelled | BlockBuilderEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BlockBuilderEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BlockBuilderStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BlockBuilderEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BlockBuilderEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BlockBuilderState::new(BlockBuilderConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BlockBuilderConfig { retry_count: 2, ..Default::default() };
        let state = BlockBuilderState::new(config);
