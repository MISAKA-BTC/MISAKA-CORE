...
pub fn validate_tx_mass(mass: u64) -> Result<(), MassError> {
    if mass > MAX_TX_MASS {
        Err(MassError::TxMassExceeded { mass, max: MAX_TX_MASS })
    } else {
        Ok(())
    }
}
...
pub fn validate_block_mass(total_mass: u64) -> Result<(), MassError> {
    if total_mass > MAX_BLOCK_MASS {
        Err(MassError::BlockMassExceeded { mass: total_mass, max: MAX_BLOCK_MASS })
    } else {
        Ok(())
    }
}
...
pub fn validate_timestamp(block_time: u64, past_median_time: u64, max_future_offset: u64) -> Result<(), TimestampError> {
    ...
    if block_time <= past_median_time {
        return Err(TimestampError::BeforeMedian { block_time, median: past_median_time });
    }
    if block_time > now + max_future_offset {
        return Err(TimestampError::TooFarFuture { block_time, max: now + max_future_offset });
    }
    Ok(())
}
