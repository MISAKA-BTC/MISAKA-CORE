...
impl From<&Header> for CompactHeaderData {
    fn from(h: &Header) -> Self {
        Self { daa_score: h.daa_score, timestamp: h.timestamp, bits: h.bits, blue_score: h.blue_score }
    }
}
...
impl DbHeadersStore {
    pub fn new(db: Arc<DB>, cache_policy: CachePolicy, compact_cache_policy: CachePolicy) -> Self {
        Self {
            db: db.clone(),
            headers_access: CachedDbAccess::new(db.clone(), cache_policy,
                DatabaseStorePrefixes::Headers.as_prefix()),
            compact_access: CachedDbAccess::new(db, compact_cache_policy,
                DatabaseStorePrefixes::HeadersCompact.as_prefix()),
        }
    }

    pub fn insert_batch(&self, batch: &mut WriteBatch, hash: Hash, header: Header) -> StoreResult<()> {
        ...
        self.headers_access.write(BatchDbWriter::new(batch), hash, header)?;
...
impl HeaderStoreReader for DbHeadersStore {
    fn get_daa_score(&self, hash: Hash) -> StoreResult<u64> { Ok(self.compact_access.read(hash)?.daa_score) }
    fn get_blue_score(&self, hash: Hash) -> StoreResult<u64> { Ok(self.compact_access.read(hash)?.blue_score) }
    fn get_timestamp(&self, hash: Hash) -> StoreResult<u64> { Ok(self.compact_access.read(hash)?.timestamp) }
    fn get_bits(&self, hash: Hash) -> StoreResult<u32> { Ok(self.compact_access.read(hash)?.bits) }
    fn get_header(&self, hash: Hash) -> StoreResult<Header> { self.headers_access.read(hash) }
    fn get_compact_header_data(&self, hash: Hash) -> StoreResult<CompactHeaderData> { self.compact_access.read(hash) }
    fn has(&self, hash: Hash) -> StoreResult<bool> { self.headers_access.has(hash) }
}
