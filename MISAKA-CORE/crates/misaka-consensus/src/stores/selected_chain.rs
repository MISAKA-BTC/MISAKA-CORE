...
impl U64KeyBytes {
    pub fn new(val: u64) -> Self { Self { bytes: val.to_le_bytes() } }
    pub fn value(&self) -> u64 { u64::from_le_bytes(self.bytes) }
}
...
impl DbSelectedChainStore {
    pub fn new(db: Arc<DB>, cache_policy: CachePolicy) -> Self {
        Self {
            db: db.clone(),
            hash_by_index: CachedDbAccess::new(db.clone(), cache_policy,
                DatabaseStorePrefixes::ChainHashByIndex.as_prefix()),
            index_by_hash: CachedDbAccess::new(db.clone(), cache_policy,
                DatabaseStorePrefixes::ChainIndexByHash.as_prefix()),
            highest_index: CachedDbItem::new(db, DatabaseStorePrefixes::ChainHighestIndex.as_prefix()),
        }
    }

    pub fn apply_new_chain_block(&mut self, batch: &mut WriteBatch, index: u64, hash: Hash) -> StoreResult<()> {
        self.hash_by_index.write(BatchDbWriter::new(batch), U64KeyBytes::new(index), hash)?;
        self.index_by_hash.write(BatchDbWriter::new(batch), hash, StoredU64(index))?;
        self.highest_index.write(BatchDbWriter::new(batch), &StoredU64(index))?;
        Ok(())
    }
}

impl SelectedChainStoreReader for DbSelectedChainStore {
    fn get_by_hash(&self, hash: Hash) -> StoreResult<u64> { Ok(self.index_by_hash.read(hash)?.0) }
    fn get_by_index(&self, index: u64) -> StoreResult<Hash> { self.hash_by_index.read(U64KeyBytes::new(index)) }
    fn get_tip(&self) -> StoreResult<(u64, Hash)> {
