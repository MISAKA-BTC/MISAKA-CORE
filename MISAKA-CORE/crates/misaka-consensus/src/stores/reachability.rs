...
impl DbReachabilityStore {
    ...
    pub fn insert_batch(&self, batch: &mut WriteBatch, hash: Hash, data: ReachabilityData) -> StoreResult<()> {
        self.access.write(BatchDbWriter::new(batch), hash, data)
    }

    pub fn insert(&self, hash: Hash, data: ReachabilityData) -> StoreResult<()> {
        self.access.write(DirectDbWriter::new(self.db.clone()), hash, data)
    }

    pub fn set_interval(&self, batch: &mut WriteBatch, hash: Hash, interval: ReachabilityInterval) -> StoreResult<()> {
...
impl ReachabilityStoreReader for DbReachabilityStore {
    fn get_interval(&self, hash: Hash) -> StoreResult<ReachabilityInterval> { Ok(self.access.read(hash)?.interval) }
    fn get_parent(&self, hash: Hash) -> StoreResult<Hash> { Ok(self.access.read(hash)?.parent) }
    fn get_children(&self, hash: Hash) -> StoreResult<Vec<Hash>> { Ok(self.access.read(hash)?.children) }
    fn get_future_covering_set(&self, hash: Hash) -> StoreResult<Vec<Hash>> { Ok(self.access.read(hash)?.future_covering_set) }
