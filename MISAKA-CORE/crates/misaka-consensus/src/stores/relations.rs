...
impl DbRelationsStore {
    pub fn new(db: Arc<DB>, level: u8, cache_policy: CachePolicy) -> Self {
        Self {
            db: db.clone(),
            parents_access: CachedDbAccess::new(db.clone(), cache_policy,
                DatabaseStorePrefixes::RelationsParents.with_bucket(level)),
            children_access: CachedDbAccess::new(db, cache_policy,
                DatabaseStorePrefixes::RelationsChildren.with_bucket(level)),
        }
    }

    pub fn insert_batch(&self, batch: &mut WriteBatch, hash: Hash, parents: Vec<Hash>) -> StoreResult<()> {
        self.parents_access.write(BatchDbWriter::new(batch), hash, HashVec(parents.clone()))?;
        for parent in &parents {
            let mut children = self.children_access.read(*parent)
                .map(|h| h.0).unwrap_or_default();
            children.push(hash);
            self.children_access.write(BatchDbWriter::new(batch), *parent, HashVec(children))?;
        }
        if !self.children_access.has(hash)? {
            self.children_access.write(BatchDbWriter::new(batch), hash, HashVec(Vec::new()))?;
        }
        Ok(())
    }
}
...
impl RelationsStoreReader for DbRelationsStore {
    fn get_parents(&self, hash: Hash) -> StoreResult<Vec<Hash>> { Ok(self.parents_access.read(hash)?.0) }
    fn get_children(&self, hash: Hash) -> StoreResult<Vec<Hash>> { Ok(self.children_access.read(hash)?.0) }
    fn has(&self, hash: Hash) -> StoreResult<bool> { self.parents_access.has(hash) }
}
