...
impl MemSizeEstimator for UtxoDiff {
    fn estimate_mem_bytes(&self) -> usize {
        std::mem::size_of::<Self>() + (self.added.len() + self.removed.len()) * std::mem::size_of::<UtxoEntry>()
    }
}
...
impl DbUtxoDiffsStore {
    ...
    pub fn insert_batch(&self, batch: &mut WriteBatch, hash: Hash, diff: UtxoDiff) -> StoreResult<()> {
        self.access.write(BatchDbWriter::new(batch), hash, diff)
    }
    pub fn get(&self, hash: Hash) -> StoreResult<UtxoDiff> { self.access.read(hash) }
