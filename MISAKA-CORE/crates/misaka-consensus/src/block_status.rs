...
/// Configuration for BlockStatus.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockStatusConfig {
    pub enabled: bool,
...
impl Default for BlockStatusConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// BlockStatus operational state.
pub struct BlockStatusState {
    config: BlockStatusConfig,
    entries: parking_lot::RwLock<HashMap<u64, BlockStatusEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BlockStatusTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BlockStatusStats,
    events: parking_lot::Mutex<Vec<BlockStatusEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BlockStatus.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockStatusEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BlockStatusEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BlockStatusEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BlockStatusTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BlockStatus.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BlockStatusEvent {
...
/// Statistics for BlockStatus.
#[derive(Debug, Default)]
pub struct BlockStatusStats {
...
impl BlockStatusStats {
    pub fn snapshot(&self) -> BlockStatusStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockStatusStatsSnapshot {
...
impl BlockStatusState {
    pub fn new(config: BlockStatusConfig) -> Self {
        Self {
            config,
            ...
            stats: BlockStatusStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BlockStatusError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BlockStatusError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BlockStatusEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BlockStatusEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BlockStatusTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BlockStatusEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BlockStatusError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockStatusError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = BlockStatusEntryState::Completed;
        ...
        self.emit(BlockStatusEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BlockStatusError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockStatusError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BlockStatusEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BlockStatusEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BlockStatusEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BlockStatusError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockStatusError::NotFound(id))?;
        entry.state = BlockStatusEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BlockStatusEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BlockStatusEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BlockStatusEntryState) -> Vec<BlockStatusEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BlockStatusEntryState::Completed | BlockStatusEntryState::Failed | BlockStatusEntryState::Cancelled | BlockStatusEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BlockStatusEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BlockStatusStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BlockStatusEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BlockStatusEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BlockStatusState::new(BlockStatusConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BlockStatusConfig { retry_count: 2, ..Default::default() };
        let state = BlockStatusState::new(config);
