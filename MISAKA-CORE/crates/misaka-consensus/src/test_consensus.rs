//! Test consensus framework for integration testing
//!
...
/// Configuration for TestConsensus.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestConsensusConfig {
    pub enabled: bool,
...
impl Default for TestConsensusConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// TestConsensus operational state.
pub struct TestConsensusState {
    config: TestConsensusConfig,
    entries: parking_lot::RwLock<HashMap<u64, TestConsensusEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<TestConsensusTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: TestConsensusStats,
    events: parking_lot::Mutex<Vec<TestConsensusEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in TestConsensus.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestConsensusEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: TestConsensusEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum TestConsensusEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct TestConsensusTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from TestConsensus.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum TestConsensusEvent {
...
/// Statistics for TestConsensus.
#[derive(Debug, Default)]
pub struct TestConsensusStats {
...
impl TestConsensusStats {
    pub fn snapshot(&self) -> TestConsensusStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestConsensusStatsSnapshot {
...
impl TestConsensusState {
    pub fn new(config: TestConsensusConfig) -> Self {
        Self {
            config,
            ...
            stats: TestConsensusStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, TestConsensusError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(TestConsensusError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = TestConsensusEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: TestConsensusEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(TestConsensusTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(TestConsensusEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), TestConsensusError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TestConsensusError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = TestConsensusEntryState::Completed;
        ...
        self.emit(TestConsensusEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, TestConsensusError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TestConsensusError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = TestConsensusEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = TestConsensusEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(TestConsensusEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), TestConsensusError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TestConsensusError::NotFound(id))?;
        entry.state = TestConsensusEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<TestConsensusEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<TestConsensusEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: TestConsensusEntryState) -> Vec<TestConsensusEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, TestConsensusEntryState::Completed | TestConsensusEntryState::Failed | TestConsensusEntryState::Cancelled | TestConsensusEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(TestConsensusEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> TestConsensusStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<TestConsensusEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: TestConsensusEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = TestConsensusState::new(TestConsensusConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = TestConsensusConfig { retry_count: 2, ..Default::default() };
        let state = TestConsensusState::new(config);
