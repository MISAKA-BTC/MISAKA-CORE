...
impl PruningProcessor {
    pub fn new(
        ...
        Self { config, db, ghostdag_store, headers_store, pruning_store }
    }

    /// Attempt to advance the pruning point based on current virtual state.
    pub fn maybe_advance_pruning_point(&self, virtual_selected_parent: Hash) -> Result<bool, PruningError> {
        let virtual_blue_score = self.ghostdag_store.get_blue_score(&virtual_selected_parent).unwrap_or(0);
        let current_pruning = self.pruning_store.read().get();

        let current_pruning_score = match &current_pruning {
            Ok(info) => self.ghostdag_store.get_blue_score(&info.pruning_point).unwrap_or(0),
            Err(_) => 0,
        };
    ...
    fn find_block_at_score(&self, start: Hash, target_score: u64) -> Result<Hash, PruningError> {
        ...
        loop {
            ...
            current = self.ghostdag_store.get_selected_parent(&current).unwrap_or(ZERO_HASH);
        }
    }
}
