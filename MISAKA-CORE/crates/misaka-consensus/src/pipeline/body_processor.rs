...
impl BodyProcessor {
    pub fn new(
        ...
        Self { db, block_txs_store, statuses_store }
    }

    /// Validate and store block body.
    pub fn process_body(&self, block_hash: Hash, transactions: Vec<StoredTransaction>) -> Result<(), BodyProcessingError> {
        ...
        self.block_txs_store.insert_batch(&mut batch, block_hash, transactions)?;
        self.statuses_store.set_batch(&mut batch, block_hash, BlockStatus::StatusBodyValid)?;
        self.db.write(batch).map_err(StoreError::from)?;

        Ok(())
    }

    fn validate_transactions_in_isolation(&self, txs: &[StoredTransaction]) -> Result<(), BodyProcessingError> {
        let mut seen_tx_ids = std::collections::HashSet::new();
        for tx in txs {
            if !seen_tx_ids.insert(tx.tx_id) {
                return Err(BodyProcessingError::DuplicateTransaction(hex::encode(tx.tx_id)));
            }
