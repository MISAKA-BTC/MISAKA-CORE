...
use crate::p2p_mode::ValidatorNetworkProfile;
...
impl RewardDistributor {
    ...
    pub fn compute_reward(
        &self,
        record: &ValidatorRecord,
        scoring: &crate::validator_scoring::ScoringConfig,
        is_active: bool,
    ) -> u64 {
        ...
        let base = if is_active {
            ...
            // Backup はより少ない
            (self.epoch_base_reward / ACTIVE_SET_SIZE as u64) * (self.backup_reward_ratio * 1000.0) as u64 / 1000
        };
...
mod tests {
    ...
    fn good_active_metrics(stake: u64) -> ValidatorMetrics {
        ValidatorMetrics { stake, uptime: 0.99, contribution: 0.99, timeouts: 0, invalid_actions: 0 }
    }

    fn bad_metrics(stake: u64) -> ValidatorMetrics {
        ValidatorMetrics { stake, uptime: 0.50, contribution: 0.50, timeouts: 10, invalid_actions: 2 }
    }
    ...
    fn make_full_registry(n_backup: u8) -> ValidatorRegistry {
        ...
        for i in 1u8..=21 {
            reg.register(make_id(i), min_active, public_profile(i)).expect("ok");
            reg.update_metrics(&make_id(i), good_active_metrics(min_active)).expect("ok");
        ...
        for i in 22u8..22 + n_backup {
            reg.register(make_id(i), min_backup, backup_profile(i)).expect("ok");
            reg.update_metrics(&make_id(i), good_active_metrics(min_backup)).expect("ok");
        }
    ...
    fn rotation_promotes_public_backup_over_local() {
        ...
        reg.register(make_id(30), min_backup * 2, public_profile(30)).expect("ok");
        reg.update_metrics(&make_id(30), good_active_metrics(min_backup * 2)).expect("ok");

        reg.register(make_id(31), min_backup * 3, backup_profile(31)).expect("ok"); // より高スコア
        reg.update_metrics(&make_id(31), good_active_metrics(min_backup * 3)).expect("ok");
    ...
    fn active_set_always_21_when_enough_backups() {
        ...
        for i in 1u8..=21 {
            reg.register(make_id(i), min_active, public_profile(i)).expect("ok");
            let m = if i <= 3 { bad_metrics(min_active) } else { good_active_metrics(min_active) };
        ...
        // 公開接続の Backup 3人
        for i in 22u8..=24 {
            reg.register(make_id(i), min_backup, public_profile(i)).expect("ok");
            reg.update_metrics(&make_id(i), good_active_metrics(min_backup)).expect("ok");
        }
    ...
    fn demoted_enters_cooldown() {
        ...
        reg.register(make_id(1), min_active, public_profile(1)).expect("ok");
        reg.update_metrics(&make_id(1), bad_metrics(min_active)).expect("ok");
    ...
    fn reward_zero_during_cooldown() {
        ...
        reg.register(make_id(1), min_active, backup_profile(1)).expect("ok");
        reg.update_metrics(&make_id(1), good_active_metrics(min_active)).expect("ok");
        // クールダウンに入れる
