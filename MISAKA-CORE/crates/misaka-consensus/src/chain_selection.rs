//! Chain selection rule — heaviest blue work wins
//!
...
/// Configuration for ChainSelection.
...
/// ChainSelection operational state.
pub struct ChainSelectionState {
    config: ChainSelectionConfig,
    entries: parking_lot::RwLock<HashMap<u64, ChainSelectionEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<ChainSelectionTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ChainSelectionStats,
    events: parking_lot::Mutex<Vec<ChainSelectionEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in ChainSelection.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ChainSelectionEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: ChainSelectionEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum ChainSelectionEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct ChainSelectionTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from ChainSelection.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ChainSelectionEvent {
...
/// Statistics for ChainSelection.
#[derive(Debug, Default)]
pub struct ChainSelectionStats {
...
impl ChainSelectionStats {
    pub fn snapshot(&self) -> ChainSelectionStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ChainSelectionStatsSnapshot {
...
impl ChainSelectionState {
    pub fn new(config: ChainSelectionConfig) -> Self {
        Self {
            ...
            stats: ChainSelectionStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, ChainSelectionError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(ChainSelectionError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = ChainSelectionEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: ChainSelectionEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(ChainSelectionTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(ChainSelectionEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), ChainSelectionError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ChainSelectionError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = ChainSelectionEntryState::Completed;
        ...
        self.emit(ChainSelectionEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, ChainSelectionError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ChainSelectionError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = ChainSelectionEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = ChainSelectionEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(ChainSelectionEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), ChainSelectionError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ChainSelectionError::NotFound(id))?;
        entry.state = ChainSelectionEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<ChainSelectionEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<ChainSelectionEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: ChainSelectionEntryState) -> Vec<ChainSelectionEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, ChainSelectionEntryState::Completed | ChainSelectionEntryState::Failed | ChainSelectionEntryState::Cancelled | ChainSelectionEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(ChainSelectionEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> ChainSelectionStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<ChainSelectionEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: ChainSelectionEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = ChainSelectionState::new(ChainSelectionConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = ChainSelectionConfig { retry_count: 2, ..Default::default() };
        let state = ChainSelectionState::new(config);
