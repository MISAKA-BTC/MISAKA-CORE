//! Complete consensus validation rule set
//!
...
/// Configuration for ValidationRules.
...
/// ValidationRules operational state.
pub struct ValidationRulesState {
    config: ValidationRulesConfig,
    entries: parking_lot::RwLock<HashMap<u64, ValidationRulesEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<ValidationRulesTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ValidationRulesStats,
    events: parking_lot::Mutex<Vec<ValidationRulesEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in ValidationRules.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ValidationRulesEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: ValidationRulesEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum ValidationRulesEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct ValidationRulesTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from ValidationRules.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ValidationRulesEvent {
...
/// Statistics for ValidationRules.
#[derive(Debug, Default)]
pub struct ValidationRulesStats {
...
impl ValidationRulesStats {
    pub fn snapshot(&self) -> ValidationRulesStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ValidationRulesStatsSnapshot {
...
impl ValidationRulesState {
    pub fn new(config: ValidationRulesConfig) -> Self {
        Self {
            ...
            stats: ValidationRulesStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, ValidationRulesError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(ValidationRulesError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = ValidationRulesEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: ValidationRulesEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(ValidationRulesTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(ValidationRulesEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), ValidationRulesError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ValidationRulesError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = ValidationRulesEntryState::Completed;
        ...
        self.emit(ValidationRulesEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, ValidationRulesError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ValidationRulesError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = ValidationRulesEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = ValidationRulesEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(ValidationRulesEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), ValidationRulesError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ValidationRulesError::NotFound(id))?;
        entry.state = ValidationRulesEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<ValidationRulesEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<ValidationRulesEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: ValidationRulesEntryState) -> Vec<ValidationRulesEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, ValidationRulesEntryState::Completed | ValidationRulesEntryState::Failed | ValidationRulesEntryState::Cancelled | ValidationRulesEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(ValidationRulesEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> ValidationRulesStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<ValidationRulesEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: ValidationRulesEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = ValidationRulesState::new(ValidationRulesConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = ValidationRulesConfig { retry_count: 2, ..Default::default() };
        let state = ValidationRulesState::new(config);
