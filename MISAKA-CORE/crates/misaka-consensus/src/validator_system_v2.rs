...
pub enum ValidatorRole { Active, Backup }

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum ConnectionMode { Public, OutboundOnly }

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum ValidatorStatus {
    Normal,
    Cooldown { until_epoch: u64 },
    Jailed { until_epoch: u64, reason: JailReason },
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum JailReason { DoubleSign, InvalidBlock, InvalidQC, FraudProof }
...
impl ValidatorAccountV2 {
    ...
    pub fn stake_factor(&self, config: &ValidatorSystemConfig) -> f64 {
        if config.min_stake_active == 0 { return 1.0; }
    ...
    pub fn credit(&self, config: &ValidatorSystemConfig) -> f64 {
        if config.credit_base_stake == 0 { return 1.0; }
        (self.stake_amount as f64 / config.credit_base_stake as f64)
            .min(config.credit_cap)
    }
    ...
    pub fn compute_penalty_factor(&self, config: &ValidatorSystemConfig) -> f64 {
        ...
        (base - config.penalty_per_timeout * self.timeouts as f64
...
impl ValidatorSystemV2 {
    ...
    pub fn register(
        ...
        self.validators.insert(validator_id, ValidatorAccountV2 {
            validator_id,
            pubkey,
            stake_amount,
    ...
    pub fn report_infraction(
        ...
        let v = self.validators.get_mut(validator_id)
        ...
        Ok(InfractionResult {
            ...
            immediately_removed,
            jailed,
            new_penalty_factor: self.validators.get(validator_id)
                .map(|v| v.penalty_factor).unwrap_or(0.0),
    ...
    pub fn enforce_critical_uptime(&mut self) -> Vec<ImmediateRemovalResult> {
        ...
        // 除外対象を特定
        let to_remove: Vec<[u8; 32]> = self.validators.values()
        ...
        for id in to_remove {
            let uptime_bps = self.validators.get(&id)
    ...
    pub fn process_epoch_update(&mut self) -> EpochUpdateResult {
        ...
        for v in self.validators.values() {
            if v.role != ValidatorRole::Active { continue; }
        ...
        for (id, score, reason) in &demotion_candidates {
            if let Some(v) = self.validators.get_mut(id) {
                ...
                demoted.push(DemotionRecord {
                    validator_id: *id, reason: *reason, score: *score,
                });
            }
        }
        ...
        let vacancies = self.config.active_set_size.saturating_sub(self.active_count());
        if vacancies > 0 {
            let mut eligible: Vec<([u8; 32], f64)> = self.validators.values()
                .filter(|v| v.role == ValidatorRole::Backup
                    && v.can_promote_to_active(&self.config, epoch))
            ...
            for (id, score) in eligible.into_iter().take(vacancies) {
                if let Some(v) = self.validators.get_mut(&id) {
                    ...
                    promoted.push(PromotionRecord { validator_id: id, score });
                }
            }
        }

        // ── Step 5: 報酬配分 ──
        let active_ids: Vec<[u8; 32]> = self.validators.values()
            .filter(|v| v.role == ValidatorRole::Active)
            .map(|v| v.validator_id).collect();
        let per_base = if active_ids.is_empty() { 0 }
            else { self.config.base_reward_per_epoch / active_ids.len() as u64 };
        ...
        for id in &active_ids {
            if let Some(v) = self.validators.get_mut(id) {
                ...
                rewards.push(RewardRecord {
                    validator_id: *id, amount: reward, score: v.score,
                });
            }
        }

        // ── Step 6: メトリクスリセット ──
        for v in self.validators.values_mut() { v.reset_epoch_metrics(); }
    ...
    fn promote_top_backup(&mut self) -> Option<[u8; 32]> {
        ...
        for v in self.validators.values_mut() {
            v.penalty_factor = v.compute_penalty_factor(&config);
            v.score = v.compute_score(&config);
        }

        let mut eligible: Vec<([u8; 32], f64)> = self.validators.values()
            .filter(|v| v.role == ValidatorRole::Backup
                && v.can_promote_to_active(&config, epoch))
            .map(|v| (v.validator_id, v.score))
            .collect();
    ...
    pub fn active_validators(&self) -> Vec<&ValidatorAccountV2> {
        let mut v: Vec<_> = self.validators.values()
            .filter(|v| v.role == ValidatorRole::Active).collect();
        v.sort_by(|a, b| b.score.partial_cmp(&a.score)
    ...
    pub fn backup_validators(&self) -> Vec<&ValidatorAccountV2> {
        let mut v: Vec<_> = self.validators.values()
            .filter(|v| v.role == ValidatorRole::Backup).collect();
        v.sort_by(|a, b| b.score.partial_cmp(&a.score)
            .unwrap_or(std::cmp::Ordering::Equal)
            .then_with(|| a.validator_id.cmp(&b.validator_id)));
        v
    ...
    pub fn get(&self, id: &[u8; 32]) -> Option<&ValidatorAccountV2> { self.validators.get(id) }
    pub fn active_count(&self) -> usize { self.validators.values().filter(|v| v.role == ValidatorRole::Active).count() }
    pub fn backup_count(&self) -> usize { self.validators.values().filter(|v| v.role == ValidatorRole::Backup).count() }
    pub fn total_active_stake(&self) -> u64 {
        self.validators.values()
            .filter(|v| v.role == ValidatorRole::Active)
            .map(|v| v.effective_stake(&self.config)).sum()
    }
}
...
mod tests {
    ...
    fn id(n: u8) -> [u8; 32] { let mut a = [0u8; 32]; a[0] = n; a }

    fn sys() -> ValidatorSystemV2 { ValidatorSystemV2::new(ValidatorSystemConfig::testnet()) }

    fn add_active(s: &mut ValidatorSystemV2, n: u8, stake: u64) {
        s.register(id(n), vec![n; 32], stake, id(n), ConnectionMode::Public, true, None).unwrap();
    ...
    fn add_backup(s: &mut ValidatorSystemV2, n: u8, stake: u64) {
        s.register(id(n), vec![n; 32], stake, id(n), ConnectionMode::OutboundOnly, true, None).unwrap();
    }
    ...
    fn stake_never_reduced_on_invalid_block() {
        ...
        add_active(&mut s, 1, 10_000_000 * DECIMALS);
        let before = s.get(&id(1)).unwrap().stake_amount;
        s.report_infraction(&id(1), Infraction::InvalidBlock).unwrap();
    ...
    fn double_sign_immediately_removes_and_jails() {
        ...
        b.public_reachable = true; b.port_open = true; b.inbound_consensus = true;
    ...
    fn critical_uptime_immediately_removes() {
        ...
        assert_eq!(s.get(&id(1)).unwrap().role, ValidatorRole::Backup);
        assert!(matches!(s.get(&id(1)).unwrap().status, ValidatorStatus::Cooldown { .. }));
        // Stake untouched
    ...
    fn low_uptime_demoted_at_monthly_update() {
        let mut s = sys();
        for i in 1..=21u8 { add_active(&mut s, i, 10_000_000 * DECIMALS); }
    ...
    fn backup_promoted_to_fill_vacancies() {
        ...
        let bid = id(100);
        s.register(bid, vec![100; 32], 10_000_000 * DECIMALS, bid, ConnectionMode::Public, true, None).unwrap();
        let b = s.validators.get_mut(&bid).unwrap();
        b.public_reachable = true; b.port_open = true; b.inbound_consensus = true;
        b.uptime = 0.99; b.contribution = 0.99;
    ...
    fn cooldown_blocks_immediate_repromotion() {
        ...
        assert_eq!(s.get(&id(1)).unwrap().role, ValidatorRole::Backup);
        assert!(!s.get(&id(1)).unwrap().can_promote_to_active(&s.config, s.current_epoch));
    }
    ...
    fn effective_stake_capped_at_saturation() {
        ...
        assert_eq!(s.get(&id(1)).unwrap().effective_stake(&s.config), s.config.max_effective_stake);
    }
    ...
    fn backup_allows_outbound_only() {
        ...
        assert_eq!(s.get(&id(1)).unwrap().connection_mode, ConnectionMode::OutboundOnly);
    }
}
