        ...
        for (validator_id_hex, account) in as_hex {
            ...
            if bytes.len() != 20 {
                return Err(D::Error::custom(format!(
                    "validator id must be 20 bytes, got {}",
                    bytes.len()
                )));
            }
...
impl StakingRegistry {
    ...
    pub fn mark_stake_verified(
        ...
        if self.used_stake_signatures.contains(&signature) {
            ...
            let account = self.validators.get(validator_id)
...
mod tests {
    ...
    fn register_and_activate(reg: &mut StakingRegistry, id: [u8; 32], stake: u64, epoch: u64) {
        reg.register(
            id, vec![1; 1952], stake, 500, id, epoch, [id[0]; 32], 0,
            true, // solana_stake_verified — pre-verified for test convenience
            Some("test_sig".to_string()),
        ).unwrap();
    ...
    fn test_full_lifecycle() {
        ...
        reg.register(id, vec![1; 1952], 10_000_000, 500, id, 0, [1; 32], 0,
            true, Some("solana_sig_abc".into()))
            .unwrap();
    ...
    fn test_below_min_stake() {
        ...
        assert!(reg
            .register(
                make_id(1), vec![], 9_999_999, 500, make_id(1), 0, [1; 32], 0,
                true, None,
            )
            .is_err());
    }
    ...
    fn test_exit_from_locked_fails() {
        let mut reg = StakingRegistry::new(test_config());
        let id = make_id(1);
        reg.register(id, vec![], 10_000_000, 500, id, 0, [1; 32], 0,
            true, None)
            .unwrap();
    ...
    fn test_reward_weight_linear_proportional() {
        ...
        assert!((ratio - 4.0).abs() < 0.01, "4x stake should yield 4x weight, got {ratio}x");
    }
    ...
    fn test_commission_too_high() {
        ...
        assert!(reg
            .register(
                make_id(1), vec![], 10_000_000, 9000, make_id(1), 0, [1; 32], 0,
                true, None,
            )
            .is_err());
    }
    ...
    fn test_reregister_after_unlock() {
        ...
        reg.register(id, vec![1; 1952], 10_000_000, 500, id, 300, [2; 32], 0,
            true, Some("new_sig".into()))
            .unwrap();
        assert_eq!(reg.get(&id).unwrap().state, ValidatorState::Locked);
    }
    ...
    fn test_activate_rejected_without_stake_verification() {
        ...
        reg.register(
            id, vec![1; 1952], 10_000_000, 500, id, 0, [1; 32], 0,
    ...
    fn test_activate_succeeds_after_mark_stake_verified() {
        ...
        reg.mark_stake_verified(&id, "5nXuqx...verified_sig".to_string()).unwrap();
        assert!(reg.get(&id).unwrap().solana_stake_verified);
        assert_eq!(
    ...
    fn test_register_with_verified_stake_can_activate_immediately() {
        ...
        reg.register(
            id, vec![1; 1952], 10_000_000, 500, id, 0, [1; 32], 0,
            true, Some("pre_verified_sig".into()),
        ).unwrap();
    ...
    fn test_testnet_1m_stake_accepted() {
        ...
        reg.register(
            id, vec![1; 1952], 1_000_000_000_000_000, 500, id, 0, [1; 32], 0,
            true, Some("testnet_sig".into()),
        ).unwrap();

        // Below 1M — should fail
        let id2 = make_id(2);
        let result = reg.register(
            id2, vec![1; 1952], 999_999_999_999_999, 500, id2, 0, [2; 32], 0,
            true, None,
        );
        assert!(result.is_err());
    }
    ...
    fn test_mainnet_10m_stake_required() {
        ...
        let result = reg.register(
            id, vec![1; 1952], 9_999_999_999_999_999, 500, id, 0, [1; 32], 0,
            true, None,
        );
        assert!(result.is_err());
        ...
        reg.register(
            id, vec![1; 1952], 10_000_000_000_000_000, 500, id, 0, [1; 32], 0,
            true, Some("mainnet_sig".into()),
        ).unwrap();
    }
    ...
    fn test_same_stake_signature_rejected_for_second_validator() {
        ...
        reg.register(
            id1, vec![1; 1952], 10_000_000, 500, id1, 0, [1; 32], 0,
            true, Some(shared_sig.clone()),
        ).unwrap();

        // Second validator tries to use the SAME signature — MUST fail
        let result = reg.register(
            id2, vec![2; 1952], 10_000_000, 500, id2, 0, [2; 32], 0,
            true, Some(shared_sig.clone()),
        );
        assert!(result.is_err());
        match result.unwrap_err() {
    ...
    fn test_stake_signature_released_after_unlock() {
        ...
        reg.register(
            id2, vec![2; 1952], 10_000_000, 500, id2, 300, [2; 32], 0,
            true, Some(sig.clone()),
        ).unwrap();
