//! Block window sampling for difficulty and median time
//!
...
/// Configuration for BlockWindow.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockWindowConfig {
    pub enabled: bool,
...
impl Default for BlockWindowConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// BlockWindow operational state.
pub struct BlockWindowState {
    config: BlockWindowConfig,
    entries: parking_lot::RwLock<HashMap<u64, BlockWindowEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BlockWindowTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BlockWindowStats,
    events: parking_lot::Mutex<Vec<BlockWindowEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BlockWindow.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockWindowEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BlockWindowEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BlockWindowEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BlockWindowTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BlockWindow.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BlockWindowEvent {
...
impl BlockWindowStats {
    pub fn snapshot(&self) -> BlockWindowStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockWindowStatsSnapshot {
...
impl BlockWindowState {
    pub fn new(config: BlockWindowConfig) -> Self {
        Self {
            config,
            ...
            stats: BlockWindowStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BlockWindowError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BlockWindowError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BlockWindowEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BlockWindowEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BlockWindowTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BlockWindowEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BlockWindowError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockWindowError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = BlockWindowEntryState::Completed;
        ...
        self.emit(BlockWindowEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BlockWindowError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockWindowError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BlockWindowEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BlockWindowEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BlockWindowEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BlockWindowError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockWindowError::NotFound(id))?;
        entry.state = BlockWindowEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BlockWindowEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BlockWindowEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BlockWindowEntryState) -> Vec<BlockWindowEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BlockWindowEntryState::Completed | BlockWindowEntryState::Failed | BlockWindowEntryState::Cancelled | BlockWindowEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BlockWindowEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BlockWindowStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BlockWindowEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BlockWindowEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BlockWindowState::new(BlockWindowConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BlockWindowConfig { retry_count: 2, ..Default::default() };
        let state = BlockWindowState::new(config);
