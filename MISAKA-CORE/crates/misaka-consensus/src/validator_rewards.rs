...
use super::validator_scoring::{ScoreBreakdown, ScoringConfig, ValidatorMetrics, compute_score};
...
pub fn distribute_epoch_rewards(
    ...
    let active_pool = (total_emission as f64 * reward_config.active_pool_share) as u64;
    let backup_pool = (total_emission as f64 * reward_config.backup_pool_share) as u64;
    ...
    for v in validators {
        ...
        let reward_credit = 1.0
            + (breakdown.credit - 1.0) * reward_config.reward_credit_weight;
        let reward_credit = reward_credit.max(0.1);

        let reward_score = breakdown.uptime
    ...
    // ── Merge results ──
    let active_total: u64 = active_rewards.iter().map(|r| r.1).sum();
    let backup_total: u64 = backup_rewards.iter().map(|r| r.1).sum();
    let carry_over = total_emission.saturating_sub(active_total + backup_total);
...
fn distribute_pool(pool: u64, scores: &[([u8; 32], f64)]) -> Vec<([u8; 32], u64)> {
    ...
    for (id, score) in scores {
        let share = ((*score / total_score) * pool as f64).floor() as u64;
...
mod tests {
    ...
    fn test_low_uptime_gets_less_reward() {
        ...
        let r1 = result.rewards.iter().find(|r| r.validator_id[0] == 1).unwrap();
        let r2 = result.rewards.iter().find(|r| r.validator_id[0] == 2).unwrap();

        // Validator 1 (uptime=1.0) should get more than validator 2 (uptime=0.5)
        assert!(r1.reward > r2.reward, "v1={} should > v2={}", r1.reward, r2.reward);
    }
    ...
    fn test_zero_uptime_zero_reward() {
        let validators = vec![
            make_input(1, 10_000_000_000_000_000, 0.0, ValidatorRole::Active),
        ];
    ...
    fn test_backup_receives_20_percent() {
        ...
        for i in 0..21u8 {
            validators.push(make_input(i, 10_000_000_000_000_000, 1.0, ValidatorRole::Active));
        }
        for i in 100..110u8 {
            validators.push(make_input(i, 1_000_000_000_000_000, 0.95, ValidatorRole::Backup));
        }
        ...
        let active_sum: u64 = result.rewards.iter()
            .filter(|r| r.role == ValidatorRole::Active)
            .map(|r| r.reward)
            .sum();
        let backup_sum: u64 = result.rewards.iter()
