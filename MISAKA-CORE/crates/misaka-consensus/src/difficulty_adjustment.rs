//! Difficulty retarget algorithm for DAA
//!
...
/// Configuration for DifficultyAdjustment.
...
/// DifficultyAdjustment operational state.
pub struct DifficultyAdjustmentState {
    config: DifficultyAdjustmentConfig,
    entries: parking_lot::RwLock<HashMap<u64, DifficultyAdjustmentEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<DifficultyAdjustmentTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: DifficultyAdjustmentStats,
    events: parking_lot::Mutex<Vec<DifficultyAdjustmentEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in DifficultyAdjustment.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DifficultyAdjustmentEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: DifficultyAdjustmentEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum DifficultyAdjustmentEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct DifficultyAdjustmentTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from DifficultyAdjustment.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DifficultyAdjustmentEvent {
...
/// Statistics for DifficultyAdjustment.
#[derive(Debug, Default)]
pub struct DifficultyAdjustmentStats {
...
impl DifficultyAdjustmentStats {
    pub fn snapshot(&self) -> DifficultyAdjustmentStatsSnapshot {
        ...
        DifficultyAdjustmentStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DifficultyAdjustmentStatsSnapshot {
...
impl DifficultyAdjustmentState {
    pub fn new(config: DifficultyAdjustmentConfig) -> Self {
        Self {
            config,
            ...
            stats: DifficultyAdjustmentStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, DifficultyAdjustmentError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(DifficultyAdjustmentError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = DifficultyAdjustmentEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: DifficultyAdjustmentEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(DifficultyAdjustmentTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(DifficultyAdjustmentEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), DifficultyAdjustmentError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DifficultyAdjustmentError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = DifficultyAdjustmentEntryState::Completed;
        ...
        self.emit(DifficultyAdjustmentEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, DifficultyAdjustmentError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DifficultyAdjustmentError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = DifficultyAdjustmentEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = DifficultyAdjustmentEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(DifficultyAdjustmentEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), DifficultyAdjustmentError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DifficultyAdjustmentError::NotFound(id))?;
        entry.state = DifficultyAdjustmentEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<DifficultyAdjustmentEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<DifficultyAdjustmentEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: DifficultyAdjustmentEntryState) -> Vec<DifficultyAdjustmentEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, DifficultyAdjustmentEntryState::Completed | DifficultyAdjustmentEntryState::Failed | DifficultyAdjustmentEntryState::Cancelled | DifficultyAdjustmentEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(DifficultyAdjustmentEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> DifficultyAdjustmentStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<DifficultyAdjustmentEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: DifficultyAdjustmentEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = DifficultyAdjustmentState::new(DifficultyAdjustmentConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = DifficultyAdjustmentConfig { retry_count: 2, ..Default::default() };
        let state = DifficultyAdjustmentState::new(config);
