//! Pruning service — automated state cleanup
//!
...
/// Configuration for PruningService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PruningServiceConfig {
    pub enabled: bool,
...
impl Default for PruningServiceConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// PruningService operational state.
pub struct PruningServiceState {
    config: PruningServiceConfig,
    entries: parking_lot::RwLock<HashMap<u64, PruningServiceEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<PruningServiceTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: PruningServiceStats,
    events: parking_lot::Mutex<Vec<PruningServiceEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in PruningService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PruningServiceEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: PruningServiceEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum PruningServiceEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct PruningServiceTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from PruningService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum PruningServiceEvent {
...
/// Statistics for PruningService.
#[derive(Debug, Default)]
pub struct PruningServiceStats {
...
impl PruningServiceStats {
    pub fn snapshot(&self) -> PruningServiceStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PruningServiceStatsSnapshot {
...
impl PruningServiceState {
    pub fn new(config: PruningServiceConfig) -> Self {
        Self {
            config,
            ...
            stats: PruningServiceStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, PruningServiceError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(PruningServiceError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = PruningServiceEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: PruningServiceEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(PruningServiceTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(PruningServiceEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), PruningServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PruningServiceError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = PruningServiceEntryState::Completed;
        ...
        self.emit(PruningServiceEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, PruningServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PruningServiceError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = PruningServiceEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = PruningServiceEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(PruningServiceEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), PruningServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PruningServiceError::NotFound(id))?;
        entry.state = PruningServiceEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<PruningServiceEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<PruningServiceEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: PruningServiceEntryState) -> Vec<PruningServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, PruningServiceEntryState::Completed | PruningServiceEntryState::Failed | PruningServiceEntryState::Cancelled | PruningServiceEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(PruningServiceEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> PruningServiceStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<PruningServiceEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: PruningServiceEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = PruningServiceState::new(PruningServiceConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = PruningServiceConfig { retry_count: 2, ..Default::default() };
        let state = PruningServiceState::new(config);
