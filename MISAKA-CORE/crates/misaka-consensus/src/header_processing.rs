//! Block header processing pipeline
//!
...
/// Configuration for the HeaderProcessing subsystem.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HeaderProcessingConfig {
    pub enabled: bool,
...
impl Default for HeaderProcessingConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 10_000,
...
/// Runtime state for HeaderProcessing.
pub struct HeaderProcessingRuntime {
    config: HeaderProcessingConfig,
    entries: parking_lot::RwLock<HashMap<u64, HeaderProcessingEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: HeaderProcessingStats,
    event_buffer: parking_lot::Mutex<Vec<HeaderProcessingEvent>>,
}

/// Individual entry tracked by HeaderProcessing.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HeaderProcessingEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by HeaderProcessing.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum HeaderProcessingEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for HeaderProcessing.
#[derive(Debug, Default)]
pub struct HeaderProcessingStats {
...
impl HeaderProcessingStats {
    pub fn snapshot(&self) -> HeaderProcessingStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        HeaderProcessingStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HeaderProcessingStatsSnapshot {
...
impl HeaderProcessingRuntime {
    /// Create a new HeaderProcessing runtime.
    pub fn new(config: HeaderProcessingConfig) -> Self {
        Self {
            config,
            entries: parking_lot::RwLock::new(HashMap::new()),
            next_id: std::sync::atomic::AtomicU64::new(1),
            stats: HeaderProcessingStats::default(),
            event_buffer: parking_lot::Mutex::new(Vec::new()),
        }
    }

    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, HeaderProcessingError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(HeaderProcessingError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, HeaderProcessingEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(HeaderProcessingEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), HeaderProcessingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HeaderProcessingError::NotFound(id))?;
        ...
        self.emit_event(HeaderProcessingEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, HeaderProcessingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HeaderProcessingError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(HeaderProcessingEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<HeaderProcessingEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<HeaderProcessingEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(HeaderProcessingEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> HeaderProcessingStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<HeaderProcessingEvent> {
    ...
    fn emit_event(&self, event: HeaderProcessingEvent) {
...
/// Errors from HeaderProcessing operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = HeaderProcessingRuntime::new(HeaderProcessingConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = HeaderProcessingConfig { retry_count: 2, ..Default::default() };
        let rt = HeaderProcessingRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = HeaderProcessingConfig { max_entries: 2, ..Default::default() };
        let rt = HeaderProcessingRuntime::new(config);
