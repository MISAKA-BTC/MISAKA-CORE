...
/// MUST be the same for ALL lattice ZKP proof schemes.
...
/// ```
pub fn canonical_key_image_bound(secret: &Poly, one_time_address: &[u8; 32]) -> [u8; 32] {
    let s_bytes = secret.to_bytes();
    let inner: [u8; 64] = {
        let mut h = Sha3_512::new();
        h.update(&s_bytes);
...
mod tests {
    ...
    fn test_canonical_ki_matches_across_schemes() {
        ...
        // All nullifier derivation paths must use this function,
    ...
    fn test_canonical_ki_differs_from_bound() {
        ...
        let ki_bound = canonical_key_image_bound(&sk, &[0xAA; 32]);
