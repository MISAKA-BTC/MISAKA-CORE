...
//!
//! Lattice ZKP proof descriptors (LRS, LogRing, Chipmunk) have been removed.
...
pub enum PrivacyBackendFamily {
    /// Lattice-based zero-knowledge proofs (UnifiedZKP-v1).
    /// This is the PRIMARY privacy backend for MISAKA.
    ZeroKnowledge,
    /// Transparent transfers (ML-DSA direct signature, no anonymity).
    Transparent,
}
...
pub fn tx_spend_semantics_for_backend(
    ...
    match backend_family {
        ...
        PrivacyBackendFamily::Transparent => TransactionSpendSemantics {
            scheme_tag: misaka_types::utxo::PROOF_SCHEME_TRANSPARENT,
            spend_identifier_model: SpendIdentifierModel::CanonicalNullifier,
            spend_identifier_label: "nullifier".to_string(),
            spend_identifiers: tx.inputs.iter().map(|inp| inp.key_image).collect(),
        },
    }
}
...
pub fn target_spend_semantics_for_backend(
    ...
    match backend_family {
        ...
        PrivacyBackendFamily::Transparent => TargetSpendSemanticsDescriptor {
            target_spend_identifier_model: SpendIdentifierModel::CanonicalNullifier,
            target_spend_identifier_label: "canonicalNullifier",
...
pub fn describe_transaction(_tx: &UtxoTransaction) -> Option<PrivacyBackendDescriptor> {
