...
///
/// SECURITY: all intermediate NTT-domain arrays are zeroized after use.
/// When called with secret polynomial inputs (e.g. during signing),
/// NTT-domain coefficients are secret-derived and must not persist on the stack.
pub fn ntt_mul(a: &Poly, b: &Poly) -> Poly {
    ...
    // Zeroize NTT-domain work arrays (may contain secret-derived data)
    zeroize_poly_coeffs(&mut a_ntt);
    zeroize_poly_coeffs(&mut b_ntt);
    zeroize_poly_coeffs(&mut c_ntt);
    result
}
