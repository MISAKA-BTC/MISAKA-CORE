...
//! # Measured Operations
//!
//! 1. Lattice ZKP proof (LRS): sign + verify for ring sizes 4, 8, 16
...
use misaka_pqc::bdlop::{BalanceExcessProof, BdlopCommitment, BdlopCrs, BlindingFactor};
...
fn zkp_benchmark_suite() {
    ...
    for anonymity_set_size in [4, 8, 16] {
        let wallets: Vec<SpendingKeypair> = (0..anonymity_set_size)
        ...
        bench(&format!("pq_sign (n={})", anonymity_set_size), || {
            let s = pq_sign(&a, &ring, 0, &wallets[0].secret_poly, &msg).unwrap();
            sig_out = Some(s);
        });

        let sig = sig_out.unwrap();
        bench(&format!("ring_verify (n={})", anonymity_set_size), || {
