...
use admission_pipeline::{
    build_privacy_constraints, build_privacy_statement, cheap_size_gate, ResolvedInputAmounts,
};
pub use admission_pipeline::PeerTxAdmissionGate;
use misaka_pqc::pq_ring::{derive_public_param, Poly, DEFAULT_A_SEED};
use misaka_pqc::{
    select_privacy_backend, PrivacyBackendFamily, PrivacyBackendPreference,
...
mod tests {
    ...
    fn test_duplicate_nullifier_rejected() {
        let mut pool = UtxoMempool::new(100);
        let utxo_set = UtxoSet::new(100);
        pool.admit(sample_v4_tx([0xAA; 32]), &utxo_set, 1000)
            .unwrap();
