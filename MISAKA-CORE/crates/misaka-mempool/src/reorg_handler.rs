...
mod tests {
    ...
    fn test_chain_conflicted_orphan_dropped() {
        ...
        let result = handler.evaluate_orphans(&orphans, &chain_nullifiers, &HashSet::new(), 1000, 1);
    ...
    fn test_deep_reorg_drops_all_orphans() {
        let handler = ReorgHandler::new(100);
        let orphans: Vec<OrphanedTx> = (0..50u8)
            .map(|i| orphan(i, &[[i; 32]], true))
            .collect();
    ...
    fn test_tx_budget_caps_evaluation() {
        ...
        let orphans: Vec<OrphanedTx> = (0..count)
            .map(|i| {
                ...
                OrphanedTx {
                    tx_hash: hash,
                    nullifiers: vec![hash], // unique nullifier per TX
                    was_applied: true,
                    original_block: [0xFF; 32],
                }
            })
