...
use misaka_shielded::{
    SharedShieldedState, ShieldedError,
    ShieldDepositTx, ShieldWithdrawTx, ShieldedTransferTx, Nullifier,
};
...
impl ShieldedMempool {
    ...
    pub fn admit(
        ...
        // ── 4. ShieldedState validate ──
        {
            ...
            match &tx {
                ShieldedMempoolTx::Deposit(t) => {
                    state.validate_deposit(t).map_err(ShieldedMempoolError::Shielded)?;
                }
                ShieldedMempoolTx::Transfer(t) => {
                    state.validate_shielded_transfer(t).map_err(ShieldedMempoolError::Shielded)?;
                }
                ShieldedMempoolTx::Withdraw(t) => {
                    state.validate_withdraw(t).map_err(ShieldedMempoolError::Shielded)?;
                }
            }
        }
        ...
        self.entries.insert(
            tx_hash,
            ShieldedMempoolEntry { tx, tx_hash, received_at_ms: now_ms },
        );
    ...
    pub fn remove(&mut self, tx_hash: &[u8; 32]) {
        if self.entries.remove(tx_hash).is_some() {
            ...
            self.shielded
                .write()
                .release_nullifier_reservation(tx_hash);
            tracing::debug!(
                "ShieldedMempool::remove: tx={}",
                hex::encode(tx_hash)
            );
        }
    }
    ...
    pub fn pending_txs(&self) -> Vec<&ShieldedMempoolTx> {
        let mut txs: Vec<_> = self.entries.values()
            .map(|e| &e.tx)
            .collect();
        // fee 降順にソート
...
mod tests {
    use super::*;
    use misaka_shielded::{
        new_shared_state, ShieldedConfig,
        ShieldDepositTx, EncryptedNote, NoteCommitment, MIN_SHIELDED_FEE,
    };

    fn make_shielded_state() -> SharedShieldedState {
        let mut state = misaka_shielded::ShieldedState::new(ShieldedConfig::testnet());
        state.register_stub_backend();
        new_shared_state(ShieldedConfig::disabled()) // disabled for basic test
    }
