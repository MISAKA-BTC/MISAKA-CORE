...
impl ScriptClass {
    ...
    pub fn from_script(script: &[u8]) -> Self {
        ...
        if script.len() > 3
            && script.last() == Some(&OP_CHECKMULTISIG)
        {
            let m = script[0];
        ...
        // PQ Multisig
        if script.len() > 3
            && script.last() == Some(&OP_CHECKMULTISIG_PQ)
        {
            let m = script[0];
            let n = script[script.len() - 2];
            if m >= OP_1 && m <= OP_16 && n >= OP_1 && n <= OP_16 && m <= n {
    ...
    pub fn is_pq(&self) -> bool {
        matches!(self, ScriptClass::PubKeyPQ | ScriptClass::PubKeyHashPQ | ScriptClass::MultiSigPQ)
    }
    ...
    pub fn required_sigs(&self) -> usize {
        match self {
            ...
            ScriptClass::PubKeyPQ | ScriptClass::PubKeyHashPQ => 1,
