...
impl DataStack {
    pub fn new() -> Self {
        Self { items: Vec::with_capacity(64) }
    }
    ...
    pub fn push(&mut self, data: Vec<u8>) -> TxScriptResult<()> {
        if self.items.len() >= MAX_STACK_SIZE {
            return Err(TxScriptError::StackOverflow(self.items.len() + 1, MAX_STACK_SIZE));
        }
    ...
    pub fn pop(&mut self) -> TxScriptResult<Vec<u8>> {
        self.items.pop().ok_or(TxScriptError::StackUnderflow { needed: 1, got: 0 })
    }

    /// Peek at the top element without removing it.
    pub fn peek(&self) -> TxScriptResult<&Vec<u8>> {
        self.items.last().ok_or(TxScriptError::StackUnderflow { needed: 1, got: 0 })
    }

    /// Peek at the n-th element from the top (0 = top).
    pub fn peek_at(&self, n: usize) -> TxScriptResult<&Vec<u8>> {
        if n >= self.items.len() {
            return Err(TxScriptError::StackUnderflow { needed: n + 1, got: self.items.len() });
        }
    ...
    pub fn dup_n(&mut self, n: usize) -> TxScriptResult<()> {
        if n > self.items.len() {
            return Err(TxScriptError::StackUnderflow { needed: n, got: self.items.len() });
        }
    ...
    pub fn rot(&mut self) -> TxScriptResult<()> {
        if self.items.len() < 3 {
            return Err(TxScriptError::StackUnderflow { needed: 3, got: self.items.len() });
        }
    ...
    pub fn swap(&mut self) -> TxScriptResult<()> {
        let len = self.items.len();
        if len < 2 {
            return Err(TxScriptError::StackUnderflow { needed: 2, got: len });
        }
    ...
    pub fn nip(&mut self) -> TxScriptResult<()> {
        if self.items.len() < 2 {
            return Err(TxScriptError::StackUnderflow { needed: 2, got: self.items.len() });
        }
