//! RPC performance metrics collection and reporting
//!
...
/// Configuration for the MetricsCollector subsystem.
...
/// Runtime state for MetricsCollector.
pub struct MetricsCollectorRuntime {
    config: MetricsCollectorConfig,
    entries: parking_lot::RwLock<HashMap<u64, MetricsCollectorEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MetricsCollectorStats,
    event_buffer: parking_lot::Mutex<Vec<MetricsCollectorEvent>>,
}

/// Individual entry tracked by MetricsCollector.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MetricsCollectorEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Events emitted by MetricsCollector.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum MetricsCollectorEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for MetricsCollector.
...
impl MetricsCollectorStats {
    pub fn snapshot(&self) -> MetricsCollectorStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        MetricsCollectorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MetricsCollectorStatsSnapshot {
...
impl MetricsCollectorRuntime {
    /// Create a new MetricsCollector runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, MetricsCollectorError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(MetricsCollectorError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, MetricsCollectorEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(MetricsCollectorEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), MetricsCollectorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MetricsCollectorError::NotFound(id))?;
        ...
        self.emit_event(MetricsCollectorEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, MetricsCollectorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MetricsCollectorError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(MetricsCollectorEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<MetricsCollectorEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<MetricsCollectorEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(MetricsCollectorEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> MetricsCollectorStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<MetricsCollectorEvent> {
    ...
    fn emit_event(&self, event: MetricsCollectorEvent) {
...
/// Errors from MetricsCollector operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = MetricsCollectorRuntime::new(MetricsCollectorConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = MetricsCollectorConfig { retry_count: 2, ..Default::default() };
        let rt = MetricsCollectorRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = MetricsCollectorConfig { max_entries: 2, ..Default::default() };
        let rt = MetricsCollectorRuntime::new(config);
