//! RPC proxy — forwards requests between protocols
//!
...
/// Runtime state for Proxy.
pub struct ProxyRuntime {
    config: ProxyConfig,
    entries: parking_lot::RwLock<HashMap<u64, ProxyEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ProxyStats,
    event_buffer: parking_lot::Mutex<Vec<ProxyEvent>>,
}

/// Individual entry tracked by Proxy.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for Proxy.
...
impl ProxyRuntime {
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, ProxyError> {
        ...
        self.entries.write().insert(id, ProxyEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(ProxyEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), ProxyError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ProxyError::NotFound(id))?;
        ...
        self.emit_event(ProxyEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, ProxyError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ProxyError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(ProxyEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<ProxyEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<ProxyEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(ProxyEvent::CleanupCompleted { removed });
        }
        removed
    }
    ...
    pub fn drain_events(&self) -> Vec<ProxyEvent> {
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = ProxyRuntime::new(ProxyConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = ProxyConfig { retry_count: 2, ..Default::default() };
        let rt = ProxyRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = ProxyConfig { max_entries: 2, ..Default::default() };
        let rt = ProxyRuntime::new(config);
