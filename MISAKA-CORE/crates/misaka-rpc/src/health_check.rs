//! RPC server health monitoring and readiness probes
//!
...
/// HealthCheck operational state.
pub struct HealthCheckState {
    config: HealthCheckConfig,
    entries: parking_lot::RwLock<HashMap<u64, HealthCheckEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<HealthCheckTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: HealthCheckStats,
    events: parking_lot::Mutex<Vec<HealthCheckEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in HealthCheck.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HealthCheckEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: HealthCheckEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum HealthCheckEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
impl HealthCheckStats {
    pub fn snapshot(&self) -> HealthCheckStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HealthCheckStatsSnapshot {
...
impl HealthCheckState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, HealthCheckError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(HealthCheckError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = HealthCheckEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: HealthCheckEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(HealthCheckTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(HealthCheckEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), HealthCheckError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HealthCheckError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = HealthCheckEntryState::Completed;
        ...
        self.emit(HealthCheckEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, HealthCheckError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HealthCheckError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = HealthCheckEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = HealthCheckEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(HealthCheckEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), HealthCheckError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(HealthCheckError::NotFound(id))?;
        entry.state = HealthCheckEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<HealthCheckEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<HealthCheckEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: HealthCheckEntryState) -> Vec<HealthCheckEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, HealthCheckEntryState::Completed | HealthCheckEntryState::Failed | HealthCheckEntryState::Cancelled | HealthCheckEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(HealthCheckEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> HealthCheckStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<HealthCheckEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: HealthCheckEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = HealthCheckState::new(HealthCheckConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = HealthCheckConfig { retry_count: 2, ..Default::default() };
