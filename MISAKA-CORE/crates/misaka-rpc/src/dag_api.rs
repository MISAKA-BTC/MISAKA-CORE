//! DAG state and virtual chain API
//!
...
/// Configuration for DagApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DagApiConfig {
    pub enabled: bool,
...
/// DagApi operational state.
pub struct DagApiState {
    config: DagApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, DagApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<DagApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: DagApiStats,
    events: parking_lot::Mutex<Vec<DagApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in DagApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DagApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: DagApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Statistics for DagApi.
#[derive(Debug, Default)]
pub struct DagApiStats {
...
impl DagApiStats {
    pub fn snapshot(&self) -> DagApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DagApiStatsSnapshot {
...
impl DagApiState {
    pub fn new(config: DagApiConfig) -> Self {
        Self {
            config,
            ...
            stats: DagApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, DagApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(DagApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = DagApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: DagApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(DagApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(DagApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), DagApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DagApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = DagApiEntryState::Completed;
        ...
        self.emit(DagApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, DagApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DagApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = DagApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = DagApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(DagApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), DagApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(DagApiError::NotFound(id))?;
        entry.state = DagApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<DagApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<DagApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: DagApiEntryState) -> Vec<DagApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, DagApiEntryState::Completed | DagApiEntryState::Failed | DagApiEntryState::Cancelled | DagApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(DagApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> DagApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<DagApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: DagApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = DagApiState::new(DagApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = DagApiConfig { retry_count: 2, ..Default::default() };
        let state = DagApiState::new(config);
