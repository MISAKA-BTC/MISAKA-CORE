//! Network peer and connection API implementation
//!
...
/// NetworkApi operational state.
pub struct NetworkApiState {
    config: NetworkApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, NetworkApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<NetworkApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: NetworkApiStats,
    events: parking_lot::Mutex<Vec<NetworkApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in NetworkApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: NetworkApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct NetworkApiTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from NetworkApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Statistics for NetworkApi.
#[derive(Debug, Default)]
pub struct NetworkApiStats {
...
impl NetworkApiStats {
    pub fn snapshot(&self) -> NetworkApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkApiStatsSnapshot {
...
impl NetworkApiState {
    pub fn new(config: NetworkApiConfig) -> Self {
        Self {
            ...
            stats: NetworkApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, NetworkApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(NetworkApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = NetworkApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: NetworkApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(NetworkApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(NetworkApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), NetworkApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NetworkApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = NetworkApiEntryState::Completed;
        ...
        self.emit(NetworkApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, NetworkApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NetworkApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = NetworkApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = NetworkApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(NetworkApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), NetworkApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NetworkApiError::NotFound(id))?;
        entry.state = NetworkApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<NetworkApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<NetworkApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: NetworkApiEntryState) -> Vec<NetworkApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, NetworkApiEntryState::Completed | NetworkApiEntryState::Failed | NetworkApiEntryState::Cancelled | NetworkApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(NetworkApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> NetworkApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<NetworkApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: NetworkApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = NetworkApiState::new(NetworkApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = NetworkApiConfig { retry_count: 2, ..Default::default() };
        let state = NetworkApiState::new(config);
