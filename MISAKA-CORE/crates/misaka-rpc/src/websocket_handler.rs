...
/// WebsocketHandler operational state.
pub struct WebsocketHandlerState {
    config: WebsocketHandlerConfig,
    entries: parking_lot::RwLock<HashMap<u64, WebsocketHandlerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<WebsocketHandlerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: WebsocketHandlerStats,
    events: parking_lot::Mutex<Vec<WebsocketHandlerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in WebsocketHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WebsocketHandlerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: WebsocketHandlerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum WebsocketHandlerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct WebsocketHandlerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from WebsocketHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum WebsocketHandlerEvent {
...
/// Statistics for WebsocketHandler.
...
impl WebsocketHandlerStats {
    pub fn snapshot(&self) -> WebsocketHandlerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WebsocketHandlerStatsSnapshot {
...
impl WebsocketHandlerState {
    pub fn new(config: WebsocketHandlerConfig) -> Self {
        Self {
            ...
            stats: WebsocketHandlerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, WebsocketHandlerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(WebsocketHandlerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = WebsocketHandlerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: WebsocketHandlerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(WebsocketHandlerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(WebsocketHandlerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), WebsocketHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WebsocketHandlerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = WebsocketHandlerEntryState::Completed;
        ...
        self.emit(WebsocketHandlerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, WebsocketHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WebsocketHandlerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = WebsocketHandlerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = WebsocketHandlerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(WebsocketHandlerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), WebsocketHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(WebsocketHandlerError::NotFound(id))?;
        entry.state = WebsocketHandlerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<WebsocketHandlerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<WebsocketHandlerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: WebsocketHandlerEntryState) -> Vec<WebsocketHandlerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, WebsocketHandlerEntryState::Completed | WebsocketHandlerEntryState::Failed | WebsocketHandlerEntryState::Cancelled | WebsocketHandlerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(WebsocketHandlerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> WebsocketHandlerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<WebsocketHandlerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: WebsocketHandlerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = WebsocketHandlerState::new(WebsocketHandlerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = WebsocketHandlerConfig { retry_count: 2, ..Default::default() };
        let state = WebsocketHandlerState::new(config);
