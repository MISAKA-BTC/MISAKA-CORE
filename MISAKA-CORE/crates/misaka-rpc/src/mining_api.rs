//! Mining and block template API implementation
//!
...
/// Configuration for MiningApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MiningApiConfig {
    pub enabled: bool,
...
impl Default for MiningApiConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// MiningApi operational state.
pub struct MiningApiState {
    config: MiningApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, MiningApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<MiningApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MiningApiStats,
    events: parking_lot::Mutex<Vec<MiningApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in MiningApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MiningApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: MiningApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct MiningApiTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from MiningApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum MiningApiEvent {
...
/// Statistics for MiningApi.
#[derive(Debug, Default)]
pub struct MiningApiStats {
...
impl MiningApiStats {
    pub fn snapshot(&self) -> MiningApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MiningApiStatsSnapshot {
...
impl MiningApiState {
    pub fn new(config: MiningApiConfig) -> Self {
        Self {
            config,
            ...
            stats: MiningApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, MiningApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(MiningApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = MiningApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: MiningApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(MiningApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(MiningApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), MiningApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MiningApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = MiningApiEntryState::Completed;
        ...
        self.emit(MiningApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, MiningApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MiningApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = MiningApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = MiningApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(MiningApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), MiningApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MiningApiError::NotFound(id))?;
        entry.state = MiningApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<MiningApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<MiningApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: MiningApiEntryState) -> Vec<MiningApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, MiningApiEntryState::Completed | MiningApiEntryState::Failed | MiningApiEntryState::Cancelled | MiningApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(MiningApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> MiningApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<MiningApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: MiningApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = MiningApiState::new(MiningApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = MiningApiConfig { retry_count: 2, ..Default::default() };
        let state = MiningApiState::new(config);
