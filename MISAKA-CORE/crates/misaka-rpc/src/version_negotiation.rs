//! Protocol version negotiation for RPC clients
//!
...
/// Configuration for VersionNegotiation.
...
/// VersionNegotiation operational state.
pub struct VersionNegotiationState {
    config: VersionNegotiationConfig,
    entries: parking_lot::RwLock<HashMap<u64, VersionNegotiationEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<VersionNegotiationTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: VersionNegotiationStats,
    events: parking_lot::Mutex<Vec<VersionNegotiationEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in VersionNegotiation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VersionNegotiationEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: VersionNegotiationEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum VersionNegotiationEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct VersionNegotiationTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from VersionNegotiation.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Statistics for VersionNegotiation.
#[derive(Debug, Default)]
pub struct VersionNegotiationStats {
...
impl VersionNegotiationStats {
    pub fn snapshot(&self) -> VersionNegotiationStatsSnapshot {
        ...
        VersionNegotiationStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VersionNegotiationStatsSnapshot {
...
impl VersionNegotiationState {
    pub fn new(config: VersionNegotiationConfig) -> Self {
        Self {
            ...
            stats: VersionNegotiationStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, VersionNegotiationError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(VersionNegotiationError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = VersionNegotiationEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: VersionNegotiationEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(VersionNegotiationTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(VersionNegotiationEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), VersionNegotiationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(VersionNegotiationError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = VersionNegotiationEntryState::Completed;
        ...
        self.emit(VersionNegotiationEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, VersionNegotiationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(VersionNegotiationError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = VersionNegotiationEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = VersionNegotiationEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(VersionNegotiationEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), VersionNegotiationError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(VersionNegotiationError::NotFound(id))?;
        entry.state = VersionNegotiationEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<VersionNegotiationEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<VersionNegotiationEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: VersionNegotiationEntryState) -> Vec<VersionNegotiationEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, VersionNegotiationEntryState::Completed | VersionNegotiationEntryState::Failed | VersionNegotiationEntryState::Cancelled | VersionNegotiationEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(VersionNegotiationEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> VersionNegotiationStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<VersionNegotiationEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: VersionNegotiationEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = VersionNegotiationState::new(VersionNegotiationConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = VersionNegotiationConfig { retry_count: 2, ..Default::default() };
        let state = VersionNegotiationState::new(config);
