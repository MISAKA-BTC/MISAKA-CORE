...
/// Configuration for BatchHandler.
...
impl Default for BatchHandlerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// BatchHandler operational state.
pub struct BatchHandlerState {
    config: BatchHandlerConfig,
    entries: parking_lot::RwLock<HashMap<u64, BatchHandlerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BatchHandlerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BatchHandlerStats,
    events: parking_lot::Mutex<Vec<BatchHandlerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BatchHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BatchHandlerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BatchHandlerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BatchHandlerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BatchHandlerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BatchHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BatchHandlerEvent {
...
/// Statistics for BatchHandler.
#[derive(Debug, Default)]
pub struct BatchHandlerStats {
...
impl BatchHandlerStats {
    pub fn snapshot(&self) -> BatchHandlerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BatchHandlerStatsSnapshot {
...
impl BatchHandlerState {
    pub fn new(config: BatchHandlerConfig) -> Self {
        Self {
            config,
            ...
            stats: BatchHandlerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BatchHandlerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BatchHandlerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BatchHandlerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BatchHandlerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BatchHandlerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BatchHandlerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BatchHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BatchHandlerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = BatchHandlerEntryState::Completed;
        ...
        self.emit(BatchHandlerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BatchHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BatchHandlerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BatchHandlerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BatchHandlerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BatchHandlerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BatchHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BatchHandlerError::NotFound(id))?;
        entry.state = BatchHandlerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BatchHandlerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BatchHandlerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BatchHandlerEntryState) -> Vec<BatchHandlerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BatchHandlerEntryState::Completed | BatchHandlerEntryState::Failed | BatchHandlerEntryState::Cancelled | BatchHandlerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BatchHandlerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BatchHandlerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BatchHandlerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BatchHandlerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BatchHandlerState::new(BatchHandlerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BatchHandlerConfig { retry_count: 2, ..Default::default() };
        let state = BatchHandlerState::new(config);
