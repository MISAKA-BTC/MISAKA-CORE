//! Message encoding and serialization utilities
//!
...
/// Encoding operational state.
pub struct EncodingState {
    config: EncodingConfig,
    entries: parking_lot::RwLock<HashMap<u64, EncodingEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<EncodingTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: EncodingStats,
    events: parking_lot::Mutex<Vec<EncodingEvent>>,
...
impl EncodingStats {
    pub fn snapshot(&self) -> EncodingStatsSnapshot {
...
impl EncodingState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, EncodingError> {
        ...
        let entry = EncodingEntry {
            ...
            state: EncodingEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(EncodingTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(EncodingEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), EncodingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EncodingError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = EncodingEntryState::Completed;
        ...
        self.emit(EncodingEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, EncodingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EncodingError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = EncodingEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = EncodingEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(EncodingEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), EncodingError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(EncodingError::NotFound(id))?;
        entry.state = EncodingEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<EncodingEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<EncodingEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: EncodingEntryState) -> Vec<EncodingEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, EncodingEntryState::Completed | EncodingEntryState::Failed | EncodingEntryState::Cancelled | EncodingEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(EncodingEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> EncodingStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<EncodingEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: EncodingEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = EncodingState::new(EncodingConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = EncodingConfig { retry_count: 2, ..Default::default() };
        let state = EncodingState::new(config);
