//! Relay notifications from consensus to RPC subscribers
//!
...
/// Configuration for NotificationRelay.
...
/// NotificationRelay operational state.
pub struct NotificationRelayState {
    config: NotificationRelayConfig,
    entries: parking_lot::RwLock<HashMap<u64, NotificationRelayEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<NotificationRelayTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: NotificationRelayStats,
    events: parking_lot::Mutex<Vec<NotificationRelayEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in NotificationRelay.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NotificationRelayEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: NotificationRelayEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum NotificationRelayEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct NotificationRelayTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from NotificationRelay.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum NotificationRelayEvent {
...
/// Statistics for NotificationRelay.
#[derive(Debug, Default)]
pub struct NotificationRelayStats {
...
impl NotificationRelayStats {
    pub fn snapshot(&self) -> NotificationRelayStatsSnapshot {
        ...
        NotificationRelayStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NotificationRelayStatsSnapshot {
...
impl NotificationRelayState {
    pub fn new(config: NotificationRelayConfig) -> Self {
        Self {
            config,
            ...
            stats: NotificationRelayStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, NotificationRelayError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(NotificationRelayError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = NotificationRelayEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: NotificationRelayEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(NotificationRelayTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(NotificationRelayEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), NotificationRelayError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NotificationRelayError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = NotificationRelayEntryState::Completed;
        ...
        self.emit(NotificationRelayEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, NotificationRelayError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NotificationRelayError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = NotificationRelayEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = NotificationRelayEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(NotificationRelayEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), NotificationRelayError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(NotificationRelayError::NotFound(id))?;
        entry.state = NotificationRelayEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<NotificationRelayEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<NotificationRelayEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: NotificationRelayEntryState) -> Vec<NotificationRelayEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, NotificationRelayEntryState::Completed | NotificationRelayEntryState::Failed | NotificationRelayEntryState::Cancelled | NotificationRelayEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(NotificationRelayEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> NotificationRelayStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<NotificationRelayEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: NotificationRelayEvent) {
...
pub enum NotificationRelayError {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = NotificationRelayState::new(NotificationRelayConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = NotificationRelayConfig { retry_count: 2, ..Default::default() };
        let state = NotificationRelayState::new(config);
