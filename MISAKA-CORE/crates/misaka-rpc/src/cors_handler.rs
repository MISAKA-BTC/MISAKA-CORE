...
/// CorsHandler operational state.
pub struct CorsHandlerState {
    config: CorsHandlerConfig,
    entries: parking_lot::RwLock<HashMap<u64, CorsHandlerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<CorsHandlerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CorsHandlerStats,
    events: parking_lot::Mutex<Vec<CorsHandlerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in CorsHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CorsHandlerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: CorsHandlerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum CorsHandlerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct CorsHandlerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from CorsHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum CorsHandlerEvent {
...
/// Statistics for CorsHandler.
#[derive(Debug, Default)]
pub struct CorsHandlerStats {
...
impl CorsHandlerStats {
    pub fn snapshot(&self) -> CorsHandlerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CorsHandlerStatsSnapshot {
...
impl CorsHandlerState {
    pub fn new(config: CorsHandlerConfig) -> Self {
        Self {
            config,
            ...
            stats: CorsHandlerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, CorsHandlerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(CorsHandlerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = CorsHandlerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: CorsHandlerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(CorsHandlerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(CorsHandlerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), CorsHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CorsHandlerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = CorsHandlerEntryState::Completed;
        ...
        self.emit(CorsHandlerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, CorsHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CorsHandlerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = CorsHandlerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = CorsHandlerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(CorsHandlerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), CorsHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CorsHandlerError::NotFound(id))?;
        entry.state = CorsHandlerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<CorsHandlerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<CorsHandlerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: CorsHandlerEntryState) -> Vec<CorsHandlerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, CorsHandlerEntryState::Completed | CorsHandlerEntryState::Failed | CorsHandlerEntryState::Cancelled | CorsHandlerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(CorsHandlerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> CorsHandlerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<CorsHandlerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: CorsHandlerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = CorsHandlerState::new(CorsHandlerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = CorsHandlerConfig { retry_count: 2, ..Default::default() };
        let state = CorsHandlerState::new(config);
