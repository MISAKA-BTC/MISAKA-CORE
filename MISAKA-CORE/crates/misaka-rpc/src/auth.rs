...
fn is_admin_method(method: &str) -> bool {
    matches!(method, "shutdown" | "addPeer" | "banPeer" | "unbanPeer" | "resolveFinalityConflict")
}

fn is_read_method(method: &str) -> bool {
    matches!(method,
        "ping" | "getSystemInfo" | "getBlock" | "getBlocks" | "getBlockCount" |
        "getBlockDagInfo" | "getHeaders" | "getMempoolEntries" | "getMempoolEntry" |
        "getUtxosByAddresses" | "getBalanceByAddress" | "getBalancesByAddresses" |
        "getVirtualChainFromBlock" | "getSinkBlueScore" | "getVirtualDaaScore" |
        "getBlockTemplate" | "estimateFeeRate" | "getPruningPoint" | "getPeerAddresses" |
        "getConnections" | "getMetrics" | "getCoinbaseAddress"
    )
}
...
impl TokenManager {
    ...
    pub fn active_token_count(&self) -> usize { self.tokens.read().len() }
}
...
impl IpAcl {
    ...
    pub fn allow(&self, ip: &IpAddr) -> bool {
        if self.blacklist.read().contains(ip) { return false; }
    ...
    pub fn add_whitelist(&self, ip: IpAddr) { self.whitelist.write().insert(ip); }
    pub fn add_blacklist(&self, ip: IpAddr) { self.blacklist.write().insert(ip); }
    pub fn remove_whitelist(&self, ip: &IpAddr) { self.whitelist.write().remove(ip); }
...
impl MethodRateLimiter {
    pub fn new() -> Self {
        ...
        limits.insert("submitTransaction".to_string(), (100, Duration::from_secs(60)));
        limits.insert("submitBlock".to_string(), (10, Duration::from_secs(60)));
        limits.insert("getBlockTemplate".to_string(), (60, Duration::from_secs(60)));
        limits.insert("getUtxosByAddresses".to_string(), (30, Duration::from_secs(60)));
    ...
    pub fn check(&self, method: &str, client_id: &str) -> bool {
        let (max_count, window) = self.limits.get(method)
...
impl InputValidator {
    ...
    pub fn validate_hash(s: &str) -> Result<[u8; 32], String> {
        ...
        if bytes.len() != 32 { return Err(format!("hash must be 32 bytes, got {}", bytes.len())); }
        let mut hash = [0u8; 32];
        hash.copy_from_slice(&bytes);
        Ok(hash)
    }
    ...
    pub fn validate_tx_submission(tx_json: &serde_json::Value) -> Result<(), Vec<String>> {
        ...
        if tx_json.get("inputs").and_then(|v| v.as_array()).map_or(true, |a| a.is_empty()) {
            errors.push("transaction must have at least one input".to_string());
        }
        if tx_json.get("outputs").and_then(|v| v.as_array()).map_or(true, |a| a.is_empty()) {
    ...
    pub fn validate_pagination(limit: u32, max: u32) -> Result<u32, String> {
        ...
        if limit > max { return Err(format!("limit {} exceeds maximum {}", limit, max)); }
        Ok(limit)
    }
}
