//! RPC middleware chain — auth, logging, metrics, compression
//!
...
/// Configuration for the Middleware subsystem.
...
/// Runtime state for Middleware.
pub struct MiddlewareRuntime {
    config: MiddlewareConfig,
    entries: parking_lot::RwLock<HashMap<u64, MiddlewareEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MiddlewareStats,
    event_buffer: parking_lot::Mutex<Vec<MiddlewareEvent>>,
}

/// Individual entry tracked by Middleware.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for Middleware.
...
impl MiddlewareRuntime {
    /// Create a new Middleware runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, MiddlewareError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(MiddlewareError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, MiddlewareEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(MiddlewareEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), MiddlewareError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MiddlewareError::NotFound(id))?;
        ...
        self.emit_event(MiddlewareEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, MiddlewareError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MiddlewareError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(MiddlewareEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<MiddlewareEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<MiddlewareEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(MiddlewareEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> MiddlewareStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<MiddlewareEvent> {
    ...
    fn emit_event(&self, event: MiddlewareEvent) {
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = MiddlewareRuntime::new(MiddlewareConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = MiddlewareConfig { retry_count: 2, ..Default::default() };
        let rt = MiddlewareRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = MiddlewareConfig { max_entries: 2, ..Default::default() };
        let rt = MiddlewareRuntime::new(config);
