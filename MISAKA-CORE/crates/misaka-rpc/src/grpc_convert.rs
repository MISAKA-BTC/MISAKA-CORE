//! gRPC to internal type conversion layer
//!
...
/// Configuration for GrpcConvert.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GrpcConvertConfig {
    pub enabled: bool,
...
impl Default for GrpcConvertConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// GrpcConvert operational state.
pub struct GrpcConvertState {
    config: GrpcConvertConfig,
    entries: parking_lot::RwLock<HashMap<u64, GrpcConvertEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<GrpcConvertTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: GrpcConvertStats,
    events: parking_lot::Mutex<Vec<GrpcConvertEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in GrpcConvert.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GrpcConvertEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: GrpcConvertEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum GrpcConvertEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct GrpcConvertTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from GrpcConvert.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum GrpcConvertEvent {
...
/// Statistics for GrpcConvert.
#[derive(Debug, Default)]
pub struct GrpcConvertStats {
...
impl GrpcConvertStats {
    pub fn snapshot(&self) -> GrpcConvertStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GrpcConvertStatsSnapshot {
...
impl GrpcConvertState {
    pub fn new(config: GrpcConvertConfig) -> Self {
        Self {
            config,
            ...
            stats: GrpcConvertStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, GrpcConvertError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(GrpcConvertError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = GrpcConvertEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: GrpcConvertEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(GrpcConvertTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(GrpcConvertEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), GrpcConvertError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(GrpcConvertError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = GrpcConvertEntryState::Completed;
        ...
        self.emit(GrpcConvertEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, GrpcConvertError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(GrpcConvertError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = GrpcConvertEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = GrpcConvertEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(GrpcConvertEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), GrpcConvertError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(GrpcConvertError::NotFound(id))?;
        entry.state = GrpcConvertEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<GrpcConvertEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<GrpcConvertEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: GrpcConvertEntryState) -> Vec<GrpcConvertEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, GrpcConvertEntryState::Completed | GrpcConvertEntryState::Failed | GrpcConvertEntryState::Cancelled | GrpcConvertEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(GrpcConvertEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> GrpcConvertStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<GrpcConvertEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: GrpcConvertEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = GrpcConvertState::new(GrpcConvertConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = GrpcConvertConfig { retry_count: 2, ..Default::default() };
        let state = GrpcConvertState::new(config);
