//! RPC connection management — WebSocket and HTTP
//!
...
/// Configuration for the ConnectionManager subsystem.
...
/// Runtime state for ConnectionManager.
pub struct ConnectionManagerRuntime {
    config: ConnectionManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, ConnectionManagerEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ConnectionManagerStats,
    event_buffer: parking_lot::Mutex<Vec<ConnectionManagerEvent>>,
}

/// Individual entry tracked by ConnectionManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ConnectionManagerEntry {
    pub id: u64,
    pub created_at: u64,
    pub updated_at: u64,
...
/// Operational statistics for ConnectionManager.
...
impl ConnectionManagerStats {
    pub fn snapshot(&self) -> ConnectionManagerStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        ConnectionManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ConnectionManagerStatsSnapshot {
...
impl ConnectionManagerRuntime {
    /// Create a new ConnectionManager runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, ConnectionManagerError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(ConnectionManagerError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, ConnectionManagerEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(ConnectionManagerEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), ConnectionManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ConnectionManagerError::NotFound(id))?;
        ...
        self.emit_event(ConnectionManagerEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, ConnectionManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ConnectionManagerError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(ConnectionManagerEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<ConnectionManagerEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<ConnectionManagerEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(ConnectionManagerEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> ConnectionManagerStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<ConnectionManagerEvent> {
    ...
    fn emit_event(&self, event: ConnectionManagerEvent) {
...
/// Errors from ConnectionManager operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = ConnectionManagerRuntime::new(ConnectionManagerConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = ConnectionManagerConfig { retry_count: 2, ..Default::default() };
        let rt = ConnectionManagerRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = ConnectionManagerConfig { max_entries: 2, ..Default::default() };
        let rt = ConnectionManagerRuntime::new(config);
