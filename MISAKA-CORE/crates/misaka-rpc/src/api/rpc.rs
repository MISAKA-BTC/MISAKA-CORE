...
pub trait RpcApi: Send + Sync {
    ...
    async fn get_blocks(&self, low_hash: Option<String>, include_txs: bool, max_blocks: u32) -> RpcResult<GetBlocksResponse>;
    ...
    // ─── Transaction queries ──────────
    async fn get_mempool_entries(&self, include_orphans: bool, filter_tx_pool: bool) -> RpcResult<GetMempoolEntriesResponse>;
    async fn get_mempool_entry(&self, tx_id: String) -> RpcResult<GetMempoolEntryResponse>;
    async fn submit_transaction(&self, tx: RpcTransaction, allow_orphan: bool) -> RpcResult<SubmitTransactionResponse>;

    // ─── UTXO queries ─────────────────
    async fn get_utxos_by_addresses(&self, addresses: Vec<String>) -> RpcResult<GetUtxosByAddressesResponse>;
    async fn get_balance_by_address(&self, address: String) -> RpcResult<GetBalanceByAddressResponse>;
    async fn get_balances_by_addresses(&self, addresses: Vec<String>) -> RpcResult<GetBalancesByAddressesResponse>;

    // ─── DAG state ────────────────────
    async fn get_virtual_chain_from_block(&self, start_hash: String, include_accepted: bool) -> RpcResult<GetVirtualChainResponse>;
    async fn get_sink_blue_score(&self) -> RpcResult<u64>;
    async fn get_virtual_daa_score(&self) -> RpcResult<u64>;

    // ─── Mining ───────────────────────
    async fn get_block_template(&self, pay_address: String, extra_data: String) -> RpcResult<GetBlockTemplateResponse>;
