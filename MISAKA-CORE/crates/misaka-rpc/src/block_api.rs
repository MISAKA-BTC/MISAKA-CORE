//! Block query API implementation
//!
...
/// Configuration for BlockApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockApiConfig {
    pub enabled: bool,
...
impl Default for BlockApiConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// BlockApi operational state.
pub struct BlockApiState {
    config: BlockApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, BlockApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BlockApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BlockApiStats,
    events: parking_lot::Mutex<Vec<BlockApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BlockApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BlockApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Events from BlockApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BlockApiEvent {
...
/// Statistics for BlockApi.
#[derive(Debug, Default)]
pub struct BlockApiStats {
...
impl BlockApiStats {
    pub fn snapshot(&self) -> BlockApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockApiStatsSnapshot {
...
impl BlockApiState {
    pub fn new(config: BlockApiConfig) -> Self {
        Self {
            config,
            ...
            stats: BlockApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BlockApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BlockApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BlockApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BlockApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BlockApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BlockApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BlockApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = BlockApiEntryState::Completed;
        ...
        self.emit(BlockApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BlockApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BlockApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BlockApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BlockApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BlockApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockApiError::NotFound(id))?;
        entry.state = BlockApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BlockApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BlockApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BlockApiEntryState) -> Vec<BlockApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BlockApiEntryState::Completed | BlockApiEntryState::Failed | BlockApiEntryState::Cancelled | BlockApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BlockApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BlockApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BlockApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BlockApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BlockApiState::new(BlockApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BlockApiConfig { retry_count: 2, ..Default::default() };
        let state = BlockApiState::new(config);
