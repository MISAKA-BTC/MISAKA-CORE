//! RPC request load balancing across worker threads
//!
...
/// LoadBalancer operational state.
pub struct LoadBalancerState {
    config: LoadBalancerConfig,
    entries: parking_lot::RwLock<HashMap<u64, LoadBalancerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<LoadBalancerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: LoadBalancerStats,
    events: parking_lot::Mutex<Vec<LoadBalancerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in LoadBalancer.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LoadBalancerEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: LoadBalancerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum LoadBalancerEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
impl LoadBalancerStats {
    pub fn snapshot(&self) -> LoadBalancerStatsSnapshot {
...
impl LoadBalancerState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, LoadBalancerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(LoadBalancerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = LoadBalancerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: LoadBalancerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(LoadBalancerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(LoadBalancerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), LoadBalancerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(LoadBalancerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = LoadBalancerEntryState::Completed;
        ...
        self.emit(LoadBalancerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, LoadBalancerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(LoadBalancerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = LoadBalancerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = LoadBalancerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(LoadBalancerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), LoadBalancerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(LoadBalancerError::NotFound(id))?;
        entry.state = LoadBalancerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<LoadBalancerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<LoadBalancerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: LoadBalancerEntryState) -> Vec<LoadBalancerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, LoadBalancerEntryState::Completed | LoadBalancerEntryState::Failed | LoadBalancerEntryState::Cancelled | LoadBalancerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(LoadBalancerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> LoadBalancerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<LoadBalancerEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = LoadBalancerState::new(LoadBalancerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = LoadBalancerConfig { retry_count: 2, ..Default::default() };
        let state = LoadBalancerState::new(config);
