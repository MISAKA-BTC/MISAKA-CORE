//! Peer management API implementation
//!
...
/// PeerApi operational state.
pub struct PeerApiState {
    config: PeerApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, PeerApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<PeerApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: PeerApiStats,
    events: parking_lot::Mutex<Vec<PeerApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in PeerApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: PeerApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Statistics for PeerApi.
#[derive(Debug, Default)]
pub struct PeerApiStats {
...
impl PeerApiStats {
    pub fn snapshot(&self) -> PeerApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerApiStatsSnapshot {
...
impl PeerApiState {
    pub fn new(config: PeerApiConfig) -> Self {
        Self {
            config,
            ...
            stats: PeerApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, PeerApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(PeerApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = PeerApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: PeerApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(PeerApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(PeerApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), PeerApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PeerApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = PeerApiEntryState::Completed;
        ...
        self.emit(PeerApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, PeerApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PeerApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = PeerApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = PeerApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(PeerApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), PeerApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PeerApiError::NotFound(id))?;
        entry.state = PeerApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<PeerApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<PeerApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: PeerApiEntryState) -> Vec<PeerApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, PeerApiEntryState::Completed | PeerApiEntryState::Failed | PeerApiEntryState::Cancelled | PeerApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(PeerApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> PeerApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<PeerApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: PeerApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = PeerApiState::new(PeerApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = PeerApiConfig { retry_count: 2, ..Default::default() };
        let state = PeerApiState::new(config);
