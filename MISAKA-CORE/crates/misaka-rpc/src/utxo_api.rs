//! UTXO query and balance API implementation
//!
...
/// Configuration for UtxoApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoApiConfig {
    pub enabled: bool,
...
impl Default for UtxoApiConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// UtxoApi operational state.
pub struct UtxoApiState {
    config: UtxoApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, UtxoApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<UtxoApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: UtxoApiStats,
    events: parking_lot::Mutex<Vec<UtxoApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in UtxoApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: UtxoApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Statistics for UtxoApi.
#[derive(Debug, Default)]
pub struct UtxoApiStats {
...
impl UtxoApiStats {
    pub fn snapshot(&self) -> UtxoApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UtxoApiStatsSnapshot {
...
impl UtxoApiState {
    pub fn new(config: UtxoApiConfig) -> Self {
        Self {
            config,
            ...
            stats: UtxoApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, UtxoApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(UtxoApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = UtxoApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: UtxoApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(UtxoApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(UtxoApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), UtxoApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = UtxoApiEntryState::Completed;
        ...
        self.emit(UtxoApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, UtxoApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = UtxoApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = UtxoApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(UtxoApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), UtxoApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(UtxoApiError::NotFound(id))?;
        entry.state = UtxoApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<UtxoApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<UtxoApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: UtxoApiEntryState) -> Vec<UtxoApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, UtxoApiEntryState::Completed | UtxoApiEntryState::Failed | UtxoApiEntryState::Cancelled | UtxoApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(UtxoApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> UtxoApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<UtxoApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: UtxoApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = UtxoApiState::new(UtxoApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = UtxoApiConfig { retry_count: 2, ..Default::default() };
        let state = UtxoApiState::new(config);
