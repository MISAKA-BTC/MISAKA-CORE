//! Comprehensive RPC request parameter validation
//!
...
/// RequestValidator operational state.
pub struct RequestValidatorState {
    config: RequestValidatorConfig,
    entries: parking_lot::RwLock<HashMap<u64, RequestValidatorEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<RequestValidatorTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: RequestValidatorStats,
    events: parking_lot::Mutex<Vec<RequestValidatorEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in RequestValidator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RequestValidatorEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: RequestValidatorEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum RequestValidatorEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct RequestValidatorTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from RequestValidator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RequestValidatorEvent {
...
/// Statistics for RequestValidator.
#[derive(Debug, Default)]
pub struct RequestValidatorStats {
...
impl RequestValidatorStats {
    pub fn snapshot(&self) -> RequestValidatorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RequestValidatorStatsSnapshot {
...
impl RequestValidatorState {
    pub fn new(config: RequestValidatorConfig) -> Self {
        Self {
            ...
            stats: RequestValidatorStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, RequestValidatorError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(RequestValidatorError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = RequestValidatorEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: RequestValidatorEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(RequestValidatorTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(RequestValidatorEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), RequestValidatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RequestValidatorError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = RequestValidatorEntryState::Completed;
        ...
        self.emit(RequestValidatorEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, RequestValidatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RequestValidatorError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = RequestValidatorEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = RequestValidatorEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(RequestValidatorEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), RequestValidatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RequestValidatorError::NotFound(id))?;
        entry.state = RequestValidatorEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<RequestValidatorEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<RequestValidatorEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: RequestValidatorEntryState) -> Vec<RequestValidatorEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, RequestValidatorEntryState::Completed | RequestValidatorEntryState::Failed | RequestValidatorEntryState::Cancelled | RequestValidatorEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(RequestValidatorEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> RequestValidatorStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<RequestValidatorEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: RequestValidatorEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = RequestValidatorState::new(RequestValidatorConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = RequestValidatorConfig { retry_count: 2, ..Default::default() };
        let state = RequestValidatorState::new(config);
