...
/// Configuration for TlsConfig.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TlsConfigConfig {
    pub enabled: bool,
...
impl Default for TlsConfigConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// TlsConfig operational state.
pub struct TlsConfigState {
    config: TlsConfigConfig,
    entries: parking_lot::RwLock<HashMap<u64, TlsConfigEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<TlsConfigTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: TlsConfigStats,
    events: parking_lot::Mutex<Vec<TlsConfigEvent>>,
...
/// Queued task.
struct TlsConfigTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from TlsConfig.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum TlsConfigEvent {
...
/// Statistics for TlsConfig.
#[derive(Debug, Default)]
pub struct TlsConfigStats {
...
impl TlsConfigStats {
    pub fn snapshot(&self) -> TlsConfigStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TlsConfigStatsSnapshot {
...
impl TlsConfigState {
    pub fn new(config: TlsConfigConfig) -> Self {
        Self {
            config,
            ...
            stats: TlsConfigStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, TlsConfigError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(TlsConfigError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = TlsConfigEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: TlsConfigEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(TlsConfigTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(TlsConfigEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), TlsConfigError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TlsConfigError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = TlsConfigEntryState::Completed;
        ...
        self.emit(TlsConfigEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, TlsConfigError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TlsConfigError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = TlsConfigEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = TlsConfigEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(TlsConfigEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), TlsConfigError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(TlsConfigError::NotFound(id))?;
        entry.state = TlsConfigEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<TlsConfigEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<TlsConfigEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: TlsConfigEntryState) -> Vec<TlsConfigEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, TlsConfigEntryState::Completed | TlsConfigEntryState::Failed | TlsConfigEntryState::Cancelled | TlsConfigEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(TlsConfigEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> TlsConfigStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<TlsConfigEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: TlsConfigEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = TlsConfigState::new(TlsConfigConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = TlsConfigConfig { retry_count: 2, ..Default::default() };
        let state = TlsConfigState::new(config);
