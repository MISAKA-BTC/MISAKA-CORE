...
/// Configuration for SubscribeApi.
...
/// SubscribeApi operational state.
pub struct SubscribeApiState {
    config: SubscribeApiConfig,
    entries: parking_lot::RwLock<HashMap<u64, SubscribeApiEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<SubscribeApiTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SubscribeApiStats,
    events: parking_lot::Mutex<Vec<SubscribeApiEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in SubscribeApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubscribeApiEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: SubscribeApiEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum SubscribeApiEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct SubscribeApiTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from SubscribeApi.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SubscribeApiEvent {
...
/// Statistics for SubscribeApi.
#[derive(Debug, Default)]
pub struct SubscribeApiStats {
...
impl SubscribeApiStats {
    pub fn snapshot(&self) -> SubscribeApiStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubscribeApiStatsSnapshot {
...
impl SubscribeApiState {
    pub fn new(config: SubscribeApiConfig) -> Self {
        Self {
            config,
            ...
            stats: SubscribeApiStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, SubscribeApiError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(SubscribeApiError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = SubscribeApiEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: SubscribeApiEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(SubscribeApiTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(SubscribeApiEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), SubscribeApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SubscribeApiError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = SubscribeApiEntryState::Completed;
        ...
        self.emit(SubscribeApiEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, SubscribeApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SubscribeApiError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = SubscribeApiEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = SubscribeApiEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(SubscribeApiEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), SubscribeApiError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SubscribeApiError::NotFound(id))?;
        entry.state = SubscribeApiEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<SubscribeApiEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<SubscribeApiEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: SubscribeApiEntryState) -> Vec<SubscribeApiEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, SubscribeApiEntryState::Completed | SubscribeApiEntryState::Failed | SubscribeApiEntryState::Cancelled | SubscribeApiEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(SubscribeApiEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> SubscribeApiStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<SubscribeApiEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: SubscribeApiEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = SubscribeApiState::new(SubscribeApiConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = SubscribeApiConfig { retry_count: 2, ..Default::default() };
        let state = SubscribeApiState::new(config);
