//! Type-safe RPC response construction
//!
...
pub struct ResponseBuilderConfig {
    pub enabled: bool,
...
impl Default for ResponseBuilderConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// ResponseBuilder operational state.
pub struct ResponseBuilderState {
    config: ResponseBuilderConfig,
    entries: parking_lot::RwLock<HashMap<u64, ResponseBuilderEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<ResponseBuilderTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ResponseBuilderStats,
    events: parking_lot::Mutex<Vec<ResponseBuilderEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in ResponseBuilder.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ResponseBuilderEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: ResponseBuilderEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum ResponseBuilderEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct ResponseBuilderTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from ResponseBuilder.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ResponseBuilderEvent {
...
/// Statistics for ResponseBuilder.
...
impl ResponseBuilderStats {
    pub fn snapshot(&self) -> ResponseBuilderStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ResponseBuilderStatsSnapshot {
...
impl ResponseBuilderState {
    pub fn new(config: ResponseBuilderConfig) -> Self {
        Self {
            config,
            ...
            stats: ResponseBuilderStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, ResponseBuilderError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(ResponseBuilderError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = ResponseBuilderEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: ResponseBuilderEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(ResponseBuilderTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(ResponseBuilderEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), ResponseBuilderError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ResponseBuilderError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = ResponseBuilderEntryState::Completed;
        ...
        self.emit(ResponseBuilderEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, ResponseBuilderError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ResponseBuilderError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = ResponseBuilderEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = ResponseBuilderEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(ResponseBuilderEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), ResponseBuilderError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ResponseBuilderError::NotFound(id))?;
        entry.state = ResponseBuilderEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<ResponseBuilderEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<ResponseBuilderEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: ResponseBuilderEntryState) -> Vec<ResponseBuilderEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, ResponseBuilderEntryState::Completed | ResponseBuilderEntryState::Failed | ResponseBuilderEntryState::Cancelled | ResponseBuilderEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(ResponseBuilderEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> ResponseBuilderStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<ResponseBuilderEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: ResponseBuilderEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = ResponseBuilderState::new(ResponseBuilderConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = ResponseBuilderConfig { retry_count: 2, ..Default::default() };
        let state = ResponseBuilderState::new(config);
