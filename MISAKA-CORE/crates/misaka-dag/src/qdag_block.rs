...
//! legacy UTXO model. `UtxoTransaction` carries `proof`, `key_image`,
//! and `utxo_refs` — all artifacts of the purged ring-signature architecture.
...
pub struct SealedTransaction {
    ...
    pub tx_bytes: Vec<u8>,
    /// Shielded transaction type tag, if this is a shielded pool tx.
    /// None = normal transparent/ring/ZKP transfer.
    /// Some = ShieldDeposit (7), ShieldedTransfer (8), ShieldWithdraw (9).
    #[serde(default)]
    pub shielded_tx_type: Option<u8>,
}
...
/// # No Legacy Leakage
///
/// This type has NO dependency on `UtxoTransaction`, `TxInput`, `key_image`,
