...
impl DiffTxResult {
    ...
    pub fn applied(tx_hash: Hash, nullifiers: Vec<Hash>) -> Self {
        Self {
            ...
            nullifiers,
            is_coinbase: false,
            conflicting_nullifier: [0u8; 32],
            prior_tx_hash: [0u8; 32],
        }
    }
    ...
    pub fn failed_nullifier(tx_hash: Hash, conflicting_nf: Hash, prior_tx: Hash) -> Self {
        Self {
            ...
            nullifiers: vec![],
            is_coinbase: false,
            conflicting_nullifier: conflicting_nf,
            prior_tx_hash: prior_tx,
        }
    }
    ...
    pub fn failed_proof(tx_hash: Hash) -> Self {
        Self {
            tx_hash,
            status: DiffTxStatus::FailedInvalidProof,
            nullifiers: vec![],
            is_coinbase: false,
            conflicting_nullifier: [0u8; 32],
            prior_tx_hash: [0u8; 32],
...
impl ReorgEngine {
    ...
    pub fn execute_reorg<S: DiffApplicable>(
        ...
        for _ in 0..revert_count {
            let diff = self.diff_stack.pop().ok_or(ReorgError::NoCommonAncestor)?;
            ...
            state
                .revert_diff(&diff)
                .map_err(|e| ReorgError::RevertFailed {
        ...
        for diff in new_diffs {
            state
                .apply_diff(&diff)
                .map_err(|e| ReorgError::ApplyFailed {
                    block: hex::encode(&diff.block_hash[..4]),
                    reason: format!("{:?}", e),
                })?;
...
impl DiffApplicable for InMemoryState {
    ...
    fn apply_diff(&mut self, diff: &StateDiff) -> Result<(), String> {
        ...
        for created in &diff.utxos_created {
            self.utxos
                .insert(created.outref.clone(), created.output.clone());
        }
    ...
    fn revert_diff(&mut self, diff: &StateDiff) -> Result<(), String> {
        ...
        for spent in &diff.utxos_spent {
            ...
            self.utxos
                .insert(spent.outref.clone(), spent.output.clone());
        }
...
mod tests {
    ...
    fn make_diff(block_id: u8, nullifiers: &[[u8; 32]], utxos: &[(u8, u32)]) -> StateDiff {
        StateDiff {
            ...
            tx_results: nullifiers
                .iter()
                .map(|nf| DiffTxResult::applied([block_id; 32], vec![*nf]))
                .collect(),
        }
    }
    ...
    fn test_multi_block_revert() {
        ...
        assert_eq!(
            state.state_root(),
            root_genesis,
            "after reverting A, state == genesis"
        );
    }
    ...
    fn test_reorg_engine_basic() {
        ...
        assert!(
            state.nullifiers.contains(&[0x01; 32]),
            "A's nullifier preserved"
        );
        assert!(
            !state.nullifiers.contains(&[0x02; 32]),
            "B's nullifier reverted"
        );
        assert!(
            !state.nullifiers.contains(&[0x03; 32]),
            "C's nullifier reverted"
        );
        assert!(
            state.nullifiers.contains(&[0x04; 32]),
            "D's nullifier applied"
        );
        assert!(
            state.nullifiers.contains(&[0x05; 32]),
            "E's nullifier applied"
        );
    }
    ...
    fn test_spent_utxo_revert_identity() {
        ...
        assert_eq!(
            restored.amount, 1000,
            "PQC metadata (amount) must survive revert"
        );
        assert_eq!(
            restored.one_time_address, [0xAA; 32],
    ...
    fn test_spent_utxo_pqc_stealth_preservation() {
        ...
        state.apply_diff(&spend_diff).unwrap();
        assert!(
            state.utxos.get(&outref).is_none(),
            "PQC UTXO must be consumed"
        );
        ...
        assert_eq!(
            restored.spending_pubkey,
            Some(vec![0x42; 48]),
            "spending_pubkey must survive revert"
        );
        let restored_stealth = restored
            .pq_stealth
            .as_ref()
            .expect("pq_stealth must survive revert");
        assert_eq!(
            restored_stealth.kem_ct, stealth.kem_ct,
            "ML-KEM-768 ciphertext must survive revert"
        );
        assert_eq!(
            restored_stealth.scan_tag, stealth.scan_tag,
