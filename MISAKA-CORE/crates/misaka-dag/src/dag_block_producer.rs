...
use crate::ghostdag::{DagStore, EpochAwareStakeProvider, GhostDagEngine, UniformStakeProvider};
use crate::reachability::ReachabilityStore;
use crate::state_diff::{CreatedUtxo, DiffTxResult, SpentUtxo, StateDiff};
...
pub struct DagNodeState {
    ...
    /// Quarantined block hashes — fast O(1) relay gate.
    ///
    /// Blocks in this set are NEVER relayed to peers.
    /// Updated when mark_quarantined() is called on the ingestion pipeline.
    /// Persisted separately via QuarantineStore (at the node layer).
    pub quarantined_blocks: std::collections::HashSet<Hash>,
    ...
    /// 21 SR: This node's Super Representative index (0..20).
    /// Used for round-robin block proposal scheduling.
    pub sr_index: usize,

    /// 21 SR: Total number of active SRs in current epoch.
    /// Defaults to NUM_SUPER_REPRESENTATIVES (21).
    pub num_active_srs: usize,
    ...
    /// SEC-FAUCET: Per-address cooldown tracker (address → last_drip_ms).
    /// Protected by the same RwLock<DagNodeState> as the rest of state,
    /// so check + record is atomic (no TOCTOU).
    pub faucet_cooldowns: HashMap<String, u64>,

    /// Pending faucet/direct transactions awaiting block inclusion.
    /// Used by the faucet to submit TXs through the normal block production path
    /// instead of bypassing consensus with direct UTXO writes.
    pub pending_transactions: HashMap<Hash, UtxoTransaction>,

    /// Shielded block hook — dispatched after every RocksDB commit.
    /// `None` = shielded module disabled (transparent-only mode / CEX node).
    /// `Some` = hook receives shielded tx payloads for ShieldedState update.
    pub shielded_hook: Option<crate::shielded_hook::SharedShieldedHook>,
}
...
impl DagMempool {
        ...
        F: Fn(&[u8; 32]) -> bool,
    {
        ...
        // HIGH-2 FIX: ML-DSA-65 signature structural check on transparent TX inputs.
        // Ensures no unsigned TXs are included in blocks.
        // Full cryptographic verification happens at RPC admission; this catches
        // TXs that bypassed RPC (e.g. P2P relay without full verification).
        if tx.proof_scheme == 0x00 { // PROOF_SCHEME_TRANSPARENT
            for (i, inp) in tx.inputs.iter().enumerate() {
                if inp.proof.is_empty() {
                    return Err(format!(
                        "input[{}]: missing ML-DSA-65 signature (transparent TX)", i
                    ));
                }
                if inp.proof.len() < 3309 {
                    return Err(format!(
                        "input[{}]: signature too short ({} bytes, need 3309 for ML-DSA-65)",
                        i, inp.proof.len()
                    ));
                }
            }
    ...
    /// Fee 降順で透明系 TX のみ取得する (TxType 0-6: Transfer, Coinbase, Faucet, Stake*, Transparent)。
    /// Fast lane (1-2秒ブロック) 用。
    pub fn top_transparent(&self, n: usize) -> Vec<&UtxoTransaction> {
        let mut sorted: Vec<&UtxoTransaction> = self
            .txs
            .iter()
            .filter(|tx| !tx.tx_type.is_shielded())
            .collect();
        sorted.sort_by(|a, b| b.fee.cmp(&a.fee));
        sorted.truncate(n);
        sorted
    ...
    /// Fee 降順で shielded 系 TX のみ取得する (TxType 7-9: ShieldDeposit, ShieldedTransfer, ShieldWithdraw)。
    /// ZKP batch lane (30秒バッチ) 用。
    pub fn top_shielded(&self, n: usize) -> Vec<&UtxoTransaction> {
        let mut sorted: Vec<&UtxoTransaction> = self
            .txs
            .iter()
            .filter(|tx| tx.tx_type.is_shielded())
...
#[deprecated(
    note = "Use VirtualState::update_virtual() for O(1) diff-based updates. replay_ordered_state is O(|history|)."
)]
    ...
    for tx in txs {
        ...
        if let Some(nf) = conflicting_nf {
            ...
            for inp in &tx.inputs {
                ...
                // 追跡するため、ここでは utxo_refs[0] を参照先として使用する。
                // 実際の UTXO 消費は nullifier ベースで管理される。
                if let Some(source_ref) = inp.utxo_refs.first() {
                    if let Some((output, creation_score)) =
                        virtual_state.get_utxo_with_score(source_ref)
                    {
...
pub async fn run_dag_block_producer(
    ...
    loop {
        ...
        //
        // HIGH-1 FIX: Use EpochAwareStakeProvider for PoS-weighted chain selection.
        // blue_work accumulates real validator stake, not uniform weight=1.
        let stake = {
            let total_stake: u128 = if s.known_validators.is_empty() {
                1u128
            } else {
                s.known_validators.iter().map(|v| v.stake_weight).sum::<u128>().max(1)
            };
            let mut provider = EpochAwareStakeProvider::new(total_stake, 1);
            for v in &s.known_validators {
                provider.set_weight(0, v.validator_id, v.stake_weight);
            }
            provider
        };
        let selected_parent = s
            .ghostdag
            .select_parent_public(&block.header.parents, &snapshot);
        ...
        if let Some(ref backend) = s.persistent_backend {
            ...
            if let Err(e) =
                backend.insert_block_atomic(block_hash, block.header.clone(), ghostdag_data)
            {
        ...
        // ── Step 6b: Shielded Block Hook dispatch ──
        //
        // RocksDB commit 後に shielded tx を ShieldedState に反映する。
        // hook が None (transparent-only / CEX mode) の場合は何もしない。
        // 失敗しても DAG consensus は止めない (hook 内でログのみ)。
        if let Some(ref hook) = s.shielded_hook {
            // Collect shielded tx payloads from the block.
            // UtxoTransaction.tx_type:
            //   ShieldDeposit      (to_byte() = 7)
            //   ShieldedTransfer   (to_byte() = 8)
            //   ShieldWithdraw     (to_byte() = 9)
            //   Other              = transparent / ring / ZKP (ignored here)
            let shielded_payloads: Vec<crate::shielded_hook::ShieldedTxPayload> = block
                .transactions
                .iter()
                .filter_map(|tx| {
                    let tx_hash = tx.tx_hash();
                    let serialized = tx.extra.clone(); // P1: extra contains serialized ShieldDepositTx/Transfer/Withdraw
                    match tx.tx_type.to_byte() {
                        7 => Some(crate::shielded_hook::ShieldedTxPayload::Deposit {
                            tx_hash,
                            serialized,
                        }),
                        8 => Some(crate::shielded_hook::ShieldedTxPayload::Transfer {
                            tx_hash,
                            serialized,
                        }),
                        9 => Some(crate::shielded_hook::ShieldedTxPayload::Withdraw {
                            tx_hash,
                            serialized,
                        }),
                        _ => None,
            ...
            hook.on_block_committed(
                block.header.blue_score, // height として blue_score を使用
                &block_hash,
                &shielded_payloads,
            );
        }
        ...
        let virtual_sp = s
            .ghostdag
            .select_parent_public(&updated_tips, &updated_snapshot);
        ...
        let diff = build_block_diff(block_hash, blue_score, &candidate_txs, &s.virtual_state);

        // TX 適用結果をストアに保存 (lossless via DiffTxResult::to_apply_status)
        for tx_result in &diff.tx_results {
            s.dag_store
                .set_tx_status(tx_result.tx_hash, tx_result.to_apply_status());
        }
        ...
        let resolve_result = match s.virtual_state.resolve(
            ...
            Err(e) => {
                error!(
                    "VirtualState resolve failed: {} — state may be inconsistent",
                    e
                );
                continue;
            }
        ...
        // ── Reorg: notify shielded hook of reverted blocks ──
        if resolve_result.reorg_depth > 0 {
            if let Some(ref hook) = s.shielded_hook {
                warn!(
                    "Reorg detected: depth={} — notifying shielded hook for nullifier rollback",
                    resolve_result.reorg_depth
                );
                // Reverted blocks' shielded TXs need nullifier un-confirmation.
                // The chain_changes.removed_chain_hashes contains reverted block hashes.
                for removed_hash in &resolve_result.chain_changes.removed_chain_hashes {
                    hook.on_block_reverted(0, removed_hash, &[]);
                }
            }
        }
        ...
        let _spent_nfs: HashSet<[u8; 32]> = resolve_result
            .chain_changes
            .acceptance_data
            .iter()
        ...
        let diff_nfs: HashSet<[u8; 32]> =
            s.virtual_state.all_nullifiers().iter().copied().collect();
...
//  ┌────────────────────────────────────────────────────────┐
//  │ Fast Lane (1-2秒)                                     │
//  │  透明送金 / Ring Sig / Faucet / Staking               │
//  │  TxType: Transfer, Coinbase, Faucet, Stake*,          │
//  │          TransparentTransfer                           │
//  │  → 即時ファイナリティ、高スループット                  │
//  ├────────────────────────────────────────────────────────┤
//  │ ZKP Batch Lane (30秒)                                 │
//  │  ShieldDeposit / ShieldedTransfer / ShieldWithdraw     │
//  │  → ZK proof 検証をバッチ処理、効率的                  │
//  │  → shielded TX が無ければスキップ                     │
//  └────────────────────────────────────────────────────────┘
//
//  両レーンは同一 DAG Store / VirtualState を共有。
//  GhostDAG の並列ブロック耐性により、2つのレーンが
//  同時にブロックを生成しても安全。

/// Dual-lane DAG block producer.
///
/// - **fast_interval_secs**: 透明送金ブロックの生成間隔 (推奨: 1-2秒)
/// - **zkp_interval_secs**: ZKP shielded バッチブロックの生成間隔 (推奨: 30秒)
/// - **max_txs**: 1ブロックあたりの最大TX数
pub async fn run_dag_block_producer_dual(
    state: Arc<RwLock<DagNodeState>>,
    fast_interval_secs: u64,
    zkp_interval_secs: u64,
    max_txs: usize,
) {
    let mut fast_ticker = interval(Duration::from_secs(fast_interval_secs));
    let mut zkp_ticker = interval(Duration::from_secs(zkp_interval_secs));
    fast_ticker.tick().await;
    zkp_ticker.tick().await;

    info!(
        "DUAL-LANE block producer started | fast={}s | zkp_batch={}s | max_txs={}",
        fast_interval_secs, zkp_interval_secs, max_txs
    );
...
/// Block lane identifier.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum BlockLane {
    /// Fast lane: transparent / ring-sig TXs (1-2秒)
    Fast,
    /// ZKP batch lane: shielded TXs (30秒)
    ZkpBatch,
}

impl std::fmt::Display for BlockLane {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BlockLane::Fast => write!(f, "⚡FAST"),
            BlockLane::ZkpBatch => write!(f, "🛡️ ZKP"),
        }
    }
}

/// Produce a block for the specified lane.
///
/// Fast lane: mempool.top_transparent() → 透明系TXのみ
/// ZKP lane:  mempool.top_shielded()   → shielded TXのみ (空ならスキップ)
async fn produce_lane_block(state: &Arc<RwLock<DagNodeState>>, lane: BlockLane, max_txs: usize) {
    let now_ms = chrono::Utc::now().timestamp_millis() as u64;
    ...
    // ── 21 SR Slot Check ──
    // Round-robin: SR at index `i` produces block when `block_count % num_srs == i`.
    // Single-node testnet: num_active_srs == 1, always produces.
    if s.num_active_srs > 1 {
        let block_count = s.dag_store.block_count() as usize;
        let current_slot = block_count % s.num_active_srs;
        if current_slot != s.sr_index {
            return; // Not our turn
        }
    }
    ...
    let tips = snapshot.get_tips();
    if tips.is_empty() {
        warn!("{} lane: DAG has no tips — skipping", lane);
        return;
    }

    // ── Step 2: Lane-specific TX 選択 ──
    let candidate_txs: Vec<UtxoTransaction> = match lane {
        BlockLane::Fast => s
            .mempool
            .top_transparent(max_txs)
            .into_iter()
            .cloned()
            .collect(),
        BlockLane::ZkpBatch => s
            .mempool
            .top_shielded(max_txs)
            .into_iter()
            .cloned()
            .collect(),
    };

    // ZKP lane: shielded TX が無ければスキップ (空ブロック不要)
    if lane == BlockLane::ZkpBatch && candidate_txs.is_empty() {
        debug!("ZKP batch lane: no shielded TXs — skipping");
        return;
    }
    ...
    if let Err(e) =
        s.dag_store
            .insert_block(block_hash, block.header.clone(), block.transactions.clone())
    {
        error!("{} lane: Failed to insert block: {}", lane, e);
        return;
    }
    ...
    let selected_parent = s
        ...
        .select_parent_public(&block.header.parents, &snapshot);
    if let Err(e) = s.reachability.add_child(selected_parent, block_hash) {
        error!("{} lane: Failed to update reachability: {}", lane, e);
        return;
    }
    let mut snapshot = s.dag_store.snapshot();
    let ghostdag_data = match s.ghostdag.try_calculate(
        ...
        Ok(data) => data,
        Err(e) => {
            error!("{} lane: GhostDAG calculation failed: {}", lane, e);
            return;
        }
    };
    let blue_score = ghostdag_data.blue_score;
    ...
    // ── Step 6a: RocksDB persist ──
    #[cfg(feature = "rocksdb")]
    if let Some(ref backend) = s.persistent_backend {
        use crate::persistent_store::PersistentDagBackend;
        if let Err(e) = backend.insert_block_atomic(block_hash, block.header.clone(), ghostdag_data)
        {
            tracing::warn!("{} lane: RocksDB write-through failed: {}", lane, e,);
        }
    }

    // ── Step 6b: Shielded Block Hook ──
    if let Some(ref hook) = s.shielded_hook {
        let shielded_payloads: Vec<crate::shielded_hook::ShieldedTxPayload> = block
            .transactions
            .iter()
        ...
        hook.on_block_committed(block.header.blue_score, &block_hash, &shielded_payloads);
    }

    // ── Step 7: VirtualState resolve ──
    let updated_snapshot = s.dag_store.snapshot();
    let updated_tips = updated_snapshot.get_tips();
    let virtual_sp = s
        .ghostdag
    ...
    // Save created UTXOs before diff is consumed by resolve
    let created_utxos: Vec<(crate::state_diff::CreatedUtxo, Option<Vec<u8>>)> = diff
        .utxos_created
        .iter()
        .map(|cu| {
            let spk = cu.output.spending_pubkey.clone();
            (cu.clone(), spk)
        })
        .collect();
    let spent_outrefs: Vec<OutputRef> = diff
        .utxos_spent
        .iter()
        .map(|su| su.outref.clone())
        .collect();
    ...
    let resolve_result = match s.virtual_state.resolve(
        ...
        Ok(result) => result,
        Err(e) => {
            error!("{} lane: VirtualState resolve failed: {}", lane, e);
            return;
        }
    };
    ...
    if resolve_result.reorg_depth > 0 {
        if let Some(ref hook) = s.shielded_hook {
            warn!(
                "{} lane: Reorg detected: depth={} — notifying shielded hook",
                lane, resolve_result.reorg_depth
            );
            for removed_hash in &resolve_result.chain_changes.removed_chain_hashes {
                hook.on_block_reverted(0, removed_hash, &[]);
            }
    ...
    // ── Step 7b: Sync UtxoSet (RPC reads from utxo_set, not VirtualState) ──
    for (cu, spk) in &created_utxos {
        let _ = s
            .utxo_set
            .add_output(cu.outref.clone(), cu.output.clone(), blue_score);
        if let Some(ref pk_bytes) = spk {
            s.utxo_set
                .register_spending_key(cu.outref.clone(), pk_bytes.clone());
        }
    }
    for outref in &spent_outrefs {
        s.utxo_set.remove_output(outref);
    }
    for tx in &candidate_txs {
        for inp in &tx.inputs {
            let _ = s.utxo_set.record_nullifier(inp.key_image);
        }
    }
    ...
    if let Err(e) = save_runtime_snapshot(
        ...
        error!("{} lane: Failed to persist snapshot: {}", lane, e);
    }
    ...
    if candidate_txs.is_empty() {
        info!(
            "⛏  {} Block {} | score={} | empty | tips={} | dag_size={}",
            lane,
            hex::encode(&block_hash[..4]),
            blue_score,
            s.dag_store.tip_count(),
        ...
        let rejected = resolve_result
            ...
            .count();
        info!(
            "⛏  {} Block {} | score={} | txs={} (ok={}, fail={}) | tips={} | mempool={}",
            lane,
            hex::encode(&block_hash[..4]),
            blue_score,
            candidate_txs.len(),
    ...
    // ── Periodic pruning (every 500 blocks) ──
    if s.blocks_produced % 500 == 0 && s.blocks_produced > 0 {
        let pruned_bodies = s.dag_store.prune_old_block_bodies(blue_score, 2000);
        let pruned_status = s.dag_store.prune_old_tx_status(100_000);
        if pruned_bodies > 0 || pruned_status > 0 {
            let (headers, bodies, statuses) = s.dag_store.memory_stats();
            info!(
                "🧹 Periodic pruning: bodies=-{} statuses=-{} | headers={} bodies={} statuses={}",
                pruned_bodies, pruned_status, headers, bodies, statuses
            );
        }
    }
}
...
mod tests {
    ...
    use misaka_types::utxo::{
        TxInput, TxOutput, TxType, PROOF_SCHEME_DEPRECATED_LOGRING, UTXO_TX_VERSION_V3,
    };
    ...
    fn test_mempool_ki_dedup() {
        ...
        let tx1 = misaka_types::utxo::UtxoTransaction {
            ...
            inputs: vec![misaka_types::utxo::TxInput {
                utxo_refs: vec![
