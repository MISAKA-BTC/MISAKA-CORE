...
use crate::ghostdag::{DagStore, GhostDagEngine, StakeWeightProvider, UniformStakeProvider};
...
pub async fn run_dag_block_producer(
    ...
    loop {
        ...
        let spent_nfs: HashSet<[u8; 32]> = resolve_result
            .chain_changes
            .acceptance_data
            .iter()
...
async fn produce_lane_block(
    ...
    if let Some(ref backend) = s.persistent_backend {
        ...
        {
            tracing::warn!(
                "{} lane: RocksDB write-through failed: {}",
                lane,
                e,
            );
        }
    ...
    let created_utxos: Vec<(crate::state_diff::CreatedUtxo, Option<Vec<u8>>)> = diff
        ...
        .collect();
    let spent_outrefs: Vec<OutputRef> = diff.utxos_spent.iter().map(|su| su.outref.clone()).collect();
    ...
    for (cu, spk) in &created_utxos {
        let _ = s.utxo_set.add_output(cu.outref.clone(), cu.output.clone(), blue_score);
        if let Some(ref pk_bytes) = spk {
            s.utxo_set.register_spending_key(cu.outref.clone(), pk_bytes.clone());
        }
    }
