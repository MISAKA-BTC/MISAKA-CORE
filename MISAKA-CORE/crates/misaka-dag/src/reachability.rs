    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingGhostDagData { hash } => {
                write!(
                    f,
                    "missing ghostdag data for block {}",
                    hex::encode(&hash[..4])
                )
            }
        }
...
impl ReachabilityStore {
    ...
    pub fn add_child(&mut self, parent: Hash, child: Hash) -> Result<(), String> {
        ...
        // Any structural interval change invalidates cached ancestor answers.
        ...
        self.nodes.insert(
            child,
            TreeNode {
                // Assigned immediately by canonical redistribute() below.
        ...
        if let Some(parent_node) = self.nodes.get_mut(&parent) {
            ...
            // Canonical sibling order keeps interval assignment insertion-order independent.
            parent_node.children.sort();
        }
        let parent_interval = self.nodes[&parent].interval;
        self.redistribute(&parent, parent_interval);
        Ok(())
    }
    ...
    fn reallocate_from(&mut self, start: &Hash) -> Result<(), String> {
        ...
        for _ in 0..50000 {
            ...
            if node.parent == ZERO_HASH || current == self.genesis {
                break;
            }
            current = node.parent;
        }
...
pub fn is_dag_ancestor_conclusive<S: DagStore>(
    ...
    let ancestor_score = store
        .get_ghostdag_data(ancestor)
        .map(|d| d.blue_score)
        .ok_or(ReachabilityError::MissingGhostDagData { hash: *ancestor })?;
    let descendant_score = store
        .get_ghostdag_data(descendant)
        .map(|d| d.blue_score)
...
pub use crate::constants::{
    is_decoy_eligible, retention_level, NodeRetentionRole, RetentionLevel,
    ACCUMULATOR_RETENTION_DEPTH, FINALITY_DEPTH, MIN_DECOY_DEPTH, PRUNING_DEPTH,
};
...
mod tests {
    ...
    fn test_conclusive_detects_side_branch_ancestor() {
        ...
        assert!(
            !reach.is_sp_tree_ancestor_of(&b, &c),
            "SPT must NOT detect B→C"
        );
        ...
        assert_eq!(
            is_dag_ancestor_conclusive(&a, &c, &reach, &store).unwrap(),
            true
        );
        assert_eq!(
            is_dag_ancestor_conclusive(&g, &c, &reach, &store).unwrap(),
            true
        );
        assert_eq!(
            is_dag_ancestor_conclusive(&c, &b, &reach, &store).unwrap(),
            false
        );
        assert_eq!(
            is_dag_anticone_conclusive(&a, &b, &reach, &store).unwrap(),
            true
        );
    }
    ...
    fn test_conclusive_detects_deep_side_branch() {
        ...
        assert_eq!(
            is_dag_ancestor_conclusive(&d, &e, &reach, &store).unwrap(),
            true
        );
        assert_eq!(
            is_dag_ancestor_conclusive(&b, &e, &reach, &store).unwrap(),
            true
        );
        assert_eq!(
            is_dag_ancestor_conclusive(&g, &e, &reach, &store).unwrap(),
            true
        );
        // E is NOT an ancestor of B
        assert_eq!(
            is_dag_ancestor_conclusive(&e, &b, &reach, &store).unwrap(),
            false
        );
    }
    ...
    fn test_conclusive_wide_dag_no_false_negative() {
        ...
        for _ in 0..depth {
            ...
            assert_eq!(
                result.unwrap(),
                true,
                "branch tip must be ancestor of merge"
            );
    ...
    fn test_conclusive_missing_data_returns_error() {
        ...
        assert!(
            result.is_err(),
            "missing data must return Error, not silent false"
        );
    ...
    fn test_conclusive_identity() {
        ...
        assert_eq!(
            is_dag_ancestor_conclusive(&g, &g, &reach, &store).unwrap(),
            true
        );
    }
    ...
    fn test_conclusive_anticone_symmetric() {
        ...
        assert_eq!(
            is_dag_anticone_conclusive(&b, &a, &reach, &store).unwrap(),
            true
        );
        // Genesis is NOT in anticone with either
        assert_eq!(
            is_dag_anticone_conclusive(&g, &a, &reach, &store).unwrap(),
            false
        );
    }
    ...
    fn test_interval_exhaustion_linear_500() {
        ...
        for i in 1..500u32 {
            ...
            assert!(
                s.is_sp_tree_ancestor_of(&hn(0), &child),
                "genesis→{} broken",
                i
            );
            parent = child;
    ...
    #[test]
    #[ignore = "pre-existing: legacy GhostDAG/virtual state test"]
    ...
    #[test]
    fn test_siblings_never_become_ancestors_by_insertion_order() {
        let g = hn(0);
        let a = hn(1);
        let b = hn(2);

        let mut first_ab = ReachabilityStore::new(g);
        first_ab.add_child(g, a).unwrap();
        first_ab.add_child(g, b).unwrap();
        assert!(!first_ab.is_sp_tree_ancestor_of(&a, &b));
        assert!(!first_ab.is_sp_tree_ancestor_of(&b, &a));
        assert!(first_ab.is_anticone(&a, &b));

        let mut first_ba = ReachabilityStore::new(g);
        first_ba.add_child(g, b).unwrap();
        first_ba.add_child(g, a).unwrap();
        assert!(!first_ba.is_sp_tree_ancestor_of(&a, &b));
        assert!(!first_ba.is_sp_tree_ancestor_of(&b, &a));
        assert!(first_ba.is_anticone(&a, &b));
    }
}
