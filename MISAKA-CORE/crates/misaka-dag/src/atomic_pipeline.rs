...
pub struct StoreWriteBatch {
    ...
    /// Shielded tx payloads to dispatch to ShieldedBlockHook after commit.
    /// These are NOT written to the DAG RocksDB — the hook handles persistence.
    pub shielded_txs: Vec<crate::shielded_hook::ShieldedTxPayload>,
}
...
impl StoreWriteBatch {
    ...
    /// # Atomicity (C3 FIX)
    ///
    /// ALL mutations are collected into a single `DagWriteBatch` and
    /// committed via `PersistentDagBackend::commit_write_batch()`, which
    /// maps to a single `rocksdb::WriteBatch::write()` call in production.
    ///
    /// Previous design called individual methods (insert_block_atomic,
    /// record_nullifier, save_state_diff, ...) as SEPARATE writes —
    /// a crash between any two writes left the DB inconsistent.
    ///
    /// # Fail-Closed
    ///
    /// If this method returns `Err`, no partial state has been persisted.
    /// The storage remains in its pre-commit state.
    pub fn commit<S: PersistentDagBackend>(mut self, storage: &S) -> Result<(), AtomicPipelineError> {
        use crate::persistent_store::DagWriteBatch;

        let mut dag_batch = DagWriteBatch::new();

        // ── Header + GhostDAG ──
        if let (Some((hash, header)), Some((_, ghostdag))) = (self.header.take(), self.ghostdag.take()) {
            dag_batch.header = Some((hash, header, ghostdag));
        }

        // ── Nullifiers ──
        dag_batch.nullifiers = std::mem::take(&mut self.nullifiers);

        // ── State diff (serialize) ──
        if let Some((hash, ref diff)) = self.state_diff.take() {
            let diff_json = serde_json::to_vec(diff).map_err(|e| {
                AtomicPipelineError::CommitFailed(format!("serialize state_diff: {}", e))
            })?;
            dag_batch.state_diff = Some((hash, diff_json));
        }

        // ── Acceptance data ──
        dag_batch.acceptance = self.acceptance.take();

        // ── Virtual state snapshot ──
        dag_batch.virtual_snapshot = self.virtual_snapshot.take();

        // ── Last accepted ──
        dag_batch.last_accepted = self.last_accepted.take();

        // ── SINGLE ATOMIC COMMIT ──
        // All mutations are written in one rocksdb::WriteBatch::write() call.
        // No window exists where some mutations are visible but others are not.
        storage.commit_write_batch(dag_batch).map_err(|e| {
            AtomicPipelineError::CommitFailed(format!("atomic batch commit: {}", e))
        })?;

        info!("StoreWriteBatch committed atomically (single WriteBatch)");
        Ok(())
    }
    ...
    pub fn is_empty(&self) -> bool {
        self.header.is_none()
            ...
            && self.shielded_txs.is_empty()
    }

    /// Push a shielded tx payload to be dispatched after commit.
    pub fn push_shielded_tx(&mut self, payload: crate::shielded_hook::ShieldedTxPayload) {
        self.shielded_txs.push(payload);
    }
}
