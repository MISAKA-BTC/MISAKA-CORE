...
pub enum BlockIngestState {
    ...
    /// Block quarantined — validation failed and the block is isolated.
    /// Quarantined blocks are NOT relayed to peers and are persisted
    /// to the QuarantineStore for audit and potential future release.
...
pub enum IngestAction {
    ...
    ValidateBlock { block_hash: Hash },

    /// Block has been accepted — caller should update VirtualState.
    BlockAccepted { block_hash: Hash },

    /// Block has been rejected — caller should penalize peer if appropriate.
    BlockRejected { block_hash: Hash, reason: String },

    /// Block has been quarantined — caller MUST persist to QuarantineStore
    /// and MUST NOT relay this block to any peer. Source peer should be
    /// penalized via PeerScoring.
    BlockQuarantined {
        block_hash: Hash,
        reason: String,
        /// The peer that sent this block (for penalization).
...
pub struct IngestionStats {
    ...
    /// Blocks quarantined (persisted to QuarantineStore, never relayed).
...
impl IngestionPipeline {
    ...
    pub fn ingest_block(
        ...
        if self.pending.len() >= MAX_PENDING_BLOCKS {
            return Err(IngestError::PoolFull {
        ...
        let missing: HashSet<Hash> = header
            .parents
            .iter()
            .filter(|p| {
                **p != ZERO_HASH
                    ...
                    && !self
                        .pending
                        .iter()
                        .any(|(_, pb)| pb.hash == **p && pb.state == BlockIngestState::Accepted)
            })
            .copied()
            .collect();
        ...
        if missing.is_empty() {
            ...
            actions.push(IngestAction::SendP2p(DagP2pMessage::GetDagBlocks {
                hashes: missing_vec,
            }));
        }
    ...
    /// Mark a block as quarantined (validation failed, persisted for audit).
    ///
    /// Quarantine is stronger than rejection:
    /// - The block is persisted to QuarantineStore (by the caller)
    /// - The block is NEVER relayed to peers
    /// - Source peer is penalized
    /// - All transitive dependents are also quarantined
    ///
    /// # When to use quarantine vs rejection
    ///
    /// - **Quarantine**: GhostDAG errors, k-cluster violation, future block,
    ///   proposer ineligible, merkle root mismatch, signature failure
    /// - **Rejection**: Structural issues that may be transient (missing parent
    ///   timeout, pool full)
    pub fn mark_quarantined(
        ...
        if let Some(block) = self.pending.get_mut(&hash) {
            block.state = BlockIngestState::Quarantined {
                reason: reason.clone(),
            };
        }

        let mut actions = vec![IngestAction::BlockQuarantined {
            block_hash: hash,
            reason: reason.clone(),
            source_peer,
        }];

        // Cascade quarantine to children (their parent is quarantined)
        if let Some(children) = self.waiting_for.remove(&hash) {
            for child_hash in children {
                let child_actions = self.mark_quarantined(
                    child_hash,
                    format!("parent {} quarantined: {}", hex::encode(&hash[..4]), reason),
                    source_peer,
                );
                actions.extend(child_actions);
            }
...
mod tests {
    ...
    fn test_ingest_all_parents_known() {
        ...
        assert!(
            matches!(actions[0], IngestAction::ValidateBlock { block_hash } if block_hash == h(1))
        );
    ...
    fn test_ingest_missing_parent_then_arrival() {
        ...
        assert!(actions
            .iter()
            .any(|a| matches!(a, IngestAction::FetchParents { .. })));
        assert!(actions
            .iter()
            .any(|a| matches!(a, IngestAction::SendP2p(..))));
        ...
        assert!(actions.iter().any(
            |a| matches!(a, IngestAction::ValidateBlock { block_hash } if *block_hash == h(2))
        ));
    }
    ...
    fn test_mark_accepted_wakes_children() {
        ...
        let validate_hashes: Vec<Hash> = actions
            .iter()
    ...
    fn test_known_parent_bypass() {
        ...
        assert!(actions
            .iter()
            .any(|a| matches!(a, IngestAction::ValidateBlock { .. })));
        assert!(!actions
            .iter()
            .any(|a| matches!(a, IngestAction::FetchParents { .. })));
    }
