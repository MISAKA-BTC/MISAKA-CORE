...
pub enum DagP2pMessage {
    ...
    // ─── BFT Consensus (v5.2) ───
    /// BFT Proposal from VRF-elected leader.
    BftProposal {
        payload: Vec<u8>,  // serde_json encoded BftProposal
    },

    /// BFT Prevote.
    BftPrevote {
        payload: Vec<u8>,  // serde_json encoded BftVote
    },
    ...
    /// Equivocation evidence for slashing.
    BftSlashEvidence {
        payload: Vec<u8>,  // serde_json encoded EquivocationEvidence
    },
}
...
pub struct PeerInfo {
    ...
    /// SHA3-256(pk) hex-encoded (32 bytes) peer identity.
    pub peer_id: String,
