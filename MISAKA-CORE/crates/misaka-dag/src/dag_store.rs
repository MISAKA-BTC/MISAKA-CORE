...
// ═══════════════════════════════════════════════════════════════
//  Pruning — メモリ節約のため古いブロック本体を削除
// ═══════════════════════════════════════════════════════════════

impl ThreadSafeDagStore {
    /// Prune block TX bodies older than `keep_depth` blocks from the current tip.
    /// Headers and GhostDAG data are retained for reachability/DAA.
    /// TX status is retained for wallet queries.
    ///
    /// Returns the number of block bodies pruned.
    pub fn prune_old_block_bodies(&self, current_score: u64, keep_depth: u64) -> usize {
        if current_score <= keep_depth {
            return 0;
        }
        let cutoff = current_score - keep_depth;
        let mut inner = self.inner.write().unwrap();
        let mut pruned = 0;

        // Collect hashes of blocks with score below cutoff
        let old_hashes: Vec<Hash> = inner
            .headers
            .iter()
            .filter(|(_, h)| h.blue_score < cutoff)
            .map(|(hash, _)| *hash)
            .collect();

        for hash in &old_hashes {
            if inner.block_txs.remove(hash).is_some() {
                pruned += 1;
            }
        }
        ...
        if pruned > 0 {
            tracing::debug!(
                "DAG Pruning: removed {} block bodies (cutoff score={})",
                pruned, cutoff
            );
        }
        pruned
    ...
    /// Prune old tx_status entries for blocks older than cutoff.
    /// Retains last `keep` entries by insertion order approximation.
    pub fn prune_old_tx_status(&self, max_entries: usize) -> usize {
        let mut inner = self.inner.write().unwrap();
        let current = inner.tx_status.len();
        if current <= max_entries {
            return 0;
        }
        // Remove oldest entries (HashMap has no order, so we just truncate)
        let to_remove = current - max_entries;
        let keys: Vec<[u8; 32]> = inner.tx_status.keys().take(to_remove).cloned().collect();
        for k in &keys {
            inner.tx_status.remove(k);
        }
        tracing::debug!("DAG Pruning: removed {} old tx_status entries", keys.len());
        keys.len()
    }

    /// Memory usage estimate (block count + TX body count).
