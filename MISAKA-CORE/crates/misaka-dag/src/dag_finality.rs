...
impl FinalityManager {
    ...
    /// Select a deterministic checkpoint candidate from the current bucket.
    ///
    ...
    ///
    /// The current rule is:
    /// 1. derive the current checkpoint bucket from the local virtual tip score
    /// 2. enumerate all known blocks with `blue_score <= bucket_boundary`
    /// 3. choose the canonical best block under `ParentSortKey`
    ///
    /// Once validators have converged on the same set of blocks up to the
    /// bucket boundary, this produces the same checkpoint target even if their
    /// virtual tips differ slightly above that boundary.
    pub fn checkpoint_candidate<S: DagStore>(
        ...
        let target_boundary = (virtual_tip_score / self.checkpoint_interval) * self.checkpoint_interval;
...
mod tests {
    ...
    #[test]
    fn test_checkpoint_candidate_uses_virtual_tip_bucket() {
