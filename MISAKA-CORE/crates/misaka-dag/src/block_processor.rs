...
pub use crate::parent_selection::{canonical_compare, select_canonical_parents, ParentSortKey};
    ...
    if !header.is_genesis() {
        ...
        match validate_timestamp(header.timestamp_ms, &header.parents, store, now_ms) {
            ...
            TimestampCheck::TooOld {
                ...
                return Err(BlockProcessError::BadTimestamp(format!(
                    "timestamp {}ms < past_median_time {}ms",
                    block_ms, past_median_ms
                )));
            }
            TimestampCheck::TooFuture { block_ms, max_ms } => {
                return Err(BlockProcessError::BadTimestamp(format!(
                    "timestamp {}ms > max_allowed {}ms",
                    block_ms, max_ms
                )));
            }
        }
