...
pub fn select_canonical_parents<S: DagStore>(
    ...
    let mut tip_keys: Vec<(ParentSortKey, Hash)> = tips
        .iter()
        .filter_map(|&tip| {
...
pub fn select_parent<S: DagStore>(parents: &[Hash], store: &S, genesis_hash: &Hash) -> Hash {
    parents
        .iter()
        .max_by(|a, b| {
            let key_a = ParentSortKey::from_store(a, store).unwrap_or(ParentSortKey {
                blue_work: 0,
                blue_score: 0,
                proposer_id: [0; 32],
                block_hash: **a,
            });
            let key_b = ParentSortKey::from_store(b, store).unwrap_or(ParentSortKey {
                blue_work: 0,
                blue_score: 0,
                proposer_id: [0; 32],
                block_hash: **b,
    ...
    pub fn compute<S: DagStore>(tips: &[Hash], store: &S, max_parents: usize) -> Self {
        if tips.is_empty() {
            return Self {
                ...
                spread: 0,
                active_tip_count: 0,
                recommended_parent_count: MIN_PARENTS,
            };
        }

        let scores: Vec<u64> = tips
            .iter()
        ...
        if scores.is_empty() {
            return Self {
                ...
                max_score: 0,
                spread: 0,
                active_tip_count: tips.len(),
                recommended_parent_count: MIN_PARENTS,
            };
        }

        // SEC-AUDIT-V5 MED-002: scores is non-empty (checked above), but
        // use unwrap_or to satisfy zero-panic policy.
...
pub fn check_block_template_quality<S: DagStore>(
    ...
    let all_present = virtual_parents
        .parents
        .iter()
    ...
    BlockTemplateQuality {
        ...
        parent_count_optimal: virtual_parents.parents.len()
            <= anticone_spread.recommended_parent_count,
    }
}
...
pub fn pick_virtual_parents<S: DagStore>(
    ...
    let parents: Vec<Hash> = candidates
        .into_iter()
        .take(max_parents)
        .map(|(_, h)| h)
        .collect();
