...
pub fn save_runtime_snapshot(
    ...
    let tmp_path = path.with_extension("json.tmp");
    {
        let file = fs::File::create(&tmp_path).map_err(|e| {
            format!("snapshot: create tmp '{}': {}", tmp_path.display(), e)
        })?;
        let mut writer = std::io::BufWriter::new(file);
        std::io::Write::write_all(&mut writer, &bytes).map_err(|e| {
            format!("snapshot: write tmp: {}", e)
        })?;
        std::io::Write::flush(&mut writer).map_err(|e| {
            format!("snapshot: flush tmp: {}", e)
        })?;
        writer.get_ref().sync_all().map_err(|e| {
            format!("snapshot: fsync tmp: {}", e)
        })?;
    }
    fs::rename(&tmp_path, path).map_err(|e| {
        format!("snapshot: rename '{}' → '{}': {}", tmp_path.display(), path.display(), e)
    })?;
