...
#[cfg(all(not(debug_assertions), feature = "faucet"))]
compile_error!(
...
#[cfg(all(not(debug_assertions), feature = "allow-tofu"))]
compile_error!(
...
struct Cli {
    ...
    /// Enable testnet mode for shielded module (allows stub proofs).
    ...
    if cli.keygen_only {
        ...
        // Now uses the same argon2id+chacha20poly1305 keystore as runtime.
        {
            ...
            let keystore = encrypt_keystore(
                ...
                &hex::encode(&pk_bytes),
                &validator_id_hex,
                if cli.chain_id == 1 { 10_000_000 } else { 1_000_000 },
                &passphrase,
            )
        ...
        println!("  4. Stake {} MISAKA",
            if cli.chain_id == 1 { "10,000,000" } else { "1,000,000" });
...
async fn start_dag_node(
    ...
    use misaka_dag::{
        ...
        dag_block_producer::run_dag_block_producer_dual,
    ...
    // selected_parent → child 関係を blue_score 昇順で再挿入する。
    {
        ...
        if all.len() > 1 {
            ...
            let mut blocks_with_score: Vec<([u8; 32], u64)> = all
                ...
                .filter_map(|h| {
                    snap.get_ghostdag_data(h).map(|gd| (*h, gd.blue_score))
                })
                .collect();
    ...
    let dag_node_state = DagNodeState {
        ...
        num_active_srs: if cli.validators <= 1 { 1 } else { cli.validators.min(21) },
        local_validator,
        genesis_hash,
    ...
    // If Solana RPC is not configured, accepts the signature format-only (backward compat).
    {
        ...
        if let Some(ref lv) = local_validator_ref.local_validator {
            ...
            if !already_registered {
                ...
                let l1_pubkey_hex = {
                    let key_path = std::path::Path::new(&cli.data_dir).join("l1-public-key.json");
                    if key_path.exists() {
                    ...
                    cli.stake_signature
                {
                    ...
                    if !rpc_url.is_empty() && !program_id.is_empty() && !l1_pubkey_hex.is_empty() {
                        ...
                        .await
                        {
                            ...
                            Err(e) => {
                                ...
                                (registry.config().min_validator_stake, false, Some(sig.clone()))
                            }
                        }
                    } else {
                ...
                match registry.register(
                    ...
                    Ok(()) => {
                        if stake_verified {
                            ...
                            // これにより VRF 選出確率とコンセンサス重みが預入枚数に比例する。
                            {
                                ...
                                if let Some(kv) = guard.known_validators.iter_mut()
                    ...
                    Err(e) => {
                        warn!(
                            "SEC-STAKE: Failed to auto-register local validator: {}",
                            e
                        );
                    }
                }
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let l1_pubkey_hex = {
                let key_path = std::path::Path::new(&stake_verify_data_dir)
                    .join("l1-public-key.json");
                if key_path.exists() {
                    let raw = std::fs::read_to_string(&key_path).unwrap_or_default();
                    let parsed: serde_json::Value =
                        serde_json::from_str(&raw).unwrap_or_default();
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Some(next_epoch) = maybe_next_epoch {
                ...
                {
                    let guard = finality_epoch_state.read().await;
                    let election_result = sr21_election::run_election(
                        &guard.known_validators,
                        next_epoch,
                    );
                    ...
                    if let Some(ref lv) = wguard.local_validator {
                        if let Some(new_index) = sr21_election::find_sr_index(
                            &election_result, &lv.identity.validator_id
                        ) {
    ...
    let shielded_state_for_rpc: Option<misaka_shielded::SharedShieldedState> = {
        use crate::shielded_hook_impl::ShieldedBootstrap;
        ...
        info!(
            "ShieldedConfig: enabled={}, testnet={}, anchor_age={}, min_fee={} (chain_id={})",
            enabled, testnet, anchor_age, min_fee, cli.chain_id
        );
        ShieldedBootstrap::from_node_config(enabled, testnet, anchor_age, min_fee)
    };
    ...
    if let Some(ref state) = shielded_state_for_rpc {
        ...
        let hook: misaka_dag::shielded_hook::SharedShieldedHook =
            std::sync::Arc::new(
    ...
    let _rpc_handle = tokio::spawn(async move {
        if let Err(e) = dag_rpc::run_dag_rpc_server_with_observation(
            ...
            shielded_state_for_rpc,  // passed to RPC server for /api/shielded/* endpoints
            rpc_addr,
            cli.chain_id,
        )
        .await
    ...
    let state: SharedState = Arc::new(RwLock::new(NodeState {
        ...
        proposer_payout_address: cli.proposer_payout_address.as_deref().and_then(|hex_str| {
            ...
            if bytes.len() == 32 {
                ...
                tracing::warn!("proposer_payout_address must be 32 bytes hex, got {} bytes", bytes.len());
                None
            }
        }),
        treasury_address: cli.treasury_address.as_deref().and_then(|hex_str| {
            ...
            if bytes.len() == 32 {
                ...
                tracing::warn!("treasury_address must be 32 bytes hex, got {} bytes", bytes.len());
                None
            }
        }),
    }));
