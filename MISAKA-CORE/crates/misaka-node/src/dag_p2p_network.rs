...
use misaka_dag::dag_p2p::{DagP2pMessage, DagSyncManager, DagSyncState, SyncAction, SyncMode};
...
pub struct DagP2pEventLoop {
    ...
    sync_poll_ticks: u64,
    /// Optional BFT message forwarding channel (v5.2).
    /// When set, BFT P2P messages are forwarded to the BFT event loop.
    /// NOTE: bft_event_loop module is not yet enabled. Using opaque bytes for now.
    bft_msg_tx: Option<mpsc::Sender<Vec<u8>>>,
...
impl DagP2pEventLoop {
        ...
        match event.message {
            ...
            DagP2pMessage::Peers { peers } => {
                ...
                for peer_info in peers {
                    ...
                    if obs.discovered_peers.iter().any(|dp| dp.address == peer_info.listen_addr) {
                        continue;
                    }
                    ...
                    if let Some(ref prefix) = subnet_prefix {
                        let same_subnet_count = obs.discovered_peers.iter()
                            .filter(|dp| {
                    ...
                    obs.discovered_peers.push(
                        crate::dag_p2p_surface::DiscoveredPeer {
                    ...
                    match action {
                        ...
                        IngestAction::BlockQuarantined {
                            ...
                            self.observation
                                .write()
                                .await
                                .record_ingest_rejected(block_hash, format!("QUARANTINED: {}", reason));
                            warn!(
    ...
    async fn run_block_validation(&self, guard: &mut DagNodeState, block_hash: Hash) {
        ...
        if computed_hash != block_hash {
            ...
            guard
                .ingestion_pipeline
                .mark_quarantined(block_hash, "hash mismatch".to_string(), None);
            return;
        }
        ...
        let ghostdag_data = match guard.ghostdag.try_calculate(
            ...
            Err(e) => {
                ...
                guard
                    .ingestion_pipeline
                    .mark_quarantined(block_hash, format!("ghostdag: {}", e), None);
                return;
            }
        };
    ...
    async fn process_sync_actions(&mut self, peer_id: misaka_p2p::PeerId, actions: Vec<SyncAction>) {
        for action in actions {
            match action {
                SyncAction::Send(msg) => {
                    self.send_to_peer(peer_id, msg).await;
    ...
    async fn test_restart_sync_does_not_fetch_known_parent_for_relay_block() {
        ...
        assert!(
            timeout(Duration::from_millis(200), outbound_rx.recv())
                .await
                .is_err(),
            "known parent should not trigger an extra fetch after restart"
        );
