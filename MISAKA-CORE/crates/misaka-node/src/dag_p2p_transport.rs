...
#[cfg(feature = "dag")]
const PEER_MAX_MESSAGES_PER_SEC: u32 = 150;
...
fn validate_message_limits(msg: &misaka_dag::dag_p2p::DagP2pMessage) -> Option<String> {
    ...
    match msg {
        DagP2pMessage::GetDagBlocks { hashes } if hashes.len() > MAX_HASHES_PER_MESSAGE => {
            Some(format!("GetDagBlocks: {} hashes (max {})", hashes.len(), MAX_HASHES_PER_MESSAGE))
        }
        DagP2pMessage::DagInventory { block_hashes, .. } if block_hashes.len() > MAX_HASHES_PER_MESSAGE => {
            Some(format!("DagInventory: {} hashes (max {})", block_hashes.len(), MAX_HASHES_PER_MESSAGE))
        }
        DagP2pMessage::BlockLocator { hashes, .. } if hashes.len() > MAX_HASHES_PER_MESSAGE => {
            Some(format!("BlockLocator: {} hashes (max {})", hashes.len(), MAX_HASHES_PER_MESSAGE))
        }
        DagP2pMessage::DagHello { tips, node_name, .. } => {
            if tips.len() > MAX_TIPS_PER_MESSAGE {
                return Some(format!("DagHello: {} tips (max {})", tips.len(), MAX_TIPS_PER_MESSAGE));
            }
            // Reject excessively long node names (potential log injection / memory waste)
            if node_name.len() > 256 {
                return Some(format!("DagHello: node_name {} bytes (max 256)", node_name.len()));
            }
            None
        }
        DagP2pMessage::DagTips { tips, .. } if tips.len() > MAX_TIPS_PER_MESSAGE => {
            Some(format!("DagTips: {} tips (max {})", tips.len(), MAX_TIPS_PER_MESSAGE))
        }
        DagP2pMessage::NewDagBlock { parents, .. } if parents.len() > MAX_PARENTS_PER_BLOCK => {
            Some(format!("NewDagBlock: {} parents (max {})", parents.len(), MAX_PARENTS_PER_BLOCK))
        }
        DagP2pMessage::Peers { peers } if peers.len() > MAX_PEERS_PER_GOSSIP => {
            Some(format!("Peers: {} entries (max {})", peers.len(), MAX_PEERS_PER_GOSSIP))
        }
        DagP2pMessage::GetBodies { hashes } if hashes.len() > MAX_HASHES_PER_MESSAGE => {
            Some(format!("GetBodies: {} hashes (max {})", hashes.len(), MAX_HASHES_PER_MESSAGE))
        }
        DagP2pMessage::Bodies { blocks } if blocks.len() > MAX_HASHES_PER_MESSAGE => {
            Some(format!("Bodies: {} blocks (max {})", blocks.len(), MAX_HASHES_PER_MESSAGE))
        }
        _ => None,
    }
}
...
fn cheap_structural_check(msg: &misaka_dag::dag_p2p::DagP2pMessage) -> Option<String> {
    ...
    match msg {
        ...
        DagP2pMessage::Headers { headers_json, count, .. } => {
        ...
        DagP2pMessage::PruningProofData { proof_json } if proof_json.len() > MAX_BLOCK_JSON_SIZE => {
            Some(format!(
        ...
        DagP2pMessage::DagSnapshotData { snapshot_json } if snapshot_json.len() > MAX_BLOCK_JSON_SIZE * 2 => {
            Some(format!(
...
async fn tcp_responder_handshake(
    ...
    let reply = responder_handle(&ephemeral_pk, &nonce_i, initiator_version, our_pk.clone(), our_sk)
...
async fn tcp_initiator_handshake(
    ...
    if let Some(expected) = expected_responder_pk {
        ...
        // TOFU (Trust On First Use): accept unverified responder for seed bootstrap.
        // On mainnet, operators MUST pin expected_responder_pk via validator registry.
        // TOFU is safe for testnet seed discovery; mainnet should never reach here
        // because all outbound connections will have pinned PKs from the SR21 set.
        warn!(
            "TOFU handshake: accepting unverified responder pk={} \
             (safe for testnet seed bootstrap; mainnet operators must pin PKs)",
            hex::encode(&responder_pk.to_bytes()[..8]),
        );
    }
        ...
        while let Some(pt) = peer_out_rx.recv().await {
            ...
            match encode_wire_frame(&current_send_key, &mut nonce, &pt) {
                ...
                Err(AeadError::NonceExhausted) => {
                    ...
                    nonce = NonceCounter::new();
                    rekey_sent = false;
                    warn!("Peer {} emergency rekey (epoch={}) — proactive rekey missed", wph, rekey_epoch);
    ...
    loop {
            ...
            Err(AeadError::DecryptFailed) => {
                ...
                for attempt in 1..=MAX_REKEY_RETRIES {
                    ...
                    let (new_seed, new_keys) =
                        derive_rekey(&recv_session_seed, try_epoch, false);
                ...
                match recovered {
                    ...
                    None => {
                        ...
                        warn!("Peer {} AEAD auth failed after rekey retries — disconnecting", ph);
                        break;
                    }
                }
            }
                ...
                if msg_type == misaka_p2p::secure_transport::MsgType::Rekey {
                    if payload.len() >= 8 {
                        let new_epoch = u64::from_le_bytes(
                            payload[..8].try_into().unwrap_or([0u8; 8]),
                        );
        ...
        match serde_json::from_slice::<misaka_dag::dag_p2p::DagP2pMessage>(&plaintext) {
            Ok(msg) => {
                ...
                if peer_msg_count > PEER_MAX_MESSAGES_PER_SEC {
                    warn!("Peer {} rate limited ({}/s), dropping message", ph, peer_msg_count);
                    continue;
                }
...
mod tests {
    ...
    async fn test_tcp_handshake_allows_first_dag_frame_roundtrip() {
        ...
        let client = tokio::spawn(async move {
            ...
            let (_hs, keys) =
                tcp_initiator_handshake(&mut stream, &keypair_b.public_key, &keypair_b.secret_key, None)
                    .await
...
pub async fn run_dag_p2p_transport(
        ...
        tokio::spawn(async move {
            ...
            for attempt in 1..=5u32 {
                match connect_to_peer(
                    ...
                    &pk,
                    &sk,
                    None, // SEC-C1: Seeds use TOFU (runtime warning on testnet)
                    &itx,
                    &r,
                    &st,
                    chain_id,
    ...
    let conn_guard: Arc<tokio::sync::Mutex<misaka_p2p::ConnectionGuard>> =
        Arc::new(tokio::sync::Mutex::new(misaka_p2p::ConnectionGuard::with_config(guard_config)));
        ...
        match listener.accept().await {
            Ok((mut stream, addr)) => {
                ...
                let slot_id = {
                    ...
                    if misaka_p2p::is_bogon_ip(&remote_ip) {
                ...
                tokio::spawn(async move {
                    ...
                    info!(
                        ...
                        pid.short_hex(), addr,
                        slot_guard.guard.lock().await.established_count(),
                        slot_guard.guard.lock().await.half_open_count(),
                    );
                    let (otx, orx) = mpsc::channel::<Vec<u8>>(PEER_OUTBOUND_CAPACITY);
