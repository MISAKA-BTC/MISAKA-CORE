...
use crate::block_apply::{execute_block, BlockResult};
...
mod tests {
    ...
    fn setup() -> (UtxoSet, Vec<SpendingKeypair>, Poly) {
        ...
        for (i, _) in wallets.iter().enumerate() {
            utxo_set
                .add_output(OutputRef { tx_hash: [(i + 1) as u8; 32], output_index: 0 },
                    TxOutput { amount: 10_000, one_time_address: [0xAA; 32], pq_stealth: None, spending_pubkey: None },
                    0)
                .unwrap();
        }
        (utxo_set, wallets, a)
    ...
    fn make_block(a: &Poly, wallets: &[SpendingKeypair], height: u64) -> BlockCandidate {
        ...
        let tx = UtxoTransaction {
            proof_scheme: PROOF_SCHEME_DEPRECATED_LRS, tx_type: TxType::Transfer, version: UTXO_TX_VERSION,
            inputs: vec![TxInput {
                utxo_refs: (0..4).map(|i| OutputRef { tx_hash: [(i + 1) as u8; 32], output_index: 0 }).collect(),
                proof: vec![], key_image: strong_ki, ki_proof: vec![],
            }],
            outputs: vec![
                TxOutput { amount: 7000, one_time_address: [0xBB; 32], pq_stealth: None, spending_pubkey: None },
                TxOutput { amount: 2900, one_time_address: [0xCC; 32], pq_stealth: None, spending_pubkey: None },
            ],
            fee: 100, extra: vec![], zk_proof: None,
        };
        let sig = pq_sign(a, &ring_pks, 0, &wallets[0].secret_poly, &tx.signing_digest()).unwrap();
        let kip = prove_key_image(a, &wallets[0].secret_poly, &wallets[0].public_poly, &strong_ki).unwrap();
        let mut tx_final = tx;
        tx_final.inputs[0].ki_proof = kip.to_bytes();
        BlockCandidate {
            height, slot: height, parent_hash: [0; 32],
            transactions: vec![VerifiedTx {
                tx: tx_final.clone(), raw_spending_keys: vec![],
            ring_pubkeys: vec![ring_pks.clone()],
                ring_amounts: vec![vec![10_000; 4]],
                ring_proofs: vec![VerifiedProof::Lrs { sig, ki_proof: Some(kip) }],
                privacy_constraints: TransactionPrivacyConstraints::from_tx_and_input_amounts(&tx_final, &[10_000]).ok(),
                privacy_statement: TransactionPrivacyConstraints::from_tx_and_input_amounts(&tx_final, &[10_000]).ok()
                    .and_then(|c| TransactionPublicStatement::from_constraints_and_resolved_rings(&tx_final, &c, &[ring_pks.clone()], misaka_pqc::PrivacyBackendFamily::ZeroKnowledge).ok()),
            }],
    ...
    fn test_sync_wrong_height_rejected() {
        let (mut utxo_set, wallets, a) = setup();
        let mut sync = SyncEngine::new();
        let block = make_block(&a, &wallets, 5);
