...
pub async fn run_rpc_server(
    ...
    write_routes = write_routes.route_layer(axum::middleware::from_fn_with_state(
        auth_state,
        require_api_key,
    ));
...
async fn submit_tx(
    ...
    // - Lattice ZKP proof verification (PQ lattice-based)
...
async fn get_anonymity_set(
    ...
    let mut leaves: Vec<[u8; 32]> = Vec::with_capacity(anonymity_set_size);
    ...
    let mut fill_idx = 0;
    for (leaf, _) in all_leaf_hashes.iter().take(anonymity_set_size - selected.len()) {
...
async fn faucet(
    ...
    let coinbase_tx = misaka_types::utxo::UtxoTransaction {
        ...
        proof_scheme: misaka_types::utxo::PROOF_SCHEME_DEPRECATED_LOGRING,
