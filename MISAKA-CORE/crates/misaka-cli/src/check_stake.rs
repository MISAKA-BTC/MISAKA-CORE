...
pub async fn run(l1_public_key: &str) -> Result<()> {
    let l1_bytes = hex::decode(l1_public_key)
        .context("L1 public key must be valid hex (64 chars)")?;
    if l1_bytes.len() != 32 {
    ...
    println!("  ══════════════════════════════════");
    println!("  L1 Key:  {}...{}", &l1_public_key[..24], &l1_public_key[56..]);
    ...
    println!("  Node:     {}", if node_name.is_empty() { "(not set)" } else { &node_name });
    ...
    for acc in accounts {
        ...
        if raw.len() >= 104 && raw[8..40] == user_bytes[..] {
            let amt = u64::from_le_bytes(
                raw[96..104].try_into().unwrap_or([0; 8])
            );
    ...
    let status = if total_misaka >= min_stake { "Active ✅" } else { "BELOW MINIMUM ⚠️" };
