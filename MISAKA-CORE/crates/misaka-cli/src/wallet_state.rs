...
impl WalletState {
    ...
    pub fn load(key_path: &str) -> Result<Self> {
        ...
        let mut state: WalletState = serde_json::from_str(&json)
            .with_context(|| "failed to parse wallet state JSON")?;
    ...
    pub fn load_or_create(key_path: &str, wallet_name: &str, master_address: &str) -> Result<Self> {
        ...
        if state_path.exists() {
            let json = std::fs::read_to_string(&state_path)
                .with_context(|| format!("failed to read wallet state: {}", state_path.display()))?;
            let mut state: WalletState = serde_json::from_str(&json)
                .with_context(|| "failed to parse wallet state JSON")?;
            state.recalculate_balance();
            Ok(state)
    ...
    pub fn save(&mut self, key_path: &str) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(self)
            .context("failed to serialize wallet state")?;
        ...
        std::fs::rename(&tmp_path, &state_path)
            .with_context(|| format!("failed to rename {} → {}", tmp_path.display(), state_path.display()))?;
        ...
        if candidates.is_empty() {
            ...
            if total_unspent >= needed {
                bail!(
                    ...
                    needed, amount, fee,
                    self.unspent_utxos().len(), total_unspent
                );
            }
            bail!(
    ...
    pub fn select_utxos_multi(&self, amount: u64, fee: u64) -> Result<(Vec<&OwnedUtxo>, u64)> {
        ...
        if accumulated < needed {
            bail!(
                "insufficient funds: need {} (amount={} + fee={}), have {} across {} UTXOs",
                needed, amount, fee, accumulated, self.unspent_utxos().len()
            );
        }
    ...
    pub fn validate(&self) -> Vec<String> {
        ...
        let computed: u64 = self.utxos.iter().filter(|u| !u.spent).map(|u| u.amount).sum();
