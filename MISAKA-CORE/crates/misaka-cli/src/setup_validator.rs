...
pub async fn run(data_dir: &str, chain_id: u32, validator_index: usize) -> Result<()> {
    ...
    let (wallet_address, _wallet_pk_hex) = if wallet_key_path.exists() {
        ...
        let pub_json: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&wallet_pub_path)?)?;
        let addr = pub_json["address"].as_str().unwrap_or("").to_string();
        let pk = pub_json["ml_dsa_pk"].as_str().unwrap_or("").to_string();
    ...
    let l1_pubkey_hex = if enc_key_path.exists() {
        ...
        if pub_key_path.exists() {
            let pk_json: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(&pub_key_path)?)?;
        ...
        println!(
            "    misaka-node --keygen-only --name validator-{} --data-dir {}",
            validator_index, data_dir
        );
        println!();
    ...
    if solana_rpc.is_empty() {
        ...
        println!(
            "    misaka-cli check-stake --key-file {}",
