//! Unified `send` command — single entry point for all transfer types.
//!
//! # Transfer Modes
//!
//! | Flag         | Mode          | TxType               | Privacy     |
//! |--------------|---------------|----------------------|-------------|
//! | *(default)*  | Transparent   | TransparentTransfer  | None        |
//! | `--shielded` | Q-DAG-CT ZKP  | ConfidentialTransfer | Full        |
//!
//! Lattice ZKP proofs have been removed. Transparent mode uses ML-DSA-65
//! direct signatures (equivalent to Kaspa's Schnorr approach but
//! post-quantum safe). For privacy, use `--shielded` which provides
//! full ZKP confidentiality (amounts + sender hidden).
//!
//! # Amount Handling
//!
...
pub enum SendMode {
    ...
    Transparent,
    /// ZKP confidential — sender + amounts fully hidden.
    Shielded,
}
...
impl std::fmt::Display for SendMode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SendMode::Shielded => write!(f, "🛡️  Shielded (ZKP confidential)"),
        }
    }
}
...
pub fn resolve_mode(shielded: bool) -> SendMode {
    if shielded {
        SendMode::Shielded
    } else {
...
fn confirm_transaction(args: &SendArgs) -> Result<bool> {
    ...
    println!("│  Total:  {:<36}│", format_amount(args.amount_base + args.fee_base));
    println!("╰──────────────────────────────────────────────╯");

    if args.mode == SendMode::Transparent {
...
pub async fn run(args: SendArgs) -> Result<()> {
    ...
    if args.mode == SendMode::Shielded && args.to_kem_pk.is_none() {
        bail!(
            "shielded mode requires --to-kem-pk <ML-KEM-768 public key hex>\n\
             Hint: use the recipient's ml_kem_pk from their .pub.json file"
        );
    }
    ...
    match args.mode {
        ...
        SendMode::Shielded => {
            let kem_pk = args.to_kem_pk.as_deref()
                .ok_or_else(|| anyhow::anyhow!("shielded mode requires --to-kem-pk"))?;
            crate::confidential_transfer::run(
                &args.wallet_path,
                kem_pk,
                args.amount_base,
                args.fee_base,
...
mod tests {
    ...
    fn test_parse_amount_whole() {
        ...
        assert_eq!(parse_amount(100.0).unwrap(), 100_000_000);
    }
    ...
    fn test_parse_amount_decimal() {
        ...
        assert_eq!(parse_amount(1234.567890).unwrap(), 1_234_567_890);
    }
    ...
    fn test_format_amount() {
        assert_eq!(format_amount(1_000_000), "1 MISAKA");
        assert_eq!(format_amount(1_500_000), "1.5 MISAKA");
        assert_eq!(format_amount(1), "0.000001 MISAKA");
        assert_eq!(format_amount(1_234_567_890), "1234.56789 MISAKA");
    ...
    fn test_resolve_mode() {
        ...
        assert_eq!(resolve_mode(true), SendMode::Shielded);
    }
}
