...
pub async fn run(
    ...
    println!("   Building anonymity set (anonymity_set_size={})...", CT_RING_SIZE);
...
async fn query_spendable_utxo(
    ...
    let outputs = resp["utxos"]
        .as_array()
        .ok_or_else(|| anyhow::anyhow!("no utxos array in response"))?;
    ...
    match resp {
        Ok(data) => {
            if let Some(leaves) = data["leaves"].as_array() {
                ...
                return generate_synthetic_anonymity_set(anonymity_set_size);
            }
        }
        Err(_) => {
...
fn generate_synthetic_anonymity_set(anonymity_set_size: usize) -> Result<AnonymitySetData> {
    use sha3::{Digest, Sha3_256};

    let mut leaf_hashes = Vec::with_capacity(anonymity_set_size);
    ...
    let signer_index = rand::random::<usize>() % anonymity_set_size;
