...
impl CheckpointManager {
    ...
    pub fn create_checkpoint(
        ...
        let payload_for_hash = serde_json::to_vec(&checkpoint)
            .map_err(|e| CheckpointError::Serde(e.into()))?;
    ...
    pub fn load_latest(&self) -> Result<Checkpoint, CheckpointError> {
        ...
        if checkpoint.content_hash != [0u8; 32] {
            ...
            let payload = serde_json::to_vec(&verify_cp)
                .map_err(|e| CheckpointError::Serde(e.into()))?;
        ...
        info!(
            ...
            hex::encode(&checkpoint.block_hash[..4]),
            hex::encode(&checkpoint.state_root[..4]),
            if checkpoint.content_hash != [0u8; 32] { "verified" } else { "legacy" },
        );
