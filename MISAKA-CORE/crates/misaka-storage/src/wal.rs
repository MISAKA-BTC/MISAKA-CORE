...
impl WriteAheadLog {
    ...
    fn append_entry(&self, entry: &JournalEntry) -> Result<(), WalError> {
        ...
        let hash = Sha3_256::digest(json.as_bytes());
        let hash_hex = hex::encode(&hash[..4]); // 4 bytes = 8 hex chars
    ...
    fn read_entries(path: &Path) -> Result<Vec<JournalEntry>, WalError> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            if let Some(ref expected) = expected_hash {
                ...
                let computed_hex = hex::encode(&computed[..4]);
...
mod tests {
    ...
    fn test_wal_integrity_hash_present() {
        ...
        assert_eq!(parts[0].len(), 8, "integrity hash must be 8 hex chars (4 bytes)");
    ...
    fn test_wal_legacy_format_accepted() {
        // Ensure backward compat: legacy entries without hash are still accepted
        let tmp = TempDir::new().unwrap();
        ...
        assert_eq!(entries.len(), 1, "legacy format without hash must be accepted");
    }
    ...
    fn test_wal_truncated_mid_write_detected() {
        ...
        let entries = WriteAheadLog::read_entries(&path).unwrap();
        assert_eq!(entries.len(), 1, "must recover valid entries before truncation");
    }
    ...
    fn test_wal_compact_preserves_incomplete_blocks() {
        ...
        wal.log_phase(h(2), AcceptPhase::Received).unwrap();
        wal.log_phase(h(2), AcceptPhase::BlockDataPersisted).unwrap();
        ...
        assert_eq!(recovery.incomplete.len(), 1);
        assert_eq!(recovery.incomplete[0].block_hash, h(2));
        assert_eq!(recovery.incomplete[0].last_phase, AcceptPhase::BlockDataPersisted);
    }
    ...
    fn test_wal_multiple_phases_tracked_correctly() {
        ...
        wal.log_phase(h(1), AcceptPhase::ConsensusPersisted).unwrap();
