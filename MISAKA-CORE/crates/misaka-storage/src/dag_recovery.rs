...
/// Discard incomplete block acceptance transactions.
///
/// For each incomplete block hash:
/// - The block was never fully committed to the DAG store
/// - It will be re-fetched from P2P during the next sync
///
/// This is NOT a rollback — it discards partially-written data.
/// The DAG continues forward from its last consistent state.
pub fn discard_incomplete_blocks(incomplete: &[[u8; 32]], _snapshot_path: &Path) -> usize {
    if incomplete.is_empty() {
        return 0;
    }

    info!(
        "DAG Recovery: discarding {} incomplete block(s)",
        incomplete.len()
    );
    ...
    for hash in incomplete {
        info!("  Discarded incomplete: {}", hex::encode(&hash[..4]));
    }
...
pub fn bootstrap(data_dir: &Path, _max_utxo_delta: usize) -> DagRecoveryResult {
    ...
    let rolled_back = if recovery.incomplete.is_empty() {
        ...
        let incomplete: Vec<[u8; 32]> = recovery.incomplete.iter().map(|b| b.block_hash).collect();
        discard_incomplete_blocks(&incomplete, &snapshot_path)
    };
...
mod tests {
    ...
    fn test_rollback_empty() {
        let rolled = discard_incomplete_blocks(&[], Path::new("/tmp"));
        assert_eq!(rolled, 0);
    }
