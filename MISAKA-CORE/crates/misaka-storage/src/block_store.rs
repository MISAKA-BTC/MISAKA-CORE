...
struct BlockMeta {
    ...
    /// Key images (nullifiers) added in this block (for SPC switch undo).
...
impl RocksBlockStore {
    ...
    pub fn open(path: &Path, sync_writes: bool) -> Result<Self, BlockStoreError> {
        ...
        // ── Per-CF options: Nullifier CF gets bloom filter ──
        let mut nullifier_opts = Options::default();
        nullifier_opts.set_paranoid_checks(true);
        // Bloom filter: ~10 bits/key, reduces disk reads for point lookups.
        // Nullifiers are 32-byte keys checked on every TX validation.
        nullifier_opts.optimize_for_point_lookup(64); // 64 MB block cache hint

        let cf_descriptors = vec![
            ColumnFamilyDescriptor::new(CF_UTXOS, Options::default()),
            ColumnFamilyDescriptor::new(CF_NULLIFIERS, nullifier_opts),
