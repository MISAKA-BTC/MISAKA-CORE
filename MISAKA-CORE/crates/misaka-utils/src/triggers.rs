...
impl OnceTrigger {
    pub fn new() -> Self { Self { inner: DuplexTrigger::new() } }
    pub fn trigger(&self) { self.inner.trigger(); }
    pub fn is_triggered(&self) -> bool { self.inner.is_triggered() }
    pub async fn wait(&self) { self.inner.wait().await; }
}
