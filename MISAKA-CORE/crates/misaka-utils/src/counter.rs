...
impl AtomicCounter {
    ...
    pub fn reset(&self) -> u64 { self.value.swap(0, Ordering::Relaxed) }
}
...
impl AtomicGauge {
    ...
    pub fn increment(&self) -> i64 { self.value.fetch_add(1, Ordering::Relaxed) }
    pub fn decrement(&self) -> i64 { self.value.fetch_sub(1, Ordering::Relaxed) }
    pub fn add(&self, n: i64) -> i64 { self.value.fetch_add(n, Ordering::Relaxed) }
...
impl CounterSet {
    pub fn new(name: impl Into<String>) -> Self {
        Self { name: name.into(), counters: Vec::new() }
    }

    pub fn register(&mut self, counter_name: impl Into<String>) -> usize {
        let idx = self.counters.len();
        self.counters.push((counter_name.into(), AtomicCounter::new(0)));
        idx
    }
    ...
    pub fn snapshot(&self) -> CounterSnapshot {
        ...
        CounterSnapshot {
            ...
            values: self.counters.iter().map(|(n, c)| (n.clone(), c.get())).collect(),
            timestamp_ms: now,
        }
    }
}
...
impl HighWaterMark {
    pub fn new() -> Self {
        Self { current: AtomicUsize::new(0), high: AtomicUsize::new(0) }
    }
    ...
    pub fn high(&self) -> usize { self.high.load(Ordering::Relaxed) }
}
