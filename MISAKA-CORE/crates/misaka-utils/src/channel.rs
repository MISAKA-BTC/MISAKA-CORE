...
impl<T: Send + 'static> MeteredChannel<T> {
    pub fn new(name: impl Into<String>, capacity: usize) -> Self {
        ...
        Self { sender, receiver: Some(receiver), capacity, name: name.into() }
    }

    pub fn sender(&self) -> mpsc::Sender<T> { self.sender.clone() }

    pub fn take_receiver(&mut self) -> Option<mpsc::Receiver<T>> { self.receiver.take() }
