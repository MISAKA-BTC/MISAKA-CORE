...
impl MemSize for String {
    fn mem_size(&self) -> usize { self.capacity() }
}
...
impl MemSize for [u8; 32] {
    fn mem_size(&self) -> usize { 0 } // Inline, no heap
}

impl<K: MemSize, V: MemSize> MemSize for std::collections::HashMap<K, V> {
    fn mem_size(&self) -> usize {
        self.iter().map(|(k, v)| k.mem_size() + v.mem_size()).sum::<usize>()
            + self.capacity() * (std::mem::size_of::<K>() + std::mem::size_of::<V>() + 8)
    }
}

impl MemSize for u64 { fn mem_size(&self) -> usize { 0 } }
impl MemSize for u32 { fn mem_size(&self) -> usize { 0 } }
impl MemSize for i64 { fn mem_size(&self) -> usize { 0 } }
impl MemSize for bool { fn mem_size(&self) -> usize { 0 } }
