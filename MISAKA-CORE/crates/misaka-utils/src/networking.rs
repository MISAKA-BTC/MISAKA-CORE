...
pub fn is_bogon(addr: &IpAddr) -> bool {
    matches!(classify_ip(addr), IpClassification::Bogon | IpClassification::Loopback | IpClassification::Multicast)
}
...
impl BanManager {
    ...
    pub fn ban(&self, addr: IpAddr, reason: &str) {
        ...
        tracing::warn!("Banned peer {} for {:?}: {}", addr, self.default_ban_duration, reason);
...
mod tests {
    ...
    fn test_ip_classification() {
        assert_eq!(classify_ip(&"8.8.8.8".parse().unwrap()), IpClassification::Public);
        assert_eq!(classify_ip(&"192.168.1.1".parse().unwrap()), IpClassification::Private);
        assert_eq!(classify_ip(&"127.0.0.1".parse().unwrap()), IpClassification::Loopback);
        assert_eq!(classify_ip(&"169.254.1.1".parse().unwrap()), IpClassification::LinkLocal);
        assert_eq!(classify_ip(&"240.0.0.1".parse().unwrap()), IpClassification::Bogon);
    }
