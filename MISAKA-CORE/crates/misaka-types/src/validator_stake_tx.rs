    ...
    pub fn validate_structure(&self) -> Result<(), StakeTxError> {
        ...
        match &self.params {
            ...
            StakeTxParams::StakeMore(p) => {
                ...
                let required = p.additional_amount
...
pub enum StakeTxError {
    ...
    #[error("duplicate stake input: tx_hash={} output_index={output_index}",
        hex::encode(tx_hash))]
    DuplicateStakeInput { tx_hash: [u8; 32], output_index: u32 },
...
mod tests {
    ...
    fn register_structure_valid() {
        make_register(10_000_000).validate_structure().expect("valid register");
    }

    #[test]
    fn stake_more_structure_valid() {
        make_stake_more(5_000_000).validate_structure().expect("valid stake_more");
    }

    #[test]
    fn begin_exit_structure_valid() {
        make_begin_exit().validate_structure().expect("valid begin_exit");
    }
    ...
    fn register_zero_fee_rejected() {
        ...
        tx.fee = 0;
        assert!(matches!(tx.validate_structure(), Err(StakeTxError::ZeroFee)));
    }
    ...
    fn register_missing_signature_rejected() {
        let mut tx = make_register(10_000_000);
        tx.signature.clear();
        assert!(matches!(tx.validate_structure(), Err(StakeTxError::MissingSignature)));
    }
