...
pub fn decode_address(s: &str, chain_id: u32) -> Result<[u8; ADDR_BYTES], AddressError> {
    ...
    // ── Hex decode ──
    let bytes =
        hex::decode(hex_part).map_err(|e| AddressError::InvalidHex(format!("{}", e)))?;
...
mod tests {
    ...
    fn sample_addr() -> [u8; 32] {
        [
            0xa1, 0xb2, 0xc3, 0xd4, 0xe5, 0xf6, 0x78, 0x90, 0x12, 0x34,
    ...
    #[test]
    fn test_invalid_hex() {
        let bad = format!("misaka1{}{}", "g".repeat(40), "0000");
        assert!(matches!(
