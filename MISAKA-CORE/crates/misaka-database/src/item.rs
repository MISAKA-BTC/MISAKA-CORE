//! Cached single-valued DB items with concurrency support.
...
impl<T> CachedDbItem<T> {
        ...
        T: Clone + DeserializeOwned,
    {
        ...
        if let Some(slice) = self.db.get_pinned(&self.key)? {
            ...
            *self.cached_item.write() = Some(item.clone());
            ...
            Err(StoreError::KeyNotFound(DbKey::prefix_only(&self.key).to_string()))
        }
    }
        ...
        F: Fn(T) -> T,
    {
        ...
        let mut item = if let Some(item) = guard.take() {
            ...
            return Err(StoreError::KeyNotFound(DbKey::prefix_only(&self.key).to_string()));
        };
