...
fn test_cached_db_access_roundtrip() {
    ...
    let key = [1u8, 2, 3, 4];
    let data = TestData { value: 42, name: "hello".into() };

    // Write
    access.write(DirectDbWriter::new(db.clone()), key, data.clone()).expect("write");
...
fn test_cached_db_access_has_and_delete() {
    ...
    let data = TestData { value: 99, name: "test".into() };
    access.write(DirectDbWriter::new(db.clone()), key, data).expect("write");
    assert!(access.has(key).expect("has after"));

    access.delete(DirectDbWriter::new(db.clone()), key).expect("delete");
...
fn test_cached_db_item_roundtrip() {
    ...
    let data = TestData { value: 123, name: "item".into() };
    item.write(DirectDbWriter::new(db.clone()), &data).expect("write");

    let result = item.read().expect("read");
    assert_eq!(result, data);

    // Update
    let updated = item.update(DirectDbWriter::new(db.clone()), |mut d| {
        d.value = 456;
        d
    }).expect("update");
...
fn test_batch_write() {
    ...
    for i in 0u8..10 {
        let key = [i, 0, 0, 0];
        let data = TestData { value: i as u64, name: format!("item_{}", i) };
        access.write(BatchDbWriter::new(&mut batch), key, data).expect("batch write");
    }
