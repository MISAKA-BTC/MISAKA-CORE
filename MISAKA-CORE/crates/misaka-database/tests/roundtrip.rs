...
fn test_cached_db_access_write_read() {
    ...
    access.write(DirectDbWriter::new(db.clone()), key, value.clone()).expect("write");
...
fn test_cached_db_access_has_and_delete() {
    ...
    access.write(DirectDbWriter::new(db.clone()), key, vec![99]).expect("write");
...
fn test_cached_db_item_roundtrip() {
    ...
    item.write(DirectDbWriter::new(db.clone()), &42u64).expect("write");
...
fn test_cached_db_item_update() {
    ...
    item.write(DirectDbWriter::new(db.clone()), &10u64).expect("write");
    let updated = item.update(DirectDbWriter::new(db.clone()), |v| v + 5).expect("update");
...
fn test_batch_writer_atomicity() {
    ...
    let k1 = [1u8; 32];
    let k2 = [2u8; 32];
    access.write(BatchDbWriter::new(&mut batch), k1, vec![10]).expect("w1");
    access.write(BatchDbWriter::new(&mut batch), k2, vec![20]).expect("w2");
