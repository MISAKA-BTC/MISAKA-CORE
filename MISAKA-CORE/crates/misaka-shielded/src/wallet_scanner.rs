...
use crate::types::{
    DecryptError, EncryptedNote, FullViewKey, IncomingViewKey, Note,
    NoteCommitment, Nullifier, NullifierKey,
};
...
impl LocalNullifierCache {
    pub fn new() -> Self {
        Self { spent: std::collections::HashSet::new() }
    }
...
impl NoteScanner {
    ...
    fn compute_view_tag(&self, epk: &[u8; 32]) -> u8 {
        ...
        hasher.update(self.ivk.as_bytes());
        hasher.update(epk);
...
mod tests {
    use super::*;
    use crate::types::{Note, IncomingViewKey, NullifierKey};
    use rand::SeedableRng;

    fn make_scanner() -> NoteScanner {
        NoteScanner::new(
            IncomingViewKey([0u8; 32]),
            NullifierKey([1u8; 32]),
        )
    }
    ...
    fn select_notes_insufficient() {
        ...
        assert!(matches!(result, Err(WalletScanError::InsufficientBalance { .. })));
    }
