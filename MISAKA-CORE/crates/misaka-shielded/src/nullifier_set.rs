...
impl NullifierSet {
    ...
    pub fn confirmed_entries(&self) -> Vec<(Nullifier, SpentRecord)> {
        self.confirmed.iter().map(|(k, v)| (*k, v.clone())).collect()
    }
...
pub enum NullifierError {
    ...
    #[error("nullifier {nullifier} already reserved by tx {}", hex::encode(reserved_by))]
    AlreadyReserved {
        nullifier: Nullifier,
        reserved_by: [u8; 32],
    },
...
mod tests {
    ...
    fn double_reserve_different_tx_fails() {
        ...
        assert!(matches!(result, Err(NullifierError::AlreadyReserved { .. })));
    }
