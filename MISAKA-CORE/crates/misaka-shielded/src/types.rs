...
impl EncryptedNote {
    ...
    pub fn encrypt(note: &Note, ivk: &[u8; 32], epk_secret: &[u8; 32]) -> Result<Self, EncryptError> {
    ...
    pub fn try_decrypt(&self, ivk: &[u8; 32]) -> Result<Note, DecryptError> {
        ...
        let mut off = 0;
        let value = u64::from_le_bytes(plaintext[off..off + 8].try_into().map_err(|_| DecryptError::Truncated)?);
        off += 8;
        let asset_id = u64::from_le_bytes(plaintext[off..off + 8].try_into().map_err(|_| DecryptError::Truncated)?);
        off += 8;
        ...
        off += 32;
        let memo_len = u32::from_le_bytes(plaintext[off..off + 4].try_into().map_err(|_| DecryptError::Truncated)?) as usize;
        off += 4;
        ...
        Ok(Note { value, asset_id, recipient_pk, rcm, memo })
    }
}
...
impl ShieldedProof {
    pub fn stub() -> Self {
        ...
        Self { bytes: vec![0xAA; 4] } // stub marker
    }
