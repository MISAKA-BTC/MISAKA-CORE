...
pub enum ShieldedTxError {
    ...
    OutputCountMismatch { commitments: usize, encrypted: usize },
...
mod tests {
    ...
    fn valid_deposit_structure() {
        make_deposit().validate_structure().expect("should be valid");
    }
    ...
    fn deposit_zero_amount_rejected() {
        ...
        tx.amount = 0;
        assert!(matches!(tx.validate_structure(), Err(ShieldedTxError::ZeroAmount)));
    }
    ...
    fn deposit_low_fee_rejected() {
        ...
        assert!(matches!(tx.validate_structure(), Err(ShieldedTxError::FeeTooLow { .. })));
    }
    ...
    fn valid_withdraw_structure() {
        make_withdraw().validate_structure().expect("should be valid");
    }
    ...
    fn withdraw_duplicate_nullifier_rejected() {
        ...
        assert!(matches!(tx.validate_structure(), Err(ShieldedTxError::DuplicateNullifier)));
    }
}
