...
use misaka_shielded::{
    CircuitVersion, EncryptedNote, FullViewKey, IncomingViewKey, LocalNullifierCache,
    NoteCommitment, NoteScanner, Nullifier, NullifierKey, PaymentProof, ScannedBlock,
    ScannedNote, ShieldDepositTx, ShieldWithdrawTx, ShieldedConfig, ShieldedError,
    ShieldedProof, ShieldedState, SpentRecord, TreeRoot,
    MIN_SHIELDED_FEE,
    types::Note,
};
...
fn make_deposit(cm_byte: u8, amount: u64) -> ShieldDepositTx {
    ...
    let mut tx = ShieldDepositTx {
        ...
        output_commitment: NoteCommitment([cm_byte; 32]),
        encrypted_note: EncryptedNote {
            epk: [cm_byte; 32],
            ciphertext: vec![cm_byte; 64],
            tag: [0u8; 16],
            view_tag: 0,
        },
...
fn test_withdraw_apply() {
    ...
    assert_eq!(credit.recipient, [9u8; 20]);
...
fn test_double_spend_rejected() {
    ...
    // 1回目は成功
    state.apply_withdraw(&make_withdraw(77, 1_000_000), [0u8; 32], 1)
        .expect("first withdraw ok");

    // 2回目は同じ nullifier → 拒否
    let err = state.validate_withdraw(&make_withdraw(77, 1_000_000))
...
fn test_cex_flow() {
    ...
    let (deposit_ws, _) = state
        .apply_deposit(&deposit_tx, [1u8; 32], 100)
        .expect("deposit ok");

    // transparent 側で debit が発生する
    assert!(deposit_ws.transparent_debit.is_some());
    assert_eq!(deposit_ws.transparent_debit.as_ref().unwrap().from, user_addr);

    // Step 2: ユーザーが shielded pool から CEX アドレスへ出金 (ShieldWithdraw)
    let withdraw_tx = ShieldWithdrawTx {
        nullifiers: vec![Nullifier([0xE1u8; 32])],
        anchor: TreeRoot::empty(),
        withdraw_amount: 9_000_000, // fee 引き後
        withdraw_recipient: cex_addr,
        fee: MIN_SHIELDED_FEE,
        proof: ShieldedProof::stub(),
        circuit_version: CircuitVersion::STUB_V1,
    };
...
fn test_nullifier_reservation_exclusive() {
    ...
    // tx1 が先に予約
    state.reserve_nullifiers(&[nf], tx1).expect("tx1 reserve ok");
    ...
    assert!(!state.is_nullifier_reserved(&nf));
    state.reserve_nullifiers(&[nf], tx2).expect("tx2 reserve after release ok");
}
...
fn test_payment_proof_create_verify() {
    ...
    // proof 生成
    let proof = PaymentProof::from_scanned_note(&sn, &ivk, 1_700_000_000_000)
...
fn test_view_tag_fast_reject() {
    ...
    // 自分宛の note を作成
    let enc = EncryptedNote::encrypt(&correct_note, ivk.as_bytes(), &epk_secret).expect("encrypt ok");
