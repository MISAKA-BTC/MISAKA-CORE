//! Key derivation functions for all wallet and node operations
//!
...
/// Kdf operational state.
pub struct KdfState {
    config: KdfConfig,
    entries: parking_lot::RwLock<HashMap<u64, KdfEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<KdfTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: KdfStats,
    events: parking_lot::Mutex<Vec<KdfEvent>>,
...
impl KdfStats {
    pub fn snapshot(&self) -> KdfStatsSnapshot {
...
impl KdfState {
    pub fn new(config: KdfConfig) -> Self {
        Self {
            ...
            stats: KdfStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, KdfError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(KdfError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = KdfEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: KdfEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(KdfTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(KdfEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), KdfError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KdfError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = KdfEntryState::Completed;
        ...
        self.emit(KdfEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, KdfError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KdfError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = KdfEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = KdfEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(KdfEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), KdfError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KdfError::NotFound(id))?;
        entry.state = KdfEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<KdfEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<KdfEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: KdfEntryState) -> Vec<KdfEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, KdfEntryState::Completed | KdfEntryState::Failed | KdfEntryState::Cancelled | KdfEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(KdfEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> KdfStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<KdfEvent> { std::mem::take(&mut *self.events.lock()) }
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = KdfState::new(KdfConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = KdfConfig { retry_count: 2, ..Default::default() };
        let state = KdfState::new(config);
