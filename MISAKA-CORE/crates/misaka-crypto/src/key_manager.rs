...
/// KeyManager operational state.
pub struct KeyManagerState {
    config: KeyManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, KeyManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<KeyManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: KeyManagerStats,
    events: parking_lot::Mutex<Vec<KeyManagerEvent>>,
...
pub struct KeyManagerEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: KeyManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct KeyManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from KeyManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl KeyManagerStats {
    pub fn snapshot(&self) -> KeyManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyManagerStatsSnapshot {
...
impl KeyManagerState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, KeyManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(KeyManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = KeyManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: KeyManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(KeyManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(KeyManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), KeyManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KeyManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = KeyManagerEntryState::Completed;
        ...
        self.emit(KeyManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, KeyManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KeyManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = KeyManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = KeyManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(KeyManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), KeyManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(KeyManagerError::NotFound(id))?;
        entry.state = KeyManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<KeyManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<KeyManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: KeyManagerEntryState) -> Vec<KeyManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, KeyManagerEntryState::Completed | KeyManagerEntryState::Failed | KeyManagerEntryState::Cancelled | KeyManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(KeyManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> KeyManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<KeyManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: KeyManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = KeyManagerState::new(KeyManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = KeyManagerConfig { retry_count: 2, ..Default::default() };
        let state = KeyManagerState::new(config);
