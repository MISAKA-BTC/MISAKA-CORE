...
/// Merkle operational state.
pub struct MerkleState {
    config: MerkleConfig,
    entries: parking_lot::RwLock<HashMap<u64, MerkleEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<MerkleTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MerkleStats,
    events: parking_lot::Mutex<Vec<MerkleEvent>>,
...
impl MerkleStats {
    pub fn snapshot(&self) -> MerkleStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MerkleStatsSnapshot {
...
impl MerkleState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, MerkleError> {
        ...
        let entry = MerkleEntry {
            ...
            state: MerkleEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(MerkleTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(MerkleEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), MerkleError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MerkleError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = MerkleEntryState::Completed;
        ...
        self.emit(MerkleEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, MerkleError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MerkleError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = MerkleEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = MerkleEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(MerkleEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), MerkleError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MerkleError::NotFound(id))?;
        entry.state = MerkleEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<MerkleEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<MerkleEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: MerkleEntryState) -> Vec<MerkleEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, MerkleEntryState::Completed | MerkleEntryState::Failed | MerkleEntryState::Cancelled | MerkleEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(MerkleEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> MerkleStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<MerkleEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: MerkleEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = MerkleState::new(MerkleConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = MerkleConfig { retry_count: 2, ..Default::default() };
        let state = MerkleState::new(config);
