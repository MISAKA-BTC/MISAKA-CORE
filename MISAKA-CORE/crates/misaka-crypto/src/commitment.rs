//! Cryptographic commitment schemes for privacy
//!
...
/// Commitment operational state.
pub struct CommitmentState {
    config: CommitmentConfig,
    entries: parking_lot::RwLock<HashMap<u64, CommitmentEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<CommitmentTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: CommitmentStats,
    events: parking_lot::Mutex<Vec<CommitmentEvent>>,
...
pub struct CommitmentEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: CommitmentEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct CommitmentTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from Commitment.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl CommitmentStats {
    pub fn snapshot(&self) -> CommitmentStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CommitmentStatsSnapshot {
...
impl CommitmentState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, CommitmentError> {
        ...
        let entry = CommitmentEntry {
            ...
            state: CommitmentEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(CommitmentTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(CommitmentEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), CommitmentError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CommitmentError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = CommitmentEntryState::Completed;
        ...
        self.emit(CommitmentEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, CommitmentError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CommitmentError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = CommitmentEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = CommitmentEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(CommitmentEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), CommitmentError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(CommitmentError::NotFound(id))?;
        entry.state = CommitmentEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<CommitmentEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<CommitmentEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: CommitmentEntryState) -> Vec<CommitmentEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, CommitmentEntryState::Completed | CommitmentEntryState::Failed | CommitmentEntryState::Cancelled | CommitmentEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(CommitmentEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> CommitmentStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<CommitmentEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: CommitmentEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = CommitmentState::new(CommitmentConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = CommitmentConfig { retry_count: 2, ..Default::default() };
        let state = CommitmentState::new(config);
