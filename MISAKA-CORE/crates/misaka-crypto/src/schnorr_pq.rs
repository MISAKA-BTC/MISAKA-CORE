//! Post-quantum Schnorr-like signature scheme adapter
//!
...
/// Configuration for SchnorrPq.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SchnorrPqConfig {
    pub enabled: bool,
...
impl Default for SchnorrPqConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// SchnorrPq operational state.
pub struct SchnorrPqState {
    config: SchnorrPqConfig,
    entries: parking_lot::RwLock<HashMap<u64, SchnorrPqEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<SchnorrPqTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SchnorrPqStats,
    events: parking_lot::Mutex<Vec<SchnorrPqEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in SchnorrPq.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SchnorrPqEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: SchnorrPqEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct SchnorrPqTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from SchnorrPq.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SchnorrPqEvent {
...
/// Statistics for SchnorrPq.
#[derive(Debug, Default)]
pub struct SchnorrPqStats {
...
impl SchnorrPqStats {
    pub fn snapshot(&self) -> SchnorrPqStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SchnorrPqStatsSnapshot {
...
impl SchnorrPqState {
    pub fn new(config: SchnorrPqConfig) -> Self {
        Self {
            config,
            ...
            stats: SchnorrPqStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, SchnorrPqError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(SchnorrPqError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = SchnorrPqEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: SchnorrPqEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(SchnorrPqTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(SchnorrPqEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), SchnorrPqError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SchnorrPqError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = SchnorrPqEntryState::Completed;
        ...
        self.emit(SchnorrPqEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, SchnorrPqError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SchnorrPqError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = SchnorrPqEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = SchnorrPqEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(SchnorrPqEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), SchnorrPqError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SchnorrPqError::NotFound(id))?;
        entry.state = SchnorrPqEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<SchnorrPqEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<SchnorrPqEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: SchnorrPqEntryState) -> Vec<SchnorrPqEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, SchnorrPqEntryState::Completed | SchnorrPqEntryState::Failed | SchnorrPqEntryState::Cancelled | SchnorrPqEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(SchnorrPqEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> SchnorrPqStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<SchnorrPqEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: SchnorrPqEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = SchnorrPqState::new(SchnorrPqConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = SchnorrPqConfig { retry_count: 2, ..Default::default() };
        let state = SchnorrPqState::new(config);
