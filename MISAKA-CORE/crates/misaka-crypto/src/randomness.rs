...
/// Randomness operational state.
pub struct RandomnessState {
    config: RandomnessConfig,
    entries: parking_lot::RwLock<HashMap<u64, RandomnessEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<RandomnessTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: RandomnessStats,
    events: parking_lot::Mutex<Vec<RandomnessEvent>>,
...
pub struct RandomnessEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: RandomnessEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct RandomnessTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from Randomness.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum RandomnessEvent {
...
impl RandomnessStats {
    pub fn snapshot(&self) -> RandomnessStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RandomnessStatsSnapshot {
...
impl RandomnessState {
    pub fn new(config: RandomnessConfig) -> Self {
        Self {
            config,
            ...
            stats: RandomnessStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, RandomnessError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(RandomnessError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = RandomnessEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: RandomnessEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(RandomnessTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(RandomnessEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), RandomnessError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RandomnessError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = RandomnessEntryState::Completed;
        ...
        self.emit(RandomnessEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, RandomnessError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RandomnessError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = RandomnessEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = RandomnessEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(RandomnessEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), RandomnessError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RandomnessError::NotFound(id))?;
        entry.state = RandomnessEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<RandomnessEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<RandomnessEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: RandomnessEntryState) -> Vec<RandomnessEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, RandomnessEntryState::Completed | RandomnessEntryState::Failed | RandomnessEntryState::Cancelled | RandomnessEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(RandomnessEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> RandomnessStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<RandomnessEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: RandomnessEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = RandomnessState::new(RandomnessConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = RandomnessConfig { retry_count: 2, ..Default::default() };
        let state = RandomnessState::new(config);
