...
pub fn sanitize_payload(payload: &[u8], max_size: usize) -> Result<&[u8], SanitizeError> {
    if payload.len() > max_size {
        return Err(SanitizeError::PayloadTooLarge { size: payload.len(), max: max_size });
    }
    Ok(payload)
}
...
pub fn sanitize_script(script: &[u8], max_size: usize) -> Result<&[u8], SanitizeError> {
    if script.len() > max_size {
        return Err(SanitizeError::ScriptTooLarge { size: script.len(), max: max_size });
    }
...
pub fn sanitize_u64(input: &str) -> Result<u64, SanitizeError> {
    let trimmed = input.trim();
    trimmed.parse::<u64>().map_err(|_| SanitizeError::InvalidNumber(trimmed.to_string()))
}
