...
impl CompoundedSubscription {
    ...
    pub fn unsubscribe(&mut self, listener: ListenerId, event: &EventType) {
        if let Some(listeners) = self.by_event.get_mut(event) { listeners.remove(&listener); }
        if let Some(events) = self.by_listener.get_mut(&listener) { events.remove(event); }
    }
    ...
    pub fn events_for(&self, listener: &ListenerId) -> impl Iterator<Item = &EventType> {
        self.by_listener.get(listener).into_iter().flat_map(|s| s.iter())
    }
}
