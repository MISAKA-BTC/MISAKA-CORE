//! Notification flow control — backpressure and batching
//!
...
/// Configuration for the FlowControl subsystem.
...
/// Runtime state for FlowControl.
pub struct FlowControlRuntime {
    config: FlowControlConfig,
    entries: parking_lot::RwLock<HashMap<u64, FlowControlEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: FlowControlStats,
    event_buffer: parking_lot::Mutex<Vec<FlowControlEvent>>,
}

/// Individual entry tracked by FlowControl.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for FlowControl.
...
impl FlowControlStats {
    pub fn snapshot(&self) -> FlowControlStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        FlowControlStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowControlStatsSnapshot {
...
impl FlowControlRuntime {
    /// Create a new FlowControl runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, FlowControlError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(FlowControlError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, FlowControlEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(FlowControlEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), FlowControlError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowControlError::NotFound(id))?;
        ...
        self.emit_event(FlowControlEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, FlowControlError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowControlError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(FlowControlEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<FlowControlEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<FlowControlEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(FlowControlEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> FlowControlStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<FlowControlEvent> {
    ...
    fn emit_event(&self, event: FlowControlEvent) {
...
/// Errors from FlowControl operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = FlowControlRuntime::new(FlowControlConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = FlowControlConfig { retry_count: 2, ..Default::default() };
        let rt = FlowControlRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = FlowControlConfig { max_entries: 2, ..Default::default() };
        let rt = FlowControlRuntime::new(config);
