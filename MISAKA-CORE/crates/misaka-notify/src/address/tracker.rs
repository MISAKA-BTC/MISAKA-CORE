...
impl AddressTracker {
    ...
    pub fn register(&mut self, listener: ListenerId, addresses: Vec<String>) -> Result<(), AddressTrackError> {
        ...
        for addr in addresses {
            self.by_address.entry(addr.clone()).or_default().insert(listener);
            current.insert(addr);
        }
        Ok(())
    }
    ...
    pub fn unregister(&mut self, listener: &ListenerId, addresses: &[String]) {
        for addr in addresses {
            if let Some(listeners) = self.by_address.get_mut(addr) {
                ...
                if listeners.is_empty() { self.by_address.remove(addr); }
            }
        }
        if let Some(addrs) = self.by_listener.get_mut(listener) {
            for addr in addresses { addrs.remove(addr); }
        }
    }
    ...
    pub fn listeners_for_address(&self, address: &str) -> Vec<ListenerId> {
        self.by_address.get(address).map_or_else(Vec::new, |s| s.iter().copied().collect())
    }
    ...
    pub fn tracked_address_count(&self) -> usize { self.by_address.len() }
    pub fn listener_count(&self) -> usize { self.by_listener.len() }
}

#[derive(Debug, thiserror::Error)]
pub enum AddressTrackError {
    #[error("too many addresses: {current} + {requested} > {max}")]
    TooManyAddresses { current: usize, requested: usize, max: usize },
}
