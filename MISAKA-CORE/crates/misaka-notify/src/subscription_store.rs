//! Persistent subscription state storage
//!
...
/// Configuration for SubscriptionStore.
...
/// SubscriptionStore operational state.
pub struct SubscriptionStoreState {
    config: SubscriptionStoreConfig,
    entries: parking_lot::RwLock<HashMap<u64, SubscriptionStoreEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<SubscriptionStoreTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: SubscriptionStoreStats,
    events: parking_lot::Mutex<Vec<SubscriptionStoreEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in SubscriptionStore.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubscriptionStoreEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: SubscriptionStoreEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum SubscriptionStoreEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct SubscriptionStoreTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from SubscriptionStore.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum SubscriptionStoreEvent {
...
/// Statistics for SubscriptionStore.
#[derive(Debug, Default)]
pub struct SubscriptionStoreStats {
...
impl SubscriptionStoreStats {
    pub fn snapshot(&self) -> SubscriptionStoreStatsSnapshot {
        ...
        SubscriptionStoreStatsSnapshot {
            total_created: total,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SubscriptionStoreStatsSnapshot {
...
impl SubscriptionStoreState {
    pub fn new(config: SubscriptionStoreConfig) -> Self {
        Self {
            ...
            stats: SubscriptionStoreStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, SubscriptionStoreError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(SubscriptionStoreError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = SubscriptionStoreEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: SubscriptionStoreEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(SubscriptionStoreTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(SubscriptionStoreEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), SubscriptionStoreError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SubscriptionStoreError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = SubscriptionStoreEntryState::Completed;
        ...
        self.emit(SubscriptionStoreEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, SubscriptionStoreError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SubscriptionStoreError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = SubscriptionStoreEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = SubscriptionStoreEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(SubscriptionStoreEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), SubscriptionStoreError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(SubscriptionStoreError::NotFound(id))?;
        entry.state = SubscriptionStoreEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<SubscriptionStoreEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<SubscriptionStoreEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: SubscriptionStoreEntryState) -> Vec<SubscriptionStoreEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, SubscriptionStoreEntryState::Completed | SubscriptionStoreEntryState::Failed | SubscriptionStoreEntryState::Cancelled | SubscriptionStoreEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(SubscriptionStoreEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> SubscriptionStoreStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<SubscriptionStoreEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: SubscriptionStoreEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = SubscriptionStoreState::new(SubscriptionStoreConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = SubscriptionStoreConfig { retry_count: 2, ..Default::default() };
        let state = SubscriptionStoreState::new(config);
