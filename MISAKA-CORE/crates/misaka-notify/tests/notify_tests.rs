...
async fn test_notifier_basic() {
    ...
    notifier.notify(Notification::BlockAdded(BlockAddedNotification { hash: [1u8; 32] }));
...
async fn test_notifier_scoped() {
    ...
    let (_id, mut rx) = notifier.register_listener(
        Scope::new(&[EventType::BlockAdded])
    );
    ...
    notifier.notify(Notification::VirtualDaaScoreChanged(
        VirtualDaaScoreChangedNotification { virtual_daa_score: 100 }
    ));
...
async fn test_notifier_unregister() {
    ...
    notifier.notify(Notification::BlockAdded(BlockAddedNotification { hash: [0u8; 32] }));
}
...
async fn test_multiple_listeners() {
    ...
    notifier.notify(Notification::SinkBlueScoreChanged(
        SinkBlueScoreChangedNotification { sink_blue_score: 42 }
    ));
