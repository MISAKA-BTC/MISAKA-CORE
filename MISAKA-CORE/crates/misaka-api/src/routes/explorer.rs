...
async fn get_block_detail(
    ...
    if let Ok(height) = hash_or_height.parse::<u64>() {
        return state
            .proxy
            .post("/api/get_block_by_height", &serde_json::json!({ "height": height }))
            .await
            .map(Json)
            .map_err(|e| (StatusCode::BAD_GATEWAY, Json(api_error("UPSTREAM_ERROR", &e.to_string()))));
    }

    if hash_or_height.len() == 64 && hash_or_height.chars().all(|c| c.is_ascii_hexdigit()) {
        return state
            .proxy
            .post("/api/get_block_by_hash", &serde_json::json!({ "hash": hash_or_height }))
            .await
            .map(Json)
            .map_err(|e| (StatusCode::BAD_GATEWAY, Json(api_error("UPSTREAM_ERROR", &e.to_string()))));
    }
    ...
    Err((
        StatusCode::BAD_REQUEST,
        Json(api_error("INVALID_PARAM", "must be a block height (number) or hash (64 hex chars)")),
    ))
}
