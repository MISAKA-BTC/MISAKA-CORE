...
/// Configuration for AddressExchange.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressExchangeConfig {
    pub enabled: bool,
...
impl Default for AddressExchangeConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// AddressExchange operational state.
pub struct AddressExchangeState {
    config: AddressExchangeConfig,
    entries: parking_lot::RwLock<HashMap<u64, AddressExchangeEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<AddressExchangeTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: AddressExchangeStats,
    events: parking_lot::Mutex<Vec<AddressExchangeEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in AddressExchange.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressExchangeEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: AddressExchangeEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum AddressExchangeEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct AddressExchangeTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from AddressExchange.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum AddressExchangeEvent {
...
/// Statistics for AddressExchange.
#[derive(Debug, Default)]
pub struct AddressExchangeStats {
...
impl AddressExchangeStats {
    pub fn snapshot(&self) -> AddressExchangeStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddressExchangeStatsSnapshot {
...
impl AddressExchangeState {
    pub fn new(config: AddressExchangeConfig) -> Self {
        Self {
            config,
            ...
            stats: AddressExchangeStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, AddressExchangeError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(AddressExchangeError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = AddressExchangeEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: AddressExchangeEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(AddressExchangeTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(AddressExchangeEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), AddressExchangeError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressExchangeError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = AddressExchangeEntryState::Completed;
        ...
        self.emit(AddressExchangeEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AddressExchangeError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressExchangeError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = AddressExchangeEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = AddressExchangeEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(AddressExchangeEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), AddressExchangeError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(AddressExchangeError::NotFound(id))?;
        entry.state = AddressExchangeEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<AddressExchangeEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<AddressExchangeEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: AddressExchangeEntryState) -> Vec<AddressExchangeEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, AddressExchangeEntryState::Completed | AddressExchangeEntryState::Failed | AddressExchangeEntryState::Cancelled | AddressExchangeEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(AddressExchangeEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> AddressExchangeStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<AddressExchangeEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: AddressExchangeEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = AddressExchangeState::new(AddressExchangeConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = AddressExchangeConfig { retry_count: 2, ..Default::default() };
        let state = AddressExchangeState::new(config);
