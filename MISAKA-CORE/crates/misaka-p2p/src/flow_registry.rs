//! Protocol flow handler registry
//!
...
/// Configuration for FlowRegistry.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowRegistryConfig {
    pub enabled: bool,
...
impl Default for FlowRegistryConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// FlowRegistry operational state.
pub struct FlowRegistryState {
    config: FlowRegistryConfig,
    entries: parking_lot::RwLock<HashMap<u64, FlowRegistryEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<FlowRegistryTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: FlowRegistryStats,
    events: parking_lot::Mutex<Vec<FlowRegistryEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in FlowRegistry.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowRegistryEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: FlowRegistryEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum FlowRegistryEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct FlowRegistryTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from FlowRegistry.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum FlowRegistryEvent {
...
/// Statistics for FlowRegistry.
#[derive(Debug, Default)]
pub struct FlowRegistryStats {
...
impl FlowRegistryStats {
    pub fn snapshot(&self) -> FlowRegistryStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowRegistryStatsSnapshot {
...
impl FlowRegistryState {
    pub fn new(config: FlowRegistryConfig) -> Self {
        Self {
            config,
            ...
            stats: FlowRegistryStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, FlowRegistryError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(FlowRegistryError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = FlowRegistryEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: FlowRegistryEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(FlowRegistryTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(FlowRegistryEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), FlowRegistryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowRegistryError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = FlowRegistryEntryState::Completed;
        ...
        self.emit(FlowRegistryEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, FlowRegistryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowRegistryError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = FlowRegistryEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = FlowRegistryEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(FlowRegistryEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), FlowRegistryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowRegistryError::NotFound(id))?;
        entry.state = FlowRegistryEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<FlowRegistryEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<FlowRegistryEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: FlowRegistryEntryState) -> Vec<FlowRegistryEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, FlowRegistryEntryState::Completed | FlowRegistryEntryState::Failed | FlowRegistryEntryState::Cancelled | FlowRegistryEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(FlowRegistryEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> FlowRegistryStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<FlowRegistryEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: FlowRegistryEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = FlowRegistryState::new(FlowRegistryConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = FlowRegistryConfig { retry_count: 2, ..Default::default() };
        let state = FlowRegistryState::new(config);
