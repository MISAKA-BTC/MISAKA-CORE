...
impl MessageRateTracker {
    pub fn new() -> Self {
        Self {
            ...
            limits: message_rate_limits().into_iter()
...
impl NonceTracker {
    pub fn new(max_age: Duration, max_entries: usize) -> Self {
        Self { seen: HashMap::new(), max_age, max_entries }
    }
...
pub enum RateCheckResult {
    Ok,
    Exceeded { msg_type: String, count: usize, limit: usize },
}

#[derive(Debug, thiserror::Error)]
pub enum MessageValidationError {
    #[error("{msg_type} too large: {size} > {max}")]
    TooLarge { msg_type: String, size: usize, max: usize },
