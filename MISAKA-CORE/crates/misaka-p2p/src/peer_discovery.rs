//! Peer discovery via DNS seeds and address exchange
//!
...
/// PeerDiscovery operational state.
pub struct PeerDiscoveryState {
    config: PeerDiscoveryConfig,
    entries: parking_lot::RwLock<HashMap<u64, PeerDiscoveryEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<PeerDiscoveryTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: PeerDiscoveryStats,
    events: parking_lot::Mutex<Vec<PeerDiscoveryEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in PeerDiscovery.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerDiscoveryEntry {
    ...
    pub created_at: u64,
    pub updated_at: u64,
    pub state: PeerDiscoveryEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum PeerDiscoveryEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
impl PeerDiscoveryStats {
    pub fn snapshot(&self) -> PeerDiscoveryStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerDiscoveryStatsSnapshot {
...
impl PeerDiscoveryState {
    pub fn new(config: PeerDiscoveryConfig) -> Self {
        Self {
            ...
            stats: PeerDiscoveryStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, PeerDiscoveryError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(PeerDiscoveryError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = PeerDiscoveryEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: PeerDiscoveryEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(PeerDiscoveryTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(PeerDiscoveryEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), PeerDiscoveryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PeerDiscoveryError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = PeerDiscoveryEntryState::Completed;
        ...
        self.emit(PeerDiscoveryEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, PeerDiscoveryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PeerDiscoveryError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = PeerDiscoveryEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = PeerDiscoveryEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(PeerDiscoveryEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), PeerDiscoveryError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(PeerDiscoveryError::NotFound(id))?;
        entry.state = PeerDiscoveryEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<PeerDiscoveryEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<PeerDiscoveryEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: PeerDiscoveryEntryState) -> Vec<PeerDiscoveryEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, PeerDiscoveryEntryState::Completed | PeerDiscoveryEntryState::Failed | PeerDiscoveryEntryState::Cancelled | PeerDiscoveryEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(PeerDiscoveryEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> PeerDiscoveryStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<PeerDiscoveryEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: PeerDiscoveryEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = PeerDiscoveryState::new(PeerDiscoveryConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = PeerDiscoveryConfig { retry_count: 2, ..Default::default() };
        let state = PeerDiscoveryState::new(config);
