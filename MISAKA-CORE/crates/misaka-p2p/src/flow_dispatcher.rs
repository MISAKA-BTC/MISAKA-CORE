//! Dispatch incoming messages to protocol flows
//!
...
/// Configuration for FlowDispatcher.
...
impl Default for FlowDispatcherConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// FlowDispatcher operational state.
pub struct FlowDispatcherState {
    config: FlowDispatcherConfig,
    entries: parking_lot::RwLock<HashMap<u64, FlowDispatcherEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<FlowDispatcherTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: FlowDispatcherStats,
    events: parking_lot::Mutex<Vec<FlowDispatcherEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in FlowDispatcher.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowDispatcherEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: FlowDispatcherEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum FlowDispatcherEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct FlowDispatcherTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from FlowDispatcher.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum FlowDispatcherEvent {
...
/// Statistics for FlowDispatcher.
#[derive(Debug, Default)]
pub struct FlowDispatcherStats {
...
impl FlowDispatcherStats {
    pub fn snapshot(&self) -> FlowDispatcherStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowDispatcherStatsSnapshot {
...
impl FlowDispatcherState {
    pub fn new(config: FlowDispatcherConfig) -> Self {
        Self {
            config,
            ...
            stats: FlowDispatcherStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, FlowDispatcherError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(FlowDispatcherError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = FlowDispatcherEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: FlowDispatcherEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(FlowDispatcherTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(FlowDispatcherEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), FlowDispatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowDispatcherError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = FlowDispatcherEntryState::Completed;
        ...
        self.emit(FlowDispatcherEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, FlowDispatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowDispatcherError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = FlowDispatcherEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = FlowDispatcherEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(FlowDispatcherEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), FlowDispatcherError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(FlowDispatcherError::NotFound(id))?;
        entry.state = FlowDispatcherEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<FlowDispatcherEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<FlowDispatcherEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: FlowDispatcherEntryState) -> Vec<FlowDispatcherEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, FlowDispatcherEntryState::Completed | FlowDispatcherEntryState::Failed | FlowDispatcherEntryState::Cancelled | FlowDispatcherEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(FlowDispatcherEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> FlowDispatcherStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<FlowDispatcherEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: FlowDispatcherEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = FlowDispatcherState::new(FlowDispatcherConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = FlowDispatcherConfig { retry_count: 2, ..Default::default() };
        let state = FlowDispatcherState::new(config);
