//! Transaction and block relay service
//!
...
/// Configuration for RelayService.
...
/// RelayService operational state.
pub struct RelayServiceState {
    config: RelayServiceConfig,
    entries: parking_lot::RwLock<HashMap<u64, RelayServiceEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<RelayServiceTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: RelayServiceStats,
    events: parking_lot::Mutex<Vec<RelayServiceEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in RelayService.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RelayServiceEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: RelayServiceEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum RelayServiceEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct RelayServiceTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from RelayService.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Statistics for RelayService.
#[derive(Debug, Default)]
pub struct RelayServiceStats {
...
impl RelayServiceStats {
    pub fn snapshot(&self) -> RelayServiceStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RelayServiceStatsSnapshot {
...
impl RelayServiceState {
    pub fn new(config: RelayServiceConfig) -> Self {
        Self {
            ...
            stats: RelayServiceStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, RelayServiceError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(RelayServiceError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = RelayServiceEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: RelayServiceEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(RelayServiceTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(RelayServiceEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), RelayServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RelayServiceError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = RelayServiceEntryState::Completed;
        ...
        self.emit(RelayServiceEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, RelayServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RelayServiceError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = RelayServiceEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = RelayServiceEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(RelayServiceEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), RelayServiceError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(RelayServiceError::NotFound(id))?;
        entry.state = RelayServiceEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<RelayServiceEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<RelayServiceEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: RelayServiceEntryState) -> Vec<RelayServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, RelayServiceEntryState::Completed | RelayServiceEntryState::Failed | RelayServiceEntryState::Cancelled | RelayServiceEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(RelayServiceEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> RelayServiceStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<RelayServiceEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: RelayServiceEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = RelayServiceState::new(RelayServiceConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = RelayServiceConfig { retry_count: 2, ..Default::default() };
        let state = RelayServiceState::new(config);
