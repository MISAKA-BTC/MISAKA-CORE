...
use tokio::sync::mpsc::{
    channel as mpsc_channel, Receiver as MpscReceiver, Sender as MpscSender,
};
use tokio::sync::oneshot::{channel as oneshot_channel, Sender as OneshotSender};
use tracing::{debug, error, info, trace, warn};

use crate::payload_type::{
    MisakaMessage, MisakaPayloadType, OverflowPolicy, BLANK_ROUTE_ID,
};
...
impl Router {
    ...
    pub async fn enqueue(&self, msg: MisakaMessage) -> Result<(), ProtocolError> {
        match self.outgoing_route.try_send(msg) {
            ...
            Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                Err(ProtocolError::OutgoingRouteCapacityReached(self.to_string()))
            }
        }
    }
    ...
    pub async fn close(self: &Arc<Router>) -> bool {
        {
            ...
            if let Some(tx) = state.shutdown_signal.take() {
                ...
                trace!(
                    "P2P Router::close called again, peer={}",
                    self.identity()
                );
                return false;
            }
