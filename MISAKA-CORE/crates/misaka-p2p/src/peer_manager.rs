...
impl PeerManagerRuntime {
    ...
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, PeerManagerError> {
        ...
        self.entries.write().insert(id, PeerManagerEntry {
            id,
            created_at: now,
            updated_at: now,
    ...
    pub fn complete_entry(&self, id: u64) -> Result<(), PeerManagerError> {
        ...
        self.emit_event(PeerManagerEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }
...
mod tests {
    ...
    fn test_retry_and_fail() {
        let config = PeerManagerConfig { retry_count: 2, ..Default::default() };
    ...
    fn test_capacity_limit() {
        let config = PeerManagerConfig { max_entries: 2, ..Default::default() };
        let rt = PeerManagerRuntime::new(config);
