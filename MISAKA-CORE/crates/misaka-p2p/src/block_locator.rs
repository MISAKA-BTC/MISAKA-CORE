//! Block locator construction for IBD header requests
//!
...
/// Configuration for BlockLocator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockLocatorConfig {
    pub enabled: bool,
...
impl Default for BlockLocatorConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// BlockLocator operational state.
pub struct BlockLocatorState {
    config: BlockLocatorConfig,
    entries: parking_lot::RwLock<HashMap<u64, BlockLocatorEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<BlockLocatorTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: BlockLocatorStats,
    events: parking_lot::Mutex<Vec<BlockLocatorEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in BlockLocator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockLocatorEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: BlockLocatorEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum BlockLocatorEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct BlockLocatorTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from BlockLocator.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BlockLocatorEvent {
...
/// Statistics for BlockLocator.
#[derive(Debug, Default)]
pub struct BlockLocatorStats {
...
impl BlockLocatorStats {
    pub fn snapshot(&self) -> BlockLocatorStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlockLocatorStatsSnapshot {
...
impl BlockLocatorState {
    pub fn new(config: BlockLocatorConfig) -> Self {
        Self {
            config,
            ...
            stats: BlockLocatorStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, BlockLocatorError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(BlockLocatorError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = BlockLocatorEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: BlockLocatorEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(BlockLocatorTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(BlockLocatorEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), BlockLocatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockLocatorError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = BlockLocatorEntryState::Completed;
        ...
        self.emit(BlockLocatorEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BlockLocatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockLocatorError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = BlockLocatorEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = BlockLocatorEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(BlockLocatorEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), BlockLocatorError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(BlockLocatorError::NotFound(id))?;
        entry.state = BlockLocatorEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<BlockLocatorEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<BlockLocatorEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: BlockLocatorEntryState) -> Vec<BlockLocatorEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, BlockLocatorEntryState::Completed | BlockLocatorEntryState::Failed | BlockLocatorEntryState::Cancelled | BlockLocatorEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(BlockLocatorEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> BlockLocatorStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<BlockLocatorEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: BlockLocatorEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = BlockLocatorState::new(BlockLocatorConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = BlockLocatorConfig { retry_count: 2, ..Default::default() };
        let state = BlockLocatorState::new(config);
