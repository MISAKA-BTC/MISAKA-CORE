//! Inventory message management for relay
//!
...
/// Configuration for InvManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InvManagerConfig {
    pub enabled: bool,
...
impl Default for InvManagerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            max_entries: 50_000,
...
/// InvManager operational state.
pub struct InvManagerState {
    config: InvManagerConfig,
    entries: parking_lot::RwLock<HashMap<u64, InvManagerEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<InvManagerTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: InvManagerStats,
    events: parking_lot::Mutex<Vec<InvManagerEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in InvManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InvManagerEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: InvManagerEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
/// Queued task.
struct InvManagerTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from InvManager.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum InvManagerEvent {
...
/// Statistics for InvManager.
#[derive(Debug, Default)]
pub struct InvManagerStats {
...
impl InvManagerStats {
    pub fn snapshot(&self) -> InvManagerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InvManagerStatsSnapshot {
...
impl InvManagerState {
    pub fn new(config: InvManagerConfig) -> Self {
        Self {
            config,
            ...
            stats: InvManagerStats::default(),
            events: parking_lot::Mutex::new(Vec::new()),
            is_running: std::sync::atomic::AtomicBool::new(false),
        }
    }

    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, InvManagerError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(InvManagerError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = InvManagerEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: InvManagerEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(InvManagerTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(InvManagerEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), InvManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(InvManagerError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = InvManagerEntryState::Completed;
        ...
        self.emit(InvManagerEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, InvManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(InvManagerError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = InvManagerEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = InvManagerEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(InvManagerEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), InvManagerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(InvManagerError::NotFound(id))?;
        entry.state = InvManagerEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<InvManagerEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<InvManagerEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: InvManagerEntryState) -> Vec<InvManagerEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, InvManagerEntryState::Completed | InvManagerEntryState::Failed | InvManagerEntryState::Cancelled | InvManagerEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(InvManagerEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> InvManagerStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<InvManagerEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: InvManagerEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = InvManagerState::new(InvManagerConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = InvManagerConfig { retry_count: 2, ..Default::default() };
        let state = InvManagerState::new(config);
