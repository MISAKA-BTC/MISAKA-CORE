//! Route P2P messages to appropriate flow handlers
//!
...
/// MessageRouter operational state.
pub struct MessageRouterState {
    config: MessageRouterConfig,
    entries: parking_lot::RwLock<HashMap<u64, MessageRouterEntry>>,
    index: parking_lot::RwLock<HashMap<String, Vec<u64>>>,
    queue: parking_lot::Mutex<VecDeque<MessageRouterTask>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: MessageRouterStats,
    events: parking_lot::Mutex<Vec<MessageRouterEvent>>,
    is_running: std::sync::atomic::AtomicBool,
}

/// Entry in MessageRouter.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MessageRouterEntry {
    pub id: u64,
    pub key: String,
    pub created_at: u64,
    pub updated_at: u64,
    pub state: MessageRouterEntryState,
    pub priority: u32,
    pub retry_count: u32,
    pub data: Vec<u8>,
...
pub enum MessageRouterEntryState {
    Queued,
    Processing,
    Completed,
    Failed,
...
/// Queued task.
struct MessageRouterTask {
    entry_id: u64,
    scheduled_at: u64,
    priority: u32,
}

/// Events from MessageRouter.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl MessageRouterStats {
    pub fn snapshot(&self) -> MessageRouterStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MessageRouterStatsSnapshot {
...
impl MessageRouterState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, MessageRouterError> {
        if self.entries.read().len() >= self.config.max_entries {
            return Err(MessageRouterError::CapacityExceeded);
        }
        let id = self.next_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let now = now_secs();
        let entry = MessageRouterEntry {
            id, key: key.clone(), created_at: now, updated_at: now,
            state: MessageRouterEntryState::Queued, priority, retry_count: 0,
            data, metadata: HashMap::new(), dependencies: Vec::new(), tags: Vec::new(),
        };
        self.entries.write().insert(id, entry);
        self.index.write().entry(key.clone()).or_default().push(id);
        self.queue.lock().push_back(MessageRouterTask { entry_id: id, scheduled_at: now, priority });
        ...
        self.emit(MessageRouterEvent::Created { id, key });
        Ok(id)
    }

    pub fn complete(&self, id: u64) -> Result<(), MessageRouterError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MessageRouterError::NotFound(id))?;
        let duration = now_secs().saturating_sub(entry.created_at) * 1000;
        let _old_state = format!("{:?}", entry.state);
        entry.state = MessageRouterEntryState::Completed;
        ...
        self.emit(MessageRouterEvent::Completed { id, duration_ms: duration });
        Ok(())
    }

    pub fn fail(&self, id: u64, reason: String) -> Result<bool, MessageRouterError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MessageRouterError::NotFound(id))?;
        entry.retry_count += 1;
        entry.updated_at = now_secs();
        ...
        if will_retry {
            entry.state = MessageRouterEntryState::Retrying;
            self.stats.total_retried.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        } else {
            entry.state = MessageRouterEntryState::Failed;
            self.stats.total_failed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
        }
        self.emit(MessageRouterEvent::Failed { id, reason, will_retry });
        Ok(will_retry)
    }

    pub fn cancel(&self, id: u64) -> Result<(), MessageRouterError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(MessageRouterError::NotFound(id))?;
        entry.state = MessageRouterEntryState::Cancelled;
        entry.updated_at = now_secs();
    ...
    pub fn get(&self, id: u64) -> Option<MessageRouterEntry> { self.entries.read().get(&id).cloned() }
    pub fn get_by_key(&self, key: &str) -> Vec<MessageRouterEntry> {
        let index = self.index.read();
        let entries = self.entries.read();
        index.get(key).map(|ids| ids.iter().filter_map(|id| entries.get(id).cloned()).collect()).unwrap_or_default()
    }
    pub fn by_state(&self, state: MessageRouterEntryState) -> Vec<MessageRouterEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries.iter()
            .filter(|(_, e)| matches!(e.state, MessageRouterEntryState::Completed | MessageRouterEntryState::Failed | MessageRouterEntryState::Cancelled | MessageRouterEntryState::Expired) && now.saturating_sub(e.updated_at) > timeout)
            .map(|(id, _)| *id)
            .collect();
        for id in &to_remove { entries.remove(id); }
        let removed = to_remove.len();
        if removed > 0 { self.emit(MessageRouterEvent::Cleanup { removed, remaining: entries.len() }); }
        removed
    }
    pub fn stats(&self) -> MessageRouterStatsSnapshot { self.stats.snapshot() }
    pub fn drain_events(&self) -> Vec<MessageRouterEvent> { std::mem::take(&mut *self.events.lock()) }
    ...
    fn emit(&self, event: MessageRouterEvent) {
...
mod tests {
    ...
    #[test]
    fn test_lifecycle() {
        let state = MessageRouterState::new(MessageRouterConfig::default());
    ...
    #[test]
    fn test_retry() {
        let config = MessageRouterConfig { retry_count: 2, ..Default::default() };
        let state = MessageRouterState::new(config);
