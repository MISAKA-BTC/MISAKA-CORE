...
impl TxRelayFlow {
    ...
    fn parse_tx_inv(payload: &[u8]) -> Result<Vec<Hash>, ProtocolError> {
        ...
        if count > MAX_TX_INV {
            return Err(ProtocolError::ProtocolViolation(
                format!("tx inv contains {} IDs (max {})", count, MAX_TX_INV),
            ));
        }
        ...
        loop {
            ...
            match msg.msg_type {
                ...
                MisakaPayloadType::RequestTransactions => {
                    ...
                    // For now, send TransactionNotFound for all.
                    // TODO: Wire to actual mempool lookup.
                    for id in tx_ids {
                ...
                MisakaPayloadType::TransactionNotFound => {
                    ...
                    trace!(
                        "P2P tx not found response from {}",
                        self.router
                    );
                }
                ...
                other => {
                    warn!(
                        "P2P TxRelayFlow unexpected message {:?}",
                        other
                    );
                }
            }
