...
/// Timeout waiting for a requested block.
...
impl BlockRelayFlow {
    ...
    fn parse_inv_hashes(payload: &[u8]) -> Result<Vec<Hash>, ProtocolError> {
        ...
        if count > MAX_INV_PER_MSG {
            return Err(ProtocolError::ProtocolViolation(
                format!("inv contains {} hashes (max {})", count, MAX_INV_PER_MSG),
            ));
        }
        ...
        loop {
            ...
            match msg.msg_type {
                ...
                MisakaPayloadType::RelayBlock => {
                    ...
                    let enqueued = self.ctx.process_queue.lock().push(
                        hash,
                        block_data,
                        peer_key,
                    );
                ...
                MisakaPayloadType::NewBlockTemplate => {
                    ...
                    debug!(
                        "P2P received NewBlockTemplate from {}",
                        self.router
                    );
                ...
                other => {
                    warn!(
                        "P2P BlockRelayFlow unexpected message {:?}",
                        other
                    );
                }
            }
