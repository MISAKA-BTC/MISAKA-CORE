//! P2P protocol message handler dispatch
//!
...
/// Configuration for the ProtocolHandler subsystem.
...
/// Runtime state for ProtocolHandler.
pub struct ProtocolHandlerRuntime {
    config: ProtocolHandlerConfig,
    entries: parking_lot::RwLock<HashMap<u64, ProtocolHandlerEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ProtocolHandlerStats,
    event_buffer: parking_lot::Mutex<Vec<ProtocolHandlerEvent>>,
}

/// Individual entry tracked by ProtocolHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Events emitted by ProtocolHandler.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ProtocolHandlerEvent {
    EntryCreated { id: u64 },
...
/// Operational statistics for ProtocolHandler.
...
impl ProtocolHandlerStats {
    pub fn snapshot(&self) -> ProtocolHandlerStatsSnapshot {
        use std::sync::atomic::Ordering::Relaxed;
        ProtocolHandlerStatsSnapshot {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProtocolHandlerStatsSnapshot {
...
impl ProtocolHandlerRuntime {
    /// Create a new ProtocolHandler runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, ProtocolHandlerError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(ProtocolHandlerError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, ProtocolHandlerEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(ProtocolHandlerEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), ProtocolHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ProtocolHandlerError::NotFound(id))?;
        ...
        self.emit_event(ProtocolHandlerEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, ProtocolHandlerError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ProtocolHandlerError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(ProtocolHandlerEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<ProtocolHandlerEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<ProtocolHandlerEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(ProtocolHandlerEvent::CleanupCompleted { removed });
        }
        removed
    }

    /// Get current statistics.
    pub fn stats(&self) -> ProtocolHandlerStatsSnapshot {
        self.stats.snapshot()
    }

    /// Drain pending events.
    pub fn drain_events(&self) -> Vec<ProtocolHandlerEvent> {
    ...
    fn emit_event(&self, event: ProtocolHandlerEvent) {
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = ProtocolHandlerRuntime::new(ProtocolHandlerConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = ProtocolHandlerConfig { retry_count: 2, ..Default::default() };
        let rt = ProtocolHandlerRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = ProtocolHandlerConfig { max_entries: 2, ..Default::default() };
        let rt = ProtocolHandlerRuntime::new(config);
