...
impl Hub {
    ...
    async fn insert_router(&self, router: Arc<Router>) {
        ...
        if let Some(old) = prev {
            ...
            old.close().await;
            warn!(
                "P2P Hub replaced duplicate peer: {}",
                old.key()
            );
        }
    }
    ...
    pub async fn broadcast_to_some(
        &self,
        msg: MisakaMessage,
        num_peers: usize,
    ) {
    ...
    pub async fn broadcast_many(
        &self,
        msgs: Vec<MisakaMessage>,
        exclude: Option<PeerKey>,
    ) {
        if msgs.is_empty() {
            return;
    ...
    pub async fn terminate_all(&self) {
        let all: Vec<Arc<Router>> =
            self.peers.write().drain().map(|(_, r)| r).collect();
        for r in all {
            r.close().await;
        }
    }
    ...
    pub fn inbound_count(&self) -> usize {
        let peers = self.peers.read();
        peers.len().saturating_sub(
            peers.values().filter(|r| r.is_outbound()).count(),
        )
    }
