//! P2P connection pool with health monitoring
//!
...
/// Configuration for the ConnectionPool subsystem.
...
/// Runtime state for ConnectionPool.
pub struct ConnectionPoolRuntime {
    config: ConnectionPoolConfig,
    entries: parking_lot::RwLock<HashMap<u64, ConnectionPoolEntry>>,
    next_id: std::sync::atomic::AtomicU64,
    stats: ConnectionPoolStats,
    event_buffer: parking_lot::Mutex<Vec<ConnectionPoolEvent>>,
}

/// Individual entry tracked by ConnectionPool.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Events emitted by ConnectionPool.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Operational statistics for ConnectionPool.
...
impl ConnectionPoolRuntime {
    /// Create a new ConnectionPool runtime.
    ...
    /// Create a new entry.
    pub fn create_entry(&self, data: Vec<u8>) -> Result<u64, ConnectionPoolError> {
        let entries = self.entries.read();
        if entries.len() >= self.config.max_entries {
            return Err(ConnectionPoolError::CapacityExceeded {
                current: entries.len(),
        ...
        self.entries.write().insert(id, ConnectionPoolEntry {
            id,
            created_at: now,
            updated_at: now,
        ...
        self.emit_event(ConnectionPoolEvent::EntryCreated { id });
        Ok(id)
    }

    /// Complete an entry successfully.
    pub fn complete_entry(&self, id: u64) -> Result<(), ConnectionPoolError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ConnectionPoolError::NotFound(id))?;
        ...
        self.emit_event(ConnectionPoolEvent::EntryCompleted { id, duration_ms: duration });
        Ok(())
    }

    /// Mark an entry as failed.
    pub fn fail_entry(&self, id: u64, reason: String) -> Result<bool, ConnectionPoolError> {
        let mut entries = self.entries.write();
        let entry = entries.get_mut(&id).ok_or(ConnectionPoolError::NotFound(id))?;
        ...
        if entry.retry_count >= self.config.retry_count {
            ...
            self.stats.active_count.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            self.emit_event(ConnectionPoolEvent::EntryFailed { id, reason });
    ...
    /// Get an entry by ID.
    pub fn get_entry(&self, id: u64) -> Option<ConnectionPoolEntry> {
        self.entries.read().get(&id).cloned()
    }

    /// Get all entries in a given state.
    pub fn entries_by_state(&self, state: EntryState) -> Vec<ConnectionPoolEntry> {
    ...
    pub fn cleanup_expired(&self) -> usize {
        ...
        if removed > 0 {
            self.emit_event(ConnectionPoolEvent::CleanupCompleted { removed });
        }
        removed
    }
    ...
    pub fn drain_events(&self) -> Vec<ConnectionPoolEvent> {
    ...
    fn emit_event(&self, event: ConnectionPoolEvent) {
...
/// Errors from ConnectionPool operations.
...
mod tests {
    ...
    fn test_create_and_complete() {
        let rt = ConnectionPoolRuntime::new(ConnectionPoolConfig::default());
    ...
    fn test_retry_and_fail() {
        let config = ConnectionPoolConfig { retry_count: 2, ..Default::default() };
        let rt = ConnectionPoolRuntime::new(config);
    ...
    fn test_capacity_limit() {
        let config = ConnectionPoolConfig { max_entries: 2, ..Default::default() };
        let rt = ConnectionPoolRuntime::new(config);
