...
impl AddressManager {
    ...
    pub fn add_address(&self, addr: SocketAddr, source: AddressSource) -> bool {
        ...
        addresses.insert(addr, AddressEntry {
            addr,
            source,
            last_connected: None,
            last_attempt: None,
    ...
    pub fn add_addresses(&self, addrs: &[SocketAddr], source: AddressSource) -> usize {
        addrs.iter().filter(|a| self.add_address(**a, source)).count()
    }
    ...
    pub fn select_for_outbound(&self, count: usize) -> Vec<SocketAddr> {
        ...
        let mut candidates: Vec<_> = addresses.values()
            .filter(|e| {
                !e.is_banned
                && !connected.contains(&e.addr)
                && e.connection_failures < self.config.max_retries
                && !tried.get(&e.addr).map_or(false, |t| t.elapsed() < self.config.retry_interval)
            })
            .collect();

        // Prefer addresses we've successfully connected to before
        candidates.sort_by(|a, b| {
            b.last_success.cmp(&a.last_success)
    ...
    pub fn known_count(&self) -> usize { self.addresses.read().len() }
    ...
    pub fn banned_count(&self) -> usize {
        self.addresses.read().values().filter(|e| e.is_banned).count()
    }
    ...
    fn evict_oldest(addresses: &mut HashMap<SocketAddr, AddressEntry>) {
        ...
        if let Some(worst) = addresses.values()
    ...
    pub fn cleanup(&self) -> usize {
        ...
        addresses.retain(|_, e| {
            e.last_success.is_some() || e.connection_failures < 10
        });
