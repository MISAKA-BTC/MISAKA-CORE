...
impl ConnectionManager {
    ...
    pub fn get_by_addr(&self, addr: &SocketAddr) -> Option<PeerConnection> {
        ...
        by_addr.get(addr).and_then(|id| self.connections.read().get(id).cloned())
    }
    ...
    pub fn idle_connections(&self) -> Vec<u64> {
        ...
        connections.values()
            .filter(|c| {
                ...
                && c.last_message_at.map_or(true, |t| t.elapsed() > self.config.idle_timeout)
            })
            .map(|c| c.id)
            .collect()
    }
    ...
    pub fn outbound_count(&self) -> usize {
        self.connections.read().values()
            .filter(|c| c.direction == ConnectionDirection::Outbound && c.state == ConnectionState::Connected)
            .count()
    }

    pub fn inbound_count(&self) -> usize {
        self.connections.read().values()
            .filter(|c| c.direction == ConnectionDirection::Inbound && c.state == ConnectionState::Connected)
            .count()
    }
