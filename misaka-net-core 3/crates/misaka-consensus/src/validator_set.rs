//! Validator set — hybrid signature verification.

use misaka_types::validator::{ValidatorId, ValidatorIdentity, ValidatorSignature};
use misaka_types::error::MisakaError;
use misaka_crypto::hybrid::{HybridPublicKey, HybridSignature, hybrid_verify};
...
impl ValidatorSet {
    ...
    pub fn total_stake(&self) -> u128 {
        self.validators.iter().filter(|v| v.is_active).map(|v| v.stake_weight).sum()
    }
...
mod tests {
    ...
    use misaka_crypto::hybrid::{generate_hybrid_keypair, hybrid_sign, HybridKeypair};

    fn make_validator(id_byte: u8, stake: u128) -> (ValidatorIdentity, HybridKeypair) {
        let kp = generate_hybrid_keypair();
        let mut vid = [0u8; 20]; vid[0] = id_byte;
        (ValidatorIdentity {
            validator_id: vid, stake_weight: stake,
