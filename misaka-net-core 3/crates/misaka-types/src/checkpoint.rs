...
use crate::{Digest, Epoch, CheckpointSeq};
...
pub fn compute_tx_digest_merkle_root(tx_digests: &[Digest]) -> Digest {
    ...
    let mut layer: Vec<Digest> = tx_digests.iter().map(|d| {
        let mut h = Sha3_256::new();
        h.update(d);
        h.finalize().into()
    }).collect();
...
mod tests {
    ...
    fn test_checkpoint_digest_deterministic() {
        let cp = CheckpointSummary {
            epoch: 1, sequence_number: 42, content_digest: [0xAA; 32],
            previous_digest: [0xBB; 32], timestamp_ms: 1700000000000,
            tx_count: 10, state_root: [0xCC; 32],
        };
    ...
    fn test_merkle_root_single() {
        ...
        let expected: Digest = { let mut h = Sha3_256::new(); h.update(&d); h.finalize().into() };
