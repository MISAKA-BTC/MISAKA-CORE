...
pub fn decode_transaction(data: &[u8]) -> Result<UtxoTransaction, String> {
    ...
    Ok(UtxoTransaction { version, inputs, outputs, fee, extra })
}

/// Compute wire size without actually encoding.
pub fn wire_size(tx: &UtxoTransaction) -> usize {
    let mut sz = 1 + 4; // version + num_inputs
    for inp in &tx.inputs {
...
mod tests {
    ...
    fn test_full_tx_with_real_ring_sig() {
        ...
        let kip = misaka_pqc::ki_proof::prove_key_image(&a, &kps[1].secret_poly, &kps[1].public_poly, &kps[1].key_image).unwrap();
