...
use shielded::{
    create_payment_proof_cmd, export_view_key_cmd, shield_deposit_cmd, shield_withdraw_cmd,
    shielded_status_cmd,
};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Transfer {
            ...
            eprintln!(
                "   For privacy, use: misaka-cli send {} {} --shielded --to-kem-pk <PK>",
                to, amount
            );
        ...
        Commands::ShieldedStatus { rpc } => shielded_status_cmd(&rpc).await?,
        Commands::ShieldedScan {
            wallet,
            rpc,
            from_block,
        } => shielded::shielded_scan_cmd(&wallet, &rpc, from_block).await?,
        Commands::ExportViewKey { wallet } => export_view_key_cmd(&wallet)?,
        Commands::CreatePaymentProof {
            commitment,
            wallet,
            output,
...
fn wallet_info(wallet_path: &str) -> Result<()> {
    ...
    println!(
        "  Transparent: {} MISAKA ({} base units)",
        state.balance as f64 / 1_000_000.0,
        state.balance
    );
    println!(
        "  Shielded:    {} MISAKA ({} base units)",
        state.shielded_balance as f64 / 1_000_000.0,
        state.shielded_balance
    );
    println!(
        "  Total:       {} MISAKA",
        (state.balance + state.shielded_balance) as f64 / 1_000_000.0
    );
