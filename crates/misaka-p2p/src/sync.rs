...
/// M-4 fix: raised from 2 to 3 to resist eclipse attacks with 2 malicious peers.
pub const PRUNING_POINT_QUORUM: usize = 3;
...
pub enum IbdPhase {
    ...
    RequestPruningPoint {
        ...
        proofs_received: HashMap<[u8; 32], Hash>,
    ...
    /// the VirtualState, reconstructing the UTXO set and spent set.
...
pub enum IbdAction {
    ...
    Ban { peer_id: [u8; 32], reason: String },
...
pub struct IbdEngine {
    ...
    peer_penalties: HashMap<[u8; 32], u32>,
}
...
impl IbdEngine {
    ...
    pub fn on_header_validated(&mut self, hash: Hash, valid: bool) -> Vec<IbdAction> {
        let (validated_queue, invalid_headers, peer_has_more, _pruning_point) =
            match &mut self.phase {
                IbdPhase::HeaderSync {
                    validated_queue,
                    invalid_headers,
    ...
    pub fn on_bodies_received(
        ...
        let (next_batch, completed, bodies_applied_total) = {
            ...
            let next_batch: Vec<Hash> = remaining
                .drain(..remaining.len().min(IBD_BODY_BATCH))
                .collect();
    ...
    fn add_peer_penalty(&mut self, peer_id: [u8; 32], points: u32) -> u32 {
...
mod tests {
    ...
    fn test_pruning_proof_invalid_penalizes_peer() {
        ...
        assert!(matches!(engine.phase, IbdPhase::RequestPruningPoint { .. }));
    }
    ...
    fn test_body_fetch_to_steady_relay() {
        ...
        assert!(actions
            .iter()
            .any(|a| matches!(a, IbdAction::CommitIbdState)));
    }
    ...
    fn test_invalid_header_excluded_from_body_fetch() {
        ...
        } else if let IbdPhase::HeaderSync {
            invalid_headers, ..
        } = &engine.phase
        {
