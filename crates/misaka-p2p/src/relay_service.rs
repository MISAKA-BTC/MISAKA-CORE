...
impl RelayServiceState {
    ...
    pub fn create(
        ...
        let entry = RelayServiceEntry {
            ...
            created_at: now,
            updated_at: now,
            state: RelayServiceEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(RelayServiceTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), RelayServiceError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(RelayServiceError::NotFound(id))?;
        ...
        self.emit(RelayServiceEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, RelayServiceError> {
        ...
        self.emit(RelayServiceEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<RelayServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    RelayServiceEntryState::Completed
                        | RelayServiceEntryState::Failed
                        | RelayServiceEntryState::Cancelled
                        | RelayServiceEntryState::Expired
        ...
        if removed > 0 {
            self.emit(RelayServiceEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> RelayServiceStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<RelayServiceEvent> {
