...
impl AddressExchangeState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, AddressExchangeError> {
        ...
        let entry = AddressExchangeEntry {
            ...
            created_at: now,
            updated_at: now,
            state: AddressExchangeEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(AddressExchangeTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), AddressExchangeError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(AddressExchangeError::NotFound(id))?;
        ...
        self.emit(AddressExchangeEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, AddressExchangeError> {
        ...
        self.emit(AddressExchangeEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<AddressExchangeEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    AddressExchangeEntryState::Completed
                        | AddressExchangeEntryState::Failed
                        | AddressExchangeEntryState::Cancelled
                        | AddressExchangeEntryState::Expired
        ...
        if removed > 0 {
            self.emit(AddressExchangeEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> AddressExchangeStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<AddressExchangeEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = AddressExchangeConfig {
            retry_count: 2,
            ..Default::default()
        };
