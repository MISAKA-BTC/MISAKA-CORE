...
impl FlowDispatcherState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, FlowDispatcherError> {
        ...
        let entry = FlowDispatcherEntry {
            ...
            created_at: now,
            updated_at: now,
            state: FlowDispatcherEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(FlowDispatcherTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), FlowDispatcherError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(FlowDispatcherError::NotFound(id))?;
        ...
        self.emit(FlowDispatcherEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, FlowDispatcherError> {
        ...
        self.emit(FlowDispatcherEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<FlowDispatcherEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    FlowDispatcherEntryState::Completed
                        | FlowDispatcherEntryState::Failed
                        | FlowDispatcherEntryState::Cancelled
                        | FlowDispatcherEntryState::Expired
        ...
        if removed > 0 {
            self.emit(FlowDispatcherEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> FlowDispatcherStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<FlowDispatcherEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = FlowDispatcherConfig {
            retry_count: 2,
            ..Default::default()
        };
