...
impl TransactionsSpread {
    ...
    /// MED-5: Check if any peer has reported this TX (dedup for relay).
    pub fn is_known(&self, tx_id: &Hash) -> bool {
        self.known_by.contains_key(tx_id)
    }
...
// ═══════════════════════════════════════════════════════════════
//  CRIT-4: TX Body Cache — serves RequestTransactions from peers
// ═══════════════════════════════════════════════════════════════

const MAX_TX_BODY_CACHE: usize = 4096;

/// Cache of recently seen transaction bodies for P2P relay.
/// When a peer sends us a Transaction message, we store the body here.
/// When another peer sends RequestTransactions, we look up and serve from here.
pub struct TxBodyCache {
...
impl TxBodyCache {
    pub fn new() -> Self {
        Self {
            bodies: HashMap::with_capacity(1024),
            order: VecDeque::with_capacity(1024),
            max_size: MAX_TX_BODY_CACHE,
        }
    }

    /// Store a TX body. Evicts oldest if at capacity.
    pub fn insert(&mut self, tx_id: Hash, body: Vec<u8>) {
        if self.bodies.contains_key(&tx_id) {
            return; // dedup
        }
        if self.order.len() >= self.max_size {
            if let Some(old) = self.order.pop_front() {
                self.bodies.remove(&old);
            }
        }
        self.order.push_back(tx_id);
        self.bodies.insert(tx_id, body);
    }

    /// Look up a TX body by hash.
    pub fn get(&self, tx_id: &Hash) -> Option<&[u8]> {
        self.bodies.get(tx_id).map(|v| v.as_slice())
    }
}

impl Default for TxBodyCache {
...
pub struct FlowContext {
    ...
    /// CRIT-4 FIX: TX body cache for serving RequestTransactions from peers.
    /// Stores serialized TX bodies keyed by TX hash (32 bytes).
    /// Bounded to MAX_TX_BODY_CACHE entries with LRU eviction.
    pub tx_body_cache: Mutex<TxBodyCache>,
...
impl FlowContext {
    pub fn new(config: FlowConfig, hub: Hub) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            tx_body_cache: Mutex::new(TxBodyCache::new()),
