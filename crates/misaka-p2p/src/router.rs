...
impl Router {
    ...
    pub fn subscribe(&self, msg_types: Vec<MisakaPayloadType>) -> Result<IncomingRoute, ProtocolError> {
    ...
    pub fn subscribe_with_capacity(
        ...
        capacity: usize,
    ) -> Result<IncomingRoute, ProtocolError> {
        ...
        for msg_type in msg_types {
            if let Some(_prev) = map_type.insert(msg_type, tx.clone()) {
                ...
                return Err(ProtocolError::DuplicateRoute(format!("{:?}", msg_type)));
            }
        ...
        if map_id.insert(route.id, tx).is_some() {
            ...
            return Err(ProtocolError::DuplicateRoute(format!("route_id={}", route.id)));
        }
