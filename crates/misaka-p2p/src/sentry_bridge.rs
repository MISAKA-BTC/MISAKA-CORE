...
impl SentryBridge {
    ...
    pub fn new(
        ...
        Self,
        mpsc::Sender<SentryOutbound>,
        mpsc::Receiver<SentryInbound>,
    ) {
        ...
        if new_state != self.operational_state {
            match new_state {
                ValidatorOperationalState::Normal => {
                    info!("Validator operational: NORMAL ({} sentries)", live);
                }
                ...
                ValidatorOperationalState::Isolated => {
                    error!(
                        "🚨 Validator ISOLATED: NO sentry connections. Block production HALTED."
                    );
                }
            }
    ...
    pub fn on_sentry_connected(&mut self, peer_id: &PeerId) {
        if let Some(state) = self.sentries.get_mut(peer_id) {
            ...
            info!(
                "Sentry connected: {} ({})",
                state.endpoint.label, state.endpoint.address
            );
        }
        self.update_operational_state();
    }
...
pub fn scrub_hello_for_validator(hello: &mut serde_json::Value, validator_alias: &str) {
...
pub fn scrub_peer_record_for_relay(record: &mut serde_json::Value, validator_peer_ids: &[PeerId]) {
