...
pub fn validate_rbf_replacement(
    ...
    let old_rate = if old_mass > 0 {
        old_fee as f64 / old_mass as f64
    } else {
        0.0
    };
    let new_rate = if new_mass > 0 {
        new_fee as f64 / new_mass as f64
    } else {
        0.0
    };

    if new_rate <= old_rate {
        return Err(MempoolRuleError::RbfViolation(format!(
            "new fee rate {:.2} must exceed old rate {:.2}",
            new_rate, old_rate
        )));
    }
