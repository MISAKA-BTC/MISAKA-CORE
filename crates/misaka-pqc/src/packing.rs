...
use super::pq_ring::{LegacyProofData, Poly, BETA, N, Q};
...
pub fn unpack_challenge(data: &[u8]) -> Result<(Poly, usize), CryptoError> {
    if data.is_empty() {
        return Err(CryptoError::ProofInvalid("empty challenge data".into()));
    }
    let count = data[0] as usize;
    let sign_bytes = (count + 7) / 8;
    let total = 1 + count + sign_bytes;
    if data.len() < total {
        return Err(CryptoError::ProofInvalid("truncated challenge".into()));
    }
