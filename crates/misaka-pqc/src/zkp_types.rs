...
// ═══════════════════════════════════════════════════════════════
//  Witness Zeroization (SEC-AUDIT-V5 HIGH-003)
// ═══════════════════════════════════════════════════════════════

impl Drop for PrivateTxWitness {
    fn drop(&mut self) {
        use zeroize::Zeroize;
        self.fee_value = 0;
        // fee_blinding: BlindingFactor already has Drop+Zeroize
        // input_witnesses / output_witnesses: dropped below
        for w in &mut self.input_witnesses {
            w.secret.coeffs.zeroize();
            w.signer_pk.coeffs.zeroize();
            w.amount = 0;
            // blinding: BlindingFactor already has Drop+Zeroize
            for h in &mut w.leaf_hashes {
                h.zeroize();
            }
        }
        for w in &mut self.output_witnesses {
            w.amount = 0;
        }
    }
}

impl Drop for InputPrivateWitness {
    fn drop(&mut self) {
        use zeroize::Zeroize;
        self.secret.coeffs.zeroize();
        self.signer_pk.coeffs.zeroize();
        self.amount = 0;
        for h in &mut self.leaf_hashes {
            h.zeroize();
        }
    }
}

impl Drop for OutputPrivateWitness {
    fn drop(&mut self) {
        self.amount = 0;
        // blinding: BlindingFactor already has Drop+Zeroize
    }
