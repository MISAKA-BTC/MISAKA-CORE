...
/// Maximum amount that can be committed confidentially.
///
/// # Security (SEC-AUDIT-V5 CRITICAL-001)
///
/// BDLOP encodes the amount as a single polynomial coefficient mod Q.
/// Any amount ≥ Q aliases with `amount mod Q`, breaking the binding
/// property and allowing balance forgery.
///
/// **All confidential amounts MUST be < Q = 12 289.**
pub const MAX_CONFIDENTIAL_AMOUNT: u64 = (Q - 1) as u64; // 12_288
...
impl BlindingFactor {
    ...
    pub fn derive(secret: &[u8], index: u32) -> Result<Self, CryptoError> {
        ...
        hk.expand(&info, &mut expanded)
            .map_err(|_| CryptoError::ProofInvalid("BDLOP blind HKDF failed".into()))?;
...
impl BdlopCommitment {
    ...
    /// # Security (SEC-AUDIT-V5 CRITICAL-001)
    ///
    /// The amount is encoded as a single coefficient mod Q. To prevent
    /// mod-Q aliasing (where `amount` and `amount + k*Q` produce identical
    /// commitments, breaking binding and balance soundness), the amount
    /// MUST be strictly less than Q.
    ///
    /// **MAX_CONFIDENTIAL_AMOUNT = Q − 1 = 12 288.**
    ///
    /// Returns `Err` if amount ≥ Q.
    pub fn commit(
        crs: &BdlopCrs,
        ...
        if amount >= Q as u64 {
            return Err(CryptoError::ProofInvalid(format!(
                "amount {} exceeds MAX_CONFIDENTIAL_AMOUNT ({})",
                amount, MAX_CONFIDENTIAL_AMOUNT,
            )));
        }
        let mut v = Poly::zero();
        v.coeffs[0] = amount as i32; // safe: amount < Q < i32::MAX
    ...
    /// Commit to an arbitrary polynomial (for SIS Merkle tree ZK proofs).
    ///
    /// `C = A₁ · r + A₂ · v`  where `v` is an arbitrary polynomial in R_q.
    ///
    /// # Homomorphic Property
    ///
    /// `commit_poly(v₁, r₁) + commit_poly(v₂, r₂) = commit_poly(v₁+v₂, r₁+r₂)`
    ///
    /// This enables verification of SIS Merkle hash relations:
    /// If `parent = A_m1·left + A_m2·right`, then:
    ///   `A_m1·C(left) + A_m2·C(right) - C(parent) = A₁·r_excess`
    /// where `r_excess` is a known blinding factor combination.
    pub fn commit_poly(crs: &BdlopCrs, blind: &BlindingFactor, value: &Poly) -> Self {
        let a1r = crs.a1.mul(blind.as_poly());
        let a2v = crs.a2.mul(value);
        Self(a1r.add(&a2v))
    }

    /// Scalar multiplication by a polynomial: `p · C = C(p·v, p·r)`
    ///
    /// Used in SIS Merkle verification:
    /// `A_m1·C(left) = commit_poly(A_m1·left, A_m1·r_left)`
    pub fn scalar_mul_poly(&self, scalar: &Poly) -> BdlopCommitment {
        BdlopCommitment(scalar.mul(&self.0))
    }
...
/// # Errors
///
/// Returns `Err` if `fee >= Q` (SEC-AUDIT-V5 CRITICAL-001).
pub fn compute_balance_diff(
    crs: &BdlopCrs,
    input_commitments: &[BdlopCommitment],
    output_commitments: &[BdlopCommitment],
    fee: u64,
) -> Result<BdlopCommitment, CryptoError> {
    if fee >= Q as u64 {
        return Err(CryptoError::ProofInvalid(format!(
            "fee {} exceeds MAX_CONFIDENTIAL_AMOUNT ({})",
            fee, MAX_CONFIDENTIAL_AMOUNT,
        )));
    }
    ...
    fee_poly.coeffs[0] = fee as i32; // safe: fee < Q checked above
...
pub fn verify_balance_with_excess(
    ...
    if !crate::secret::ct_eq_32(&expected_challenge, &r_excess_proof.challenge) {
...
impl BalanceExcessProof {
    ...
    pub fn prove(
        ...
        use crate::pq_ring::{hash_to_challenge, sample_masking_poly, BETA, MAX_SIGN_ATTEMPTS};
        ...
        Err(CryptoError::ProofInvalid(
            "balance proof: max attempts".into(),
        ))
    }
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        if data.len() != 32 + N * 2 {
            return Err(CryptoError::ProofInvalid("balance proof size".into()));
        }
...
mod tests {
    ...
    fn test_commit_different_amounts_differ() {
        ...
        let c1 = BdlopCommitment::commit(&c, &r, 100).unwrap();
        let c2 = BdlopCommitment::commit(&c, &r, 200).unwrap();
    ...
    fn test_commit_different_blinds_differ() {
        ...
        let c1 = BdlopCommitment::commit(&c, &r1, 100).unwrap();
        let c2 = BdlopCommitment::commit(&c, &r2, 100).unwrap();
        assert_ne!(c1.to_bytes(), c2.to_bytes());
    }
    ...
    fn test_homomorphic_addition() {
        ...
        let c1 = BdlopCommitment::commit(&crs, &r1, 100).unwrap();
        let c2 = BdlopCommitment::commit(&crs, &r2, 200).unwrap();
        let c_sum = c1.add(&c2);
        ...
        for i in 0..N {
            ...
            if r_combined.coeffs[i] < 0 {
                r_combined.coeffs[i] += Q;
            }
        }
        let c_direct = BdlopCommitment::commit(&crs, &BlindingFactor(r_combined).unwrap(), 300);
    ...
    fn test_balance_proof_roundtrip() {
        ...
        let c_in = BdlopCommitment::commit(&crs, &r_in, 500).unwrap();
        let c_out = BdlopCommitment::commit(&crs, &r_out, 400).unwrap();
        let fee = 100u64;
    ...
    fn test_serialization_roundtrip() {
        let crs = crs();
        let r = BlindingFactor::random();
        let c = BdlopCommitment::commit(&crs, &r, 42).unwrap();
