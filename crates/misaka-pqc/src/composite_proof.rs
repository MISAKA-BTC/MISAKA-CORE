...
impl CompositeProof {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        serde_json::from_slice(data)
            .map_err(|e| CryptoError::ProofInvalid(format!("CompositeProof deserialize: {}", e)))
    }
...
pub fn verify_composite(
    ...
    // ── 3. Binding digest (anti-transplant, constant-time) ──
    ...
    for (commitment, range_proof) in proof
        .output_commitments
        .iter()
        .zip(proof.range_proofs.iter())
    {
        if verify_range(crs, commitment, range_proof).is_err() {
            return Err(reject());
        }
    }
