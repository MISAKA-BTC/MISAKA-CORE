...
use crate::bdlop::{BdlopCommitment, BdlopCrs};
use crate::error::CryptoError;
use crate::membership::{
    compute_sis_root, prove_membership_v2, sis_leaf, sis_root_hash, verify_membership_v2,
    SisMerkleCrs, ZkMembershipProofV2, MAX_MEMBERSHIP_PROOF_SIZE,
};
use crate::nullifier::{
    canonical_nullifier_hash, derive_nullifier_param_ctx, NullifierContext, OutputId, TxDomain,
};
use crate::pq_ring::{hash_to_challenge, sample_masking_poly, Poly, BETA, MAX_SIGN_ATTEMPTS, N, Q};
use crate::secret::ct_eq_32;
use crate::transcript::{domain, TranscriptBuilder, PROTOCOL_VERSION};
...
impl UnifiedMembershipProof {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        ...
        if data.len() > MAX_UNIFIED_PROOF_SIZE {
            return Err(CryptoError::ProofInvalid(format!(
                "UnifiedProof: exceeds max size ({} > {})",
                data.len(),
                MAX_UNIFIED_PROOF_SIZE
            )));
        }
        ...
        if version != UNIFIED_PROOF_VERSION {
            return Err(CryptoError::ProofInvalid(format!(
                "UnifiedProof: unknown version 0x{:02X} (expected 0x{:02X})",
                version, UNIFIED_PROOF_VERSION
            )));
        }

        // ── 2. Fixed Σ-protocol fields ──
        let sigma_w_pk = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;
        let sigma_w_null = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;
        let response = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;
        let nullifier_poly = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;
        let nullifier_param = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;

        // ── 3. NullifierContext hash ──
        let mut ctx_hash = [0u8; 32];
        ctx_hash.copy_from_slice(&data[off..off + 32]);
        off += 32;

        // ── 3b. P0-1: Key-ownership binding proof ──
        let sigma_w_bind = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;
        let binding_response = Poly::from_bytes(&data[off..off + N * 2])?;
        off += N * 2;
...
pub fn compute_merkle_root(leaf_hashes: &[[u8; 32]]) -> Result<[u8; 32], CryptoError> {
    ...
    if leaf_hashes.is_empty() {
        return Err(CryptoError::ProofInvalid("empty".into()));
    }
...
pub fn unified_prove_ctx(
    ...
    let all_leaf_polys: Vec<Poly> = leaf_hashes
        .iter()
        .map(|lh| {
            let mut p = Poly::zero();
            for (i, chunk) in lh.chunks(2).enumerate() {
                if i < N {
                    p.coeffs[i] =
                        (u16::from_le_bytes([chunk[0], chunk.get(1).copied().unwrap_or(0)]) as i32)
                            % Q;
                }
            }
            p
    ...
    let (membership_proof, r_leaf) =
        prove_membership_v2(&bdlop_crs, &sis_crs, &leaf_polys, signer_index, signer_pk)?;
    ...
    let _a_leaf_poly = &sis_crs.a_leaf;
    ...
    for _ in 0..MAX_SIGN_ATTEMPTS {
        ...
        for i in 0..N {
            let yr_c = if y_r.coeffs[i] > Q / 2 {
                y_r.coeffs[i] - Q
            } else {
                y_r.coeffs[i]
            };
            let cr_c = if cr_leaf.coeffs[i] > Q / 2 {
                cr_leaf.coeffs[i] - Q
            } else {
                cr_leaf.coeffs[i]
            };
    ...
    Err(CryptoError::ProofInvalid(
        "unified_prove: max attempts".into(),
    ))
}
...
pub fn unified_verify(
    ...
    if proof.response.norm_inf() >= BETA {
        return Err(CryptoError::ProofInvalid(format!(
            "response norm {} >= β",
            proof.response.norm_inf()
        )));
    }
    // P0-1: binding response norm check
    if proof.binding_response.norm_inf() >= BETA {
        return Err(CryptoError::ProofInvalid(format!(
            "binding_response norm {} >= β",
            proof.binding_response.norm_inf()
        )));
    }
    ...
    let w_bind_recon = bdlop_crs
        .a1
        .mul(&proof.binding_response)
    ...
    if !ct_eq_32(&proof.membership.sis_root_hash, expected_root_hash) {
        return Err(CryptoError::ProofInvalid("root hash mismatch".into()));
    }
...
fn poly_eq_mod_q(a: &Poly, b: &Poly) -> bool {
    for i in 0..N {
        if ((a.coeffs[i] - b.coeffs[i]) % Q + Q) % Q != 0 {
            return false;
        }
    }
    true
...
mod tests {
    ...
    fn make_ring(size: usize) -> (Poly, Vec<Poly>, Vec<Poly>, Vec<[u8; 32]>) {
        ...
        let root_hash = sis_root_hash(&root_poly);
        let fake_leaves: Vec<[u8; 32]> = leaf_polys
            .iter()
            .map(|p| {
                let mut h = [0u8; 32];
    ...
    fn test_unified_prove_verify_no_ring_pubkeys() {
        ...
        unified_verify(
            &a,
            &proof.membership.sis_root_hash,
            &msg,
            &null_hash,
            &proof,
        )
    ...
    fn test_unified_prove_verify_ctx() {
        ...
        let (proof, null_hash) =
            unified_prove_ctx(&a, &leaves, 0, &secrets[0], &pks[0], &msg, &ctx).unwrap();
    ...
    fn test_wrong_context_rejected() {
        ...
        assert!(
            ...
            .is_err(),
            "wrong NullifierContext must be rejected"
        );
    }
    ...
    fn test_wrong_epoch_rejected() {
        ...
        assert!(
            ...
            .is_err(),
            "wrong anonymity_root_epoch must be rejected"
        );
    }
    ...
    fn test_wrong_domain_rejected() {
        ...
        assert!(
            ...
            .is_err(),
            "wrong tx_domain must be rejected"
        );
    }
    ...
    fn test_nullifier_deterministic() {
        ...
        let (_, h1) =
            unified_prove(&a, &leaves, 0, &secrets[0], &pks[0], &[1; 32], &out, 2).unwrap();
        let (_, h2) =
            unified_prove(&a, &leaves, 0, &secrets[0], &pks[0], &[2; 32], &out, 2).unwrap();
    ...
    fn test_binary_roundtrip() {
        ...
        unified_verify(
            &a,
            &proof2.membership.sis_root_hash,
            &msg,
            &null_hash,
            &proof2,
        )
    ...
    fn test_binary_trailing_bytes_rejected() {
        ...
        assert!(
            UnifiedMembershipProof::from_bytes(&bytes).is_err(),
    ...
    fn test_binary_truncated_rejected() {
        ...
        let bytes = proof.to_bytes();
        assert!(
            UnifiedMembershipProof::from_bytes(&bytes[..bytes.len() - 1]).is_err(),
