...
use crate::bdlop::{BalanceExcessProof, BdlopCommitment};
...
pub struct ConfidentialStealthData {
    ...
    /// Previously [u8; 32] which was insufficient for post-quantum security margins.
    ...
    pub fn validate_structure(&self) -> Result<(), CryptoError> {
        ...
        if self.version != QDAG_VERSION {
            return Err(CryptoError::ProofInvalid(format!(
                "unsupported QDAG version: 0x{:02x}",
                self.version
            )));
        }
        ...
        for (i, inp) in self.inputs.iter().enumerate() {
            if inp.membership_proof.len() > MAX_MEMBERSHIP_PROOF_SIZE {
                return Err(CryptoError::ProofInvalid(format!(
                    "input[{}] membership_proof too large: {} > {}",
                    i,
                    inp.membership_proof.len(),
...
mod tests {
    ...
    fn test_transcript_deterministic() {
        let tx = QdagTransaction {
            ...
            outputs: vec![],
            fee: dummy_fee(),
            balance_proof: dummy_balance_proof(),
            extra: vec![],
        };
    ...
    fn test_transcript_changes_with_chain_id() {
        ...
        assert_ne!(
            t1, t2,
            "different chain_id must produce different transcript"
        );
    }
    ...
    fn test_transcript_changes_with_nullifier() {
        let tx1 = QdagTransaction {
            ...
            chain_id: 2,
            parents: vec![],
            inputs: vec![dummy_input([0x11; 32])],
            outputs: vec![],
            fee: dummy_fee(),
            balance_proof: dummy_balance_proof(),
            extra: vec![],
        ...
        let tx2 = QdagTransaction {
            ...
            chain_id: 2,
            parents: vec![],
            inputs: vec![dummy_input([0x22; 32])],
            outputs: vec![],
            fee: dummy_fee(),
            balance_proof: dummy_balance_proof(),
            extra: vec![],
