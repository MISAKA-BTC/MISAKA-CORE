...
// ═══════════════════════════════════════════════════════════════
//  Constant-Time Comparison (SEC-AUDIT-V5 HIGH-001)
// ═══════════════════════════════════════════════════════════════

/// Constant-time byte-slice comparison.
///
/// Returns `true` iff `a == b` (same length AND same contents).
/// Runs in time proportional to `a.len()` regardless of content,
/// preventing timing side-channel attacks on digest/challenge comparisons.
///
/// # Security
///
/// Uses XOR-accumulation with volatile reads to resist compiler
/// short-circuit optimisations.  For verification paths only --- this
/// is **not** for sorting or ordering.
///
...
pub fn ct_eq(a: &[u8], b: &[u8]) -> bool {
    ...
    let mut acc = 0u8;
    for i in 0..a.len() {
        // Use read_volatile to prevent the compiler from
        // short-circuiting on first mismatch.
        unsafe {
            let x = core::ptr::read_volatile(&a[i]);
            let y = core::ptr::read_volatile(&b[i]);
            acc |= x ^ y;
        }
    }
    acc == 0
...
/// Fixed-size 32-byte constant-time comparison (most common case).
#[inline(never)]
