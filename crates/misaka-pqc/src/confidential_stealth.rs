...
impl CtStealthScanner {
    ...
    pub fn try_recover_verified(
        ...
        // 6. Decrypt amount
        let amount_plain =
            match aead_decrypt(&mat.k_amt, &mat.nonce_amt, &stealth_data.amount_ct, &aad) {
                Ok(pt) => pt,
        ...
        let blind_plain =
            match aead_decrypt(&mat.k_blind, &mat.nonce_blind, &stealth_data.blind_ct, &aad) {
                Ok(pt) => pt,
                Err(_) => return Ok(None),
            };
...
/// Constant-time comparison for 16-byte tags.
///
/// NOTE: This is a local copy matching the pattern in
/// `misaka_security::constant_time::ct_eq`. It is duplicated here
/// to avoid a circular dependency (misaka-pqc cannot depend on
/// misaka-security). A compile-time test in misaka-security verifies
...
mod tests {
    ...
    fn test_confidential_stealth_roundtrip() {
        ...
        let output = create_confidential_stealth(&kp.public_key, amount, &blind, 2).unwrap();

        let scanner = CtStealthScanner::new(kp.secret_key);
        // Audit Fix G: atomic recover + verify
        let recovered = scanner
            .try_recover_verified(&output.stealth_data, &commitment, &crs, 2)
            .unwrap()
            .expect("should recover output for correct key");
    ...
    fn test_wrong_view_key_no_match() {
        ...
        let output = create_confidential_stealth(&sender_kp.public_key, 500, &blind, 2).unwrap();
    ...
    fn test_wrong_chain_id_fails() {
        ...
        let output = create_confidential_stealth(&kp.public_key, 100, &blind, 2).unwrap();

        let scanner = CtStealthScanner::new(kp.secret_key);
        let result = scanner
            .try_recover_verified(&output.stealth_data, &commitment, &crs, 3)
            .unwrap();
    ...
    fn test_commitment_mismatch_is_error() {
        ...
        let scanner = CtStealthScanner::new(kp.secret_key);
        let result = scanner.try_recover_verified(&output.stealth_data, &wrong_commitment, &crs, 2);
        assert!(
            result.is_err(),
    ...
    fn test_tampered_blind_ct_fails() {
        ...
        let mut output = create_confidential_stealth(&kp.public_key, 100, &blind, 2).unwrap();
