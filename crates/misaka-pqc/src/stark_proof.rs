...
impl TxConstraints {
    ...
    pub fn validate(&self) -> Result<(), CryptoError> {
        ...
        if self.output_amounts.len() != self.num_outputs {
            return Err(CryptoError::ProofInvalid("output count mismatch".into()));
        }
        // Output sum consistency
        let actual_sum: u64 = self.output_amounts.iter().sum();
        if actual_sum != self.sum_outputs {
            return Err(CryptoError::ProofInvalid("output sum mismatch".into()));
        }
        Ok(())
    }
...
impl StarkProof {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        ...
        let query_rounds = u32::from_le_bytes(
            data[33..37]
                .try_into()
                .map_err(|_| CryptoError::ProofInvalid("invalid STARK query_rounds".into()))?,
        );
        let proof_len = u32::from_le_bytes(
            data[37..41]
                .try_into()
                .map_err(|_| CryptoError::ProofInvalid("invalid STARK proof length".into()))?,
        ) as usize;
