...
pub fn encode_transaction(tx: &UtxoTransaction) -> Vec<u8> {
    ...
    for inp in &tx.inputs {
        // Ring members
        write_u32(&mut buf, inp.utxo_refs.len() as u32);
        for m in &inp.utxo_refs {
            buf.extend_from_slice(&m.tx_hash);
            write_u32(&mut buf, m.output_index);
        }
        // Lattice ZKP proof (raw bytes, caller can use v2 packing)
        write_u32(&mut buf, inp.proof.len() as u32);
        buf.extend_from_slice(&inp.proof);
        // Key image
    ...
    for out in &tx.outputs {
        ...
        if tx.version >= UTXO_TX_VERSION_V4 {
            ...
            // v10: full 32-byte address on wire (PQ-safe)
            buf.extend_from_slice(&out.one_time_address);
        }
...
pub fn decode_transaction(data: &[u8]) -> Result<UtxoTransaction, String> {
    ...
    let proof_scheme = if off < data.len() {
    ...
    for _ in 0..n_inputs {
        ...
        let mut utxo_refs = Vec::with_capacity(n_members);
        ...
        let proof = read_bytes(data, &mut off, sig_len)?;
    ...
    for _ in 0..n_outputs {
        ...
        if version >= UTXO_TX_VERSION_V4 {
            ...
            one_time_address.copy_from_slice(&legacy);
        }
...
pub fn wire_size(tx: &UtxoTransaction) -> usize {
    let mut sz = 1 + 1 + 1 + 1 + 4; // version + proof_scheme + marker + tx_type + num_inputs
    for inp in &tx.inputs {
        sz += 4; // num_ring_members
        sz += inp.utxo_refs.len() * 36; // 32 + 4 per member
        sz += 4 + inp.proof.len(); // sig
        sz += 32; // key image
...
mod tests {
    ...
    fn test_encode_decode_simple_tx() {
        ...
        assert_eq!(decoded.inputs[0].utxo_refs.len(), 4);
    ...
    fn test_full_tx_with_real_ring_sig() {
        ...
        let sig = pq_sign(&a, &ring_pks, 1, &kps[1].secret_poly, &digest).unwrap();

        // Use v2 compact packing
        tx.inputs[0].proof = pack_ring_sig_v2(&sig);
        ...
        let decoded_sig =
            crate::packing::unpack_legacy_proof_v2(&decoded.inputs[0].proof, 4).unwrap();
        ring_verify(&a, &ring_pks, &digest, &decoded_sig).unwrap();

        println!("\n  ═══ Real TX Wire Size Breakdown ═══");
        println!("  Total wire:    {} bytes", wire_bytes.len());
        println!("  Ring sig (v2): {} bytes", tx.inputs[0].proof.len());
        println!(
            "  Stealth data:  {} bytes",
        ...
        println!(
            ...
            raw_sig_size,
            tx.inputs[0].proof.len(),
            (1.0 - tx.inputs[0].proof.len() as f64 / raw_sig_size as f64) * 100.0
        );
    }
