...
///
/// # Usage by DAG State Manager
///
/// ```ignore
/// // Old (unsafe): accepts any TX, no verification guarantee
/// fn apply_tx(tx: &OrderedTxData) { ... }
///
/// // New (safe): only accepts verified TXs
/// fn apply_tx(envelope: &VerifiedTransactionEnvelope) { ... }
/// ```
...
impl std::fmt::Debug for VerifiedTransactionEnvelope {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "VerifiedTxEnvelope(digest={:?}, nullifiers={})",
            self.tx_digest, self.nullifiers.len())
    }
}
