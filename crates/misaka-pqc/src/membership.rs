...
pub fn compute_sis_root(crs: &SisMerkleCrs, leaves: &[Poly]) -> Result<Poly, CryptoError> {
    if leaves.is_empty() {
        return Err(CryptoError::ProofInvalid("empty leaf set".into()));
    }
    if leaves.len() == 1 {
    ...
    layer
        .into_iter()
        .next()
        .ok_or_else(|| CryptoError::ProofInvalid("SIS root: empty after reduction".into()))
}
...
pub fn prove_membership(
    ...
    for (level, (sib_poly, &is_right)) in siblings.iter().zip(directions.iter()).enumerate() {
        ...
        let _real_branch = if is_right { 1u8 } else { 0u8 };
...
pub fn verify_membership(
    ...
    if !ct_eq_32(&proof.sis_root_hash, expected_root_hash) {
        return Err(CryptoError::ProofInvalid("SIS root hash mismatch".into()));
    }
    ...
    let current_node_comm = proof.leaf_commitment.clone();
...
pub fn verify_membership_v2(
    ...
    if proof.level_proofs.len() > ZKMP_MAX_DEPTH {
        return Err(CryptoError::ProofInvalid("depth exceeded".into()));
    }
    ...
    if proof.root_opening_response.norm_inf() >= BETA {
        return Err(CryptoError::ProofInvalid("root opening norm >= β".into()));
    }
