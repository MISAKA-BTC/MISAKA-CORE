...
use crate::auth::{MethodRateLimiter, TokenManager};
...
impl RpcHandler {
    ...
    fn check_auth(&self, request: &RpcRequest) -> RpcResult<()> {
        ...
        if self.admin_methods.contains(&request.method) {
            let token = request.auth_token.as_deref().ok_or(RpcError::Unauthorized(
                "admin method requires authentication".into(),
            ))?;
