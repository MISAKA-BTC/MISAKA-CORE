...
pub enum AuthConfigError {
    #[error(
        "FATAL: MISAKA_RPC_API_KEY must be set in production (chain_id=1 or MISAKA_RPC_AUTH_MODE=required). Set the key or explicitly set MISAKA_RPC_AUTH_MODE=open for development."
    )]
...
impl ApiKeyState {
    ...
    pub fn from_env_checked(chain_id: u32) -> Result<Self, AuthConfigError> {
        ...
        let auth_mode = std::env::var("MISAKA_RPC_AUTH_MODE")
            .unwrap_or_default()
            .to_lowercase();
        let auth_required = chain_id == 1 || auth_mode == "required" || auth_mode == "production";
