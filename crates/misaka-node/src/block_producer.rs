...
fn verified_tx_to_stored(
    ...
    StoredTx {
        ...
        privacy_scheme_tag: descriptor.map(|d| d.scheme_tag).unwrap_or(tx.proof_scheme),
        privacy_scheme_name: descriptor
            .map(|d| d.scheme_name.to_string())
            .unwrap_or_else(|| format!("Unknown-{}", tx.proof_scheme)),
    ...
    loop {
        ...
        match exec_result {
            Ok(result) => {
                ...
                // LOW-3 FIX: Use protocol-fixed epochs_per_year (2s block time = 15,768,000 blocks/year).
                // This ensures inflation rate is deterministic regardless of config changes.
                const PROTOCOL_BLOCK_TIME_SECS: u64 = 2;
                let blocks_per_year: u64 = 365 * 24 * 60 * 60 / PROTOCOL_BLOCK_TIME_SECS;
                ...
                if rewards.proposer_amount > 0 {
                    let coinbase_tx = UtxoTransaction {
                        ...
                        proof_scheme: 0x00, // N/A for coinbase
                        tx_type: TxType::Coinbase,
                        inputs: vec![],
                        outputs: {
                            let mut outs = vec![UtxoTxOutput {
...
mod tests {
    ...
    use misaka_types::utxo::{
        ...
        ZeroKnowledgeProofCarrier, PROOF_SCHEME_DEPRECATED_LOGRING, UTXO_TX_VERSION_V3,
    };
