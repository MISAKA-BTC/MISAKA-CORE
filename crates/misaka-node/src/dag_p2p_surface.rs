...
/// A discovered peer address with optional transport public key.
///
/// When the PK is known (e.g., from a signed PeerRecord), outbound dials
/// can verify the responder's identity (MITM protection, SEC-C1).
/// When the PK is None, the dial uses TOFU (Trust On First Use).
#[derive(Debug, Clone)]
pub struct DiscoveredPeer {
    pub address: String,
    /// ML-DSA-65 transport public key bytes (1952 bytes), if known.
...
impl DagP2pObservationState {
    pub fn record(
        ...
        peer_id: Option<&misaka_p2p::PeerId>,
    ) {
        ...
        self.last_peer_prefix = peer_id.map(|id| id.short_hex());
...
pub fn classify_dag_p2p_surface(message: &DagP2pMessage) -> DagP2pSurface {
    match message {
        ...
        _ => DagP2pSurface::SteadyStateRelay,
    }
}
...
mod tests {
    ...
    fn test_observation_state_records_counts() {
        ...
        let peer_id = misaka_p2p::PeerId::from([0xAB; 32]);
