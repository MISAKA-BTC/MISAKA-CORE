...
impl DagP2pEventLoop {
    ...
    async fn run_block_validation(&self, guard: &mut DagNodeState, block_hash: Hash) {
        ...
        // HIGH-1 FIX: Validate header fields that were previously unchecked.
        // tx_root, timestamp, and chain_id validation for P2P ingested blocks.
        {
            // Timestamp: reject blocks more than 2 minutes in the future
            let now_ms = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_millis() as u64;
            let max_future_ms = now_ms + 120_000; // 2 minutes
            if header.timestamp_ms > max_future_ms {
                let reason = format!(
                    "block timestamp {}ms > max allowed {}ms ({}ms in future)",
                    header.timestamp_ms, max_future_ms, header.timestamp_ms - now_ms
                );
                self.observation.write().await.record_ingest_rejected(block_hash, reason.clone());
                guard.ingestion_pipeline.mark_quarantined(block_hash, reason, None);
                return;
            }

            // TX root: recompute using same formula as DagBlock::compute_tx_root()
            if !received_txs.is_empty() {
                use sha3::{Digest, Sha3_256};
                let mut h = Sha3_256::new();
                h.update(b"MISAKA_TXROOT_V2:");
                h.update((received_txs.len() as u32).to_le_bytes());
                for tx in &received_txs {
                    h.update(&tx.signing_digest());
                }
                let computed_tx_root: [u8; 32] = h.finalize().into();
                if header.tx_root != computed_tx_root {
                    let reason = format!(
                        "tx_root mismatch: header={} computed={}",
                        hex::encode(&header.tx_root[..8]),
        ...
        let ghostdag_data = match guard.ghostdag.try_calculate(
            ...
            &snapshot,
            &guard.reachability,
            // HIGH-1 FIX: Use EpochAware provider for PoS chain selection.
            // For P2P ingestion, we use a default provider since validator
            // stake is resolved at block production time, not relay time.
            // P2P relay uses default_weight=1 (uniform) as a safe fallback.
            &misaka_dag::EpochAwareStakeProvider::new(1, 1),
        ) {
            Ok(data) => data,
            Err(e) => {
                self.observation.write().await.record_ingest_rejected(
