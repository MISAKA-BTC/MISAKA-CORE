        ...
        match serde_json::from_slice::<misaka_dag::dag_p2p::DagP2pMessage>(&plaintext) {
            Ok(msg) => {
                ...
                if peer_msg_count > PEER_MAX_MESSAGES_PER_SEC {
                    warn!(
                        "Peer {} rate limited ({}/s), dropping message",
                        ph, peer_msg_count
                    );
                    continue;
                }
...
mod tests {
    ...
    async fn test_seed_connection_registry_clears_after_disconnect() {
        ...
        let server = tokio::spawn(async move {
            let (mut stream, _) = listener.accept().await.expect("accept");
            let _ =
                tcp_responder_handshake(&mut stream, &responder.public_key, &responder.secret_key)
                    .await
                    .expect("responder handshake");
...
pub async fn run_dag_p2p_transport(
    ...
    for addr in seed_addrs {
        let (pk, sk, itx, r, st, name, mode, seed_connections) = (
            our_pk.clone(),
            our_sk.clone(),
            inbound_tx.clone(),
            reg.clone(),
