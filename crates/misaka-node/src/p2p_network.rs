...
impl P2pNetwork {
    ...
    pub async fn get_advertisable_peers(&self) -> Vec<(String, String)> {
        self.peers
            ...
            .filter_map(|p| {
                p.advertise_addr
                    .map(|addr| (addr.to_string(), p.node_name.clone()))
            })
            .collect()
    }
                ...
                match TcpStream::connect(addr).await {
                    Ok(stream) => {
                        ...
                        let hello_enc = match hello.encode() {
                            ...
                            Err(e) => {
                                error!(
                                    "Failed to encode Hello for {}: {} — dropping connection",
                                    addr, e
                                );
                                return;
                            }
                            ...
                            loop {
                                ...
                                match P2pMessage::decode(&msg_buf) {
                                    ...
                                    Ok(P2pMessage::GetPeers) if serves_discovery => {
                                        ...
                                        let advertisable: Vec<(String, String)> = peer_list
                                            .values()
                                            .filter(|p| {
                                                p.advertisable && p.advertise_addr.is_some()
                                            })
                                            .filter_map(|p| {
                                                p.advertise_addr
                                                    .map(|a| (a.to_string(), p.node_name.clone()))
                                            })
                                            .collect();
                                        drop(peer_list);
        ...
        loop {
            ...
            match P2pMessage::decode(&msg_buf) {
                Ok(P2pMessage::GetPeers) if serves_discovery => {
                    ...
                    let peer_list: Vec<(String, String)> = peers_for_read
                        .read()
                        .await
                        .values()
                        ...
                        .collect();
                    let _ = response_tx
                        .send(P2pMessage::Peers { addrs: peer_list })
                        .await;
                }
