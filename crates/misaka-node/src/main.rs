...
// The `testnet` feature bypasses these guards for testnet release builds.
...
compile_error!(
    "FATAL: 'faucet' feature MUST NOT be compiled in release mode. \
     ...
     For testnet: use --features testnet instead."
);

#[cfg(all(not(debug_assertions), feature = "legacy-p2p", not(feature = "testnet")))]
compile_error!(
...
#[cfg(all(not(debug_assertions), feature = "allow-tofu", not(feature = "testnet")))]
compile_error!(
    "FATAL: 'allow-tofu' feature MUST NOT be compiled in release mode. \
     TOFU handshakes allow unverified peer connections, enabling MITM attacks. \
     Production builds require pinned public keys for all outbound connections. \
     For testnet: use --features testnet instead."
);
...
#[cfg(feature = "dag")]
pub mod shielded_hook_impl;
#[cfg(feature = "dag")]
pub mod shielded_rpc;
#[cfg(feature = "dag")]
pub mod shielded_verifier_adapters;
...
struct Cli {
    ...
    /// ZKP batch lane interval (shielded TXs) in seconds.
    ...
    /// Enable the shielded module. Default: true on testnet, false on mainnet.
    /// Set to false for CEX / exchange operator nodes (transparent-only mode).
    #[arg(long, env = "MISAKA_SHIELDED_ENABLED")]
    shielded_enabled: Option<bool>,

    /// Enable testnet/dev bootstrap mode for the shielded module.
    /// This is the only runtime path that may register legacy stub proofs.
    /// MUST be false on mainnet (chain_id == 1). Auto-derived from chain_id if not set.
    #[arg(long, env = "MISAKA_SHIELDED_TESTNET")]
    shielded_testnet: Option<bool>,

    /// Maximum anchor age in blocks for shielded transfers.
    #[arg(long, default_value = "100")]
    shielded_max_anchor_age: u64,

    /// Minimum fee for shielded transactions (base units).
    #[arg(long, default_value = "1000")]
    shielded_min_fee: u64,

    /// Authoritative Layer 4 verifier target contract.
    /// This fixes operator/runtime intent only; it does not widen accepted circuits.
    #[arg(
        long,
        env = "MISAKA_SHIELDED_AUTHORITATIVE_TARGET",
        value_parser = ["groth16", "plonk", "groth16_or_plonk"]
    )]
    shielded_authoritative_target: Option<String>,

    /// Optional Groth16 verifying key artifact path for Layer 4 shell contract.
    /// This updates runtime status only; it does not enable Groth16 verification yet.
    #[arg(long, env = "MISAKA_SHIELDED_GROTH16_VK_PATH")]
    shielded_groth16_vk_path: Option<String>,

    /// VK policy for the Groth16 shell contract: disabled, observe, require.
    /// If omitted, path presence implies require; no path implies disabled.
    #[arg(
        long,
        env = "MISAKA_SHIELDED_GROTH16_VK_POLICY",
        value_parser = ["disabled", "observe", "require"]
    )]
    shielded_groth16_vk_policy: Option<String>,

    /// Optional PLONK verifying key artifact path for Layer 4 shell contract.
    /// This updates runtime status only; it does not enable PLONK verification yet.
    #[arg(long, env = "MISAKA_SHIELDED_PLONK_VK_PATH")]
    shielded_plonk_vk_path: Option<String>,

    /// VK policy for the PLONK shell contract: disabled, observe, require.
    /// If omitted, path presence implies require; no path implies disabled.
    #[arg(
        long,
        env = "MISAKA_SHIELDED_PLONK_VK_POLICY",
        value_parser = ["disabled", "observe", "require"]
    )]
    shielded_plonk_vk_policy: Option<String>,

    /// Explicitly enable startup-time real backend registration.
    /// Default is shell-only status wiring; actual Groth16/PLONK registration stays off
    /// until a production adapter is compiled and this flag is set.
    #[arg(long, env = "MISAKA_SHIELDED_REAL_BACKEND_BOOTSTRAP")]
    shielded_real_backend_bootstrap: bool,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // ════════════════════════════════════════════════════════
    //  MAINNET SECURITY GATE — fail-closed startup checks
    // ════════════════════════════════════════════════════════
    {
        let is_mainnet = cli.chain_id == 1;
        let data_path = std::path::Path::new(&cli.data_dir);

        // 1. Run all startup checks
        let check_result = misaka_security::startup_checks::run_startup_checks(
            data_path, is_mainnet, cli.chain_id,
        );

        if !check_result.is_ok() {
            error!("╔═══════════════════════════════════════════════════════════╗");
            error!("║  STARTUP CHECKS FAILED — node cannot start safely       ║");
            error!("╚═══════════════════════════════════════════════════════════╝");
            for err in &check_result.errors {
                error!("  ✗ {}", err);
            }
            if is_mainnet {
                error!("FATAL: Mainnet requires ALL startup checks to pass.");
                std::process::exit(1);
            } else {
                warn!("Testnet: continuing despite failed checks (NOT safe for production)");
            }
        } else {
            info!(
                "Startup checks: {}/{} passed",
                check_result.checks_passed,
                check_result.checks_passed + check_result.checks_failed
            );
        }

        // 2. Mainnet: BUILD_MANIFEST.json must exist
        if is_mainnet {
            let manifest_path = data_path.join("BUILD_MANIFEST.json");
            // Also check in the binary's directory
            let exe_dir_manifest = std::env::current_exe()
                .ok()
                .and_then(|p| p.parent().map(|d| d.join("BUILD_MANIFEST.json")));

            let manifest_exists = manifest_path.exists()
                || exe_dir_manifest.as_ref().map(|p| p.exists()).unwrap_or(false);

            if !manifest_exists {
                error!("FATAL: BUILD_MANIFEST.json not found for mainnet build.");
                error!("This binary may not be from an official release.");
                error!("Checked: {} and exe directory", manifest_path.display());
                std::process::exit(1);
            }
            info!("Release manifest verified.");
        }

        // 3. Admin config enforcement (RPC admin listener)
        let admin_bind: std::net::SocketAddr = format!("127.0.0.1:{}", cli.rpc_port + 1)
            .parse()
            .unwrap_or_else(|_| "127.0.0.1:3002".parse().unwrap());

        if let Err(errors) = misaka_rpc::admin_listener::enforce_admin_config(
            &admin_bind, is_mainnet, false, &None, &None,
        ) {
            for e in &errors {
                error!("Admin config: {}", e);
            }
            if is_mainnet {
                error!("FATAL: Admin listener configuration invalid for mainnet.");
                std::process::exit(1);
            }
        }
        info!("Admin listener config: validated (bind={})", admin_bind);

        // 4. Bridge persistence validation (if bridge data dir exists)
        let bridge_data_dir = data_path.join("bridge");
        if bridge_data_dir.exists() {
            let persistence = misaka_bridge::persistence::BridgePersistence::new(&bridge_data_dir);
            match persistence.validate_on_startup() {
                Ok(health) => {
                    info!(
                        "Bridge persistence: OK (nullifiers={}, pending_approvals={})",
                        health.nullifier_count, health.pending_approvals
                    );
                }
                Err(errors) => {
                    for e in &errors {
                        error!("Bridge persistence: {}", e);
                    }
                    if is_mainnet {
                        error!("FATAL: Bridge persistence corrupted. Manual recovery required.");
                        error!("See docs/BRIDGE_OPERATIONS_RUNBOOK.md for recovery steps.");
                        std::process::exit(1);
                    } else {
                        warn!("Testnet: continuing despite bridge persistence errors");
                    }
                }
            }
        }
        ...
        if let Err(errors) = misaka_security::secrets::validate_required_secrets(cli.chain_id as u64) {
            error!("Secret validation: {}", errors);
            if is_mainnet {
                error!("FATAL: Required secrets not configured for mainnet.");
                std::process::exit(1);
            }
        }

        info!("═══ All mainnet security gates passed ═══");
    }
...
fn normalize_experimental_validator_identity(
    ...
    //
    // TODO(P1): リモートバリデータの stake_weight を Solana から自動照会して更新する。
    // 現時点では 1 に固定し、全バリデータ等重みでコンセンサスを行う。
    // これは「預入枚数がコンセンサス重みに反映されない」制限だが、
    // 偽の stake_weight を主張する攻撃は防いでいる。
...
async fn start_dag_node(
    ...
    info!("╔═══════════════════════════════════════════════════════════╗");
    info!("║  MISAKA Network — DAG Consensus                         ║");
    info!("╚═══════════════════════════════════════════════════════════╝");
    ...
    match role {
        NodeRole::Sr => info!("Role: SR (Super Representative) — finality committee"),
        NodeRole::Candidate => info!("Role: Candidate — SR candidate, scoring tracked"),
        NodeRole::Archive => info!("Role: Archive — full history, data serving"),
        NodeRole::Relay => info!("Role: Relay — network propagation bridge"),
        NodeRole::Observer => info!("Role: Observer — light validation"),
    }

    // Storage strategy based on role
    let storage_config = storage_strategy::StorageConfig::for_role(role);
    info!(
        "Storage: retention={}rounds history={} certificates={} max_db={}GB",
        if storage_config.dag_retention_rounds == u64::MAX { "unlimited".to_string() }
        else { storage_config.dag_retention_rounds.to_string() },
        storage_config.store_full_history,
        storage_config.store_certificates,
        storage_config.max_db_size_gb,
    );
    ...
    match consensus_mode {
        consensus_engine::ConsensusMode::NarwhalBullshark => {
            ...
            // Extract ML-DSA-65 key pair for block signing + committee public key
            let validator_sk_bytes: Option<Vec<u8>> = transport_keys
                .as_ref()
                .map(|(_, sk)| sk.pq_sk.clone());
            let validator_pk_bytes: Option<Vec<u8>> = transport_keys
                .as_ref()
                .map(|(pk, _)| pk.pq_pk.clone());

            let nb_config = consensus_engine::NarwhalConfig {
                authority_index: cli.validator_index as u32,
                epoch: 0,
                committee_size: cli.validators as u32,
                leaders_per_round: 2,
                wave_length: 3,
                gc_depth: 100,
                max_transactions_per_block: cli.dag_max_txs,
                sr_count: std::cmp::min(cli.validators, 21),
                // ML-DSA-65 key pair — loaded from validator keystore.
                signing_key: validator_sk_bytes.clone(),
                signing_public_key: validator_pk_bytes.clone(),
            };

            let engine = consensus_engine::create_consensus_engine(nb_config);

            // P0-3: Load persisted state
            {
                let mut guard = engine.write().await;
                let data_path = std::path::Path::new(&cli.data_dir);
                if let Err(e) = guard.load_state(data_path) {
                    warn!("Failed to load Narwhal state: {} (starting fresh)", e);
                }
            }

            // #6 FIX: Weak subjectivity check — reject all-zero checkpoint on mainnet.
            // An all-zero checkpoint provides NO long-range attack protection.
            // New nodes syncing from genesis with checkpoint=[0;32] could accept
            // a forged chain from a long-range attacker.
            let ws_config = consensus_engine::WeakSubjectivityConfig::default();
            if cli.chain_id == 1 && ws_config.trusted_checkpoint_digest == [0u8; 32]
                && ws_config.trusted_checkpoint_sequence == 0 {
                // HIGH-3 FIX: On mainnet, zero checkpoint is ONLY acceptable for
                // initial genesis validators (must explicitly opt in).
                // After genesis, all joining nodes MUST have a real checkpoint.
                let allow_genesis_bootstrap = std::env::var("MISAKA_ALLOW_GENESIS_BOOTSTRAP")
                    .map(|v| v == "1" || v.to_lowercase() == "true")
                    .unwrap_or(false);

                if allow_genesis_bootstrap {
                    warn!(
                        "WEAK SUBJECTIVITY: Genesis bootstrap mode (MISAKA_ALLOW_GENESIS_BOOTSTRAP=1). \
                         After the first finalized epoch, set a real checkpoint and remove this env var."
                    );
                } else {
                    error!(
                        "FATAL: Mainnet requires a non-zero weak subjectivity checkpoint. \
                         Set [weak_subjectivity].checkpoint in configs/mainnet.toml to a \
                         recent finalized checkpoint hash. \
                         For INITIAL genesis validators ONLY: set MISAKA_ALLOW_GENESIS_BOOTSTRAP=1"
                    );
                    std::process::exit(1);
                }
            }
            {
                ...
                if let Err(e) = guard.validate_weak_subjectivity(&ws_config) {
                    error!("FATAL: {}", e);
                    error!("Node must resync from a trusted checkpoint.");
                    if cli.chain_id == 1 {
                        std::process::exit(1);
                    } else {
                        warn!("Testnet: continuing despite weak subjectivity violation");
                    }
                }
            }

            // P0-3: Periodic state persistence (every 30 seconds)
            let engine_persist = engine.clone();
            let data_dir_persist = cli.data_dir.clone();
            tokio::spawn(async move {
                let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(30));
                loop {
                    interval.tick().await;
                    let guard = engine_persist.read().await;
                    if let Err(e) = guard.save_state(std::path::Path::new(&data_dir_persist)) {
                        tracing::warn!("Failed to persist Narwhal state: {}", e);
                    }
                }
            });

            info!(
                "Narwhal/Bullshark engine initialized: committee={} leaders_per_round=2 sr_count={}",
                cli.validators,
                std::cmp::min(cli.validators, 21),
            );

            // Run the new engine's block production loop
            if role.produces_blocks() {
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        if guard.check_liveness_timeout() {
                            // Try to propose even without full quorum (degraded mode)
                            // This helps the network make progress during partial connectivity
                            tracing::warn!("Liveness timeout: attempting forced proposal");
                        }
                        if let Some(block) = guard.try_propose() {
                            let committed = guard.process_block(block);
                            for sub_dag in &committed {
                                tracing::info!(
                                    "Narwhal COMMIT #{}: {} blocks finalized",
                                    sub_dag.index, sub_dag.blocks.len(),
                                );
                            }
                        }
                    }
                });
                info!("Narwhal block producer started (200ms interval)");
            }

            // ── Narwhal RPC Server ──
            // Full RPC for Narwhal consensus mode — covers all endpoints
            // that wallets, explorers, faucets, and E2E tests need.
            let narwhal_rpc_engine = engine.clone();
            ...
            tokio::spawn(async move {
                ...
                let app = Router::new()
                    ...
                    .route("/health", get(|| async {
                        Json(serde_json::json!({"status": "ok", "consensus": "narwhal_bullshark"}))
                    }))
                    // ── Chain info (POST) ──
                    .route("/api/get_chain_info", post(move || async move {
                        let guard = e_chain.read().await;
                        let f = guard.finality_status();
                        Json(serde_json::json!({
                            "nodeName": node_name,
                            "chainId": chain_id,
                            "consensus": "narwhal_bullshark",
                            "currentRound": f.current_round,
                            "lastCommittedRound": f.last_committed_round,
                            "committedCount": f.committed_count,
                            "txCount": f.tx_count,
                            "lastFinalizedSequence": f.last_finalized_sequence,
                            "isFinalized": f.is_finalized,
                            "equivocationsDetected": f.equivocations_detected,
                        }))
                    }))
                    // ── DAG info (POST) ──
                    .route("/api/get_dag_info", post(move || async move {
                        let guard = e_dag.read().await;
                        Json(serde_json::json!({
                            "consensus": "narwhal_bullshark",
                            "currentRound": guard.current_round(),
                            "lastCommittedRound": guard.last_committed_round(),
                            "committedCount": guard.committed_count,
                            "txCount": guard.tx_count,
                        }))
                    }))
                    // ── DAG tips (POST) ──
                    .route("/api/get_dag_tips", post(move || async move {
                        let guard = e_tips.read().await;
                        Json(serde_json::json!({
                            "tips": [{
                                "round": guard.current_round(),
                                "committedRound": guard.last_committed_round(),
                            }],
                            "tipCount": 1,
                        }))
                    }))
                    // ── Mempool info (GET) ──
                    .route("/api/get_mempool_info", get(|| async {
                        Json(serde_json::json!({
                            "mempoolSize": 0,
                            "mempoolBytes": 0,
                            "minFee": 100,
                    ...
                    // ── Fee estimate (GET) ──
                    .route("/api/fee_estimate", get(|| async {
                        Json(serde_json::json!({
                            "low": 100,
                            "medium": 100,
                            "high": 200,
                    ...
                    // ── Finality status (GET) ──
                    .route("/api/finality_status", get(move || async move {
                        let guard = e_finality.read().await;
                        Json(serde_json::json!(guard.finality_status()))
                    }))
                    // ── Shielded status (POST) — disabled in Narwhal mode for now ──
                    .route("/api/shielded/status", post(|| async {
                        Json(serde_json::json!({
                            "enabled": false,
                ...
                let addr: SocketAddr = format!("0.0.0.0:{}", narwhal_rpc_port)
                    .parse()
                    .expect("valid RPC addr");
                let listener = match tokio::net::TcpListener::bind(addr).await {
                    Ok(l) => l,
                    Err(e) => {
                        tracing::error!("FATAL: Narwhal RPC failed to bind {}: {}", addr, e);
                        return;
                    }
                };
                tracing::info!("Narwhal RPC server listening on {} (10 endpoints)", addr);
                if let Err(e) = axum::serve(
                    listener,
                    app.into_make_service_with_connect_info::<SocketAddr>(),
                ).await {
                    tracing::error!("Narwhal RPC server error: {}", e);
                }
            });
    ...
    let dag_node_state = DagNodeState {
        ...
        shielded_hook: None, // Set below after ShieldedState initialization
    };
    #[allow(unused_mut)]
                ...
                if let Some(account) = registry.get(&validator_id) {
                    if !account.solana_stake_verified {
                        if let Some(ref sig) = cli.stake_signature {
                            match registry.mark_stake_verified(&validator_id, sig.clone()) {
                                Ok(()) => {
                                    info!(
        ...
        loop {
            ...
            if let Some(next_epoch) = maybe_next_epoch {
                ...
                // ── SR21 Auto-Election at epoch boundary ──
                {
                    let guard = finality_epoch_state.read().await;
                    let election_result =
                        sr21_election::run_election(&guard.known_validators, next_epoch);

                    // Update num_active_srs based on election
                    drop(guard);
                    let mut wguard = finality_epoch_state.write().await;
                    wguard.num_active_srs = election_result.num_active.max(1);

                    // If our local validator is in the active set, update sr_index
                    if let Some(ref lv) = wguard.local_validator {
                        if let Some(new_index) = sr21_election::find_sr_index(
                            &election_result,
                            &lv.identity.validator_id,
                        ) {
                            wguard.sr_index = new_index;
                            info!(
    ...
    // ── Shielded module initialization (via ShieldedBootstrap) ──
    //
    // Reads shielded config from NodeConfig::shielded.
    // The same SharedShieldedState is passed to:
    //   1. DAG RPC server (for /api/shielded/* endpoints)
    //   2. NodeShieldedHook (for block commit callbacks) — P1
    //   3. ShieldedMempool (for nullifier reservation) — P1
    let shielded_state_for_rpc: Option<misaka_shielded::SharedShieldedState> = {
        use crate::shielded_hook_impl::{
            resolve_startup_verifier_adapters, validate_startup_verifier_adapter_contract,
            ShieldedBootstrap,
        };
        // SEC-FIX [Audit #8]: Always read from CLI/env, never hardcode.
        // Default: enabled on testnet (chain_id != 1), disabled on mainnet.
        let is_mainnet = cli.chain_id == 1;
        let enabled = cli.shielded_enabled.unwrap_or(!is_mainnet);
        let testnet = cli.shielded_testnet.unwrap_or(!is_mainnet);
        let anchor_age = cli.shielded_max_anchor_age;
        let min_fee = cli.shielded_min_fee;
        let authoritative_target = cli.shielded_authoritative_target.as_deref();
        let groth16_vk_policy = cli.shielded_groth16_vk_policy.as_deref();
        let groth16_vk_path = cli.shielded_groth16_vk_path.as_deref();
        let plonk_vk_policy = cli.shielded_plonk_vk_policy.as_deref();
        let plonk_vk_path = cli.shielded_plonk_vk_path.as_deref();
        let real_backend_bootstrap = cli.shielded_real_backend_bootstrap;

        // Safety check: mainnet MUST NOT have testnet_mode = true
        if is_mainnet && testnet {
            error!("FATAL: shielded testnet_mode=true is forbidden on mainnet (chain_id=1)");
            anyhow::bail!("shielded testnet_mode=true on mainnet is a fatal misconfiguration");
        }

        info!(
            "ShieldedConfig: enabled={}, testnet={}, anchor_age={}, min_fee={}, authoritative_target={}, groth16_vk_policy={}, groth16_vk_configured={}, plonk_vk_policy={}, plonk_vk_configured={}, real_backend_bootstrap={} (chain_id={})",
            enabled,
            testnet,
            anchor_age,
            min_fee,
            authoritative_target.unwrap_or("groth16_or_plonk"),
            groth16_vk_policy.unwrap_or("auto"),
            groth16_vk_path.is_some(),
            plonk_vk_policy.unwrap_or("auto"),
            plonk_vk_path.is_some(),
            real_backend_bootstrap,
            cli.chain_id
        );
        let verifier_adapters = resolve_startup_verifier_adapters(real_backend_bootstrap)?;
        ...
        ShieldedBootstrap::from_node_config_with_adapters(
            enabled,
            testnet,
            anchor_age,
            min_fee,
    ...
    // ── Shielded Hook — wire DAG block producer → ShieldedState ──
    // Build ShieldedBlockHookImpl from the same SharedShieldedState.
    // Then inject into DagNodeState so the block producer can call it
    // after each block commit.
    if let Some(ref state) = shielded_state_for_rpc {
        // A-2: Load persisted shielded state snapshot
        crate::shielded_hook_impl::ShieldedBootstrap::load_snapshot(state, &cli.data_dir);
        let hook: misaka_dag::shielded_hook::SharedShieldedHook = std::sync::Arc::new(
            crate::shielded_hook_impl::NodeShieldedHook::new(state.clone(), &cli.data_dir),
        );
        shared_state.write().await.shielded_hook = Some(hook);
        info!("ShieldedModule: block hook registered on DagNodeState (persistence enabled)");
    }
...
async fn start_v1_node(
    ...
    // Genesis — MED-4 FIX: use deterministic timestamp (CRIT-2 fixed in chain_store.rs)
    // The timestamp parameter is ignored by store_genesis() which uses GENESIS_TIMESTAMP_MS.
    let now_ms = 0u64; // ignored — genesis uses ChainStore::GENESIS_TIMESTAMP_MS
    ...
    async fn test_remote_vote_gossip_forms_live_local_quorum_when_checkpoint_matches() {
        ...
        let server = tokio::spawn(async move {
            crate::dag_rpc::run_dag_rpc_server_with_observation(
                ...
                None,
                None, // shielded: disabled in test
                addr,
                31337,
            )
            .await
