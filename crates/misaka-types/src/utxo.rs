...
///
/// PR C: merged snapshot tightened this from 8 KiB to 2 KiB. Revert to
/// 8 KiB for backward compat — the existing stake-envelope format
/// (`ValidatorStakeTx::encode_for_extra`, used by `misaka-cli` and the
/// `misaka-mempool` stake-tx admission path) serializes to ~2.1 KiB for
/// a Register (due to the 1952-byte ML-DSA-65 consensus pubkey), which
/// overflows 2 KiB. Tightening has to wait until the stake flow moves
/// out of `UtxoTransaction::extra` (PR D / post-merge refactor).
