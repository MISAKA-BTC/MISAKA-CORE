...
// ═══ Network Identity ═══
// MED-2 NOTE: MAINNET_CHAIN_ID (0x4D534B01) is used for address encoding
// and cross-chain replay protection. The runtime guard in main.rs uses
// chain_id == 1 (CLI argument). These are different namespaces:
//   - 0x4D534B01: address prefix / wire protocol identifier
//   - 1: runtime chain_id from CLI/config (mainnet.toml chain_id = 1)
...
// ═══ SR Consensus (initial SR15, expandable to SR21) ═══
// See docs/SR_COMMITTEE_DESIGN.md and misaka-consensus/sr_promotion.rs CommitteePolicy.
/// Initial mainnet committee size (SR15). Expandable to 18/21 at epoch boundaries.
pub const NUM_SUPER_REPRESENTATIVES: usize = 15;
/// Maximum committee size the protocol supports (array layouts, bridge PDA).
pub const MAX_ACTIVE_VALIDATORS: usize = 21;
pub const MIN_VALIDATORS: usize = 4;
/// BFT quorum threshold in basis points: ceil(2/3) = 66.67%.
/// For SR15: ceil(2*15/3) = 10 votes = 66.67%.
/// For SR21: ceil(2*21/3) = 14 votes = 66.67%.
pub const QUORUM_THRESHOLD_BPS: u16 = 6667;
/// Round-robin size = current committee size.
...
/// CRIT-2 NOTE: This DEFAULT_BPS (1) is for finality depth / TPS estimation.
/// The DAG layer (misaka-dag/constants.rs) uses DEFAULT_BPS=10 for block production.
/// DAA (misaka-dag/daa.rs) uses TARGET_BLOCK_INTERVAL_MS=10,000 for difficulty.
/// These are different layers with different timing targets — this is by design.
...
// ═══ P2P ═══
// MED-3 NOTE: These are Kaspa-inherited defaults. MISAKA configs use:
//   - P2P: 6690 (mainnet.toml/testnet.toml)
//   - RPC: 3001 (mainnet.toml/testnet.toml)
// These constants are only used as fallbacks if no config is provided.
...
// ═══ Compile-Time Safety ═══
const _: () = {
    assert!(NUM_SUPER_REPRESENTATIVES >= 4); // BFT minimum
    assert!(NUM_SUPER_REPRESENTATIVES <= MAX_ACTIVE_VALIDATORS);
