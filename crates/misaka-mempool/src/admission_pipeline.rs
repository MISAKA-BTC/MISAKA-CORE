...
// ═══════════════════════════════════════════════════════════════
//  D4: Lightweight ZKP Pre-Validation (Red Team audit fix)
//
//  Full ZKP verification (~10ms) remains deferred to block validation.
//  This adds O(1) structural checks that catch obviously forged proofs
//  BEFORE they enter the mempool and propagate via gossip.
//
//  An attacker sending random-byte proofs will be caught here.
//  Only a sophisticated attacker who crafts structurally-valid-but-
//  cryptographically-invalid proofs would pass (and be caught at
//  block validation).
// ═══════════════════════════════════════════════════════════════

/// Minimum proof size for a valid UnifiedZKP membership proof.
/// Even the smallest valid proof (depth-1 Merkle, single commitment)
/// requires at least this many bytes for the commitment + path + range.
pub const MIN_PROOF_BYTES: usize = 128;

/// Magic prefix for UnifiedZKP proofs (domain separation).
/// All valid UnifiedZKP proofs start with this 4-byte tag.
pub const UNIFIED_ZKP_PROOF_TAG: [u8; 4] = [0x4D, 0x5A, 0x50, 0x01]; // "MZP\x01"

/// Lightweight ZKP pre-validation.
///
/// Performs O(1) structural checks on proof bytes WITHOUT doing any
/// cryptographic verification. Catches:
/// - Random-byte proofs (wrong prefix / wrong internal structure)
/// - Empty proofs on inputs that claim to have proofs
/// - Proofs with impossible internal length fields
///
/// Cost: ~100ns per input (vs ~10ms for full ZKP verify)
pub fn lightweight_zkp_precheck(tx: &UtxoTransaction) -> Result<(), MempoolError> {
    for (i, inp) in tx.inputs.iter().enumerate() {
        let proof = &inp.proof;

        // Skip empty proofs (handled by structural validation)
        if proof.is_empty() {
            continue;
        }

        // D4-1: Minimum proof size
        if proof.len() < MIN_PROOF_BYTES {
            return Err(MempoolError::Structural(format!(
                "input[{}] proof too small for valid ZKP: {} bytes < {} min",
                i, proof.len(), MIN_PROOF_BYTES
            )));
        }

        // D4-2: Check proof tag (first 4 bytes must match domain tag)
        // This rejects ~99.99999994% of random-byte proofs immediately.
        if proof.len() >= 4 && proof[..4] != UNIFIED_ZKP_PROOF_TAG {
            return Err(MempoolError::Structural(format!(
                "input[{}] proof has invalid ZKP tag: {:02x}{:02x}{:02x}{:02x}",
                i, proof[0], proof[1], proof[2], proof[3]
            )));
        }

        // D4-3: Internal length field sanity (bytes 4-8 encode inner proof len)
        if proof.len() >= 8 {
            let inner_len = u32::from_le_bytes([proof[4], proof[5], proof[6], proof[7]]) as usize;
            // Inner length must be <= remaining bytes and > 0
            let remaining = proof.len().saturating_sub(8);
            if inner_len == 0 || inner_len > remaining {
                return Err(MempoolError::Structural(format!(
                    "input[{}] proof internal length invalid: inner={} remaining={}",
                    i, inner_len, remaining
                )));
            }
        }

        // D4-4: Entropy check — reject proofs that are all-same-byte
        // (catches memset'd buffers, compressed zeros, etc.)
        if proof.len() > 32 {
            let first = proof[8]; // skip tag + length
            let all_same = proof[8..].iter().all(|&b| b == first);
            if all_same {
                return Err(MempoolError::Structural(format!(
                    "input[{}] proof has zero entropy (all bytes = 0x{:02x})",
                    i, first
                )));
            }
        }
    ...
    // D4-5: Transaction-level ZK proof carrier check
    if let Some(ref zk) = tx.zk_proof {
        if zk.proof_bytes.len() < MIN_PROOF_BYTES {
            return Err(MempoolError::ZeroKnowledgeProof(format!(
                "zk_proof carrier too small: {} bytes < {} min",
                zk.proof_bytes.len(), MIN_PROOF_BYTES
            )));
        }
    }
...
pub fn validate_resolved_privacy_constraints(
    constraints: &TransactionPrivacyConstraints,
    tx: &UtxoTransaction,
    _resolved: &ResolvedInputAmounts,
    backend_family: PrivacyBackendFamily,
