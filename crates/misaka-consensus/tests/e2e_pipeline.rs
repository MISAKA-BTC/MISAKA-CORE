use misaka_consensus::pipeline::body_processor::BodyProcessor;
...
#[test]
#[ignore = "pre-existing: test fixture lacks ML-DSA-65 signatures required by consensus"]
fn test_e2e_genesis_to_two_blocks_current_pipeline() {
    ...
    let body_processor =
        BodyProcessor::new(db.clone(), block_txs_store.clone(), statuses_store.clone());
    ...
    body_processor
        .process_body(block1_hash, Vec::new())
        .expect("block1 body");
    assert_eq!(
    ...
    body_processor
        .process_body(block2_hash, Vec::new())
        .expect("block2 body");
    assert_eq!(
