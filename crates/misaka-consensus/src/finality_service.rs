...
impl FinalityServiceState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, FinalityServiceError> {
        ...
        let entry = FinalityServiceEntry {
            ...
            created_at: now,
            updated_at: now,
            state: FinalityServiceEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(FinalityServiceTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), FinalityServiceError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(FinalityServiceError::NotFound(id))?;
        ...
        self.emit(FinalityServiceEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, FinalityServiceError> {
        ...
        self.emit(FinalityServiceEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<FinalityServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    FinalityServiceEntryState::Completed
                        | FinalityServiceEntryState::Failed
                        | FinalityServiceEntryState::Cancelled
                        | FinalityServiceEntryState::Expired
        ...
        if removed > 0 {
            self.emit(FinalityServiceEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> FinalityServiceStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<FinalityServiceEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = FinalityServiceConfig {
            retry_count: 2,
            ..Default::default()
        };
