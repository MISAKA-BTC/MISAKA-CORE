...
impl DagTraversalState {
    ...
    pub fn create(
        ...
        let entry = DagTraversalEntry {
            ...
            created_at: now,
            updated_at: now,
            state: DagTraversalEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(DagTraversalTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), DagTraversalError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(DagTraversalError::NotFound(id))?;
        ...
        self.emit(DagTraversalEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, DagTraversalError> {
        ...
        self.emit(DagTraversalEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<DagTraversalEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    DagTraversalEntryState::Completed
                        | DagTraversalEntryState::Failed
                        | DagTraversalEntryState::Cancelled
                        | DagTraversalEntryState::Expired
        ...
        if removed > 0 {
            self.emit(DagTraversalEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> DagTraversalStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<DagTraversalEvent> {
