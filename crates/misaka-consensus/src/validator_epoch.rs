...
use super::validator_scoring::{compute_score, ScoreBreakdown, ScoringConfig, ValidatorMetrics};
...
pub fn run_epoch_rotation(
    ...
    for (i, v) in validators.iter().enumerate() {
        ...
        let fails_uptime = breakdown
            .map(|b| b.uptime < epoch_config.min_uptime_active)
            .unwrap_or(true);
        let fails_contribution = breakdown
            .map(|b| b.contribution < epoch_config.min_contribution_active)
            .unwrap_or(true);
        let fails_penalty = breakdown
            .map(|b| b.penalty_factor < epoch_config.min_penalty_active)
            .unwrap_or(true);
...
mod tests {
    ...
    fn test_cooldown_prevents_immediate_repromotion() {
        ...
        assert!(!r3.promoted.iter().any(|p| p.validator_id[0] == 2));
    }

    #[test]
    fn test_severe_offense_ejection() {
        let (_, _, cc) = default_configs();
        let mut validators = vec![make_validator(
            1,
            RotationRole::Active,
        ...
        let mut cooldowns = CooldownRegistry::new();
        let validator_id = validators[0].validator_id;
