...
impl PruningServiceState {
    ...
    pub fn create(
        ...
        data: Vec<u8>,
        priority: u32,
    ) -> Result<u64, PruningServiceError> {
        ...
        let entry = PruningServiceEntry {
            ...
            created_at: now,
            updated_at: now,
            state: PruningServiceEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(PruningServiceTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), PruningServiceError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(PruningServiceError::NotFound(id))?;
        ...
        self.emit(PruningServiceEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, PruningServiceError> {
        ...
        self.emit(PruningServiceEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<PruningServiceEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    PruningServiceEntryState::Completed
                        | PruningServiceEntryState::Failed
                        | PruningServiceEntryState::Cancelled
                        | PruningServiceEntryState::Expired
        ...
        if removed > 0 {
            self.emit(PruningServiceEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> PruningServiceStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<PruningServiceEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = PruningServiceConfig {
            retry_count: 2,
            ..Default::default()
        };
