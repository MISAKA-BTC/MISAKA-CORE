...
impl ValidatorRecord {
    ...
    /// Active 維持条件を満たしているか
    pub fn satisfies_active_requirements(&self, config: &RotationConfig) -> bool {
...
impl ValidatorRegistry {
    ...
    pub fn register(
        ...
        let record = ValidatorRecord::new_backup(validator_id, stake, self.current_epoch, network);
    ...
    pub fn update_metrics(
        ...
        let breakdown = crate::validator_scoring::compute_score(&metrics, &self.config.scoring);
    ...
    pub fn is_in_cooldown(&self, validator_id: &[u8; 32]) -> bool {
        self.cooldown
            .is_in_cooldown(validator_id, self.current_epoch)
    }
    ...
    pub fn ranked_backup_candidates(&self) -> Vec<(&ValidatorRecord, f64)> {
        let mut candidates: Vec<_> = self
            ...
            .filter(|r| {
                !self
                    .cooldown
                    .is_in_cooldown(&r.validator_id, self.current_epoch)
            })
...
mod tests {
    ...
    fn register_backup_ok() {
        ...
        reg.register(make_id(1), stake, backup_profile(1))
            .expect("ok");
    ...
    fn metrics_update_computes_score() {
        ...
        reg.register(make_id(1), stake, public_profile(1))
            .expect("ok");
        let breakdown = reg
    ...
    fn backup_candidates_sorted_by_score() {
        ...
        for i in 1u8..=3 {
            reg.register(make_id(i), stake, backup_profile(i))
                .expect("ok");
    ...
    fn active_count_matches_set() {
        ...
        reg.register(make_id(1), stake, public_profile(1))
            .expect("ok");
        reg.register(make_id(2), stake, public_profile(2))
            .expect("ok");
