...
impl HeaderProcessor {
    ...
    fn validate_header_in_isolation(&self, header: &Header) -> Result<(), HeaderProcessingError> {
        ...
        // Validate PQC signature (ML-DSA-65) — MANDATORY for non-genesis blocks
        if header.hash != self.config.genesis_hash {
            if header.pqc_signature.is_empty() {
                return Err(HeaderProcessingError::InvalidSignature(
                    "missing PQC signature".into(),
                ));
            }
            if header.proposer_pk.is_empty() {
                return Err(HeaderProcessingError::InvalidSignature(
                    "missing proposer public key".into(),
                ));
            }

            // Build the signing payload: all header fields except signature itself
            let signing_payload = {
                let mut h = sha3::Sha3_256::new();
                use sha3::Digest;
                h.update(b"MISAKA:block:header:v1:");
                ...
                h.update(&header.blue_score.to_le_bytes());
                h.finalize().to_vec()
            };

            // Verify using misaka-pqc ML-DSA-65
            match (
                misaka_pqc::pq_sign::MlDsaPublicKey::from_bytes(&header.proposer_pk),
                misaka_pqc::pq_sign::MlDsaSignature::from_bytes(&header.pqc_signature),
            ) {
                (Ok(pk), Ok(sig)) => {
                    misaka_pqc::pq_sign::ml_dsa_verify(&pk, &signing_payload, &sig)
                        .map_err(|e| {
                            HeaderProcessingError::InvalidSignature(format!(
                ...
                (Err(e), _) => {
                    // CRIT-2 FIX: No fallback — parse failure is always a hard reject.
                    return Err(HeaderProcessingError::InvalidSignature(format!(
                        "invalid ML-DSA-65 public key: {:?}", e
                    )));
                }
                (_, Err(e)) => {
                    return Err(HeaderProcessingError::InvalidSignature(format!(
                        "invalid ML-DSA-65 signature: {:?}", e
                    )));
                }
            }
        }
