...
use crate::stores::acceptance_data::{AcceptanceData, DbAcceptanceDataStore};
use crate::stores::block_transactions::DbBlockTransactionsStore;
use crate::stores::ghostdag::{
    DbGhostdagStore, GhostdagData, GhostdagStoreReader, Hash, ZERO_HASH,
};
...
impl VirtualStateProcessor {
    ...
    pub fn process_block(&self, block_hash: Hash) -> Result<(), VirtualProcessingError> {
        ...
        self.statuses_store
            .set_batch(&mut batch, block_hash, BlockStatus::StatusUTXOValid)
            .map_err(VirtualProcessingError::StoreError)?;
        ...
        self.db
            .write(batch)
            .map_err(|e| VirtualProcessingError::StoreError(StoreError::from(e)))?;

        Ok(())
    }

    fn compute_virtual_ghostdag(
        &self,
        tips: &[Hash],
    ) -> Result<GhostdagData, VirtualProcessingError> {
    ...
    fn resolve_virtual_utxo_diff(
        &self,
        _ghostdag: &GhostdagData,
    ) -> Result<UtxoDiff, VirtualProcessingError> {
    ...
    fn compute_acceptance_data(
        &self,
        _ghostdag: &GhostdagData,
    ) -> Result<AcceptanceData, VirtualProcessingError> {
        Ok(Vec::new())
    }
}
