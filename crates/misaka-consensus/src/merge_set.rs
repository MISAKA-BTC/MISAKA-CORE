...
impl MergeSetState {
    ...
    pub fn create(&self, key: String, data: Vec<u8>, priority: u32) -> Result<u64, MergeSetError> {
        ...
        let entry = MergeSetEntry {
            ...
            created_at: now,
            updated_at: now,
            state: MergeSetEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(MergeSetTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), MergeSetError> {
        ...
        self.emit(MergeSetEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, MergeSetError> {
        ...
        self.emit(MergeSetEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<MergeSetEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    MergeSetEntryState::Completed
                        | MergeSetEntryState::Failed
                        | MergeSetEntryState::Cancelled
                        | MergeSetEntryState::Expired
        ...
        if removed > 0 {
            self.emit(MergeSetEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> MergeSetStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<MergeSetEvent> {
