...
impl BlockBuilderState {
    ...
    pub fn create(
        ...
        let entry = BlockBuilderEntry {
            ...
            created_at: now,
            updated_at: now,
            state: BlockBuilderEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(BlockBuilderTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), BlockBuilderError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(BlockBuilderError::NotFound(id))?;
        ...
        self.emit(BlockBuilderEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, BlockBuilderError> {
        ...
        self.emit(BlockBuilderEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<BlockBuilderEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    BlockBuilderEntryState::Completed
                        | BlockBuilderEntryState::Failed
                        | BlockBuilderEntryState::Cancelled
                        | BlockBuilderEntryState::Expired
        ...
        if removed > 0 {
            self.emit(BlockBuilderEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> BlockBuilderStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<BlockBuilderEvent> {
