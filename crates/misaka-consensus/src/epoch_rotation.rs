...
impl<'a> EpochRotationEngine<'a> {
    ...
    pub fn run_rotation(&mut self) -> RotationResult {
        ...
        // SEC-FIX: If active count drops below BFT safety threshold (2/3 + 1),
        // undo demotions to maintain consensus liveness. Without this guard,
        // rotation can reduce the active set below the BFT quorum requirement.
        // SEC-FIX NM-5: Use count-based BFT minimum as a safety floor.
        // True BFT safety requires stake-weighted quorum, which is enforced
        // by `validator_set.quorum_threshold()` at consensus time. This guard
        // ensures we never drop below the minimum validator COUNT needed for
        // the committee to function at all.
        let bft_minimum = (ACTIVE_SET_SIZE * 2 / 3) + 1;
        if active_count < bft_minimum {
            tracing::error!(
                "EpochRotation[{}]: Active set {} < BFT minimum {} — \
                 reverting demotions to prevent consensus halt",
                epoch,
                active_count,
                bft_minimum
            );
            // Revert demotions: restore demoted validators to Active
            for record in &demoted_records {
                if let Some(v) = self.registry.validators.get_mut(&record.validator_id) {
                    v.role = ValidatorRole::Active;
                }
                if !self.registry.active_set.contains(&record.validator_id) {
                    self.registry.active_set.push(record.validator_id);
                }
                self.registry.cooldown.remove(&record.validator_id);
            }
            // Clear demotion records since they were reverted
            let reverted_count = demoted_records.len();
            demoted_records.clear();
            tracing::warn!(
                "EpochRotation[{}]: Reverted {} demotions — safe mode active",
                epoch,
                reverted_count
            );
        } else if active_count < ACTIVE_SET_SIZE {
            tracing::warn!(
...
mod tests {
    ...
    fn demoted_enters_cooldown() {
        ...
        // Register 20 good validators so BFT safety guard doesn't revert the demotion
        for i in 2..=21 {
            reg.register(make_id(i), min_active, public_profile(i))
                .expect("ok");
            reg.update_metrics(&make_id(i), good_active_metrics(min_active))
                .expect("ok");
            active_set.push(make_id(i));
        }
        // Register the bad validator to be demoted
        reg.register(make_id(1), min_active, public_profile(1))
            .expect("ok");
        reg.update_metrics(&make_id(1), bad_metrics(min_active))
            .expect("ok");
        active_set.push(make_id(1));
        reg.set_active_set(active_set);
