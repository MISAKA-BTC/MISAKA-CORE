...
impl TestConsensusState {
    ...
    pub fn create(
        ...
        let entry = TestConsensusEntry {
            ...
            created_at: now,
            updated_at: now,
            state: TestConsensusEntryState::Queued,
            priority,
            retry_count: 0,
            data,
        ...
        self.queue.lock().push_back(TestConsensusTask {
            entry_id: id,
            scheduled_at: now,
            priority,
        });
    ...
    pub fn complete(&self, id: u64) -> Result<(), TestConsensusError> {
        ...
        let entry = entries
            .get_mut(&id)
            .ok_or(TestConsensusError::NotFound(id))?;
        ...
        self.emit(TestConsensusEvent::Completed {
            id,
    ...
    pub fn fail(&self, id: u64, reason: String) -> Result<bool, TestConsensusError> {
        ...
        self.emit(TestConsensusEvent::Failed {
            id,
            reason,
            will_retry,
        });
    ...
    pub fn get(&self, id: u64) -> Option<TestConsensusEntry> {
    ...
    pub fn cleanup(&self) -> usize {
        ...
        let to_remove: Vec<u64> = entries
            ...
            .filter(|(_, e)| {
                matches!(
                    e.state,
                    TestConsensusEntryState::Completed
                        | TestConsensusEntryState::Failed
                        | TestConsensusEntryState::Cancelled
                        | TestConsensusEntryState::Expired
        ...
        if removed > 0 {
            self.emit(TestConsensusEvent::Cleanup {
                removed,
    ...
    pub fn stats(&self) -> TestConsensusStatsSnapshot {
        self.stats.snapshot()
    }
    pub fn drain_events(&self) -> Vec<TestConsensusEvent> {
...
mod tests {
    ...
    #[test]
    fn test_retry() {
        let config = TestConsensusConfig {
            retry_count: 2,
            ..Default::default()
        };
