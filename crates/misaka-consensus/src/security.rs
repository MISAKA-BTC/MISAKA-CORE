...
impl BlockSecurityChecker {
    ...
    pub fn check_body(
        ...
        if body.transactions.len() > MAX_BLOCK_TRANSACTIONS {
            return Err(BlockSecurityError::TooManyTransactions(
        ...
        if total_mass > MAX_BLOCK_MASS {
            return Err(BlockSecurityError::MassExceeded {
                mass: total_mass,
                max: MAX_BLOCK_MASS,
            });
        }
        ...
        let total_sig_ops: u64 = body
            .transactions
            .iter()
            .map(|t| t.sig_op_count as u64)
            .sum();
        if total_sig_ops > MAX_BLOCK_SIG_OPS {
            return Err(BlockSecurityError::SigOpsExceeded {
                count: total_sig_ops,
                max: MAX_BLOCK_SIG_OPS,
            });
        }
        ...
        for tx in &body.transactions {
            if !seen_txids.insert(tx.tx_id) {
                return Err(BlockSecurityError::DuplicateTransaction(hex::encode(
                    tx.tx_id,
                )));
            }
        }
...
impl TxSecurityChecker {
    ...
    pub fn check_in_isolation(tx: &TxForCheck) -> Result<(), TxSecurityError> {
        ...
        // 5. Check mass limits
        if tx.mass > MAX_TRANSACTION_MASS {
            return Err(TxSecurityError::MassExceeded {
                mass: tx.mass,
                max: MAX_TRANSACTION_MASS,
            });
        }
        ...
        for output in &tx.outputs {
            if output.script_public_key.len() > 10_000 {
                return Err(TxSecurityError::ScriptTooLarge(
