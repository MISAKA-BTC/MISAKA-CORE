...
impl MerkleFrontier {
    ...
    fn compute_root_from_scratch(&self) -> TreeRoot {
        // frontier の確定ノードを bottom-up に合成しつつ、
        // そのレベルに存在しない右側サブツリーは empty_root_at_depth(i) で埋める。
        //
        // carry は「ここまでに構成できた右側部分木」を表し、常に現在レベル i の
        // サブツリーサイズと同じ高さに正規化された状態で次のレベルへ進む。
        let mut carry: Option<[u8; 32]> = None;

        for i in 0..TREE_DEPTH {
            let bit = (self.size >> i) & 1;
            let empty = empty_root_at_depth(i);

            if bit == 1 {
                let left = self.nodes[i].unwrap_or(empty);
                carry = Some(match carry {
                    Some(right) => node_hash(&left, &right),
                    None => node_hash(&left, &empty),
                });
            } else if let Some(prev) = carry {
                carry = Some(node_hash(&prev, &empty));
            }
        }

        TreeRoot(carry.unwrap_or_else(|| TreeRoot::empty().0))
    }
...
mod tests {
    ...
    fn recompute_from_witness(cm: NoteCommitment, witness: &MerkleWitness) -> TreeRoot {
        let mut cur = cm.0;
        let mut pos = witness.position as usize;
        for sibling in &witness.auth_path {
            cur = if pos & 1 == 0 {
    ...
    #[test]
    fn witness_matches_root_for_single_leaf() {
        let mut tree = CommitmentTree::new(100);
        let cm = make_cm(7);
        tree.append(cm).unwrap();
        let witness = tree.witness(0).unwrap();
        assert_eq!(recompute_from_witness(cm, &witness), tree.root());
    }

    #[test]
    fn witness_matches_root_for_unbalanced_tree() {
        let mut tree = CommitmentTree::new(100);
        for i in 0..3u8 {
            tree.append(make_cm(i + 1)).unwrap();
        }
        let witness = tree.witness(2).unwrap();
        assert_eq!(recompute_from_witness(make_cm(3), &witness), tree.root());
    }
}
