...
pub fn compute_fee_shares(total_fee: u64) -> FeeShares {
    let proposer = ((total_fee as u128) * (PROPOSER_FEE_BPS as u128) / 10_000) as u64;
    let treasury = ((total_fee as u128) * (TREASURY_FEE_BPS as u128) / 10_000) as u64;
    let burned = total_fee.saturating_sub(proposer).saturating_sub(treasury);
    FeeShares {
        proposer,
        treasury,
        burned,
