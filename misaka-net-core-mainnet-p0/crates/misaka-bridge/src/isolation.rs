...
impl BridgeOrigin {
    ...
    pub fn new_non_pq(
        ...
        assert!(!chain.is_pq_secured(), "use new_pq() for PQ-secured chains");
        Self {
            chain,
            lock_tx_hash,
            source_token,
    ...
    pub fn new_pq(
        ...
        assert!(chain.is_pq_secured(), "use new_non_pq() for non-PQ chains");
        Self {
            chain,
            lock_tx_hash,
            source_token,
...
mod tests {
    ...
    fn test_origin_id_unique() {
        let o1 = BridgeOrigin::new_non_pq(
            ExternalChain::Solana, vec![0xAA; 64], vec![0xBB; 32], 1700000000,
        );
        let o2 = BridgeOrigin::new_non_pq(
            ExternalChain::Ethereum, vec![0xAA; 64], vec![0xBB; 32], 1700000000,
        );
    ...
    fn test_security_labels() {
        assert_eq!(TokenType::Native.security_label(), "PQ-secured (native MISAKA)");
        let wrapped = TokenType::Wrapped(BridgeOrigin::new_non_pq(
            ExternalChain::Solana, vec![], vec![], 0,
        ));
        assert_eq!(wrapped.security_label(), "non-PQ (bridged from classical chain)");
    }
    ...
    #[test]
    #[should_panic]
    fn test_new_pq_panics_on_non_pq_chain() {
        BridgeOrigin::new_pq(ExternalChain::Solana, vec![], vec![], 0);
    }

    #[test]
    #[should_panic]
    fn test_new_non_pq_panics_on_pq_chain() {
        BridgeOrigin::new_non_pq(ExternalChain::PqExternal, vec![], vec![], 0);
    }
}
