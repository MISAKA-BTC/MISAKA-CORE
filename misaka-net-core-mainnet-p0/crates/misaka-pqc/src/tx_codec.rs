...
pub fn decode_transaction(data: &[u8]) -> Result<UtxoTransaction, String> {
    ...
    for _ in 0..n_outputs {
        ...
        outputs.push(TxOutput { amount, one_time_address, pq_stealth });
    }
    ...
    Ok(UtxoTransaction { version, ring_scheme, inputs, outputs, fee, extra })
}
...
mod tests {
    ...
    fn test_full_tx_with_real_ring_sig() {
        ...
        let kip = crate::ki_proof::prove_key_image(&a, &kps[1].secret_poly, &kps[1].public_poly, &kps[1].key_image).unwrap();
        ...
        assert_eq!(decoded.inputs[0].key_image, kps[1].key_image);
