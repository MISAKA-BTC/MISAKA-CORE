...
use super::pq_ring::{Poly, Q, N, BETA, RingSig};
...
pub fn unpack_response(data: &[u8]) -> Result<Poly, CryptoError> {
    ...
    if data.len() != expected_bytes {
        return Err(CryptoError::RingSignatureInvalid(
            format!("packed response: {} bytes, expected {}", data.len(), expected_bytes)));
    }
...
/// Pack an entire ring signature (compressed format).
///
/// Format: challenge(256B) || packed_responses(416B each) || key_image(32B)
pub fn pack_ring_sig(sig: &RingSig) -> Vec<u8> {
    ...
    buf.extend_from_slice(&sig.c0.challenge_to_bytes());
    // Packed responses
...
/// Unpack a compressed ring signature.
pub fn unpack_ring_sig(data: &[u8], ring_size: usize) -> Result<RingSig, CryptoError> {
    let expected = N + ring_size * PACKED_RESPONSE_SIZE + 32;
    if data.len() != expected {
        return Err(CryptoError::RingSignatureInvalid(
            format!("packed sig: {} bytes, expected {}", data.len(), expected)));
    }
    ...
    for _ in 0..ring_size {
        responses.push(unpack_response(&data[offset..offset + PACKED_RESPONSE_SIZE])?);
        offset += PACKED_RESPONSE_SIZE;
    }
    let mut ki = [0u8; 32];
    ki.copy_from_slice(&data[offset..]);

    Ok(RingSig { c0, responses, key_image: ki })
}
...
mod tests {
    ...
    fn test_pack_unpack_response_roundtrip() {
        ...
        for i in 0..N {
            ...
            let a = if p.coeffs[i] > Q / 2 { p.coeffs[i] - Q } else { p.coeffs[i] };
            let b = if unpacked.coeffs[i] > Q / 2 { unpacked.coeffs[i] - Q } else { unpacked.coeffs[i] };
    ...
    fn test_pack_ring_sig_roundtrip() {
        ...
        let sig = ring_sign(&a, &ring, 1, &kps[1].secret_poly, &msg).unwrap();
        ...
        // Unpack and verify
        let sig2 = unpack_ring_sig(&packed, 4).unwrap();
        ring_verify(&a, &ring, &msg, &sig2).expect("packed→unpacked sig must verify");
    }

    #[test]
    fn test_packed_sizes() {
        println!("  PACKED_RESPONSE_SIZE: {} bytes (vs 512 unpacked)", PACKED_RESPONSE_SIZE);
        println!("  Ring=4 packed sig: {} bytes", N + 4 * PACKED_RESPONSE_SIZE + 32);
        println!("  Ring=4 unpacked:   {} bytes", N + 4 * N * 2 + 32);
        println!("  Ring=8 packed sig: {} bytes", N + 8 * PACKED_RESPONSE_SIZE + 32);
...
pub fn unpack_challenge(data: &[u8]) -> Result<(Poly, usize), CryptoError> {
    if data.is_empty() {
        return Err(CryptoError::RingSignatureInvalid("empty challenge data".into()));
    }
    let count = data[0] as usize;
    let sign_bytes = (count + 7) / 8;
    let total = 1 + count + sign_bytes;
    if data.len() < total {
        return Err(CryptoError::RingSignatureInvalid("truncated challenge".into()));
    }
    ...
    for i in 0..count {
        ...
        if pos >= N {
            return Err(CryptoError::RingSignatureInvalid("challenge position out of range".into()));
        }
...
/// Pack ring signature v2 (compact challenge + 14-bit responses).
///
/// Format: compact_challenge || packed_responses(448B each) || key_image(32B)
pub fn pack_ring_sig_v2(sig: &RingSig) -> Vec<u8> {
...
/// Unpack ring signature v2.
pub fn unpack_ring_sig_v2(data: &[u8], ring_size: usize) -> Result<RingSig, CryptoError> {
    // Decode challenge (variable length header)
    let (c0, c_len) = unpack_challenge(data)?;
    let remaining = &data[c_len..];
    let expected_remaining = ring_size * PACKED_RESPONSE_SIZE + 32;
    if remaining.len() != expected_remaining {
        return Err(CryptoError::RingSignatureInvalid(
            format!("v2 sig remaining: {} bytes, expected {}", remaining.len(), expected_remaining)));
    }

    let mut responses = Vec::with_capacity(ring_size);
    let mut offset = 0;
    for _ in 0..ring_size {
        responses.push(unpack_response(&remaining[offset..offset + PACKED_RESPONSE_SIZE])?);
        offset += PACKED_RESPONSE_SIZE;
    }
    let mut ki = [0u8; 32];
...
mod tests_v2 {
    ...
    fn test_v2_ring_sig_roundtrip() {
        ...
        println!("  v0→v2 saving: {} bytes ({:.1}%)",
            v1_size - v2_packed.len(),
            (1.0 - v2_packed.len() as f64 / v1_size as f64) * 100.0);

        // Unpack and verify
        let sig2 = unpack_ring_sig_v2(&v2_packed, 4).unwrap();
    ...
    fn test_v2_sizes_comparison() {
        ...
        for ring in [4, 8, 16] {
            ...
            println!("  ring={:2}: v0={}B  v1={}B  v2={}B  (v0→v2: -{:.1}%)",
                ring, v0, v1, v2, (1.0 - v2 as f64 / v0 as f64) * 100.0);
        }
    }
}
