//! PQ Stealth output extension (§6 of spec).
//!
//! Minimal extension attached to transaction outputs
//! for ML-KEM-768 based stealth addressing.

use crate::mcs1;
use crate::error::MisakaError;

/// Version tag for the stealth protocol.
pub const PQ_STEALTH_VERSION: u8 = 0x01;

/// On-chain stealth extension data attached to an output.
///
    ...
    pub fn mcs1_decode(data: &[u8], offset: &mut usize) -> Result<Self, MisakaError> {
        ...
        if version != PQ_STEALTH_VERSION {
            return Err(MisakaError::DeserializationError(
                format!("unsupported stealth version: 0x{:02x}", version),
            ));
        }
        let kem_ct = mcs1::read_bytes(data, offset)?;
        if kem_ct.len() != 1088 {
            return Err(MisakaError::DeserializationError(
                format!("invalid kem_ct length: {} (expected 1088)", kem_ct.len()),
            ));
        }
        ...
        Ok(Self { version, kem_ct, scan_tag, amount_ct, payload_ct })
    }
...
mod tests {
    ...
    #[test]
    fn test_stealth_data_bad_version() {
        let mut buf = Vec::new();
