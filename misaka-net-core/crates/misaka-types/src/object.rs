...
impl OwnerKind {
    pub fn from_u8(v: u8) -> Result<Self, MisakaError> {
        match v {
            0 => Ok(Self::AddressOwner), 1 => Ok(Self::ObjectOwner),
            2 => Ok(Self::SharedOwner), 3 => Ok(Self::Immutable),
            _ => Err(MisakaError::DeserializationError(format!("invalid OwnerKind: {v}"))),
        }
    }
}
...
mod tests {
    ...
    fn test_object_digest_deterministic() {
        let obj = Object {
            ...
            owner: [0xBB; 20], type_tag: "coin".into(), data: vec![1, 2, 3],
        };
