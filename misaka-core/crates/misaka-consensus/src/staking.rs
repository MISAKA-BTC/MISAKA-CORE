...
mod validator_map_serde {
    ...
    pub fn serialize<S>(
        map: &HashMap<[u8; 20], ValidatorAccount>,
        serializer: S,
    ) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    ...
    pub fn deserialize<'de, D>(
        deserializer: D,
    ) -> Result<HashMap<[u8; 20], ValidatorAccount>, D::Error>
    where
        D: Deserializer<'de>,
    {
...
impl StakingRegistry {
    ...
    pub fn get(&self, id: &[u8; 20]) -> Option<&ValidatorAccount> {
...
mod tests {
    ...
    fn register_and_activate(reg: &mut StakingRegistry, id: [u8; 20], stake: u64, epoch: u64) {
        reg.register(id, vec![1; 1952], stake, 500, id, epoch, [id[0]; 32], 0)
            .unwrap();
