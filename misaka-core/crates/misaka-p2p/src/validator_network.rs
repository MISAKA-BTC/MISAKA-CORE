...
impl ValidatorNetworkConfig {
    ...
    pub fn is_allowed_inbound(&self, peer_id: &PeerId) -> bool {
        ...
        self.sentry_allowlist.contains(peer_id) || self.validator_peers.contains(peer_id)
    }
}
...
mod tests {
    ...
    fn test_validator_accepts_allowlisted_sentry() {
        ...
        let decision = evaluate_inbound(NetworkRole::Validator, &pid(1), Some(&config), 0, 10);
        assert_eq!(decision, AdmissionDecision::Accept);
    }

    #[test]
    fn test_full_node_accepts_any_peer() {
        let decision = evaluate_inbound(NetworkRole::Full, &pid(99), None, 0, 48);
        assert_eq!(decision, AdmissionDecision::Accept);
    }
