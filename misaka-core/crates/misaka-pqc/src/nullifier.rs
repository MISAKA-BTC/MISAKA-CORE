...
use crate::pq_ring::{
    derive_public_param, hash_to_challenge, sample_masking_poly, Poly, BETA, MAX_SIGN_ATTEMPTS, N,
    Q,
};
...
impl TxDomain {
    ...
    pub fn from_byte(b: u8) -> Result<Self, CryptoError> {
        match b {
            ...
            0x04 => Ok(Self::Bridge),
            _ => Err(CryptoError::RingSignatureInvalid(format!(
                "unknown TxDomain: 0x{:02X}",
                b
            ))),
        }
    }
...
impl NullifierContext {
    ...
    pub fn to_bytes(&self) -> [u8; Self::WIRE_SIZE] {
        ...
        let mut off = 0;
        buf[off..off + 4].copy_from_slice(&self.chain_id.to_le_bytes());
        off += 4;
        buf[off] = self.tx_domain.to_byte();
        off += 1;
        buf[off..off + 36].copy_from_slice(&self.spent_output_id.to_bytes());
        off += 36;
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        if data.len() != Self::WIRE_SIZE {
            return Err(CryptoError::RingSignatureInvalid(format!(
                "NullifierContext: expected {} bytes, got {}",
                Self::WIRE_SIZE,
                data.len()
            )));
        }
        ...
        off += 4;
        let tx_domain = TxDomain::from_byte(data[off])?;
        off += 1;
        let mut tx_hash = [0u8; 32];
        tx_hash.copy_from_slice(&data[off..off + 32]);
        off += 32;
        let output_index =
...
pub fn compute_nullifier_ctx(secret: &Poly, ctx: &NullifierContext) -> ([u8; 32], Poly, Poly) {
...
pub fn compute_nullifier(secret: &Poly, output_id: &OutputId, chain_id: u32) -> ([u8; 32], Poly) {
...
impl NullifierProof {
    ...
    pub fn prove(
        ...
        for _ in 0..MAX_SIGN_ATTEMPTS {
            ...
            return Ok(Self {
                ...
                challenge,
                response: z,
                nullifier_poly: null_poly,
            });
        }
        Err(CryptoError::RingSignatureInvalid(
            "nullifier proof: max attempts".into(),
        ))
    }
    ...
    pub fn verify(&self, a: &Poly, pubkey: &Poly, nullifier: &[u8; 32]) -> Result<(), CryptoError> {
        // ── 0. Norm bound ──
        if self.response.norm_inf() >= BETA {
            return Err(CryptoError::RingSignatureInvalid(
                "nullifier proof: norm".into(),
            ));
        }
        ...
        let w_null_prime = a_null
            .mul(&self.response)
            .sub(&c_poly.mul(&self.nullifier_poly));
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        ...
        if data.len() != expected {
            return Err(CryptoError::RingSignatureInvalid(format!(
                "nullifier proof: expected {} bytes, got {}",
                expected,
                data.len()
            )));
        }
...
mod tests {
    ...
    fn test_proof_wrong_nullifier_rejected() {
        ...
        assert!(
            proof.verify(&a, &pk, &[0xFF; 32]).is_err(),
    ...
    fn test_proof_wrong_pk_rejected() {
        ...
        let proof = NullifierProof::prove(&a, &s, &pk, &out, 2, &null).unwrap();
        assert!(
            proof.verify(&a, &pk2, &null).is_err(),
    ...
    fn test_proof_cross_output_rejected() {
        ...
        assert!(
            proof.verify(&a, &pk, &null_a).is_ok(),
            "correct proof must pass"
        );
        ...
        assert!(
            tampered_proof.verify(&a, &pk, &null_a).is_err(),
