...
fn derive_fee_hint_key_nonce(ss: &[u8; 32]) -> Result<([u8; 32], [u8; 24]), CryptoError> {
    ...
    hk.expand(FEE_HINT_KEY_LABEL, &mut key).map_err(|_| {
        CryptoError::RingSignatureInvalid("HKDF expand for fee hint key failed".into())
    })?;
    hk.expand(FEE_HINT_NONCE_LABEL, &mut nonce).map_err(|_| {
        CryptoError::RingSignatureInvalid("HKDF expand for fee hint nonce failed".into())
    })?;
    Ok((key, nonce))
}
