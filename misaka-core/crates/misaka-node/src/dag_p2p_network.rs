...
impl DagP2pEventLoop {
    ...
    async fn run_block_validation(&self, guard: &mut DagNodeState, block_hash: Hash) {
        ...
        if computed_hash != block_hash {
            ...
            guard
                .ingestion_pipeline
                .mark_rejected(block_hash, "hash mismatch".to_string());
            return;
        }
        ...
        let ghostdag_data = match guard.ghostdag.try_calculate(
            ...
            Err(e) => {
                ...
                guard
                    .ingestion_pipeline
                    .mark_rejected(block_hash, format!("ghostdag calculation failed: {}", e));
                return;
            }
        };
        ...
        if let Err(e) = guard
            .dag_store
            .insert_block(block_hash, header.clone(), vec![])
        {
            self.observation
                .write()
                .await
        ...
        for action in actions {
            match action {
                ...
                SyncAction::Ban(reason) => {
                    warn!("Banning peer {}: {}", hex::encode(&peer_id[..4]), reason);
