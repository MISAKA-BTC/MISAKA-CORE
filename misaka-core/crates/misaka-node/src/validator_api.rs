...
async fn handle_register(
    ...
    let validator_id = {
        let pq_pubkey = ValidatorPqPublicKey::from_bytes(&validator_pubkey.bytes)
            .map_err(|e| ApiResult::err(format!("invalid validator_pubkey: {}", e)))?;
        pq_pubkey.to_legacy_validator_id()
    };
    ...
    let response = ValidatorResponse::from_account(registry.get(&validator_id).unwrap(), &config);
    drop(registry);
...
async fn handle_activate(
    ...
    let config = registry.config().clone();
    let response = ValidatorResponse::from_account(registry.get(&id).unwrap(), &config);
    drop(registry);
    if let Err(err) = crate::validator_lifecycle_persistence::persist_global_state(
        &state.registry,
        &state.current_epoch,
...
mod tests {
    ...
    async fn test_validator_api_register_rejects_invalid_pubkey_and_rate() {
        let state = test_state();
        let auth = ApiKeyState { required_key: None };
        let app = axum::Router::new()
            .nest(
                "/api/v1/validators",
                validator_api_public_router(state.clone()),
