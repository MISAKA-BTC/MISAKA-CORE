...
pub fn compute_network_summary(
    epoch: u64,
    snapshots: &[ValidatorWorkloadSnapshot],
...
pub fn compute_workload_score_from_raw(
    ...
    let norm_finality = if finalized_contribution > 0 {
        NORM_SCALE
    } else {
        0
    };
...
mod tests {
    ...
    fn test_workload_score_perfect_validator() {
        ...
        assert!(
            score > 0,
            "perfect validator must have positive score: {score}"
        );
    ...
    fn test_junk_work_no_advantage() {
        ...
        let quality =
            compute_workload_score_from_raw(&w, 20, 0, 500, 0, 500, 500, 50_000, 1000, 1000, 0);
        // Validator B: 100 blocks but 80 rejected (gaming attempt)
        let junk =
            compute_workload_score_from_raw(&w, 20, 80, 500, 0, 500, 500, 50_000, 1000, 1000, 0);
        assert!(
            junk < quality,
    ...
    fn test_accumulator_into_snapshot() {
        ...
        assert!(
            snap.workload_score > 0,
            "snapshot should have computed score"
        );
    }
