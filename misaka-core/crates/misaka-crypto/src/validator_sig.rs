...
impl ValidatorPqPublicKey {
    ...
    /// Derive the legacy 20-byte validator address for compatibility only.
    ///
    /// This truncates the canonical SHA3-256 validator ID and is retained for
    /// display / compatibility paths that still expect the older 20-byte form.
    /// Consensus and slashing code should continue to use `to_canonical_id()`.
    pub fn to_legacy_validator_id(&self) -> [u8; 20] {
...
mod tests {
    ...
    fn test_address_derivation() {
        let kp = generate_validator_keypair();
        let addr = kp.public_key.to_legacy_validator_id();
        assert_eq!(addr.len(), 20);
        assert_eq!(kp.public_key.to_legacy_validator_id(), addr); // deterministic
    }
